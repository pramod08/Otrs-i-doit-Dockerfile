...
class isys_update_modules extends isys_update
{
                ...
                if (strpos($l_file, ".") !== 0)
                {
                    ...
                    if (is_dir($p_path . $l_file))
                    {
                        ...
                        if (is_array($p_preselection))
                        {
                            ...
                            else $l_modules[$i]["selected"] = false;
                        }
                        else
                        {
                        ...
                        if (!is_null($g_comp_database) && isset($l_modules[$i]["const"]) && !empty($l_modules[$i]["const"]))
                        {
                            ...
                            if (is_object($l_check)) $l_installed = ($l_check->num_rows() > 0);

                        }
                        else $l_installed = true;
    ...
    public function install($p_module_path, $p_active_databases, $p_mandator_databases, $p_system_database)
    {
        ...
        $l_id = $l_log->add(
            "Module: <span class=\"grey\">" . str_replace($g_upd_dir . "/" . C__DIR__MODULES, "", $p_module_path) . "</span>",
            C__MESSAGE,
            "bold"
        );
        ...
        if (is_object($p_system_database) || $p_system_database == $g_comp_database_system->get_db_name())
        {
            ...
            $l_id = $l_log->add(
                "Updating system database " . $g_comp_database_system->get_db_name() . "..",
                C__MESSAGE,
                "bold indent"
            );
        ...
        else
        {
            ...
            $l_log->add(
                " - No database(s) selected. It is required to select the databases in order to install a module.",
                C__MESSAGE,
                "bold indent",
                C__HIGH,
                C__ERR

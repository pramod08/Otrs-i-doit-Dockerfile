...
class isys_update_migration extends isys_update
{
     ...
     * @return  boolean
     */
    public function is_migration_done($p_migration_identifier)
    {
        ...
        $l_sql = "SELECT isys_migration__done
			...
			AND isys_migration__version = '" . substr($_SESSION["update_directory"], 1, strlen($_SESSION["update_directory"])) . "';";
    ...
    public function get_foreign_key($p_table, $p_field = null, $p_checkTableExists = true)
    {
        ...
        if ($p_checkTableExists)
        {
            ...
            if ($g_comp_database->num_rows($l_check) > 0) $l_exec = true;
        }
        else  $l_exec = true;
        ...
        if ($l_exec)
        {
            ...
            if ($p_field)
            {
                if (isset($l_parsed[$p_field])) return $l_parsed[$p_field][C__SQL__FOREIGN_KEY];
            }
            else
            {
                return $l_parsed;
    ...
    public function reinit_foreign_key($p_table, $p_field, $p_refTable, $p_refField, $p_onDelete, $p_onUpdate)
    {
        ...
        $l_check = $g_comp_database->query("SHOW TABLES LIKE '" . $p_table . "'");
        if ($g_comp_database->num_rows($l_check) < 1) return false;

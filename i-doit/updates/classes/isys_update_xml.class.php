    ...
    public function load_xml($p_file, $p_do_version_change = false)
    {
        ...
        if (is_file($p_file))
        {
            ...
            if (isset($l_xml->queries->query))
            {
                ...
                {
                    $l_log->debug(
                        " - Processing XML-Node: " . strval($l_query->title) . " (" . strval($l_query->id) . ")"
                    );
            ...
            if ($p_do_version_change && isset($l_xml->info->version) && isset($l_xml->info->revision)) $l_statements[] = $this->db_init_statement($l_xml);

...
<script type="text/javascript">
	(function () {
		...
		$('fcport_browser_save').on('click', function () {
			var selection_str = [],
				$returnview = $('[{$returnfield}]__VIEW'),
				$returnhidden = $('[{$returnfield}]__HIDDEN'),
				$returnprimary = $('[{$returnfield}]__VIEW__PRIM');
		...
		if (typeof fc_ports == 'object' && Object.keys(fc_ports).length > 0) {
			...
			$fc_port_list.update(new Element('p', {className:'p5 m5 exception'}).update('[{isys type="lang" ident="LC_FC_PORT_POPUP__NO_PORTS"}]'));
		}

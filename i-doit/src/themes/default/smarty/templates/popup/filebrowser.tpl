...
<div id="file_browser">
	...
	<div class="popup-content" style="height:487px;">
		...
		<div id="file_browser_fileinfo">
			...
			<fieldset class="overview">
				...
				<div id="file_browser_new_file" class="p5 mt10">
					...
					[{isys type="f_text" name="file_browser_new_file_name" p_strStyle="width:260px;" p_bInfoIconSpacer=0 p_strPlaceholder="LC__CMDB__CATS__FILE_NAME"}]

					[{isys type="f_label" name="file_browser_new_file_category" ident="LC__CMDB__CATG__GLOBAL_CATEGORY"}]
					[{isys type="f_popup" p_strPopupType="dialog_plus" name="file_browser_new_file_category" p_strStyle="width:260px;" p_strTable="isys_file_category" p_bInfoIconSpacer=0}]
...
<script type="text/javascript">
	(function () {
		...
		var $new_filename = $('file_browser_new_file_name'),
			...
			file_infos,
			selected_file = parseInt('[{$selected_file}]'),
			selection = null;

		try {
	        file_infos = '[{$file_infos}]'.evalJSON();
	    } catch (e) {
			try {
	           file_infos = '[{$file_infos|escape:"quotes"}]'.evalJSON();
			} catch (e) {
				idoit.Notify.error('[{isys type="lang" ident="LC_FILEBROWSER__INVALID_JSON"}]', {sticky:true});
			}
	    }
		...
		$file_tree.on('click', '.file-object', function (ev) {
			...
			$file_tree.select('.file-object.bold').invoke('removeClassName', 'bold');
		...
		$('file_browser_save').on('click', function () {
			if (selection !== null) {
				...
				if($('[{$return_name}]__HIDDEN').value != selection.obj_id)
				{
				...
				$('[{$return_name}]__VIEW').value = '[{isys type="lang" ident="LC__CMDB__OBJTYPE__FILE"}] >> ' + selection.obj_title;
				$('[{$return_name}]__HIDDEN').value = selection.obj_id;
			}

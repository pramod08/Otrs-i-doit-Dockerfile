...
function mysearch_addElement() {
	...
	html += "<td width=\"200px\">";
	html += "<input type=\"search\" name=\"C__SEARCH_TEXT[]\" class=\"input input-small\" autosave=\"idoit.search\" results=\"5\" placeholder=\"[{isys type='lang' ident='LC__UNIVERSAL__SEARCH'}]\" id=\"searchText1\" size=\"25\" style=\"padding:3px;\" onkeypress=\"if((event.which&&event.which==13)||(event.keyCode&&event.keyCode==13)) { mysearch_change('index.php', 'ResponseContainer'); return false;}else return true;\" /></td>";
	html += "<td>&nbsp;</td>";
	html += "</tr></table>";
...
function mysearch_change (script, element, p_elemAdvSearch) {
    ...
    if($('searchText'))
    {
        var $object_type_selection = $('obj_input').removeClassName('error'),
            $category_selection = $('catgs_input').removeClassName('error'),
            $searchfield = $('searchText').removeClassName('error'),
            $last_searchfield;

        for (var i = 1, $search_field_x; ($search_field_x = $("searchText" + i)); i++) {
            if ($search_field_x.getValue().length < 3) {
                $last_searchfield = $search_field_x.addClassName('error');
            } else {
                $search_field_x.removeClassName('error');
            }
        }
        ...
        if ($searchfield) {
            if ($searchfield.getValue().length < 3) {
                $searchfield.addClassName('error').focus();
                ...
                $object_type_selection.addClassName('error').focus();
                ...
                $category_selection.addClassName('error').focus();

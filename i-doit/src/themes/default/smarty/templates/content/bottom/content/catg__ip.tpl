...
<table class="contentTable">
	...
	<tr>
		...
		<td class="value">
			...
			[{isys type="f_dialog" name="C__CATP__IP__ACTIVE" p_strClass="input input-mini" p_bDbFieldNN="1" p_bInfoIconSpacer="0"}]
		</td>
	</tr>
	<tr>
		...
		<td class="value">
			...
			[{isys type='f_button' name="" id="net_list_button" p_strValue="[{isys type='lang' ident='LC__CMDB__CATG__IP__LOAD_IP_NET'}]"}]
			[{/if}]
		</td>
	</tr>
</table>

<table class="contentTable form mt5" id="form_[{$smarty.const.C__CATS_NET_TYPE__IPV4}]" [{if $type != $smarty.const.C__CATS_NET_TYPE__IPV4}]style="display:none;"[{/if}]>
	<tr id="ip_range">
		...
		<td class="value">
			...
			<span class="ml5 mr5">-</span>
			[{isys type="f_text" name="C__CATP__IP__ADDRESS_V4_TO" p_strClass="input input-mini" p_bReadonly="1" p_bInfoIconSpacer=0}]
	...
	<tr>
		...
		<td class="value">
			<span id="ip_calc">
				[{isys type="f_text" name="C__CATP__IP__ADDRESS_V4" p_onChange="idoit.callbackManager.triggerCallback('validate_ip');" p_strClass="reEnableWhenNetAssigned input input-mini ipv4"}]
			</span>

			[{if $smarty.const.C__MODULE__PRO && isys_glob_is_edit_mode()}]
			<button type="button" class="btn" id="btn_ip_calc">
				<img src="[{$dir_images}]icons/silk/arrow_refresh.png"/>
			</button>
			<button type="button" class="btn" id="btn_detach_ip">
				<img class="vam" src="[{$dir_images}]icons/silk/detach.png"/>
			</button>
			<span id="C__CATG__IP__MESSAGES" class="inputText" style="display:none;padding:0 10px 2px 10px;"></span>
			[{/if}]
		</td>
	</tr>
	<tr>
		...
		<td class="value">
			...
			<span class="ml5 mr5">/</span>
			[{isys type="f_text" name="C__CATS__NET__CIDR" p_bInfoIconSpacer="0" p_strClass="input input-mini" p_bReadonly=true}]
		</td>
	</tr>
	<tr>
...
<table class="contentTable form mt5" id="form_[{$smarty.const.C__CATS_NET_TYPE__IPV6}]" [{if $type != $smarty.const.C__CATS_NET_TYPE__IPV6}]style="display:none;"[{/if}]>
	<tr>
	...
	<tr>
		...
		<td class="value">
			<div class="fl">
				[{isys type="f_text" id="C__CMDB__CATG__IP__IPV6_ADDRESS" name="C__CMDB__CATG__IP__IPV6_ADDRESS" p_strClass="reEnableWhenNetAssigned inputText ipv6" tab="7"}]
				[{if isys_glob_is_edit_mode()}]
				<button type="button" class="btn" id="btn_ip_calc_v6">
					<img src="[{$dir_images}]icons/navbar/recycle_icon.png"/>
				</button>
				[{/if}]
			</div>
			<div class="fl">
				<span id="C__CATG__IPV6__MESSAGES" class="inputText" style="display:none;padding:0px 10px 2px 10px;">
			</div>
		</td>
	</tr>
	<tr>
...
<table class="contentTable form mt5" id="ip_additional" [{if $type != $smarty.const.C__CATS_NET_TYPE__IPV4 && $type != $smarty.const.C__CATS_NET_TYPE__IPV6}]style="display:none;"[{/if}]>
	<tr>
		<td class="key">[{isys type='f_label' name='C__CATP__IP__HOSTNAME' ident="LC__CATP__IP__HOSTNAME"}]</td>
		<td class="value">
			[{isys
			type="f_text"
			name="C__CATP__IP__HOSTNAME"
			tab="1"}]
		</td>
	</tr>
	<tr class="IP">
	...
	<tr class="IP">
		<td class="key">[{isys type="f_label" name="C__CATP__IP__DNS_DOMAIN" ident="LC__CMDB__CATS__NET__DNS_DOMAIN"}]</td>
		<td class="value">[{isys type="f_popup" p_strPopupType="dialog_plus" name="C__CATP__IP__DNS_DOMAIN"}]</td>
	</tr>
	[{if count($fqdns) > 0}]
	<tr class="IP">
		<td class="key" style="vertical-align: top;">FQDN</td>
		<td class="value">
			[{foreach from=$fqdns item="fqdn"}]
			[{isys type="f_data" p_strValue=$fqdn}]
		<br/>
			[{/foreach}]
		</td>
	</tr>
...
<script type="text/javascript">
	...
	(function () {
		...
		var $btn_ip_calc = $('btn_ip_calc'),
			...
			$net_type = $('C__NET__TYPE');

		// Validation of the ip address.
		var validate_ip = function () {
			var ipv4_assignment = $('ipv4_assign').value,
			...
			switch (ipv4_assignment) {
				...
				case '[{$smarty.const.C__CATP__IP__ASSIGN__DHCP}]':
					if (dhcp_dynamic_exists) {
						...
						// We found a free address.
						if (found_free_ipv4) {
							ip_messages(2, 0);
							return;
						}

						// All addresses are already taken.
						ip_messages(1, 3);
						return;
					} else {
						if(true == true){
						...
						if (!used_ips.in_array(ipv4_long)) {
							ip_messages(2, 0);
							return
						}
					}
		...
		var net_assigned_v4 = function () {
			new Ajax.Request('?ajax=1&call=net', {
				...
				onSuccess: function (transport) {
					...
					if (json)
					{
						$('C__CATG__IP__NET__VIEW').value = json.object_browser_title;
						...
						// Set DNS Server.
						var dns_server_ids = $('C__CATG__IP__ASSIGNED_DNS_SERVER__HIDDEN').value.evalJSON();
						var dns_server_view = $('C__CATG__IP__ASSIGNED_DNS_SERVER__VIEW').value + ', ';
						...
						$('C__CATG__IP__ASSIGNED_DNS_SERVER__VIEW').value = dns_server_view.replace(/^[,\s]+/, '').replace(/[,\s]+$/, '');

						// Object for multiselectbox
						var $dns_domain = $('C__CATP__IP__DNS_DOMAIN');

						if ($F('old_net_id') != $net_selection_hidden.getValue()) {
							$dns_domain.setValue([]);

							if ($net_type.disabled) {
								var selected_dns_domains = [];

								json.assigned_dns_domain.each(function (item) {
									selected_dns_domains.push(item.value);
								});
			...
			new Ajax.Request('?ajax=1&call=net',
				{
					...
					onSuccess: function(transport) {
						...
						if (json) {
							...
							$('old_net_id').value = $net_selection_hidden.getValue();
						}
					}
				});
		};
		...
		var net_find_free_ipv4 = function () {
			...
			new Ajax.Request('?call=calc_ip_address&func=find_free_v4&ajax=1', {
				parameters: {
					...
					'ip_assignment': $('ipv4_assign').value
				},
				method: 'post',
				onSuccess: function (transport) {
			...
			new Ajax.Request('?call=ipv6&ajax=1',
				{
					parameters: {
						'method': 'find_free_v6',
						'net_obj_id': ((! $net_selection_hidden.getValue().blank()) ? $net_selection_hidden.getValue() : '[{$smarty.const.C__OBJ__NET_GLOBAL_IPV6}]'),
						'ip_assignment': $('C__CMDB__CATG__IP__IPV6_ASSIGNMENT').value
					},
					method: 'post',
					onSuccess: function(transport){
						var json = transport.responseJSON;
						...
						if (json !== false) {
							$('C__CMDB__CATG__IP__IPV6_ADDRESS').value = json;
		...
		var validate_ipv6 = function (){
			...
			el.setStyle({background:''})
				.previous('img')
				.writeAttribute({src: '[{$dir_images}]empty.gif', title: ''});
			isys_glob_enable_save();
			...
			if (checkipv6(el.value)) {
						...
						onSuccess: function(transport) {
							...
							if (! inside_net_range) {
								el.setStyle({background:'#f99'})
									.previous('img')
									.writeAttribute({src: '[{$dir_images}]icons/infoicon/error.png', title: '[{isys type="lang" ident="LC__CMDB__CATG__IP__IPV6_ADDRESS_OUTSIDE_RANGE" p_bHtmlEncode=false}]'});
								isys_glob_disable_save();
								return;
							}
				...
				el.setStyle({background:'#f99'})
					.previous('img')
					.writeAttribute({src: '[{$dir_images}]icons/infoicon/error.png', title: '[{isys type="lang" ident="LC__UNIVERSAL__FIELD_VALUE_IS_INVALID" p_bHtmlEncode=false}]'});
				isys_glob_disable_save();
			}
		};
		...
		var ip_messages = function(msg_level, error_level) {
			...
			if ($ip_message) {
				$ip_message.update(message).setStyle({backgroundColor: color}).show().highlight();
			} else {
				$ip_message.update().hide();
			}
		};
		...
		var is_ipv4_free = function (ipv4) {
			new Ajax.Request('?call=calc_ip_address&func=is_free_v4&ajax=1', {
				...
				onSuccess: function (transport) {
					...
					if (json.net)
					{
						$('C__CATG__IP__NET__HIDDEN').value = json.net.netID;

						// At first, we take a look of which type we are currently editing.
						if ($('C__NET__TYPE').value == '[{$smarty.const.C__CATS_NET_TYPE__IPV4}]')
						{
							net_assigned_v4();
						}
					}
		...
		[{if isys_glob_is_edit_mode()}]
			...
			idoit.callbackManager.registerCallback('validate_ip', function () {
				...
				if ($net_selection_hidden.getValue() > 0) {
					...
					if ($('C__NET__TYPE').value == '[{$smarty.const.C__CATS_NET_TYPE__IPV6}]') {
						net_assigned_v6();
					}
			...
			if ($net_type) {
				$net_type.on('change', function () {
					var p_index = $net_type.getValue();

					$('contentBottomContent').select('.form').invoke('hide');
					$('form_' + p_index).show();

					if (p_index == '[{$smarty.const.C__CATS_NET_TYPE__IPV4}]' || p_index == '[{$smarty.const.C__CATS_NET_TYPE__IPV6}]') {
						$('ip_additional').show();
						if (p_index == '[{$smarty.const.C__CATS_NET_TYPE__IPV4}]') {
							if ($net_selection_hidden.getValue() == '[{$smarty.const.C__OBJ__NET_GLOBAL_IPV6}]') {
								$net_selection_hidden.setValue('[{$smarty.const.C__OBJ__NET_GLOBAL_IPV4}]');
								$('C__CATG__IP__NET__VIEW').value = global_net_ipv4_title;
							}
							net_assigned_v4();
						} else if (p_index == '[{$smarty.const.C__CATS_NET_TYPE__IPV6}]') {
							if ($net_selection_hidden.getValue() == '[{$smarty.const.C__OBJ__NET_GLOBAL_IPV4}]') {
								$net_selection_hidden.setValue('[{$smarty.const.C__OBJ__NET_GLOBAL_IPV6}]');
								$('C__CATG__IP__NET__VIEW').value = global_net_ipv6_title;
							}
							net_assigned_v6();
						}
					} else {
			...
			var $dns_input = $('C__CATP__IP__DNS_DOMAIN'),
			...
			idoit.callbackManager
				...
				.triggerCallback('cmdb-catg-ip-dns_domain-update', $F('C__CATP__IP__DNS_DOMAIN'));

<div class="p10">
	<div class="mt10 p10" style="background:#eee url('[{$dir_images}]gradient.png') repeat-x; border:1px solid #B6BABF; -moz-border-radius:4px; -webkit-border-radius:4px;overflow-x: auto">

		<div class="toolbar fr">
			[{isys type="lang" ident="LC__CATG__CLUSTER_SHARED_VIRTUAL_SWITCH__UNUSED"}]
			<button type="button" class="btn" onclick="idoit.callbackManager.triggerCallback('cluster_shared_virtual_switch__show_unused');"><img src="[{$dir_images}]icons/silk/lightbulb.png" class="mr5" /><span>[{isys type="lang" ident="LC__UNIVERSAL__SHOW"}]</span></button>
			<button type="button" class="btn" onclick="idoit.callbackManager.triggerCallback('cluster_shared_virtual_switch__hide_unused');"><img src="[{$dir_images}]icons/silk/lightbulb_off.png" class="mr5" /><span>[{isys type="lang" ident="LC__UNIVERSAL__HIDE"}]</span></button>
		</div>
		<div class="cb"></div>
		...
		<div style="overflow-x: auto;position:relative;" id="matrix_scroller" class="mt10">
			<table class="sanmatrix" cellspacing="0" cellpadding="0" >
				...
				[{foreach from=$c_members item="cm"}]
					...
					<td id="content_[{$obj_id}]_[{$san_id}]" class="center [{if isset($coords[$obj_id][$san_id])}]gradient" style="background-color:#efefef" title="Virtueller Switch: [{$coords[$obj_id][$san_id].isys_catg_virtual_switch_list__title}]"[{/if}]">

[{if $net}]
	...
	<script type="text/javascript">
		...
		window.calc_subnet_mask_by_address_amount = function (amount) {
			for (var i in subnet_masks) {
	            // console.log(i + ' > ' + subnet_masks[i]);
	        }
	    };
		...
		$$('table#ip-table tr').each(function (el, i) {
			if ((i%2) == 0) { // el.hasClassName('subnet')
	    ...
	    window.add_new_layer3_net = function () {
		    ...
		    var td = this.hide().up(),
				tr = td.up(),
				available_net_masks = window.calc_subnet_mask_by_address_amount(tr.readAttribute('data-address-amount'));

		    // console.log(available_net_masks);
		    ...
		    td.insert(
			    new Element('div', {className:'new-net-form'})
				    ...
				    .insert(new Element('select', {name:'new_net_cidr', id:'new_net_cidr', className:'inputDialog verysmall', value:td.up().readAttribute('data-to')}))
		    );

	        $('new_net_ip').on('change', function () {
	            if (validate_ipv4(this.value)) {

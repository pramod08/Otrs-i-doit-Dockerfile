<table class="contentTable">
	...
	<tr>
		...
		<td class="value">
			<div class="ml20">
				...
				<label><input type="radio" name="CONTACT_NAME_SELECTION" value="[{$smarty.const.C__NAGIOS__PERSON_OPTION__OBJECT_TITLE}]" [{if $contact_name_selection == $smarty.const.C__NAGIOS__PERSON_OPTION__OBJECT_TITLE}]checked="checked"[{/if}] /> [{isys type="lang" ident="LC__UNIVERSAL__OBJECT_TITLE"}] ("<span class="grey">[{$obj_title}]</span>")</label><br />
				<label><input type="radio" name="CONTACT_NAME_SELECTION" value="[{$smarty.const.C__NAGIOS__PERSON_OPTION__USERNAME}]" [{if $contact_name_selection == $smarty.const.C__NAGIOS__PERSON_OPTION__USERNAME}]checked="checked"[{/if}] /> [{isys type="lang" ident="LC__CONTACT__PERSON_USER_NAME"}] ("<span class="grey">[{$user_name}]</span>")</label><br />
				<input type="radio" name="CONTACT_NAME_SELECTION" value="[{$smarty.const.C__NAGIOS__PERSON_OPTION__INPUT}]" [{if $contact_name_selection == $smarty.const.C__NAGIOS__PERSON_OPTION__INPUT}]checked="checked"[{/if}] /> [{isys type="f_text" name="CONTACT_NAME"}]
				[{else}]
				[{$contact_name}]
				[{/if}]
			</div>
    ...
    <tr>
        ...
        <td class="value">
            ...
            [{if isys_glob_is_edit_mode()}]<input type="radio" name="CONTACT_HOST_NOTIFICATION_PERIOD_SELECTION" disabled="disabled" />[{/if}]
            [{isys type="f_dialog" name="CONTACT_HOST_NOTIFICATION_PERIOD"}]<br />
    ...
    <tr>
        ...
        <td class="value">
            <img src="[{$dir_images}]empty.gif" width="20" height="1" />
            [{if isys_glob_is_edit_mode()}]<input type="radio" name="CONTACT_SERVICE_NOTIFICATION_PERIOD_SELECTION" disabled="disabled" />[{/if}]
            [{isys type="f_dialog" name="CONTACT_SERVICE_NOTIFICATION_PERIOD"}]<br />
    ...
    <tr>
        ...
        <td class="value">[{isys type="f_dialog_list" name="HOST_NOTIFICATION_OPTIONS" p_bLinklist="1" p_bDialogMode=true}]</td>
    </tr>
    <tr>
        <td class="key">[{isys type="f_label" ident="service_notification_options" name="SERVICE_NOTIFICATION_OPTIONS"}]</td>
        <td class="value">[{isys type="f_dialog_list" name="SERVICE_NOTIFICATION_OPTIONS" p_bLinklist="1" p_bDialogMode=true}]</td>
    </tr>
    <tr>
        <td class="key">[{isys type="f_label" ident="host_notification_commands" name="CONTACT_HOST_NOTIFICATION_COMMANDS"}]</td>
        <td class="value">[{isys type="f_dialog_list" name="CONTACT_HOST_NOTIFICATION_COMMANDS" p_bLinklist="1" p_bDialogMode=true}]</td>
    </tr>
    <tr>
        <td class="key">[{isys type="f_label" ident="service_notification_commands" name="CONTACT_SERVICE_NOTIFICATION_COMMANDS"}]</td>
        <td class="value">[{isys type="f_dialog_list" name="CONTACT_SERVICE_NOTIFICATION_COMMANDS" p_bLinklist="1" p_bDialogMode=true}]</td>
    </tr>
    <tr>
...
<script>
    ...
    if (host_notification_period && host_notification_period_plus) {
	    ...
	    if ($F(host_notification_period) > 0) {
		    host_notification_period.previous('input[type="radio"]').checked = true;
	    }

	    if ($F(host_notification_period_plus) > 0) {
		    host_notification_period_plus.previous('input[type="radio"]').checked = true;
	    }
    }
    ...
    if (service_notification_period && service_notification_period_plus) {
	    ...
	    if ($F(service_notification_period) > 0) {
		    service_notification_period.previous('input[type="radio"]').checked = true;
	    }

	    if ($F(service_notification_period_plus) > 0) {
		    service_notification_period_plus.previous('input[type="radio"]').checked = true;
	    }
    }
</script>
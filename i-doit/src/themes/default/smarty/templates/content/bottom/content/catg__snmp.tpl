			...
			<tr>
				<td>
					[{if isys_glob_is_edit_mode()}]
						<input type="text" name="C__CATG__SNMP_OID_TITLES[]" value="[{$key}]" class="inputText small" />
					[{else}]
						[{$key}]
					[{/if}]
				</td>
				<td>
					[{if isys_glob_is_edit_mode()}]
						<input type="text" name="C__CATG__SNMP_OIDS[]" value="[{$oid}]" placeholder="OID" class="inputText" />
					[{else}]
						[{if $has_primary}]
		...
		idoit.callbackManager.registerCallback('snmp__add_oid', function() {
			...
			$('oids').down('tbody').insert(
				new Element('tr').update(
						new Element('td').update(new Element('input', {type:'text', name: 'C__CATG__SNMP_OID_TITLES[]', className: 'inputText small', value: oid_counter}))
					).insert(
						new Element('td').update(new Element('input', {type:'text', name: 'C__CATG__SNMP_OIDS[]', placeholder:"OID", className: 'inputText'}))
					));
		});
	}());
</script>
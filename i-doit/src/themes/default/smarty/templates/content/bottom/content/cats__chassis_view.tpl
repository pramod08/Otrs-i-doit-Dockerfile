<table class="contentTable mb10">
	...
	<tr id="matrix-size-notice">
		<td colspan="2">
			<p class="red">* [{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__MATRIX_SIZE_CHANGE_NOTICE"}]</p>
		</td>
	</tr>
</table>
		...
		<td class="vat">
			...
			<div id="slot_options" class="box">
				...
				<div class="m5">
					...
					<p id="option-assign-description" class="m5 p5 note"><img src="[{$dir_images}]icons/silk/information.png" alt="Info" class="vam" /> [{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__DESCRIPTION"}]</p>
...
<script type="text/javascript">
	...
	window.change_gridsize = function () {
        ...
        $('chassis_front').down('table').setStyle({width: (x*window.chassis_front.get_size_in_pixel()) + 'px'});
        ...
        $('chassis_rear').down('table').setStyle({width: (x*window.chassis_rear.get_size_in_pixel()) + 'px'});
	};
        ...
        new Ajax.Request('?ajax=1&call=chassis&func=get_assigned_devices',
                {
                    ...
                    onSuccess: function (transport) {
                        ...
                        {
                            div_container = new Element('div', {'class' : 'error p5'}).update(json.message);
                            $('assigned_devices').insert(div_container);
                        }
                    }
                });
	...
	window.save_slot_position = function () {
		...
		if (checkboxes.length == 1) {
			...
			$('notification').removeClassName('note').addClassName('error').update('Bitte wählen Sie mindestens ein Feld aus.').show();
			return;
		}
		...
		new Ajax.Request('?ajax=1&call=chassis&func=assign_slot_position',
			{
				...
				onSuccess:function (transport) {
					...
					// Remove the "error" and "note" classes and add new ones + the message.
					$('notification')
		...
		new Ajax.Request('?ajax=1&call=chassis&func=remove_slot_position',
			{
				...
				onSuccess:function (transport) {
					...
					if (slot_action != 'option-reassign' && slot_action != 'slot-option-reassign') {
						// Remove the "error" and "note" classes and add new ones + the message.
						$('notification').removeClassName('note').removeClassName('error').addClassName(notification_class).update(json.message).show();
						window.reset_slot_options();
					}
				}
			});
	...
	window.collision_callback = function (collision) {
		if (collision === true) {
			$('option-save').hide();
			$('notification').addClassName('error').update('[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_SLOTS__ALREADY_ASSIGNED"}]').show();
		} else {
			$('option-save').show();
			$('notification').hide().update().removeClassName('error');
		}
	};

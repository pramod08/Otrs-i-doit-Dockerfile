<div class="p10">
	...
	<div class="mt10 p10" style="background:#eee url('[{$dir_images}]gradient.png') repeat-x; border:1px solid #B6BABF; -moz-border-radius:4px; -webkit-border-radius:4px;overflow-x: auto">
		...
		<div class="toolbar fr">
			...
			<button type="button" class="btn" onclick="$$('.sanmatrix .hidden').invoke('show');"><img src="[{$dir_images}]icons/silk/lightbulb.png" class="mr5" /><span>[{isys type="lang" ident="LC__UNIVERSAL__SHOW"}]</span></button>
	...
	[{if count($sanlist) > 0 && count($c_members) > 0}]
				...
				[{foreach from=$sanlist item="s" key="san_id"}]
					...
					<tr id="san_row_[{$san_id}]">
						[{foreach from=$c_members item="cm"}]
							...
							<td class="center minheight [{if isset($coords[$obj_id][$san_id])}]gradient" style="background-color:#efefef;" title="LDEV Client: [{$coords[$obj_id][$san_id].isys_catg_ldevclient_list__title}][{/if}]">
								<label>

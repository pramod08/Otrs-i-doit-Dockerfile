<div class="p10">
	<div class="p10" style="background:#eee url('[{$dir_images}]gradient.png') repeat-x; border:1px solid #B6BABF; -moz-border-radius:4px; -webkit-border-radius:4px;overflow-x: auto">

	[{if (count($cluster_service_list) > 0 || count($virtual_machine_list) > 0) && count($c_members) > 0}]
		<div class="toolbar fr">
			<a class="bold" href="javascript:" onclick="idoit.callbackManager.triggerCallback('cluster_vitality__member_click', '', $F('services_ids_arr'), $F('vms_ids_arr'))">[{isys type="lang" ident="LC__UNIVERSAL__SHOW"}]/[{isys type="lang" ident="LC__UNIVERSAL__HIDE"}]</a>
		...
		<div style="overflow-x:auto;" id="matrix_scroller">
			...
			[{foreach from=$cluster_service_list item="s" key="cluster_service_id"}]
				...
				[{foreach from=$c_members item="cm"}]
					...
					<td style="vertical-align: top;" class="maxheight [{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_active) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_aktiv [{/if}]
											[{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_passive) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_passiv [{/if}]
											[{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_hpc) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_hpc [{/if}]">
				...
				[{foreach from=$c_members item="cm"}]
					...
					<td style="vertical-align: top;" class="[{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_active) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_aktiv [{/if}]
											[{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_passive) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_passiv [{/if}]
											[{if ($coords_two[$s_id][$obj_id].cluster_type == $cluster_type_hpc) && isset($coords_two[$s_id][$obj_id])}] clusterdienst_hpc [{/if}] minheight">
						<p style="text-align:center; margin-top:3px;"><strong>[{isys type="lang" ident=$coords_two[$s_id][$obj_id].cluster_type}]</strong></p>
					</td>

...
[{if count($g_categories) > 0}]
	...
	[{foreach from=$g_categories item='l_cat' name="overview_loop"}]
		...
		[{if !empty($l_cat.id) && $l_cat.id != $smarty.const.C__CATG__OVERVIEW && $auth->is_allowed_to($auth_view_id, "OBJ_ID/$obj_id")}]
			...
			[{if is_object($l_cat.dao)}]
				[{if method_exists($l_cat.dao, "get_ui")}]
					...
					<fieldset id="[{$l_cat.dao|get_class}]" data-list-id="[{$l_cat.dao->get_list_id()}]" class="overview[{if !$l_cat.dao->get_validation()}] exception[{/if}]" style="[{if !$l_cat.dao->get_validation()}]border:1px solid #ff0000;[{/if}]">
						<legend><span[{if $smarty.foreach.overview_loop.first}] style="border-top:0;"[{/if}]>[{$l_cat.title}]</span></legend>
							...
							[{if ($l_cat.multivalued == 0) || ($g_navmode == $smarty.const.C__NAVMODE__NEW || $g_navmode == $smarty.const.C__NAVMODE__SAVE) || $show_detail || ($smarty.const.C__CATG__IP == $l_cat_id)}]
                                ...
                                [{* Check authorization if we are in editMode *}]
								[{if ($smarty.get.editMode == 1 || $g_navmode == $smarty.const.C__NAVMODE__NEW) && !$auth->has_rights_in_obj_and_category($auth_edit_id, $obj_id, $cat_const)}]
                                <p class="exception p5">[{isys type=lang ident="LC__AUTH__CMDB_EXCEPTION__OVERVIEW_CATEGORY_NOT_EDITABLE"}]</p>

                                <script type="text/javascript">
                                    /* Add FieldSet-ID to our array to deactivate it later */
                                    g_unauth_categories.push('[{$l_cat.dao|get_class}]');
                                </script>
                                [{/if}]
							[{/if}]
	...
	[{/foreach}]
    <script type="text/javascript">
        /* Disable marked input-fields to prevent unauthorized editing. */
        if (g_unauth_categories.length) {
	        for (var i = 0; i < g_unauth_categories.length; i++) {
		        $$('#' + g_unauth_categories[i] + ' input,#' + g_unauth_categories[i] + ' select, #' + g_unauth_categories[i] + ' textarea, #' + g_unauth_categories[i] + ' div[contenteditable]').each(function (e) {
			        // WYSIWYG-Disabling.
			        e.removeAttribute('contenteditable');

			        // input,select, textarea disableing.
			        if (typeof e.disable === 'function') {
				        e.disable()
			        }
		        });
	        }
    ...
    [{if ($displayed_catgs == 0)}]
        ...
        <p class="exception p5">[{isys type="lang" ident="LC__AUTH__CMDB_EXCEPTION__EMPTY_OVERVIEW_PAGE"}]</p>
    [{/if}]
[{else}]
	<p class="p10">
		...
		<br /><br />
		Quicklink:<br />
		<a href="?objGroupID=[{$smarty.get.objGroupID|default:2}]&viewMode=[{$smarty.const.C__CMDB__VIEW__CONFIG_OBJECTTYPE}]&tvMode=[{$smarty.const.C__CMDB__VIEW__TREE_OBJECTTYPE}]&objTypeID=[{$smarty.get.objTypeID}]">[{isys type="lang" ident="LC__CMDB__OBJTYPE__CONFIGURATION_MODUS"}]</a>
	</p>
[{/if}]

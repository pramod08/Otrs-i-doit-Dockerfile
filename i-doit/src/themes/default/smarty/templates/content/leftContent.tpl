...
<script type="text/javascript">
    ...
    $$('#dmenu_tree0 div.clip div.node a').each(function (el) {
	    var dir_anchor;
        if (typeof el.down('span.noentries') == 'undefined') {

	        // We found a filled sub-category, now to go back to the "directory"-element and remove it's "noentries" class.
            dir_anchor = el.up('div.clip').previous().down('a span');

	        if (dir_anchor && dir_anchor.hasClassName('noentries')) {
                var dir_text = dir_anchor.innerHTML;

                dir_anchor.up('div.node').show();
                dir_anchor.up().update(dir_text);
            }
        }
    });
</script>
var AuthConfiguration = Class.create({
	...
	initialize: function (element, options) {
		...
		for (var i in this.options.rights) {
			if (this.options.rights.hasOwnProperty(i)) {
				thead_row.insert(new Element('th', {style:'width:25px;'}).update(
					new Element('img', {src:'[{$dir_images}]' + this.options.rights[i].icon, alt:this.options.rights[i].title, title:this.options.rights[i].title})
				));
			}
		}
		...
		thead_row.insert(
				new Element('th', {style:'width:' + calculated_size + 'px;'}).update(idoit.Translate.get('LC__AUTH_GUI__CONDITION'))
			).insert(
				new Element('th', {style:'width:' + calculated_size + 'px;'}).update(idoit.Translate.get('LC__AUTH_GUI__PARAMETER'))
			).insert(
		...
		for (i in this.options.inherited_paths) {
			if (this.options.inherited_paths.hasOwnProperty(i)) {
				...
				for (var i2 in this.options.inherited_paths[i]) {
		...
		for (i in this.options.paths) {
			if (this.options.paths.hasOwnProperty(i)) {
				// Now check the next right-level.
				for (var i2 in this.options.paths[i]) {
	...
	display_path: function (method, param, rights, inherited) {
		var i,
			...
			options = new Element('select', {id: this.element_id + '_method_' + this.path_counter, name: 'method_' + this.path_counter, className: 'input input-small method-select', disabled: !this.options.edit_mode}),
            innerTD = '';
		...
		for (i in this.options.rights) {
			if (this.options.rights.hasOwnProperty(i)) {
				...
				if (this.options.rights[i].value == 1) {
					...
					if (Prototype.Browser.IE7) {
						tr.down('input:last').checked = true;
					}
				} else {
					tr.insert(new Element('td', {className: 'center'}).update(new Element('input', {type: 'checkbox', disabled: !this.options.edit_mode, className: 'right-checkbox', name: 'right_' + this.path_counter + '[]', value: this.options.rights[i].value, checked: rights.in_array(parseInt(this.options.rights[i].value))})));

					// Special treatment for IE7.
					if (Prototype.Browser.IE7 && rights.in_array(parseInt(this.options.rights[i].value))) {
						tr.down('input:last').checked = true;
					}
				}
			}
		...
		if (!param.blank()) {
			if ((this.options.edit_mode == 0 || inherited) && param == '*') {
				...
				innerTD = new Element('td', {id: this.element_id + '_param_' + this.path_counter}).update(new Element('span', {className:'fl mr5'}).update(idoit.Translate.get('LC__UNIVERSAL__ALL')));
	            ...
	            innerTD = new Element('td', {id: this.element_id + '_param_' + this.path_counter});
	            innerTD
	                .insert(new Element('img', {className: 'vam mr5', src:'[{$dir_images}]ajax-loading.gif'}))
		...
		tr
			...
			.insert(new Element('td')
				.update((this.options.edit_mode && ! inherited) ? new Element('button', {className: 'btn btn-small remove-path-button', type: 'button', title: idoit.Translate.get('LC__UNIVERSAL__REMOVE')}).update(new Element('img', {src:'[{$dir_images}]icons/silk/cross.png'})) : ''));
	...
	create_new_path: function () {
		var i,
			tr = new Element('tr', {id: this.element_id + '-row-' + this.path_counter, 'data-counter': this.path_counter}),
			options = new Element('select', {id: this.element_id + '_method_' + this.path_counter, name: 'method_' + this.path_counter, className: 'input input-small method-select'});
		...
		for (i in this.options.rights) {
			if (this.options.rights.hasOwnProperty(i)) {
				...
				if (this.options.rights[i].value == 1) {
					...
					tr.insert(new Element('td', {className: 'center'}).update(new Element('input', {type: 'checkbox', className: 'right-checkbox', name: 'right_' + this.path_counter + '[]', value: this.options.rights[i].value, checked: false})));
				}
			}
		}
		...
		tr
			.insert(new Element('td').update(new Element('span', {className: 'mr5'}).update(idoit.Translate.get('LC__AUTH_GUI__REFERS_TO'))).insert(options))
			.insert(new Element('td', {id: this.element_id + '_param_' + this.path_counter})
				.update(new Element('img', {className: 'vam', src:'[{$dir_images}]ajax-loading.gif'}))
				.insert(new Element('span', {className: 'vam'}).update(idoit.Translate.get('LC__UNIVERSAL__LOADING'))))
			.insert(new Element('td')
				.update(new Element('button', {className: 'btn btn-small remove-path-button', type: 'button', title: idoit.Translate.get('LC__UNIVERSAL__REMOVE')}).update(new Element('img', {src: '[{$dir_images}]icons/silk/cross.png'}))));
		...
		new Ajax.Request(this.options.ajax_url + '&func=retrieve_parameter',
			{
				...
				onComplete: function (r) {
					...
					if (json && json.success) {
						...
						$(element).update(new Element('p', {className:'p5 error'}).update(json.message || r.responseText));
					}
				}.bind(this)
			});
	...
	display_parameter: function (element, json, inherited) {
		var el = $(element),
			...
			last_checked_checkbox = el.up('tr').down('input.right-checkbox:checked:last');
		...
		if (this.options.edit_mode == 0 || inherited) {
            ...
            temp_obj = new Element('div')
                .insert(new Element('button', {id: 'auth_param_button_' + json.data.counter, type: 'button', className: 'fl mr5 btn param-button' + (button_active ? ' btn-green' : '')})
                    .update(new Element('span').update(idoit.Translate.get('LC__UNIVERSAL__ALL'))))
                .insert(new Element('input', {id: 'auth_param_button_val_' + json.data.counter, name: 'auth_param_button_val_' + json.data.counter, type: 'hidden', value: (button_active ? '1' : '0')}));
		...
		el.update((json.data.param == this.options.empty_id) ? '' : button)
			.insert(new Element('div', {className: 'fl', style: 'position:relative;'})
				.update(new Element('div', {className: 'param-hider', style: 'position:absolute;width:100%;height:100%;background:#fff;opacity:0.5;z-index:100;' + ((json.data.param == this.options.wildchar) ? 'display:block;' : 'display:none;')}))
		...
		if (this.options.edit_mode == 1) {
			...
			if (last_checked_checkbox) {
				last_checked_checkbox.simulate('change');
		    }
	...
	change_method: function (ev) {
		var counter = ev.findElement().up('tr').readAttribute('data-counter'),
	...
	change_rights: function (method, cnt) {
		...
		checkboxes.each(function (el) {
			if (!available_rights.in_array(parseInt(el.value))) {
				...
				el.writeAttribute('data-not-available', 1).disable().setValue('');
			}

			if (default_rights.in_array(parseInt(el.value)) && !el.match('[data-not-available="1"]')) {
				el.setValue(1);
			}
		});
	...
	update_parameter_state: function (ev) {
		...
		if (el.hasClassName('btn-green')) {
			...
			if (use_hider) {
                ...
                div.next('input,select').enable().fire('chosen:updated');
            }
		} else {
			...
			if (use_hider) {
                ...
                div.next('input,select').disable().fire('chosen:updated');
            }
		}
	},
	...
	set_observer: function () {
		...
		if (Prototype.Browser.IE && Prototype.Browser.IEVersion < 9) {
			...
			this.element.on('click', 'select.method-select', this.change_method.bindAsEventListener(this));
		} else {
	...
	update_rights: function (ev) {
		...
		if (value == 2048) {
			...
			row.select('input.right-checkbox').each(function($checkbox) {
				if ($checkbox.match('[data-not-available="1"]')) {
					$checkbox.disable().setValue(0);
				} else {
					if (el.checked) {
						$checkbox.disable().setValue(1);
					} else {
						$checkbox.enable().setValue(0);
					}
				}
			});
		}

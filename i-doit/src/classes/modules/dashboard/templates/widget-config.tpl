...
<div id="widget-config-popup">
	[{isys type="f_dialog" id="widget-config-dialog" p_strClass="normal" p_arData=$widget_selection p_bDbFieldNN=true p_bInfoIconSpacer=0}]
	<button type="button" id="widget-config-add-button" class="btn btn-small ml5">
	...
	<table class="two-col mb10">
		<tr>
			...
			<td class="vat">
				...
				[{isys name="overwrite-other-contacts" type="f_popup" p_strPopupType="browser_object_ng" catFilter="C__CATS__PERSON" p_strClass="mt5" p_bInfoIconSpacer=0 multiselection=true p_strStyle="width:250px;"}]<br />
...
<script type="text/javascript">
	...
	if (define_standard_button) {
		define_standard_button.on('click', function () {
			...
			var result_container = $('define-standard-result').removeClassName('note').removeClassName('exception').addClassName('hide');
			...
			new Ajax.Request('[{$define_default_ajax_url}]', {
				...
				onSuccess: function (response) {
					...
					if (json.success) {
						result_container.addClassName('note').update('[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_DEFAULT__SAVED"}]').removeClassName('hide');
					} else {
						result_container.addClassName('exception').update(json.message).removeClassName('hide');
					}
				}.bind(this)
			});
		});
	...
	if (overwrite_others_button) {
		overwrite_others_button.on('click', function () {
			...
			var result_container = $('overwrite-others-result').removeClassName('note').removeClassName('exception').addClassName('hide');
			...
			new Ajax.Request('[{$overwrite_dashboard_ajax_url}]', {
				...
				onSuccess: function (response) {
					...
					if (json.success) {
						result_container.addClassName('note').update('[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_OTHERS__SAVED"}]').removeClassName('hide');
					} else {
						result_container.addClassName('exception').update(json.message).removeClassName('hide');
					}
				}.bind(this)

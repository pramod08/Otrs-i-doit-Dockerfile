...
class isys_module_dashboard extends isys_module implements isys_module_interface, isys_module_authable
{
    ...
    {
        $l_cmdb_module = new isys_module_cmdb();
        $l_cmdb_module->build_tree($p_tree, $p_system_module, $p_parent);
    } // function
    ...
    public function process_view()
    {
        ...
        $template->assign(
            'dashboard',
            $template->fetch(self::get_tpl_dir() . 'main.tpl')
        )
    ...
    public function load_user_dashboard($p_user_id = null)
    {
        ...
        if ($l_res->num_rows() > 0)
        {
            ...
            while ($l_row = $l_res->get_row())
            {
                ...
                if (empty($l_row['isys_widgets_config__configuration']))
                {
                ...
                $this->add_widget(
                    [
                        ...
                        'config'       => isys_glob_htmlentities($l_config),

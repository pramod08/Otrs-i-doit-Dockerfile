<div id="widget-config-popup">
	<table class="contentTable">
		<tr>
			...
			<td class="value">
				[{isys type="f_text" id="widget-config-title" p_strClass="normal" p_strValue=$rules.title}]
			</td>
		</tr>
		<tr>
			<td class="key">
	...
	<table class="contentTable" style="border-top: none;">
		<tr>
			...
			<td class="value">
				[{isys type="f_text" id="event-title" p_strClass="normal"}]
			</td>
		</tr>
		<tr>
			<td class="key">
			...
			<td class="value">
				[{isys type="f_popup" name="event-date" p_strPopupType="calendar" p_bTime="0" p_strClass="small" p_strPlaceholder="dd.mm.yyyy"}]
			</td>
		</tr>
		<tr>
			<td class="key">
			...
			<td class="value">
				[{isys type="f_dialog" name="event-type" p_arData=$event_types p_strClass="normal" p_bDbFieldNN=true}]
			</td>
		</tr>
		<tr>
			<td></td>
...
//				'holiday_events':$('widget-popup-config-holiday-events').checked
	...
	$('event-add-button').on('click', function () {
		$('event-title', 'event-date__VIEW').invoke('up', 'tr').invoke('removeClassName', 'error');

		if ($F('event-title').blank()) {
			$('event-title').up('tr').addClassName('error');
			return;
		}

		var date_regex = new RegExp(/(\d\d|\d|\*)\.(\d\d|\d|\*)\.(\d\d\d\d|\d\d|\*)/);

		if ($F('event-date__VIEW').blank() || ! date_regex.test($F('event-date__VIEW'))) {
			$('event-date__VIEW').up('tr').addClassName('error');
			return;
		}

...
class isys_cmdb_view_tree_location extends isys_cmdb_view_tree
{
    ...
    public function tree_build()
    {
        ...
        {
            if (in_array(
                $l_user_locale->get_setting('tree_type'),
                [
            ...
            ))
            {
                if (!empty($l_objid))
                {
                    $l_dao_loc   = ($l_user_locale->get_setting('tree_type') == C__CMDB__VIEW__TREE_LOCATION__LOGICAL_UNITS) ? new isys_cmdb_dao_category_g_logical_unit(
                        $l_db
                    ) : new isys_cmdb_dao_location($l_db);

...
class isys_cmdb_view_list_object extends isys_cmdb_view_list
{
        ...
        switch ($p_navmode)
        {
            ...
            case C__NAVMODE__EDIT:
                ...
                if ($l_objid)
                {
                    ...
                    $l_gets[C__CMDB__GET__CATG]     = ($l_overview == 1) ? C__CATG__OVERVIEW : C__CATG__GLOBAL;
    ...
    public function list_process()
    {
        ...
        $l_load_complete_list = $l_gets['load_all'];

        // Here we check, if the "load complete list" configuration has been set.
        if (!$l_load_complete_list)
        {
            $l_load_complete_list = isys_tenantsettings::get('cmdb.registry.show_full_lists', 0);
        } // if

        $this->m_dao_list->set_load_all(!!$l_load_complete_list);
        ...
        $l_edit_onclick = "var checks = window.object_list.options.checkedBoxes;" . "if (checks.length > 1) { " . "document.location='?" . C__CMDB__GET__VIEWMODE . "=" .
            C__CMDB__VIEW__MULTIEDIT . "&" . C__CMDB__GET__CATG . "=" . C__CATG__GLOBAL . "&preselect=' + Object.toJSON(checks); " . "} else if (checks.length == 1) {" .
            "document.isys_form.navMode.value='" . C__NAVMODE__EDIT . "'; " . "get_tree_by_object(checks[0], '" . C__CMDB__VIEW__TREE_OBJECT . "'); " .
            "form_submit('?viewMode=" . C__CMDB__VIEW__CATEGORY . "&objTypeID=" . $this->m_id . "&objID='+checks[0]+'" . "&call=category" . "&catgID=" .
            C__CATG__OVERVIEW . "'); }";
        ...
        $l_tpl->assign('list_display', true)
            ->assign("content_title", _L($this->m_dao_cmdb->get_objtype_name_by_id_as_string($l_objtypeid)))
        ...
        try
        {
            ...
            if ($_POST[C__GET__NAVMODE] == C__NAVMODE__EXPORT_CSV)
            {
                ...
                {
                    isys_notify::warning(_L('LC__UNIVERSAL__CHOOSEN_EMPTY'));
                } // if
            } // if
            ...
            if ($l_objtypeid == C__OBJTYPE__RELATION || $l_objtypeid == C__OBJTYPE__PARALLEL_RELATION)
            {
        ...
        $l_preload_objects   = isys_glob_get_pagelimit() * $this->m_dao_list->get_preload_pages();
        ...
        if ($l_obj_type_rows > $l_preload_objects && !$l_load_complete_list)
        {
            ...
            $l_load_all = '<div class="info m5 p5 bold"><img src="' . $g_dirs['images'] . 'icons/infobox/blue.png" class="vam" /> <span class="vam">' . _L(
                    'LC__CMDB__LIST_MORE_ITEMS_NOTICE'
                ) . ' - <a href="javascript:" id="load_all_button">' . _L('LC__CMDB__LIST_LOAD_ALL') . ' (' . $l_obj_type_rows . ' ' . _L(
                    'LC__CMDB__CATG__OBJECT'
                ) . ')</a></span></div>';
        } // if
        ...
        if ($l_jsonresult != '[]')
        {
            ...
            if (isys_auth_cmdb::instance()->is_allowed_to(isys_auth::EXECUTE, 'list_config'))
            {
                ...
                $l_list_config = '<a href="' . $l_list_config_url . '">' .
                    '<img width="15px" height="15px" title="' . _L('LC__CMDB__LIST_CONFIGURE') . '" alt="' . _L('LC__CMDB__LIST_CONFIGURE') . '" src="' . $g_dirs['images'] . 'icons/silk/table_edit.png" class="listConfigureIcon" />' .
                    '</a>';
            } // if
            ...
            $l_return = '<div style="overflow: auto;">
					...
					<script type="text/javascript">
						...
						window.object_list = new Lists.Objects(\'mainList\', {
							...
							ajax_pager_url: "?ajax=1&call=object_list&func=load_objtype_list&dao=' . get_class(
                    $this->m_dao_list
                ) . '&object_type=' . (int) isys_glob_get_param(C__CMDB__GET__OBJECTTYPE) . '",
							ajax_pager_preload: ' . $this->m_dao_list->get_preload_pages() . ',
							data: ' . $l_jsonresult . ',
							filter: "top",
                    ...
                    [
                        ...
                        C__CMDB__GET__OBJECTTYPE => $this->m_id
							...
							' . ((!empty($l_default_sorting)) ? ',order_direction: "' . $l_sorting_direction . '", order_field:"' . _L($l_default_sorting) . '"' : '') . '
						});';
            ...
            {
                $l_return .= "$('data-grid-mainList-" . _L($l_default_sorting) . "').className = '" . $l_sorting_direction . "';";
            } // if
        ...
        else
        {
            ...
            $l_return = '<p class="p10"><img src="' . $g_dirs['images'] . 'icons/infobox/blue.png" class="vam" /> ' . str_replace(
                    '[{var1}]',
                    preg_replace('/ \([0-9]+\)/', '', $l_arData[$this->m_dao_list->get_rec_status()]),
                    _L('LC__CMDB__FILTER__NOTHING_FOUND')
                ) . '</p>';
        } // if
    ...
    private function prepare_navbar($p_recstatus = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_navbar = isys_component_template_navbar::getInstance()
            ...
            ->set_active(($l_admin_right && (isys_settings::get('cmdb.quickpurge') == '1')), C__NAVBAR_BUTTON__QUICK_PURGE); // See ID-885
    ...
    {
        $this->m_id = isys_tenantsettings::get('defaults.cmdb.object-list.type', C__OBJTYPE__SERVER);

...
class isys_cmdb_view_category extends isys_cmdb_view
{
    ...
    public function handle_navmode($p_navmode)
    {
        ...
        $l_dao_lock = new isys_component_dao_lock(
        ...
        $l_multi_gui = $this->m_dao_cmdb->gui_is_multivalued_by_category(
            $l_catmeta["type"],
            ($this->m_cat_dao->get_category_id() != C__CATG__CUSTOM_FIELDS) ? $this->m_cat_dao->get_category_id() : $l_catmeta["id"]
        );
        ...
        switch ($p_navmode)
        {
                    ...
                    throw new isys_exception_auth(
                        _L(
                            ...
                            [
                                ...
                                _L($this->m_cat_dao->get_catg_name_by_id_as_string($this->m_cat_dao->get_category_id()))
                            ]
                        )
                    );
                } // if
            ...
            case C__NAVMODE__SAVE:
                ...
                $l_actionproc->insert(
                    C__CMDB__ACTION__CATEGORY_UPDATE,
                    [
                        &$this->m_cat_dao,
                        &$this->m_cat_ui
                    ]
            ...
            default:
                ...
                $l_auth_instance->check(isys_auth::VIEW, 'OBJ_ID/' . $l_gets[C__CMDB__GET__OBJECT]);
                ...
                if ($l_catmeta["type"] != C__CMDB__CATEGORY__TYPE_SPECIFIC && $l_catmeta["type"] != C__CMDB__CATEGORY__TYPE_CUSTOM)
                {
                    ...
                    $l_srctable = $this->m_dao_cmdb->gui_get_source_table_by_category(
                        $l_catmeta["type"],
                        $this->m_cat_dao->get_category_id()
                    );

                    // Set entrystatus to normal in overview category, because there will never be data inside isys_catg_overview_list.
                    if ($l_gets[C__CMDB__GET__CATG] == C__CATG__OVERVIEW || $l_catmeta["table"] == "isys_catg_virtual")
                    {
    ...
    public function category_process($p_result = null)
    {
        ...
        // Rights-Check: Are we allowd to view this category.
    ...
    public function process_save()
    {
        ...
        catch (isys_exception_auth $e)
        {
            ...
            isys_notify::error(_L('LC__INFOBOX__DATA_WAS_NOT_SAVED') . ' ' . $e->getMessage(), ['sticky' => true]);
            $e->write_log();
        }
        catch (isys_exception_cmdb $e)
        {
    ...
    {
        if (is_object($this->m_cat_dao) && $this->m_cat_dao->get_category_id() != C__CATG__OVERVIEW)
        {

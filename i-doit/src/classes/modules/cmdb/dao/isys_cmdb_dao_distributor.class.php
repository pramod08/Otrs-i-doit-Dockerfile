        ...
        if ($p_objid > 0 && $l_cattype)
        {
            ...
            while ($l_row = $l_cats->get_row())
            {
                ...
                if ($l_row[$l_cattable . "__list_multi_value"] == "0" && (($l_num_cats == 1 && empty($l_list_id)) || $l_num_cats > 0))
                {
                    ...
                    if ($p_disttype == C__CMDB__CATEGORY__TYPE_SPECIFIC || $p_disttype == C__CMDB__CATEGORY__TYPE_CUSTOM) $l_table = $l_spectable;
                    else
                    ...
                    if ($p_disttype === C__CMDB__CATEGORY__TYPE_CUSTOM)
                    {
                        ...
                        $l_query           = "SELECT {$l_cat_entry_field} FROM {$l_table} " . "WHERE ({$l_table}__isys_obj__id = '" . $this->m_db->escape_string(
                                $p_objid
                ...
                {
                    // Build the category.
                    $l_catobj = isys_cmdb_dao_category::manufacture(
                        $l_dao_cmdb, // cmdb access object
                        $p_objid,     // object id

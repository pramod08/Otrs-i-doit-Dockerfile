...
class isys_cmdb_dao_category_s_relation_details extends isys_cmdb_dao_category_specific
{
    ...
    public function callback_property_itservice(isys_request $p_request)
    {
        ...
        $l_objects    = $this->get_objects_by_type_id(C__OBJTYPE__IT_SERVICE, C__RECORD_STATUS__NORMAL);
    ...
    public function save($p_cat_level, $p_newRecStatus, $p_obj_id_master, $p_obj_id_slave, $p_relation_type, $p_weighting, $p_itservice, $p_description)
    {
        ...
        $l_strSql = "UPDATE isys_catg_relation_list SET " . " isys_catg_relation_list__isys_weighting__id  = " . $this->convert_sql_text(
                $p_weighting
                ...
                $p_itservice
            ) . ", " . " isys_catg_relation_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . " isys_catg_relation_list__status = " . $this->convert_sql_id($p_newRecStatus) . " " . "WHERE isys_catg_relation_list__id = " . $this->convert_sql_id(
                $p_cat_level
            );
        ...
        if ($this->update($l_strSql))
        {
            ...
            if ($l_bRet && $p_relation_type == C__RELATION_TYPE__DEFAULT)
            {
     ...
     * (bool)
     */
    public function create($p_relation_id, $p_master_id, $p_slave_id, $p_relation_type = null, $p_weighting = null, $p_it_service = null, $p_description = null, $p_record_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        if ($this->update($l_query) && $this->apply_update())
        {
            ...
            $l_obj_title     = $this->get_obj_name_by_id_as_string($p_master_id) . ' ' . _L(
                    $l_relation_type['isys_relation_type__master']
                ) . ' ' . $this->get_obj_name_by_id_as_string($p_slave_id);

            $l_update_sql = 'UPDATE isys_obj SET isys_obj__title = ' . $this->convert_sql_text($l_obj_title) . ' ' . 'WHERE isys_obj__id = ' . $this->convert_sql_id(
                    $p_relation_id
                );
    ...
    public function get_data($p_catg_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        ...
        $l_sql = "SELECT * FROM isys_catg_relation_list " . "LEFT JOIN isys_relation_type " . "ON " . "isys_catg_relation_list__isys_relation_type__id = " . "isys_relation_type__id " . "LEFT JOIN isys_weighting " . "ON " . "isys_catg_relation_list__isys_weighting__id = " . "isys_weighting__id " . "INNER JOIN isys_obj " . "ON " . "isys_catg_relation_list__isys_obj__id = " . "isys_obj__id " . "WHERE TRUE ";
        ...
        return [
            ...
            'relation_type' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI       => [
                        C__PROPERTY__UI__ID     => 'C__CATS__RELATION_DETAILS__RELATION_TYPE_VALUE',
                        C__PROPERTY__UI__PARAMS => [
            ...
            'description'   => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI       => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__RELATION_DETAILS
                    ],
                    C__PROPERTY__PROVIDES => [

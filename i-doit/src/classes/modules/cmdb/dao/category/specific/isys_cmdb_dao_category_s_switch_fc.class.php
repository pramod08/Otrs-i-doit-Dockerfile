...
class isys_cmdb_dao_category_s_switch_fc extends isys_cmdb_dao_category_specific
{
        ...
        return [
            ...
            'description' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__SWITCH_FC
                    ]
                ]
            )
        ];
    ...
    public function save_element($p_cat_level, &$p_intOldRecStatus)
    {
        ...
        if (empty($l_list_id)) $l_list_id = $this->create_connector("isys_cats_switch_fc_list", $_GET[C__CMDB__GET__OBJECT]);

        $l_bRet = $this->save(
            $l_list_id,
            C__RECORD_STATUS__NORMAL,
            $_POST["C__CATS__SWITCH_FC_TITLE"],
            $_POST["C__CATS__SWITCH_FC_ACTIVE"],
    ...
    {
        $l_strSql = "UPDATE isys_cats_switch_fc_list SET " . "isys_cats_switch_fc_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_cats_switch_fc_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_cats_switch_fc_list__unit_active = " . $this->convert_sql_boolean(
                $p_active
            ) . ", " . "isys_cats_switch_fc_list__status = " . $this->convert_sql_id($p_newRecStatus) . " " . "WHERE isys_cats_switch_fc_list__id = " . $this->convert_sql_id(
                $p_cat_level
            );
    ...
    {
        $l_strSql = "INSERT IGNORE INTO isys_cats_switch_fc_list SET " . "isys_cats_switch_fc_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_cats_switch_fc_list__title = " . $this->convert_sql_text($p_title) . ", " . "isys_cats_switch_fc_list__unit_active = " . $this->convert_sql_id(
                $p_active
            ) . ", " . "isys_cats_switch_fc_list__status = " . $this->convert_sql_id(
                $p_newRecStatus
            ) . ", " . "isys_cats_switch_fc_list__isys_obj__id = " . $this->convert_sql_id($p_objID) . ";";

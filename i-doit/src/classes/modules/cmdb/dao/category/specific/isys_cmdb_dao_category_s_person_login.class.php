...
class isys_cmdb_dao_category_s_person_login extends isys_cmdb_dao_category_s_person
{
    ...
    protected function properties()
    {
        return [
            ...
            'user_pass'   => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'user_pass2'  => array_replace_recursive(
            ...
            'description' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__PERSON_LOGIN
                    ]
                ]
            )
        ];
    ...
    public function validate(array $p_data = [], $p_prepend_table_field = false)
    {
        ...
        {
            $l_return['user_pass'] = $l_return['user_pass2'] = _L("LC__LOGIN__PASSWORDS_DONT_MATCH");
        } // if

        if (isset($p_data['user_pass']) && isys_strlen($p_data['user_pass']) < $l_password_minlength && $p_data['user_pass'] != '')
        {
            $l_return['user_pass'] = _L("LC__LOGIN__SAVE_ERROR", $l_password_minlength);
        } // if

        if (isset($p_data['user_pass2']) && isys_strlen($p_data['user_pass2']) < $l_password_minlength && $p_data['user_pass2'] != '')
        {
            $l_return['user_pass2'] = _L("LC__LOGIN__SAVE_ERROR", $l_password_minlength);
        } // if

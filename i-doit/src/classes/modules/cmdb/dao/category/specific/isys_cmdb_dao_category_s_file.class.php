...
class isys_cmdb_dao_category_s_file extends isys_cmdb_dao_category_specific
{
    ...
    protected function properties()
    {
        return [
            ...
            'file_category'       => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        C__PROPERTY__DATA__FIELD      => 'isys_cats_file_list__isys_file_category__id',
                        C__PROPERTY__DATA__REFERENCES => [
                            'isys_file_category',
                            'isys_file_category__id'
                        ]
                    ],
                    C__PROPERTY__UI       => [
                        C__PROPERTY__UI__ID     => 'C__CATS__FILE_CATEGORY',
            ...
            'description'         => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI       => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__FILE
                    ],
                    C__PROPERTY__PROVIDES => [
                        C__PROPERTY__PROVIDES__REPORT => false,
                        C__PROPERTY__PROVIDES__LIST   => false
            ...
            'current_version'     => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog_plus(),
                [
                    C__PROPERTY__INFO     => [
                        C__PROPERTY__INFO__TITLE       => 'LC__CMDB__CATG__CATEGORY',
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            ...
            if ($p_status === isys_import_handler_cmdb::C__CREATE)
            {
                ...
                if ($this->update_cats_file_list($p_category_data['data_id'], $l_version_id, null, $p_category_data['properties']['description'][C__DATA__VALUE]))
                {
                    $l_indicator = true;
                } // if
            }
     ...
     * @author   Dennis Stuecken <dstuecken@synetics.de>
     */
    public function update_cats_file_list($p_cats_id, $p_version_id, $p_category_id, $p_description = '')
    {
        if ($p_cats_id > 0)
        {
            $l_sql = 'UPDATE isys_cats_file_list SET
				...
				isys_cats_file_list__description = ' . $this->convert_sql_text($p_description) . ',
				isys_cats_file_list__isys_file_category__id = ' . $this->convert_sql_id($p_category_id) . '
				WHERE isys_cats_file_list__id = ' . $this->convert_sql_id($p_cats_id) . ';';
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus, $p_create = false)
    {
        ...
        if (!empty($_FILES['C__CATS__FILE_UPLOAD']))
        {
            ...
            try
            {
                $l_file = isys_helper_upload::save($_FILES['C__CATS__FILE_UPLOAD'], $l_new_filename, $g_dirs["fileman"]["target_dir"]);
            }
            ...
            if ($l_file !== false)
            {
                ...
                $l_physical_id = $this->create_physical_file(
                    $l_new_filename,
                    $l_filename,
                    $p_md5_hash,
                    $p_user_id
        ...
        $l_retVal = $this->update_cats_file_list(
            $l_cats_id,
            $l_version_id,
            $l_category_id,
            $l_description

...
class isys_cmdb_dao_category_s_database_gateway extends isys_cmdb_dao_category_specific
{
    ...
    public function get_data($p_cats_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        ...
        $l_sql = "SELECT * FROM isys_cats_database_gateway_list " . "INNER JOIN isys_obj ON isys_cats_database_gateway_list__isys_obj__id = isys_obj__id " . "LEFT JOIN isys_connection ON isys_connection__id = isys_cats_database_gateway_list__isys_connection__id " . "WHERE TRUE ";
    ...
    protected function properties()
    {
        return [
            ...
            'target_schema' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__DATABASE_GATEWAY,
            ...
            'description'   => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__DATABASE_GATEWAY
                    ]
                ]
            )
        ];
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            if ($p_status === isys_import_handler_cmdb::C__CREATE)
            {
                if ($p_category_data['data_id'] = $this->create(
                    ...
                    $p_category_data['properties']['host'][C__DATA__VALUE],
                    $p_category_data['properties']['port'][C__DATA__VALUE],
                    $p_category_data['properties']['user'][C__DATA__VALUE],
                    $p_category_data['properties']['target_schema'][C__DATA__VALUE],
    ...
    public function save_element($p_cat_level, &$p_intOldRecStatus)
    {
        ...
        if (empty($l_list_id)) $l_list_id = $this->create($_GET[C__CMDB__GET__OBJECT], C__RECORD_STATUS__NORMAL, "", "", "", "", null);
        ...
        if ($l_list_id)
        {
            $l_bRet = $this->save(
                $l_list_id,
                C__RECORD_STATUS__NORMAL,
                $_POST['C__CATS__DATABASE_GATEWAY__GATEWAY_TYPE'],
                $_POST['C__CATS__DATABASE_GATEWAY__HOST'],
                $_POST['C__CATS__DATABASE_GATEWAY__PORT'],
                $_POST['C__CATS__DATABASE_GATEWAY__USER'],
                $_POST['C__CATS__DATABASE_GATEWAY__TARGET_SCHEMA__HIDDEN'],
    ...
    public function save($p_cat_level, $p_newRecStatus, $p_type, $p_host, $p_port, $p_user, $p_connectedObjID, $p_description)
    {
        ...
        $l_strSql = "UPDATE isys_cats_database_gateway_list " . "SET " . "isys_cats_database_gateway_list__isys_connection__id = " . $this->convert_sql_id(
                $this->handle_connection($p_cat_level, $p_connectedObjID)
            ) . ", " . "isys_cats_database_gateway_list__type = " . $this->convert_sql_text(
                $p_type
            ) . ", " . "isys_cats_database_gateway_list__host = " . $this->convert_sql_text(
                $p_host
            ) . ", " . "isys_cats_database_gateway_list__port = " . $this->convert_sql_text(
                $p_port
            ) . ", " . "isys_cats_database_gateway_list__user = " . $this->convert_sql_text(
                $p_user
            ) . ", " . "isys_cats_database_gateway_list__status = " . $this->convert_sql_id(
                $p_newRecStatus
            ) . ", " . "isys_cats_database_gateway_list__description = " . $this->convert_sql_text(
                $p_description
            ) . " " . "WHERE isys_cats_database_gateway_list__id = " . $this->convert_sql_id($p_cat_level);
        ...
        if ($this->update($l_strSql) && $this->apply_update())
        {
            ...
            $l_relation_dao->handle_relation(
                ...
                C__RELATION_TYPE__DATABASE_GATEWAY,
                $l_data["isys_cats_database_gateway_list__isys_catg_relation_list__id"],
                $p_connectedObjID,
                $l_data["isys_cats_database_gateway_list__isys_obj__id"]
            );
    ...
    public function create($p_objID, $p_newRecStatus, $p_type = "", $p_host = "", $p_port = "", $p_user = "", $p_connectedObjID = null, $p_description = "")
    {
        ...
        $l_strSql = "INSERT INTO isys_cats_database_gateway_list SET " . "isys_cats_database_gateway_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_cats_database_gateway_list__isys_connection__id = " . $this->convert_sql_id(
                $l_conn_id
            ) . ", " . "isys_cats_database_gateway_list__type = " . $this->convert_sql_text(
                $p_type
            ) . ", " . "isys_cats_database_gateway_list__host = " . $this->convert_sql_text(
    ...
    public function get_target_schema($p_cats_id)
    {
        ...
        if (!is_null($p_cats_id))
        {
            ...
            $l_sql = "SELECT * FROM isys_cats_database_gateway_list_2_isys_obj " . "INNER JOIN isys_obj ON isys_obj__id = isys_cats_database_gateway_list_2_isys_obj.isys_obj__id " . "WHERE isys_cats_database_gateway_list__id = " . $this->convert_sql_id(
                    $p_cats_id
                );

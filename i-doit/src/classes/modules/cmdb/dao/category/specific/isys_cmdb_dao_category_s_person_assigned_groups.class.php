...
class isys_cmdb_dao_category_s_person_assigned_groups extends isys_cmdb_dao_category_specific implements ObjectBrowserReceiver
{
            ...
            if (!in_array($l_row['isys_obj__id'], $p_objects))
            {
                if ($l_auth_obj->is_allowed_to(isys_auth::EDIT, 'OBJ_ID/' . $l_row['isys_obj__id']))
                {
                    $l_sql = 'DELETE FROM isys_person_2_group
						...
						AND isys_person_2_group__isys_obj__id__person = ' . $this->convert_sql_id($p_object_id) . ' LIMIT 1';
        ...
        foreach ($p_objects as $l_object)
        {
            ...
            if (count($l_not_allowed_objects) > 0)
            {
                ...
                isys_component_template_infobox::instance()
                    ->set_message($l_message, 1, null, null, C__LOGBOOK__ALERT_LEVEL__3);
            }
        } // if
    ...
    public function get_assigned_contacts($p_obj_id)
    {
        ...
        $l_sql = 'SELECT obj.isys_obj__id, obj.isys_obj__title, obj.isys_obj__isys_obj_type__id, obj.isys_obj__sysid ' . 'FROM isys_person_2_group AS p2g ' . 'LEFT JOIN isys_obj AS obj ON obj.isys_obj__id = p2g.isys_person_2_group__isys_obj__id__group ' . 'WHERE p2g.isys_person_2_group__isys_obj__id__person = ' . $this->convert_sql_id(
                $p_obj_id
            ) . ';';
    ...
    {
        $l_sql = 'SELECT *, mail_person.isys_catg_mail_addresses_list__title AS isys_cats_person_list__mail_address, ' . 'mail_pgroup.isys_catg_mail_addresses_list__title AS isys_cats_person_group_list__email_address ' . 'FROM isys_person_2_group ' . 'INNER JOIN isys_cats_person_list ON isys_person_2_group__isys_obj__id__person = isys_cats_person_list__isys_obj__id ' . 'INNER JOIN isys_cats_person_group_list ON isys_person_2_group__isys_obj__id__group = isys_cats_person_group_list__isys_obj__id ' . 'LEFT JOIN isys_catg_mail_addresses_list AS mail_person ON mail_person.isys_catg_mail_addresses_list__isys_obj__id = isys_cats_person_list__isys_obj__id AND mail_person.isys_catg_mail_addresses_list__primary = 1 ' . 'LEFT JOIN isys_catg_mail_addresses_list AS mail_pgroup ON mail_pgroup.isys_catg_mail_addresses_list__isys_obj__id = isys_cats_person_group_list__isys_obj__id AND mail_pgroup.isys_catg_mail_addresses_list__primary = 1 ' . 'WHERE TRUE ' . $p_condition . ' ' . $this->prepare_filter(
                $p_filter
            );
            ...
            'connected_object' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__PERSON_ASSIGNED_GROUPS,

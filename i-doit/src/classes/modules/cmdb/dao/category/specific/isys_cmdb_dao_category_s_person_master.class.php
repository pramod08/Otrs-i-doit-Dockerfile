...
class isys_cmdb_dao_category_s_person_master extends isys_cmdb_dao_category_s_person
{
     ...
     * @var  integer
     */
    protected $m_category_id = C__CATS__PERSON;
    /**
     * Category's table.
     *
     * @var  string
    ...
    public function dynamic_property_callback_organisation(array $p_row)
    {
        ...
        if ($l_row["isys_cats_organization_list__isys_obj__id"] > 0)
        {
            return isys_factory::get_instance('isys_ajax_handler_quick_info')
                ->get_quick_info(
                    $l_row["isys_cats_organization_list__isys_obj__id"],
    ...
    {
        return isys_cmdb_dao_category_g_mail_addresses::instance($this->m_db)->dynamic_property_callback_primary_email($p_row);
    } // function
    ...
    public function callback_property_organisation(isys_request $p_request)
    {
        ...
        $l_res = $this->get_objects_by_type_id(C__OBJTYPE__ORGANIZATION, C__RECORD_STATUS__NORMAL);
    ...
    public function save_element($p_cat_level, &$p_intOldRecStatus)
    {
        ...
        if ($l_list_id)
        {
            $l_bRet = $this->save(
                ...
                ((empty($_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . $this->get_category_id(
                )])) ? $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . C__CATS__PERSON] : $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type(
     ...
     * @throws  isys_exception_general
     */
    public function save($p_id, $p_firstname, $p_lastname, $p_mail, $p_phoneOffice, $p_phonePrivate, $p_phoneCell, $p_fax, $p_department = "", $p_organization = null, $p_description = "", $p_ldap_server = null, $p_ldap_dn = "", $p_status = C__RECORD_STATUS__NORMAL, $p_personnel_number = '', $p_academic_degree = "", $p_function = "", $p_service_designation = "", $p_city = "", $p_zip_code = "", $p_street = '', $p_pager = '', $p_salutation = '')
    {
        if ($p_id > 0)
        {
            ...
            {
                $p_organization = $this->get_obj_id_by_title(trim($p_organization), C__OBJTYPE__ORGANIZATION);
            } // if
            ...
            $l_sql = "UPDATE isys_cats_person_list
						...
						SET isys_obj__title = " . $this->convert_sql_text($p_firstname . " " . $p_lastname) . ",
							isys_cats_person_list__first_name = " . $this->convert_sql_text($p_firstname) . ",
							...
							isys_cats_person_list__salutation = " . $this->convert_sql_text($p_salutation) . ",
            ...
            if ($p_ldap_dn != "")
            {
                $l_sql .= "isys_cats_person_list__ldap_dn = " . $this->convert_sql_text($p_ldap_dn) . ",
					isys_cats_person_list__isys_ldap__id = " . $this->convert_sql_text($p_ldap_server) . ", ";

                $l_ldap_dao = new isys_cmdb_dao_category_g_ldap_dn($this->get_database_component());
                $l_ldap_dao->save_single_value($this->get_object_id_by_category_id($p_id), ['title' => $p_ldap_dn]);
            } // if
            ...
            if ($this->update($l_sql) && $this->apply_update())
            {
                ...
                if ($p_organization > 0)
                {
                    $l_dao_relation->handle_relation(
                        ...
                        C__RELATION_TYPE__ORGANIZATION,
     ...
     * @throws  isys_exception_general
     */
    public function create($p_obj_id = null, $p_username, $p_firstname, $p_lastname, $p_mail, $p_phoneOffice, $p_phonePrivate, $p_phoneCell, $p_fax, $p_department = "", $p_organization = "", $p_description = "", $p_ldap_server = null, $p_ldap_dn = "", $p_status = C__RECORD_STATUS__NORMAL, $p_personnel_number = '', $p_academic_degree = "", $p_function = "", $p_service_designation = "", $p_city = "", $p_zip_code = "", $p_street = "", $p_pager = '', $p_salutation = '')
    {
        $l_created  = false;
        $l_entry_id = false;
        ...
        {
            {
                ...
                if ($l_orga_id > 0)
                {
                    ...
                    $l_dao_relation->handle_relation($l_last_id, "isys_cats_person_list", C__RELATION_TYPE__ORGANIZATION, null, $l_orga_id, $p_obj_id);
                } // if
            } // if
        }
        else
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            ...
            if ($p_status === isys_import_handler_cmdb::C__CREATE || $p_status === isys_import_handler_cmdb::C__UPDATE)
            {
                 ...
                 *
                 * @var $l_dao_orga isys_cmdb_dao_category_s_organization_master
                 */
                $l_dao_orga = isys_factory_cmdb_dao::get_instance('isys_cmdb_dao_category_s_organization_master', ($this->get_database_component()));

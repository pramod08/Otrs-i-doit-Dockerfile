...
class isys_cmdb_dao_category_s_database_access extends isys_cmdb_dao_category_specific implements ObjectBrowserReceiver
{
            ...
            'access' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__DATABASE_ACCESS,
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            ...
            $l_condition = " AND isys_catg_relation_list__isys_obj__id__master = " . $this->convert_sql_id(
                    $p_category_data['properties']['access'][C__DATA__VALUE][0]
                ) . " AND isys_catg_relation_list__isys_obj__id__slave = " . $this->convert_sql_id(
                    $p_category_data['properties']['access'][C__DATA__VALUE][1]
                ) . " AND isys_catg_relation_list__isys_relation_type__id = " . C__RELATION_TYPE__SOFTWARE;
    ...
    public function save($p_id, $p_connectedObjID, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_strSql = "UPDATE isys_cats_database_access_list " . "SET " . "isys_cats_database_access_list__isys_connection__id = " . $this->convert_sql_id(
                $this->handle_connection($p_id, $p_connectedObjID)
        ...
        if ($this->update($l_strSql))
        {
            if ($this->apply_update())
            {
                ...
                $l_dao_relation->handle_relation(
                    ...
                    $l_data["isys_cats_database_access_list__isys_catg_relation_list__id"],
                    $l_data["isys_cats_database_access_list__isys_obj__id"],
                    $p_connectedObjID
                );
    ...
    public function create($p_object_id, $p_connectedObjID, $p_status = C__RECORD_STATUS__NORMAL, $p_commentary = '')
    {
        $l_sql = "INSERT INTO isys_cats_database_access_list SET
			...
			isys_cats_database_access_list__isys_connection__id = " . $this->convert_sql_id(
                isys_factory::get_instance('isys_cmdb_dao_connection', $this->m_db)
    ...
    {
        $l_sql = 'SELECT obj.isys_obj__id, obj.isys_obj__title FROM isys_cats_database_access_list AS al ' . 'LEFT JOIN isys_connection AS conn ON conn.isys_connection__id = al.isys_cats_database_access_list__isys_connection__id ' . 'LEFT JOIN isys_obj AS obj ON conn.isys_connection__isys_obj__id = obj.isys_obj__id ' . 'WHERE al.isys_cats_database_access_list__isys_obj__id = ' . $this->convert_sql_id(
                $p_id
            ) . ';';
                ...
                while ($l_row = $l_relations->get_row())
                {
                    ...
                    if ($l_obj_row['isys_obj__isys_obj_type__id'] == C__OBJTYPE__RELATION)
                    {
                        $l_return[] = [
                            ...
                            _L('LC__CMDB__CATP__TITLE') => $l_row['isys_obj__title'],
                            _L('LC__CMDB__CATG__TYPE')  => _L('LC__CMDB__OBJTYPE__RELATION'),
                            _L('LC__CMDB__CATG__TYPE')  => _L($l_obj_row['isys_obj_type__title'])
                        ];
                    } // if
                } // while

...
class isys_cmdb_dao_category_s_layer2_net extends isys_cmdb_dao_category_specific
{
    ...
    {
        $l_sql = "INSERT INTO isys_cats_layer2_net_2_iphelper VALUES (NULL, " . $this->convert_sql_id($p_cat_id) . ", " . $this->convert_sql_id(
                $p_type
     ...
     */
    public function create($p_obj_id, $p_status = C__RECORD_STATUS__NORMAL, $p_ident = null, $p_type = null, $p_subtype = null, $p_standard = null, $p_description = '', $p_vrf = null, $p_vrf_capacity = null, $p_vrf_capacity_unit = null)
    {
    ...
    protected function properties()
    {
        return [
            ...
            'vrf_capacity'        => array_replace_recursive(
                isys_cmdb_dao_category_pattern::int(),
                [
                    C__PROPERTY__INFO   => [
            ...
            'vrf_capacity_unit'   => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO => [
                        C__PROPERTY__INFO__TITLE       => 'LC__CATS__LAYER2__VRF_CAPACITY_UNIT',
                        C__PROPERTY__INFO__DESCRIPTION => 'VRF capacity unit'
                    ],
                    C__PROPERTY__DATA => [
                        C__PROPERTY__DATA__FIELD => 'isys_cats_layer2_net_list__isys_wan_capacity_unit'
                    ],
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID     => 'C__CATS__LAYER2__VRF_CAPACITY_UNIT',
                        C__PROPERTY__UI__PARAMS => [
    ...
    {
        $l_sql_condition = ' CASE isys_cats_layer2_net_list__isys_layer2_net_subtype__id ' . 'WHEN ' . (int) C__CATS__LAYER2_NET__SUBTYPE__STATIC_VLAN . ' THEN CONCAT(isys_obj__title, " ", isys_cats_layer2_net_list__ident, " (Static)") ' . 'WHEN ' . (int) C__CATS__LAYER2_NET__SUBTYPE__DYNAMIC_VLAN . ' THEN CONCAT(isys_obj__title, " ", isys_cats_layer2_net_list__ident, " (Dynamic)") ' . 'ELSE CONCAT(isys_obj__title, "") END ';
    ...
    {
        if ($this->clean_layer3($p_cat_id) && count($p_layer3))
        {
            $l_parts = [];
    ...
    {
        $l_sql = 'SELECT isys_obj.isys_obj__id, isys_obj.isys_obj__title ' . 'FROM isys_cats_layer2_net_2_layer3 ' . 'INNER JOIN isys_obj ON isys_obj.isys_obj__id = isys_cats_layer2_net_2_layer3.isys_obj__id ' . 'WHERE isys_cats_layer2_net_list__id = ' . $this->convert_sql_id(
                $p_cat_id
            ) . ';';

...
class isys_cmdb_dao_category_s_database_schema extends isys_cmdb_dao_category_specific
{
    ...
    public function dynamic_property_callback_instance(array $p_row)
    {
        ...
        if ($l_row !== false && $l_row['isys_cats_database_instance_list__id'] > 0)
        {
            ...
            $l_return = $l_instance_row["isys_obj__title"] . " " . strtolower(_L("LC__UNIVERSAL__ON")) . " " . $l_dao->get_obj_name_by_id_as_string(
                    $l_instance_row["isys_connection__isys_obj__id"]
                );
        } // if
    ...
    public function callback_property_instance(isys_request $p_request)
    {
        ...
        {
            $l_arInstances[$l_row["isys_obj__id"]] = $l_row["isys_obj__title"] . " " . $l_on . " " . $l_dao_instance->get_obj_name_by_id_as_string(
    ...
    public function save_element($p_cat_level, &$p_status, $p_create = false)
    {
        ...
        if (!$l_catdata)
        {
            $l_list_id = $this->create(
                ...
                $_POST["C__CMDB__CATS__DB_SCHEMA__RUNS_ON"],
                $_POST["C__CMDB__CATS__DB_SCHEMA__TITLE"],
                $_POST["C__CMDB__CATS__DB_SCHEMA__STORAGE_ENGINE"],
        ...
        else
        {
            ...
            $l_bRet    = $this->save(
                $l_catdata["isys_cats_database_schema_list__id"],
                $_POST["C__CMDB__CATS__DB_SCHEMA__RUNS_ON"],
                $_POST["C__CMDB__CATS__DB_SCHEMA__TITLE"],
                $_POST["C__CMDB__CATS__DB_SCHEMA__STORAGE_ENGINE"],
                $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . $this->get_category_id()]
    ...
    public function save($p_id, $p_runs_on, $p_title, $p_storage_engine, $p_description, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_strSql = "UPDATE isys_cats_database_schema_list " . "SET " . "isys_cats_database_schema_list__isys_connection__id = " . $this->convert_sql_id(
                $this->handle_connection($p_id, $p_runs_on)
            ) . ", " . "isys_cats_database_schema_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_cats_database_schema_list__storage_engine = " . $this->convert_sql_text(
                $p_storage_engine
            ) . ", " . "isys_cats_database_schema_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_cats_database_schema_list__status = " . $this->convert_sql_id($p_status) . " " .
        ...
        if ($this->update($l_strSql) && $this->apply_update())
        {
            ...
            if (isset($p_runs_on) && $p_runs_on > 0)
            {
                ...
                $l_dao_db_instance->attach_connected_database_schema(
                    $l_catdata["isys_cats_database_schema_list__isys_obj__id"],
                    $p_runs_on,
                ...
                $l_dao_relation->handle_relation(
                    ...
                    $l_catdata["isys_cats_database_schema_list__isys_catg_relation_list__id"],
                    $p_runs_on,
                    $l_catdata["isys_cats_database_schema_list__isys_obj__id"]
                );
            }
    ...
    public function create($p_object_id, $p_runs_on, $p_title, $p_storage_engine, $p_description, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_strSql = "INSERT IGNORE INTO isys_cats_database_schema_list SET " . "isys_cats_database_schema_list__isys_obj__id = " . $this->convert_sql_id(
                $p_object_id
            ) . ", " . "isys_cats_database_schema_list__isys_connection__id = " . $this->convert_sql_id(
                $l_connection_id
            ) . ", " . "isys_cats_database_schema_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_cats_database_schema_list__storage_engine = " . $this->convert_sql_text(
        ...
        if ($this->update($l_strSql) && $this->apply_update())
        {
            ...
            if (isset($p_runs_on) && $p_runs_on > 0)
            {
                ...
                $l_dao_relation->handle_relation($l_last_id, "isys_cats_database_schema_list", C__RELATION_TYPE__DATABASE_INSTANCE, null, $p_runs_on, $p_object_id);
            } // if
    ...
    public function detach_dbms($p_obj_id)
    {
        ...
        $l_update = "UPDATE isys_cats_database_schema_list SET " . "isys_cats_database_schema_list__isys_cats_db_instance_list__id = NULL, " . "isys_cats_database_schema_list__isys_catg_relation_list__id = NULL " . "WHERE isys_cats_database_schema_list__isys_obj__id = " . $this->convert_sql_id(
                $p_obj_id
            );

        if ($this->update($l_update) && $this->apply_update())
        {
            if ($l_dao_relation->delete_relation($l_catdata["isys_cats_database_schema_list__isys_catg_relation_list__id"])) return true;
            else
                return false;
        }
        else
    ...
    public function get_data($p_cats_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        $l_sql = 'SELECT * FROM isys_cats_database_schema_list
			...
			INNER JOIN isys_connection ON isys_connection__id = isys_cats_database_schema_list__isys_connection__id
    ...
    protected function properties()
    {
        return [
            ...
            'instance'       => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__DATABASE_INSTANCE,
            ...
            'storage_engine' => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO => [
            ...
            'description'    => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_SPECIFIC . C__CATS__DATABASE_SCHEMA
                    ]
                ]
            )
        ];
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            ...
            {
                $l_dao_db_instance  = new isys_cmdb_dao_category_s_database_instance($this->get_database_component());
                $l_db_instance_data = $l_dao_db_instance->get_data(
                    null,
                ...
                {
                    $l_dao_db_instance->create(
                        $p_category_data['properties']['link'][C__DATA__VALUE],
                        $this->get_obj_name_by_id_as_string($p_category_data['properties']['link'][C__DATA__VALUE]),
                        null,
                        null,
                        null,
                        null
            ...
            if ($p_status === isys_import_handler_cmdb::C__CREATE)
            {
                if (($p_category_data['data_id'] = $this->create(
                    ...
                    $p_category_data['properties']['storage_engine'][C__DATA__VALUE],

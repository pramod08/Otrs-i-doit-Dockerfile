...
class isys_cmdb_dao_category_s_net_dhcp extends isys_cmdb_dao_category_specific
{
    ...
    {
        $l_ip_assignment = C__CATP__IP__ASSIGN__STATIC;
        switch ($p_type)
        {
            case C__NET__DHCP_RESERVED:
                $l_ip_assignment = C__CATP__IP__ASSIGN__DHCP_RESERVED;
                break;
            case C__NET__DHCP_DYNAMIC:
                $l_ip_assignment = C__CATP__IP__ASSIGN__DHCP;
                break;
        } // switch
    ...
    {
        $l_ip_assignment = C__CMDB__CATG__IP__STATIC;
        switch ($p_type)
        {
            case C__NET__DHCPV6__SLAAC_AND_DHCPV6_RESERVED:
                $l_ip_assignment = C__CMDB__CATG__IP__SLAAC_AND_DHCPV6_RESERVED;
                break;
            case C__NET__DHCPV6__SLAAC_AND_DHCPV6:
                $l_ip_assignment = C__CMDB__CATG__IP__SLAAC_AND_DHCPV6;
                break;
            case C__NET__DHCP_RESERVED:
            case C__NET__DHCPV6__DHCPV6_RESERVED:
                $l_ip_assignment = C__CMDB__CATG__IP__DHCPV6_RESERVED;
                break;
            case C__NET__DHCP_DYNAMIC:
            case C__NET__DHCPV6__DHCPV6:
                $l_ip_assignment = C__CMDB__CATG__IP__DHCPV6;
                break;
        } // switch
        return $l_ip_assignment;
    } // function
    ...
    public function create($p_obj_id = null, $p_type = null, $p_typev6 = null, $p_ip_range_from = null, $p_ip_range_to = null, $p_description = null, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_sql = 'INSERT INTO ' . $this->m_table . ' (' . $this->m_table . '__isys_obj__id, ' . $this->m_table . '__isys_net_dhcp_type__id, ' . $this->m_table . '__isys_net_dhcpv6_type__id, ' . $this->m_table . '__range_from, ' . $this->m_table . '__range_from_long, ' . $this->m_table . '__range_to, ' . $this->m_table . '__range_to_long, ' . $this->m_table . '__description, ' . $this->m_table . '__status' . ') VALUES (' . $this->convert_sql_id(
                $p_obj_id
            ) . ', ' . $this->convert_sql_id($p_type) . ', ' . $this->convert_sql_id($p_typev6) . ', ' . $this->convert_sql_text(
                $p_ip_range_from
            ) . ', ' . $this->convert_sql_text(Ip::ip2long($p_ip_range_from)) . ', ' . $this->convert_sql_text($p_ip_range_to) . ', ' . $this->convert_sql_text(
                Ip::ip2long($p_ip_range_to)
            ) . ', ' . $this->convert_sql_text($p_description) . ', ' . $this->convert_sql_int($p_status) . ');';
    ...
    public function delete($p_id)
    {
        ...
        $l_sql = 'DELETE FROM ' . $this->m_table . ' ' . 'WHERE ' . $this->m_table . '__id = ' . $this->convert_sql_id($p_id);
    ...
    public function find_range_conflicts($p_obj_id, $p_from, $p_to, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        {
            $l_sql = 'SELECT * FROM ' . $this->m_table . ' ' . 'WHERE (( ' . $this->m_table . '__range_from_long BETWEEN ' . Ip::ip2long($p_from) . ' ' . 'AND ' . Ip::ip2long(
                    $p_to
                ) . ') ' . 'OR (' . $this->m_table . '__range_to_long BETWEEN ' . Ip::ip2long($p_from) . ' ' . 'AND ' . Ip::ip2long(
                    $p_to
                ) . ') ' . 'OR (' . $this->m_table . '__range_from_long <= ' . Ip::ip2long($p_from) . ' ' . 'AND ' . $this->m_table . '__range_to_long >= ' . Ip::ip2long(
                    $p_to
                ) . ') ' . 'OR (' . $this->m_table . '__range_from_long >= ' . Ip::ip2long($p_from) . ' ' . 'AND ' . $this->m_table . '__range_to_long <= ' . Ip::ip2long(
                    $p_to
                ) . ')) ' . 'AND ' . $this->m_table . '__isys_obj__id = ' . $this->convert_sql_id(
                    $p_obj_id
                ) . ' ' . 'AND ' . $this->m_table . '__status = ' . $this->convert_sql_int($p_status) . ' ' . 'ORDER BY ' . $this->m_table . '__range_from_long ASC;';
    ...
    public function find_ipv6_range_conflicts($p_obj_id, $p_from, $p_to = null, $p_status = C__RECORD_STATUS__NORMAL)
    {
        if (!empty($p_from))
        {
            ...
            $l_sql = 'SELECT * FROM ' . $this->m_table . ' ' . 'WHERE (( ' . $this->m_table . '__range_from BETWEEN "' . $l_from . '" ' . 'AND "' . $l_to . '") ' . 'OR (' . $this->m_table . '__range_to BETWEEN "' . $l_from . '" ' . 'AND "' . $l_to . '") ' . 'OR (' . $this->m_table . '__range_from <= "' . $l_from . '" ' . 'AND ' . $this->m_table . '__range_to >= "' . $l_to . '") ' . 'OR (' . $this->m_table . '__range_from >= "' . $l_from . '" ' . 'AND ' . $this->m_table . '__range_to <= "' . $l_to . '")) ' . 'AND ' . $this->m_table . '__isys_obj__id = ' . $this->convert_sql_id(
                    $p_obj_id
                ) . ' ' . 'AND ' . $this->m_table . '__status = ' . $this->convert_sql_int($p_status) . ' ' . 'ORDER BY ' . $this->m_table . '__range_from_long ASC;';
    ...
    public function get_data($p_id = null, $p_obj_id = null, $p_condition = '', $p_filter = null, $p_status = null)
    {
        ...
        $l_sql = 'SELECT * FROM ' . $this->m_table . ' ' . ' LEFT JOIN isys_net_dhcp_type ON ' . $this->m_table . '__isys_net_dhcp_type__id = isys_net_dhcp_type__id ' . ' LEFT JOIN isys_net_dhcpv6_type ON ' . $this->m_table . '__isys_net_dhcpv6_type__id = isys_net_dhcpv6_type__id ' . 'WHERE TRUE ' . $p_condition . ' ';
    ...
    public function validate(array $p_data = [], $p_prepend_table_field = false)
    {
        ...
        if ($l_net_type == C__CATS_NET_TYPE__IPV4)
        {
            ...
            if (isset($p_data['range_from']) && isset($p_data['range_to']))
            {
                ...
                {
                    return ['range_from' => _L('LC__CMDB__CATS__NET_IP_ADDRESSES__IP_INVALID')];
                } // if

                if (!Ip::validate_ip($p_data['range_to']))
                {
                    return ['range_to' => _L('LC__CMDB__CATS__NET_IP_ADDRESSES__IP_INVALID')];
                } // if

                if (Ip::ip2long($p_data['range_from']) > Ip::ip2long($p_data['range_to']))
                {
                    return [
                        'range_from' => _L('LC__CMDB__CATS__NET_IP_ADDRESSES__IP_INVALID'),
                        'range_to'   => _L('LC__CMDB__CATS__NET_IP_ADDRESSES__IP_INVALID')
                    ];
                } // if
            } // if
        }
    ...
    public function save($p_id, $p_obj_id = null, $p_type = null, $p_typev6 = null, $p_ip_range_from = null, $p_ip_range_to = null, $p_description = null, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $l_ip_dao->update_ip_assignment_by_ip_range(
            $p_obj_id,
            ((Ip::validate_ip($l_old['isys_cats_net_dhcp_list__range_from'])) ? C__CATP__IP__ASSIGN__STATIC : C__CMDB__CATG__IP__STATIC),
            $l_old['isys_cats_net_dhcp_list__range_from'],
            $l_old['isys_cats_net_dhcp_list__range_to']
        );
        ...
        $l_sql = 'UPDATE ' . $this->m_table . ' SET ' . $this->m_table . '__isys_obj__id = ' . $this->convert_sql_id(
                $p_obj_id
            ) . ', ' . $this->m_table . '__isys_net_dhcp_type__id = ' . $this->convert_sql_id(
                $p_type
            ) . ', ' . $this->m_table . '__isys_net_dhcpv6_type__id = ' . $this->convert_sql_id(
                $p_typev6
            ) . ', ' . $this->m_table . '__range_from = ' . $this->convert_sql_text(
                $p_ip_range_from
            ) . ', ' . $this->m_table . '__range_from_long = ' . $this->convert_sql_text(
                Ip::ip2long($p_ip_range_from)
            ) . ', ' . $this->m_table . '__range_to = ' . $this->convert_sql_text($p_ip_range_to) . ', ' . $this->m_table . '__range_to_long = ' . $this->convert_sql_text(
                Ip::ip2long($p_ip_range_to)
            ) . ', ' . $this->m_table . '__description = ' . $this->convert_sql_text($p_description) . ', ' . $this->m_table . '__status = ' . $this->convert_sql_int(
                $p_status
    ...
    public function update_ranges($p_id, $p_from = null, $p_to = null)
    {
        ...
        if ($p_from !== null)
        {
            $l_update[] = $this->m_table . '__range_from = ' . $this->convert_sql_text($p_from) . ', ' . $this->m_table . '__range_from_long = ' . Ip::ip2long($p_from);
        } // if

        if ($p_to !== null)
        {
            $l_update[] = $this->m_table . '__range_to = ' . $this->convert_sql_text($p_to) . ', ' . $this->m_table . '__range_to_long = ' . Ip::ip2long($p_to);
        } // if

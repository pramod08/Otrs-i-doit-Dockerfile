...
class isys_cmdb_dao_category_g_drive extends isys_cmdb_dao_category_global
{
    ...
    public function dynamic_property_callback_capacity($p_row)
    {
        ...
        $l_obj_id = $p_row['isys_obj__id'] ?: ($p_row['isys_catg_drive_list__isys_obj__id'] ?: false);

        if ($l_obj_id)
        {
            $l_drive_res = isys_cmdb_dao_category_g_drive::instance($g_comp_database)
            ...
            if (count($l_drive_res) > 0)
            {
                ...
                {
                    $l_return[] = $l_drive_row['isys_catg_drive_list__driveletter'] . ' ' . isys_convert::memory(
                            $l_drive_row['isys_catg_drive_list__capacity'],
                            $l_drive_row['isys_memory_unit__const'],
     ...
     * @return  string
     */
    public function dynamic_property_callback_free_space ($p_row)
    {
        global $g_comp_database;
        ...
        if($l_obj_id)
        {
            ...
            if (count($l_drive_res) > 0)
            {
                ...
                {
                    $l_return[] = $l_drive_row['isys_catg_drive_list__driveletter'] . ' ' . isys_convert::memory(
                            $l_drive_row['isys_catg_drive_list__free_space'],
                            $l_drive_row['isys_catg_drive_list__free_space__isys_memory_unit__id'],
                            C__CONVERT_DIRECTION__BACKWARD
                        ) . ' ' . isys_factory_cmdb_dialog_dao::get_instance(
                            'isys_memory_unit',
                            isys_application::instance()->database
                        )->get_data($l_drive_row["isys_catg_drive_list__free_space__isys_memory_unit__id"])['isys_memory_unit__title'];
                } // while
     ...
     * @return  string
     */
    public function dynamic_property_callback_used_space ($p_row)
    {
        global $g_comp_database;
            ...
            if (count($l_drive_res) > 0)
            {
                ...
                while ($l_drive_row = $l_drive_res->get_row())
                {
                    $l_return[] = $l_drive_row['isys_catg_drive_list__driveletter'] . ' ' .
                        isys_convert::memory($l_drive_row['isys_catg_drive_list__used_space'], $l_drive_row['isys_catg_drive_list__used_space__isys_memory_unit__id'], C__CONVERT_DIRECTION__BACKWARD) .
                        ' ' . isys_factory_cmdb_dialog_dao::get_instance('isys_memory_unit', isys_application::instance()->database)
                                  ->get_data($l_drive_row["isys_catg_drive_list__used_space__isys_memory_unit__id"])['isys_memory_unit__title'];
                } // while
    ...
    public function create($p_objID, $p_newRecStatus, $p_typeID = null, $p_letter = null, $p_title = null, $p_fsID = null, $p_capacity = null, $p_unitID = null, $p_deviceID = null, $p_raidID = null, $p_raidLevelID = null, $p_ldevclientID = null, $p_const = null, $p_software_raid = null, $p_description = null, $p_system_drive = null, $p_serial = null, $p_free_space = null, $p_free_space_unit = null, $p_used_space = null, $p_used_space_unit = null)
    {
        ...
        $l_update = "INSERT INTO isys_catg_drive_list SET " . "isys_catg_drive_list__isys_stor_raid_level__id = " . $this->convert_sql_id(
                $p_raidLevelID
            ) . ", " . "isys_catg_drive_list__isys_filesystem_type__id = " . $this->convert_sql_id(
                $p_fsID
            ) . ", " . "isys_catg_drive_list__isys_memory_unit__id = " . $this->convert_sql_id(
                $p_unitID
            ) . ", " . "isys_catg_drive_list__isys_catd_drive_type__id = " . $this->convert_sql_id(
                $p_typeID
            ) . ", " . "isys_catg_drive_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_drive_list__capacity = '" . $p_capacity . "', " . "isys_catg_drive_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_catg_drive_list__driveletter = " . $this->convert_sql_text(
                $p_letter
            ) . ", " . "isys_catg_drive_list__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_deviceID
            ) . ", " . "isys_catg_drive_list__isys_catg_raid_list__id = " . $this->convert_sql_id(
                $p_raidID
            ) . ", " . "isys_catg_drive_list__isys_catg_ldevclient_list__id = " . $this->convert_sql_id(
                $p_ldevclientID
            ) . ", " . "isys_catg_drive_list__status = " . $this->convert_sql_id($p_newRecStatus) . ", " . "isys_catg_drive_list__const = " . $this->convert_sql_text(
                $p_const
            ) . ", " . "isys_catg_drive_list__id__raid_pool = " . $this->convert_sql_id(
                $p_software_raid
            ) . ", " . "isys_catg_drive_list__system_drive = " . $this->convert_sql_int($p_system_drive) . ", " . "isys_catg_drive_list__serial = " . $this->convert_sql_text(
                $p_serial
            ) . ", " . "isys_catg_drive_list__free_space = '" . $p_free_space . "', " . "isys_catg_drive_list__free_space__isys_memory_unit__id = " . $this->convert_sql_id(
                $p_free_space_unit
            ) . ", " . "isys_catg_drive_list__used_space = '" . $p_used_space . "', " . "isys_catg_drive_list__used_space__isys_memory_unit__id = " . $this->convert_sql_id(
                $p_used_space_unit
    ...
    public function save($p_catLevel, $p_newRecStatus, $p_typeID = null, $p_letter = null, $p_title = null, $p_fsID = null, $p_capacity = null, $p_unitID = null, $p_deviceID = null, $p_raidID = null, $p_raidLevelID = null, $p_ldevclientID = null, $p_const = null, $p_software_raid = null, $p_description = null, $p_system_drive = null, $p_serial = null, $p_free_space = null, $p_free_space_unit = null, $p_used_space = null, $p_used_space_unit = null)
    {
        $p_capacity = isys_convert::memory($p_capacity, $p_unitID);
        $p_free_space = isys_convert::memory($p_free_space, $p_free_space_unit);
        $p_used_space = isys_convert::memory($p_used_space, $p_used_space_unit);

        $l_update = "UPDATE isys_catg_drive_list SET " . "isys_catg_drive_list__isys_stor_raid_level__id = " . $this->convert_sql_id(
                $p_raidLevelID
            ) . ", " . "isys_catg_drive_list__isys_filesystem_type__id = " . $this->convert_sql_id(
                $p_fsID
            ) . ", " . "isys_catg_drive_list__isys_memory_unit__id = " . $this->convert_sql_id(
                ...
                $p_description
            ) . ", " . "isys_catg_drive_list__driveletter = " . $this->convert_sql_text($p_letter) . ", " . "isys_catg_drive_list__const = " . $this->convert_sql_text(
                $p_const
            ) . ", " . "isys_catg_drive_list__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_deviceID
            ) . ", " . "isys_catg_drive_list__isys_catg_raid_list__id = " . $this->convert_sql_id(
                ...
                $p_used_space_unit
            ) . ", " . "isys_catg_drive_list__status = " . $this->convert_sql_id($p_newRecStatus) . " " . "WHERE isys_catg_drive_list__id = " . $this->convert_sql_id(
                $p_catLevel
            );
    ...
    {
        $l_sql = 'UPDATE isys_catg_drive_list SET isys_catg_drive_list__id__raid_pool = NULL WHERE isys_catg_drive_list__id__raid_pool = ' . $this->convert_sql_id(
                $p_raidID
            ) . ';';
    ...
    public function compare_category_data(&$p_category_data_values, &$p_object_category_dataset, &$p_used_properties, &$p_comparison, &$p_badness, &$p_mode, &$p_category_id, &$p_unit_key, &$p_category_data_ids, &$p_local_export, &$p_dataset_id_changed, &$p_dataset_id, &$p_logger, &$p_category_name = null, &$p_table = null, &$p_cat_multi = null, &$p_category_type_id = null, &$p_category_ids = null, &$p_object_ids = null, &$p_already_used_data_ids = null)
    {
        ...
        $l_capacity = $p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['capacity'][C__DATA__VALUE];
        $l_free_space = $p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['free_space'][C__DATA__VALUE];
        $l_used_space = $p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['used_space'][C__DATA__VALUE];
        ...
        foreach ($p_object_category_dataset as $l_dataset_key => $l_dataset)
        {
            ...
            $l_dataset_capacity = isys_convert::memory($l_dataset['isys_catg_drive_list__capacity'], $l_dataset['isys_memory_unit__const'], C__CONVERT_DIRECTION__BACKWARD);
            $l_dataset_free_space = isys_convert::memory($l_dataset['isys_catg_drive_list__free_space'], $l_dataset['isys_catg_drive_list__free_space__isys_memory_unit__id'], C__CONVERT_DIRECTION__BACKWARD);
            $l_dataset_used_space = isys_convert::memory($l_dataset['isys_catg_drive_list__used_space'], $l_dataset['isys_catg_drive_list__used_space__isys_memory_unit__id'], C__CONVERT_DIRECTION__BACKWARD);

            if ($l_dataset_title === $l_title && $l_dataset_mount_point === $l_mount_point && $l_dataset_capacity == $l_capacity && $l_dataset_used_space == $l_used_space && $l_dataset_free_space == $l_free_space)
            {
            ...
            else if ($l_dataset_title === $l_title && $l_dataset_mount_point === $l_mount_point && ($l_dataset_capacity != $l_capacity || $l_dataset_used_space != $l_used_space || $l_dataset_free_space != $l_free_space))
            {

...
class isys_cmdb_dao_category_g_controller extends isys_cmdb_dao_category_global
{
                ...
                foreach ($l_types as $l_data)
                {
                    if ($l_type)
                    {
                        ...
                        if ($l_type_id)
                        {
                            ...
                            $this->save_element($l_cat_level, $l_catdata['isys_catg_controller_list__status']);
                        }
                    }
                }
            }
    ...
    {
        $l_update = "INSERT INTO isys_catg_controller_list SET " . "isys_catg_controller_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_catg_controller_list__status = " . $this->convert_sql_id(
                $p_recStatus
            ) . ", " . "isys_catg_controller_list__isys_controller_type__id = " . $this->convert_sql_id(
                $p_typeID
            ) . ", " . "isys_catg_controller_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_controller_list__isys_controller_manufacturer__id = " . $this->convert_sql_id(
                $p_manufacturerID
            ) . ", " . "isys_catg_controller_list__isys_controller_model__id = " . $this->convert_sql_id(
                $p_modelID
            ) . ", " . "isys_catg_controller_list__description = " . $this->convert_sql_text($p_description);
    ...
    {
        $l_update = "UPDATE isys_catg_controller_list SET " . "isys_catg_controller_list__status = " . $this->convert_sql_id(
                $p_recStatus
            ) . ", " . "isys_catg_controller_list__isys_controller_type__id = " . $this->convert_sql_id(
                $p_typeID
            ) . ", " . "isys_catg_controller_list__title = " . $this->convert_sql_text(
                ...
                $p_modelID
            ) . ", " . "isys_catg_controller_list__description = " . $this->convert_sql_text(
                $p_description
            ) . " " . "WHERE isys_catg_controller_list__id = " . $this->convert_sql_id($p_cat_level);
        ...
        return [
            ...
            'description'  => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__CONTROLLER
                    ]
                ]
            )
        ];

...
class isys_cmdb_dao_category_g_net_listener extends isys_cmdb_dao_category_global
{
    ...
    {
        $l_sql = 'SELECT ' . 'isys_net_protocol__title as protocol, isys_net_protocol_layer_5__title as protocol_layer_5, source_obj.isys_obj__title as source_object, source_obj.isys_obj__id as source_object_id, source.isys_cats_net_ip_addresses_list__title as source_ip, isys_catg_net_connector_list__port_from source_port_from, isys_catg_net_connector_list__port_to as source_port_to, ' . 'bind_obj.isys_obj__title as bind_object, bind_obj.isys_obj__id as bind_object_id, bind_app_obj.isys_obj__title as bind_application, bind.isys_cats_net_ip_addresses_list__title as bind_ip, isys_catg_net_listener_list__port_from as bind_port_from, isys_catg_net_listener_list__port_to as bind_port_to, ' . 'rel_obj.isys_obj__title as relation_object, rel_obj.isys_obj__id as relation_object_id, isys_cats_net_list__address as net_address, network.isys_obj__title as network, source_gateway.isys_obj__title as source_gateway, bind_gateway.isys_obj__title as bind_gateway ' . 'FROM isys_catg_net_connector_list ' . 'INNER JOIN isys_catg_net_listener_list ON isys_catg_net_listener_list__id = isys_catg_net_connector_list__isys_catg_net_listener_list__id ' . 'INNER JOIN isys_obj source_obj ON isys_catg_net_connector_list__isys_obj__id = source_obj.isys_obj__id ' . 'INNER JOIN isys_obj bind_obj ON isys_catg_net_listener_list__isys_obj__id = bind_obj.isys_obj__id ' . 'LEFT JOIN isys_obj bind_app_obj ON isys_catg_net_listener_list__opened_by = bind_app_obj.isys_obj__id ' . 'LEFT JOIN isys_catg_relation_list ON isys_catg_relation_list__id = isys_catg_net_connector_list__isys_catg_relation_list__id ' . 'LEFT JOIN isys_obj rel_obj ON isys_catg_relation_list__isys_obj__id = rel_obj.isys_obj__id ' . 'INNER JOIN isys_net_protocol ON isys_catg_net_listener_list__isys_net_protocol__id = isys_net_protocol__id ' . 'INNER JOIN isys_cats_net_ip_addresses_list source ON isys_catg_net_connector_list__ip_addresses_list__id = source.isys_cats_net_ip_addresses_list__id ' . 'INNER JOIN isys_cats_net_ip_addresses_list bind ON isys_catg_net_listener_list__isys_cats_net_ip_addresses_list__id = bind.isys_cats_net_ip_addresses_list__id ' . 'INNER JOIN isys_cats_net_list ON bind.isys_cats_net_ip_addresses_list__isys_obj__id = isys_cats_net_list__isys_obj__id ' . 'INNER JOIN isys_obj network ON bind.isys_cats_net_ip_addresses_list__isys_obj__id = network.isys_obj__id ' . 'LEFT JOIN isys_net_protocol_layer_5 ON isys_catg_net_listener_list__isys_net_protocol_layer_5__id = isys_net_protocol_layer_5__id ' . 'LEFT JOIN isys_obj source_gateway ON isys_catg_net_connector_list__gateway = source_gateway.isys_obj__id ' . 'LEFT JOIN isys_obj bind_gateway ON isys_catg_net_listener_list__gateway = bind_gateway.isys_obj__id ' . 'WHERE (' . 'bind_obj.isys_obj__status = ' . $this->convert_sql_id(
                $p_status
            ) . ' AND ' . 'source_obj.isys_obj__status = ' . $this->convert_sql_id($p_status) . ' AND ' . 'isys_catg_net_connector_list__status = ' . $this->convert_sql_id(
                $p_status
            ) . ')';

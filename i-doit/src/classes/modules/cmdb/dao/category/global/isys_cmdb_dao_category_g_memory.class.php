...
class isys_cmdb_dao_category_g_memory extends isys_cmdb_dao_category_global
{
    ...
    public function dynamic_property_callback_capacity(array $p_row)
    {
        ...
        $l_res = $l_dao->retrieve(
            'SELECT isys_catg_memory_list__capacity, isys_memory_unit__const, isys_memory_unit__title
            FROM isys_catg_memory_list
            LEFT JOIN isys_memory_unit ON isys_memory_unit__id = isys_catg_memory_list__isys_memory_unit__id
            WHERE isys_catg_memory_list__isys_obj__id = ' . $l_dao->convert_sql_id($p_row['__id__']) . '
        ...
        if (count($l_res) > 0)
        {
            ...
            while ($l_row = $l_res->get_row())
            {
                $l_return[] = isys_convert::memory(
                        $l_row['isys_catg_memory_list__capacity'],
                        $l_row['isys_memory_unit__const'],
                        C__CONVERT_DIRECTION__BACKWARD
                    ) . ' ' . $l_row['isys_memory_unit__title'];
            } // while
    ...
    public function dynamic_property_callback_total_capacity(array $p_row)
    {
        ...
        if (count($l_res))
        {
            ...
            return isys_convert::memory($l_capacity, $l_unit, C__CONVERT_DIRECTION__BACKWARD) . ' ' . $l_unit_title;
        } // if
    ...
    public function import($p_data)
    {
        ...
        if (is_array($p_data))
        {
            ...
            foreach ($p_data as $l_data)
            {
                ...
                /* Manufacturer */
                $_POST['C__CATG__MEMORY_MANUFACTURER'] = isys_import::check_dialog(
                ...
                /* Capacity */
                if (empty($l_data["capacity"])) $l_capacity = $l_data["size"];
                else $l_capacity = $l_data["capacity"];
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus, $p_create = false)
    {
        ...
        if ($p_create)
        {
            ...
            if (isys_glob_get_param(
                    C__CMDB__GET__CATG
                ) == C__CATG__OVERVIEW && $_POST['C__CATG__MEMORY_MANUFACTURER'] == -1 && $_POST['C__CATG__MEMORY_TITLE_ID'] == -1 && $_POST['C__CATG__MEMORY_TYPE'] == -1 && $_POST['C__CATG__MEMORY_UNIT'] == -1 && $_POST['C__CATG__MEMORY_CAPACITY'] == "0"
            )
            {
                return null;
            } // if
            ...
            if (!is_numeric($l_quantity) || $l_quantity < 1) throw new isys_exception_dao("Invalid value for quantity");
            ...
            while ($i++ <= $l_quantity)
            {
                $l_id = $this->create(
                    ...
                    $_POST['C__CATG__MEMORY_TITLE_ID'],
                    $_POST['C__CATG__MEMORY_MANUFACTURER'],
                    $_POST['C__CATG__MEMORY_TYPE'],
                    $_POST['C__CATG__MEMORY_UNIT'],
                    $_POST['C__CATG__MEMORY_CAPACITY'],
                    $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . $this->get_category_id()]
                );
        ...
        else
        {
            ...
            if ($l_catdata['isys_catg_memory_list__id'] != "")
            {
                ...
                $l_bRet = $this->save(
                    $l_catdata["isys_catg_memory_list__id"],
                    C__RECORD_STATUS__NORMAL,
                    $_POST['C__CATG__MEMORY_TITLE_ID'],
                    $_POST['C__CATG__MEMORY_MANUFACTURER'],
                    $_POST['C__CATG__MEMORY_TYPE'],
    ...
    public function compare_category_data(&$p_category_data_values, &$p_object_category_dataset, &$p_used_properties, &$p_comparison, &$p_badness, &$p_mode, &$p_category_id, &$p_unit_key, &$p_category_data_ids, &$p_local_export, &$p_dataset_id_changed, &$p_dataset_id, &$p_logger, &$p_category_name = null, &$p_table = null, &$p_cat_multi = null, &$p_category_type_id = null, &$p_category_ids = null, &$p_object_ids = null, &$p_already_used_data_ids = null)
    {
        ...
        else
        {
            ...
            $l_capacity = strval(
                isys_convert::memory(
                    $p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['capacity']['value'],
                    $l_unit,
    ...
    {
        return [
            'quantity'     => array_replace_recursive(
            ...
            'capacity'     => array_replace_recursive(
                ...
                [
                    C__PROPERTY__INFO     => [
                        C__PROPERTY__INFO__TITLE       => 'LC__CMDB__CATS__SAN_CAPACITY_VALUE',
                        C__PROPERTY__INFO__DESCRIPTION => 'Capacity'
                    ],
                    C__PROPERTY__DATA     => [
            ...
            'description'  => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__MEMORY
                    ]
                ]
            )
        ];

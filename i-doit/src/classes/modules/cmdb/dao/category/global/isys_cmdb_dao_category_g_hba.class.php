...
class isys_cmdb_dao_category_g_hba extends isys_cmdb_dao_category_global
{
    ...
    public function get_data($p_catg_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        ...
        $l_sql = "SELECT * FROM isys_catg_hba_list " . "INNER JOIN isys_obj " . "ON " . "isys_obj__id = " . "isys_catg_hba_list__isys_obj__id " . "LEFT JOIN isys_controller_manufacturer " . "ON " . "isys_controller_manufacturer__id = " . "isys_catg_hba_list__isys_controller_manufacturer__id " . "LEFT JOIN isys_controller_model " . "ON " . "isys_controller_model__id = " . "isys_catg_hba_list__isys_controller_model__id " . "LEFT JOIN isys_hba_type " . "ON " . "isys_hba_type__id = " . "isys_catg_hba_list__isys_hba_type__id " .
        ...
        return [
            ...
            'description'  => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__HBA
                    ]
                ]
            )
        ];
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus, $p_create = false)
    {
        ...
        if ($p_create)
        {
            $l_ret       = $this->create(
                ...
                C__RECORD_STATUS__NORMAL,
                $_POST[C__CATG__STORAGE_CONTROLLER_TYPE],
                $_POST[C__CATG__STORAGE_CONTROLLER_TITLE],
                $_POST[C__CATG__STORAGE_CONTROLLER_MANUFACTURER],
                $_POST[C__CATG__STORAGE_CONTROLLER_MODEL],
                ...
                foreach ($l_types as $l_data)
                {
                    if ($l_type)
                    {
                        ...
                        if ($l_type_id)
                        {
                            ...
                            if (!$_GET[C__CMDB__GET__OBJECT] && !is_null($p_object_id)) $_GET[C__CMDB__GET__OBJECT] = $p_object_id;
    ...
    {
        $l_update = "INSERT INTO isys_catg_hba_list SET " . "isys_catg_hba_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_catg_hba_list__status = " . $this->convert_sql_id($p_recStatus) . ", " . "isys_catg_hba_list__isys_hba_type__id = " . $this->convert_sql_id(
                $p_typeID
            ) . ", " . "isys_catg_hba_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_hba_list__isys_controller_manufacturer__id = " . $this->convert_sql_id(
                $p_manufacturerID
            ) . ", " . "isys_catg_hba_list__isys_controller_model__id = " . $this->convert_sql_id(
                $p_modelID
    ...
    {
        $l_update = "UPDATE isys_catg_hba_list SET " . "isys_catg_hba_list__status = " . $this->convert_sql_id(
                $p_recStatus
            ) . ", " . "isys_catg_hba_list__isys_hba_type__id = " . $this->convert_sql_id($p_typeID) . ", " . "isys_catg_hba_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_hba_list__isys_controller_manufacturer__id = " . $this->convert_sql_id(
                $p_manufacturerID
            ) . ", " . "isys_catg_hba_list__isys_controller_model__id = " . $this->convert_sql_id(
                $p_modelID
            ) . ", " . "isys_catg_hba_list__description = " . $this->convert_sql_text($p_description) . " " . "WHERE isys_catg_hba_list__id = " . $this->convert_sql_id(
                $p_cat_level
            );
    ...
    {
        $l_q = "SELECT isys_catg_hba_list__title, isys_catg_hba_list__id, isys_catg_hba_list__status, isys_hba_type__const " . "FROM isys_catg_hba_list " . "INNER JOIN isys_hba_type ON isys_catg_hba_list__isys_hba_type__id = isys_hba_type__id " . "WHERE isys_catg_hba_list__isys_obj__id = " . $this->convert_sql_id(
                $p_obj_id
            ) . " " . "AND isys_hba_type__const = 'C__STOR_TYPE_FC_CONTROLLER';";
    ...
    public function object_browser($p_context, array $p_parameters)
    {
        ...
        switch ($p_context)
        {
            case isys_popup_browser_object_ng::C__CALL_CONTEXT__REQUEST:
                ...
                $l_obj = isys_cmdb_dao_category_g_hba::instance($this->m_db);
                $l_sql = "SELECT isys_obj__isys_obj_type__id, isys_catg_hba_list__id, isys_catg_hba_list__title, isys_obj_type__title " . "FROM isys_catg_hba_list " . "LEFT JOIN isys_obj ON isys_obj__id = isys_catg_hba_list__isys_obj__id " . "LEFT JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id " . "WHERE isys_catg_hba_list__isys_obj__id = " . $l_obj->convert_sql_id(
                        $_GET[C__CMDB__GET__OBJECT]
                    );
                ...
                if ($l_objects->num_rows() > 0)
                {
                    while ($l_row = $l_objects->get_row())
                    {
                        $l_return[] = [
                            ...
                            _L('LC__CMDB__LOGBOOK__TITLE') => $l_row["isys_catg_hba_list__title"]
                        ];
                    } // while
                } // if
            ...
            case isys_popup_browser_object_ng::C__CALL_CONTEXT__PREPARATION:
                ...
                {
                    $l_sql = "SELECT isys_obj__isys_obj_type__id, isys_catg_hba_list__id, isys_catg_hba_list__title, isys_obj_type__title " . "FROM isys_catg_hba_list " . "LEFT JOIN isys_obj ON isys_obj__id = isys_catg_hba_list__isys_obj__id " . "LEFT JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id " . "WHERE isys_catg_hba_list__id IN (" . $p_preselection . ")";

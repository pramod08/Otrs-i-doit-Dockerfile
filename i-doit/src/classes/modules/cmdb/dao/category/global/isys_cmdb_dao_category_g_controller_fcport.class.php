...
class isys_cmdb_dao_category_g_controller_fcport extends isys_cmdb_dao_category_global
{
    ...
    public function callback_property_connected_controller(isys_request $p_request)
    {
        ...
        return serialize($l_return);
    } // function

    public function get_subcategory_id()
    {
        return C__CMDB__SUBCAT__STORAGE__FCPORT;
    }
    ...
    public function get_data($p_catg_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        ...
        $l_strSQL = "SELECT isys_catg_fc_port_list.*, isys_fc_port_type.*, isys_fc_port_medium.*, isys_catg_hba_list.*, isys_obj.*, isys_port_speed.* " . ", connected.isys_catg_connector_list__title AS connector_name, connected.isys_catg_connector_list__id AS con_connector, mine.isys_catg_connector_list__isys_catg_relation_list__id AS isys_catg_connector_list__isys_catg_relation_list__id " . "FROM " . "isys_catg_fc_port_list " . "LEFT JOIN " . "isys_fc_port_type " . "ON " . "isys_catg_fc_port_list__isys_fc_port_type__id = " . "isys_fc_port_type__id " . "LEFT OUTER JOIN " . "isys_fc_port_medium " . "ON " . "isys_catg_fc_port_list__isys_fc_port_medium__id = " . "isys_fc_port_medium__id " . "LEFT JOIN " . "isys_catg_hba_list " . "ON " . "isys_catg_fc_port_list__isys_catg_hba_list__id = " . "isys_catg_hba_list__id " . "LEFT JOIN " . "isys_obj " . "ON " . "isys_obj__id = isys_catg_fc_port_list__isys_obj__id " . "LEFT JOIN " . "isys_port_speed " . "ON " . "isys_port_speed__id = isys_catg_fc_port_list__isys_port_speed__id " .

            "LEFT JOIN isys_catg_connector_list AS mine " . "ON " . "mine.isys_catg_connector_list__id = isys_catg_fc_port_list__isys_catg_connector_list__id " . "LEFT JOIN isys_cable_connection " . "ON " . "mine.isys_catg_connector_list__isys_cable_connection__id = " . "isys_cable_connection__id " . "LEFT JOIN isys_catg_connector_list AS connected ON " . "connected.isys_catg_connector_list__isys_cable_connection__id = isys_cable_connection__id " . "AND (connected.isys_catg_connector_list__id != mine.isys_catg_connector_list__id OR connected.isys_catg_connector_list__id IS NULL) " .
    ...
    protected function properties()
    {
        return [
            ...
            'connected_controller' => array_replace_recursive(
            ...
            'medium'               => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog_plus(),
                [
                    C__PROPERTY__INFO => [
            ...
            'speed_unit'           => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'san_zones'            => array_replace_recursive(
            ...
            'description'          => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__CONTROLLER_FC_PORT
                    ]
                ]
            ),
                    ...
                    if ($p_category_data['data_id'])
                    {
                        if (is_array($l_san_zones))
                        {
                            foreach ($l_san_zones as $l_value)
                            {
                                ...
                                {
                                    $this->set_san_zone_fc_port_port_selection(
                                        $p_category_data['data_id'],
                                        [$l_value[C__DATA__VALUE]]
                                    );
                                } // if
                                if ($l_value['wwn_selected'])
                                {
                                    $this->set_san_zone_fc_port_wwn_selection(
                                        $p_category_data['data_id'],
                                        [$l_value[C__DATA__VALUE]]
                                    );
                                } // if
    ...
    public function set_san_zone_fc_port_port_selection($p_id, $p_selected_zones)
    {
        ...
        if (is_array($l_zone_arr))
        {
            foreach ($l_zone_arr AS $l_zone_id)
            {
                if ($l_zone_id > 0)
                {
                    $l_sql_cond[] = "(isys_san_zoning_fc_port__isys_cats_san_zoning_list__id = " . $this->convert_sql_id(
                            $l_zone_id
    ...
    public function attach_to_zones($p_id, $p_zones)
    {
        if (!empty($p_zones))
        {
            ...
            {
                $l_sql = "INSERT INTO isys_san_zoning_fc_port (isys_san_zoning_fc_port__id, " . "isys_san_zoning_fc_port__isys_cats_san_zoning_list__id, " . "isys_san_zoning_fc_port__isys_catg_fc_port_list__id) VALUES";
     ...
     * @return  mixed
     */
    public function create($p_objID, $p_status, $p_controllerID, $p_title, $p_typeID, $p_mediumID, $p_speedValue, $p_speedID, $p_wwn, $p_wwpn, $p_description, $p_connector_sibling = null, $p_connector_id = null, $p_cable_name = null, $p_cableID = null, $p_is_master_obj = null)
    {
        ...
        $l_update = "INSERT INTO isys_catg_fc_port_list SET " . "isys_catg_fc_port_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_catg_fc_port_list__status = " . $this->convert_sql_id(
                $p_status
            ) . ", " . "isys_catg_fc_port_list__isys_catg_hba_list__id = " . $this->convert_sql_id(
                $p_controllerID
            ) . ", " . "isys_catg_fc_port_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_fc_port_list__isys_fc_port_type__id = " . $this->convert_sql_id(
                $p_typeID
            ) . ", " . "isys_catg_fc_port_list__isys_fc_port_medium__id = " . $this->convert_sql_id(
                $p_mediumID
            ) . ", " . "isys_catg_fc_port_list__port_speed = '" . $l_speedValue . "', " . "isys_catg_fc_port_list__isys_port_speed__id = " . $this->convert_sql_id(
                $p_speedID
            ) . ", " . "isys_catg_fc_port_list__wwn = " . $this->convert_sql_text($p_wwn) . ", " . "isys_catg_fc_port_list__wwpn = " . $this->convert_sql_text(
                $p_wwpn
            ) . ", " . "isys_catg_fc_port_list__description = " . $this->convert_sql_text(
                $p_description
     ...
     */
    public function save($p_catLevel, $p_status, $p_controllerID, $p_title, $p_typeID, $p_mediumID, $p_speedValue, $p_speedID, $p_wwn, $p_wwpn, $p_connector_id, $p_cable_name, $p_cable_id = null, $p_description = "", $p_connector_sibling = null, $p_is_master_obj = null)
    {
        $l_speedValue = isys_convert::speed($p_speedValue, $p_speedID);

        $l_update = "UPDATE isys_catg_fc_port_list SET " . "isys_catg_fc_port_list__status = " . $this->convert_sql_id(
                $p_status
            ) . ", " . "isys_catg_fc_port_list__isys_catg_hba_list__id = " . $this->convert_sql_id(
                $p_controllerID
            ) . ", " . "isys_catg_fc_port_list__title = " . $this->convert_sql_text(
                ...
                $p_description
            ) . " " . "WHERE isys_catg_fc_port_list__id = " . $this->convert_sql_id($p_catLevel);
        ...
        if (is_numeric($l_myconnector_id) && $l_myconnector_id > 0)
        {
            ...
            $l_strSQL_connector .= "isys_catg_connector_list__title = " . $this->convert_sql_text(
                    $p_title
                ) . " " . "WHERE isys_catg_connector_list__id = " . $this->convert_sql_id($l_myconnector_id) . "";
        ...
        if ($this->update($l_update))
        {
            ...
            if ($p_connector_id != null && $p_connector_id != "")
            {
                /**
                 * Add / Change cable connection
                 */
                if ($p_cable_id == "")
                {
                    $l_cableID = isys_cmdb_dao_cable_connection::add_cable($p_cable_name);
                }
                else
                {
                    $l_cableID = $p_cable_id;
                }

                if ($p_is_master_obj)
                {
                    $l_master_connector = $l_myconnector_id;
                }
                else
                {
                    $l_master_connector = $p_connector_id;
                }

                $this->connection_save($l_myconnector_id, $p_connector_id, $l_cableID, $l_master_connector);
            }
            else
            {

                $l_dao_cable = new isys_cmdb_dao_cable_connection($this->m_db);

                if (empty($p_connector_id))
                {
                    $l_dao_cable->delete_cable_connection($l_dao_cable->get_cable_connection_id_by_connector_id($l_myconnector_id));
                } // if
            } // if
    ...
    {
        $l_query = 'SELECT isys_catg_fc_port_list__isys_catg_connector_list__id FROM isys_catg_fc_port_list WHERE isys_catg_fc_port_list__id = ' . $this->convert_sql_id(
                $p_cat_id
            ) . ';';
    ...
    {
        $l_query = 'SELECT * FROM isys_obj_type INNER JOIN isys_obj ON isys_obj__isys_obj_type__id = isys_obj_type__id ' . 'WHERE isys_obj__id IN (SELECT isys_catg_fc_port_list__isys_obj__id FROM isys_catg_fc_port_list WHERE isys_obj_type__id = ' . $this->convert_sql_id(
                $p_objtype_id
            ) . ')';
    ...
    {
        $l_query = 'SELECT * FROM isys_catg_fc_port_list WHERE isys_catg_fc_port_list__isys_obj__id = ' . $this->convert_sql_id(
                $p_obj_id
                ...
                if ($l_fc_res->num_rows() > 0)
                {
                    while ($l_fc_row = $l_fc_res->get_row())
                    {
                        $l_return[$l_fc_row['isys_obj__id']][] = [
                            ...
                            'obj_type_title'   => _L($l_dao->get_objtype_name_by_id_as_string($l_fc_row['isys_obj__isys_obj_type__id'])),

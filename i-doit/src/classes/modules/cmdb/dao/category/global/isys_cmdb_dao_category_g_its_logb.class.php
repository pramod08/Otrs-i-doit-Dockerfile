        ...
        if ($l_objects->num_rows() > 0)
        {
            ...
            while ($l_row = $l_objects->get_row())
            {
                ...
                if (!in_array($l_row["isys_connection__isys_obj__id"], $l_arObjects))
                {
                    ...
                    if (intval($l_rowInfo["isys_obj_type__id"]) == C__OBJTYPE__RELATION)
                    {
    ...
    public function sync($p_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        try
        {
            ...
            foreach ($p_data as $l_data)
            {
                ...
                $l_dao->set_entry(
                    $l_data[$l_map["isys_logbook__event_static"]],
                    $l_data["log"],
                    $l_date,
                    $l_alert_level,
                    $p_object_id,
                    $l_data[$l_map["isys_logbook__obj_name_static"]],
                    $l_data[$l_map["isys_logbook__obj_type_static"]],
                    $l_data[$l_map["isys_logbook__category_static"]],
                    C__LOGBOOK_SOURCE__ALL,
                    "",
                    $l_data[$l_map["isys_logbook__description"]]
                );
            } // foreach
        }

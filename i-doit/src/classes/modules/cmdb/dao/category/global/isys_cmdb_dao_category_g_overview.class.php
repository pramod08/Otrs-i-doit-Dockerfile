...
class isys_cmdb_dao_category_g_overview extends isys_cmdb_dao_category_global
{
    ...
    public function validate_user_data()
    {
        ...
        {
            if (count($l_categories) === 0) continue;
    ...
    public function get_categories_as_array($p_obj_type, $p_obj_id, $p_category_type = C__CMDB__CATEGORY__TYPE_GLOBAL, $p_rec_status = C__RECORD_STATUS__NORMAL, $p_overview_only = true, $p_cats_childs = false)
    {
        ...
        if (!is_null($l_categories) && $l_categories->num_rows() > 0)
        {
            ...
            {
                if ($p_overview_only && ($l_allowed_categories === false || (is_array($l_allowed_categories) && !in_array(
                                $l_row['isysgui_cat' . $l_cat_prefix . '__const'],
                                $l_allowed_categories
                            )))
                )
                {
                    continue;
            ...
            if (isset($l_array) && is_array($l_array))
            {
                try
                {
                    $l_dist = new isys_cmdb_dao_distributor(
                        $this->m_db, $p_obj_id, $p_category_type, null, isset($l_distcat) ? $l_distcat : null
                    );
     ...
     * @return array
     */
    public function get_custom_categories_as_array($p_obj_type, $p_obj_id, $p_category_type = C__CMDB__CATEGORY__TYPE_CUSTOM, $p_rec_status = C__RECORD_STATUS__NORMAL, $p_overview_only = true)
    {
        global $g_comp_template_language_manager;
            ...
            if (is_array($p_categories))
            {
                foreach ($p_categories as $l_cat)
                {
                    ...
                    )
                    {
                        ...
                        $l_dist = new isys_cmdb_dao_distributor(
                            $this->m_db, $p_object_id, $p_cat_type, null, [$l_cat => true]
                        );
                        ...
                        if ($l_dist && $l_dist->count() > 0)
                        {
                            ...
                            */
                            ...
                            if (!in_array(get_class($l_current_cat), array_keys($l_changed)) && get_class(
                                    $l_current_cat
                                ) != 'isys_cmdb_dao_category_g_global' /*&& $l_custom_category === false*/
                            )
                            {
                                continue;
                            }
                            ...
                            else
                            {
                                ...
                                if (method_exists($l_current_cat, "save_element"))
                                {
                                    ...
                                    $l_saved = $l_current_cat->save_element(
                                        $p_cat_level,
                                        $l_category_id,
                                        $l_category_id ? false : true
                                    );
                                }
                                else
                                {
    ...
    public function assign_global_category($p_objecttype_id)
    {
        ...
        {
            $l_sql = "INSERT INTO isys_obj_type_2_isysgui_catg_overview VALUES(" . $this->convert_sql_id($p_objecttype_id) . ", " . $this->convert_sql_id(
                    C__CATG__GLOBAL
                ) . ", 0)";

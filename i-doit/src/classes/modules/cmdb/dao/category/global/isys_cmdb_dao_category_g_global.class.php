...
class isys_cmdb_dao_category_g_global extends isys_cmdb_dao_category_global
{
    ...
    public function dynamic_property_callback_created(array $p_row)
    {
        ...
        if (!empty($p_row["isys_obj__created_by"]))
        {
            ...
            return '<span data-date="' . $p_row["isys_obj__created"] . '" class="hide"></span>' . $g_loc->fmt_date(
                $p_row["isys_obj__created"]
            ) . " (" . $p_row["isys_obj__created_by"] . ")";
        }
        else
        {
    ...
    public function dynamic_property_callback_changed(array $p_row)
    {
        ...
        {
            return '<span data-date="' . $p_row["isys_obj__updated"] . '" class="hide"></span>' . $g_loc->fmt_date(
                $p_row["isys_obj__updated"]
            ) . " (" . $p_row["isys_obj__updated_by"] . ")";
        }
        else
        {
...
(
       ...
       OR (isys_obj.isys_obj__description ' . $p_filter->get() . ')
        ...
        {
            $l_sql .= ' AND isys_obj__isys_obj_type__id NOT IN (' . implode(
                    ',',
                    [
    ...
    {
        $l_sql = "UPDATE isys_obj " . "SET isys_obj__description = " . $this->convert_sql_text($p_description) . " " . "WHERE isys_obj__id = " . $this->convert_sql_id(
                $p_object_id
            ) . ";";
     ...
     */
    public function save($p_catlevel, $p_status, $p_title, $p_sysid, $p_catID, $p_purposeID, $p_description, $p_obj_id, $p_overwrite_sysid = false, $p_cmdb_status_id = null, $p_objtype_id = null, $p_tags = null)
    {
        $l_old_title = $this->get_obj_name_by_id_as_string($p_obj_id);

        if (isys_cmdb_dao_category_g_accounting::has_placeholders($p_title) && $p_status != C__RECORD_STATUS__TEMPLATE && $p_status != C__RECORD_STATUS__MASS_CHANGES_TEMPLATE)
        {
            $p_title = isys_cmdb_dao_category_g_accounting::instance($this->get_database_component())
                ->replace_placeholders(
                    $p_title,
                    $this->get_last_obj_id_from_type(),
                    (($p_objtype_id === null) ? $this->get_objTypeID($p_obj_id) : $p_objtype_id),
                    null,
                    $this->get_sysid_by_obj_id($p_obj_id),
        ...
        $l_strSql = "UPDATE isys_obj SET " . "isys_obj__title = " . $this->convert_sql_text($p_title) . ", " . "isys_obj__description = " . $this->convert_sql_text(
                $p_description
            ) . ", ";
        ...
        $l_strSql2 = "UPDATE isys_catg_global_list SET " . "isys_catg_global_list__isys_purpose__id = " . $this->convert_sql_id(
                $p_purposeID
            ) . ", " . "isys_catg_global_list__isys_catg_global_category__id = " . $this->convert_sql_id(
                $p_catID
            ) . ", " . "isys_catg_global_list__status = " . C__RECORD_STATUS__NORMAL . " " . "WHERE isys_catg_global_list__id = " . $this->convert_sql_id($p_catlevel);
    ...
    public function save_element(&$p_cat_level, &$p_new_id, $p_create)
    {
        ...
        if ($l_success)
        {
            ...
            if (C__RECORD_STATUS__BIRTH == (int) $l_intOldRecStatus && $l_new_status != C__RECORD_STATUS__TEMPLATE &&
                $l_new_status != C__RECORD_STATUS__MASS_CHANGES_TEMPLATE && ($l_auto_inventory = trim(
                    isys_tenantsettings::get('cmdb.objtype.' . $_POST["C__OBJ__TYPE"] . '.auto-inventory-no', '')
                )) !== ''
            )
            {
                if (strpos(' ' . $l_auto_inventory, '%OBJTITLE%'))
                {
                    ...
                    if ($l_new_inventory_no)
                    {
                        $l_update = 'UPDATE isys_catg_accounting_list SET
							isys_catg_accounting_list__inventory_no = ' . $this->convert_sql_text(
                                $l_new_inventory_no
                            ) . ' WHERE isys_catg_accounting_list__isys_obj__id = \'' . $l_catdata["isys_obj__id"] . '\';';
            ...
            elseif (C__RECORD_STATUS__BIRTH == $l_intOldRecStatus && ($l_new_status == C__RECORD_STATUS__TEMPLATE || $l_new_status == C__RECORD_STATUS__MASS_CHANGES_TEMPLATE))
            {
                $l_update = 'UPDATE isys_catg_accounting_list SET
            ...
            else
            {
                ...
                {
                    if (!isset($_POST['C__CONTACT__PERSON_FIRST_NAME']) && !isset($_POST['C__CONTACT__PERSON_LAST_NAME']) && $l_dao->has_cat(
                            $this->get_objTypeID($l_catdata["isys_obj__id"]),
                            ['C__CATS__PERSON']
                        )
                    )
                    {
    ...
    public function handle_template_status($p_current_status, $p_object_id)
    {
        ...
        {
            if ($p_current_status == C__RECORD_STATUS__BIRTH && $_POST["template"] == C__RECORD_STATUS__MASS_CHANGES_TEMPLATE)
            {
     ...
     * @param int   $p_status        Decision whether category data shouldx be created or
     *                               just updated.
     *
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties']))
        {
            ...
            {
                $l_data_id = $this
                    ->retrieve("SELECT isys_catg_global_list__id FROM isys_catg_global_list WHERE isys_catg_global_list__isys_obj__id = '" . $p_object_id . "' LIMIT 0,1")
            ...
            if ($p_status === isys_import_handler_cmdb::C__CREATE || $p_status === isys_import_handler_cmdb::C__UPDATE)
            {
                if (!empty($this->m_data))
                {
                    ...
                    {
                        $l_catdata = array_pop($this->get_category_data());
                    }
                    else
                    {
                        $l_catdata = $this->get_category_data();
                    } // if
                }
                else
                {

...
class isys_cmdb_dao_category_g_cluster_members extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver
{
    ...
    public function get_data($p_catg_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        ...
        $l_sql = "SELECT " . "isys_obj.*, isys_connection.*, isys_catg_cluster_members_list.*, " . "ob2.isys_obj__status, ob2.isys_obj__id as memberID, ob2.isys_obj__sysid as memberSYSID, " . "ob2.isys_obj__title as memberTitle, ob2.isys_obj__isys_obj_type__id as memberType " . "FROM isys_catg_cluster_members_list " . "INNER JOIN isys_obj ON isys_obj__id = isys_catg_cluster_members_list__isys_obj__id " . "LEFT JOIN isys_connection ON isys_connection__id = isys_catg_cluster_members_list__isys_connection__id " . "LEFT JOIN isys_obj AS ob2 ON ob2.isys_obj__id = isys_connection__isys_obj__id " . "WHERE TRUE ";
        ...
        return [
            'member'      => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__CLUSTER_MEMBERSHIPS,
            ...
            'description' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA => [
                        C__PROPERTY__DATA__FIELD => 'isys_catg_cluster_members_list__description'
                    ],
                    C__PROPERTY__UI   => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__CLUSTER_MEMBERS
                    ]
                ]
            )
        ];
                ...
                case isys_import_handler_cmdb::C__CREATE:
                    if ($p_object_id > 0)
                    {
                        return $this->create(
                            ...
                            $p_category_data['properties']['member'][C__DATA__VALUE],
    ...
    {
        $l_strSql = "UPDATE isys_catg_cluster_members_list " . "INNER JOIN isys_connection ON isys_connection__id = isys_catg_cluster_members_list__isys_connection__id " . "SET " . "isys_connection__isys_obj__id = " . $this->convert_sql_id(
                $p_connectedObjID
            ) . ", " . "isys_catg_cluster_members_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_catg_cluster_members_list__status = " . $this->convert_sql_id(
                $p_newRecStatus
        ...
        if ($this->update($l_strSql))
        {
            if ($this->apply_update())
            {
                ...
                $l_relation_dao->handle_relation(
                    ...
                    C__RELATION_TYPE__CLUSTER_MEMBERSHIPS,
                    $l_data["isys_catg_cluster_members_list__isys_catg_relation_list__id"],
                    $p_connectedObjID,
                    $l_data["isys_catg_cluster_members_list__isys_obj__id"]
                );
    ...
    public function create($p_objID, $p_newRecStatus, $p_connectedObjID, $p_description)
    {
        ...
        $l_strSql = "INSERT INTO isys_catg_cluster_members_list SET " . "isys_catg_cluster_members_list__isys_obj__id  = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_catg_cluster_members_list__description	 = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_catg_cluster_members_list__status = " . $this->convert_sql_id(
                $p_newRecStatus
            ) . ", " . "isys_catg_cluster_members_list__isys_connection__id = " . $this->convert_sql_id($l_connection_dao->add_connection($p_connectedObjID)) . ';';
    ...
    {
        $l_sql = "SELECT isys_obj.*, isys_catg_cluster_members_list__id FROM isys_catg_cluster_members_list " . "INNER JOIN isys_connection " . "ON isys_connection__id = isys_catg_cluster_members_list__isys_connection__id " . "INNER JOIN isys_obj " . "ON isys_connection__isys_obj__id = isys_obj__id " . "WHERE isys_obj__status = '" . C__RECORD_STATUS__NORMAL . "' " . "AND isys_catg_cluster_members_list__isys_obj__id = " . $this->convert_sql_id(
                $p_obj_id
            );
    ...
    public function get_assigned_members_as_array($p_obj_id, $p_as_string = false)
    {
        ...
        if ($l_res->num_rows() > 0) while ($l_row = $l_res->get_row())
        {
            $l_arr[] = $l_row["isys_connection__isys_obj__id"];
        }
        else return false;

        if ($p_as_string) return implode(",", $l_arr);
        else return $l_arr;

...
class isys_cmdb_dao_category_g_virtual_devices extends isys_cmdb_dao_category_global
{
    ...
    {
        return [
            C__VIRTUAL_DEVICE__NETWORK   => _L('LC__CMDB__CATG__NETWORK'),
            C__VIRTUAL_DEVICE__STORAGE   => _L('LC__CMDB__CATG__VD__LOCAL_DEVICE'),
            C__VIRTUAL_DEVICE__INTERFACE => _L('LC__CMDB__CATG__VD__LOCAL_DEVICE')
        ];
    } // function
        ...
        switch ($l_dao_vm->get_objTypeID($l_host_obj_id))
        {
            case C__OBJTYPE__CLUSTER:
                ...
                while ($l_row = $l_res->get_row())
                {
                    if (!empty($l_return))
                    {
                        $l_puffer = $this->get_dialog_stor_content_by_host($l_row['isys_connection__isys_obj__id']);
                        if (!empty($l_puffer))
                        {
                    ...
                    {
                        $l_return = $this->get_dialog_stor_content_by_host($l_row['isys_connection__isys_obj__id']);
                    } // if
                } // while
                break;
            default:
    ...
    public function callback_property_local_host_port(isys_request $p_request, $p_type)
    {
        ...
        if ($p_type == 'host') $l_host_obj_id = $l_dao_vm->get_host_system($p_request->get_object_id());
        else $l_host_obj_id = $p_request->get_object_id();
    ...
    public function callback_property_general(isys_request $p_request, $p_class)
    {
        ...
        if (class_exists($p_class))
        {
            ...
            $l_class_table = (is_int(strpos($l_class_table, 'list'))) ? $l_class_table : ((is_int(
                strpos($l_class_table, '_2_')
            )) ? $l_class_table : $l_class_table . '__list');
    ...
    public function get_dialog_content_by_host($p_obj_id)
    {
        ...
        while ($l_row = $l_res->get_row())
        {
            $l_id                                            = $l_row["isys_catg_stor_list__id"] . "," . C__CATG__STORAGE;
        ...
        while ($l_row = $l_res->get_row())
        {
            $l_id                                                     = $l_row["isys_catg_ldevclient_list__id"] . "," . C__CATG__LDEV_CLIENT;
            $l_host_storage[_L('LC__CMDB__CATG__LDEV_CLIENT')][$l_id] = $l_row["isys_catg_ldevclient_list__title"] . " (" . _L($l_row["isys_catg_sanpool_list__title"]) . ")";
        } // while

        $l_res = $l_dao_driv->get_data(null, $p_obj_id);
        while ($l_row = $l_res->get_row())
        {
            $l_id                                           = $l_row["isys_catg_drive_list__id"] . "," . C__CATG__DRIVE;
    ...
    {
        $l_sql = "SELECT * FROM isys_catg_virtual_device_list " . "INNER JOIN isys_virtual_device_local ON isys_virtual_device_local__isys_catg_virtual_device_list__id = isys_catg_virtual_device_list__id " . "LEFT JOIN isys_virtual_device_host ON isys_virtual_device_host__isys_catg_virtual_device_list__id = isys_catg_virtual_device_list__id " . "LEFT JOIN isys_virtual_storage_type ON isys_virtual_device_local__isys_virtual_storage_type__id = isys_virtual_storage_type__id " . "LEFT JOIN isys_virtual_network_type ON isys_virtual_device_local__isys_virtual_network_type__id = isys_virtual_network_type__id " . "INNER JOIN isys_obj ON isys_catg_virtual_device_list__isys_obj__id = isys_obj__id " . "WHERE TRUE " . $p_condition . " " . $this->prepare_filter(
                $p_filter
            );
    ...
    {
        return [
            'host_stor_device'     => array_replace_recursive(
            ...
            'host_ldev_client'     => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'host_drive'           => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'host_port'            => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'host_interface'       => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'local_stor_device'    => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'local_port'           => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'local_interface'      => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'storage_type'         => array_replace_recursive(
            ...
            'network_type'         => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog_plus(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'switch_port_group'    => array_replace_recursive(
                isys_cmdb_dao_category_pattern::dialog(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'cluster_storage'      => array_replace_recursive(
            ...
            'cluster_interface'    => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO     => [
            ...
            'disk_image_location'  => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO     => [
                        C__PROPERTY__INFO__TITLE       => 'LC__CMDB__CATG__VD__LOCAL_STORAGE',
            ...
            'device_type'          => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO     => [
                        C__PROPERTY__INFO__TITLE       => 'LC__CMDB__CATG__VD__DEVICETYPE',
                    ...
                    C__PROPERTY__UI       => [
                        ...
                        C__PROPERTY__UI__PARAMS => [
                            'p_arData' => serialize($this->get_device_types())
                        ]
                    ],
                    C__PROPERTY__PROVIDES => [
                        C__PROPERTY__PROVIDES__SEARCH => false,
            ...
            'description'          => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__UI       => [
                        C__PROPERTY__UI__ID => 'C__CMDB__CAT__COMMENTARY_' . C__CMDB__CATEGORY__TYPE_GLOBAL . C__CATG__VIRTUAL_DEVICE,
                    ],
                    C__PROPERTY__PROVIDES => [
                        C__PROPERTY__PROVIDES__SEARCH => false,
                        C__PROPERTY__PROVIDES__REPORT => false,
            ...
            'virtual_network_type' => array_replace_recursive(
                isys_cmdb_dao_category_pattern::text(),
                [
                    C__PROPERTY__INFO     => [
            ...
            switch ($p_status)
            {
                case isys_import_handler_cmdb::C__CREATE:
                    if (($p_category_data['data_id'] = $this->create(
                        $p_object_id,
                        $this->get_property('disk_image_location'),
                        $this->get_property('device_type'),
                    ...
                    )
                    {
                        ...
                        {
                            $this->create_local_device(
                                $p_category_data['data_id'],
                                $this->get_property('local_stor_device'),
                                null,
                                null,
                                $this->get_property('storage_type'),
                                null
                            );
                            if ($l_host_stor_device)
                            {
                                $this->create_host_device(
                                    $p_category_data['data_id'],
                                    $l_host_stor_device,
                                    null,
                                    null,
                        ...
                        if ($this->get_property('local_port'))
                        {
                            $this->create_local_device(
                                $p_category_data['data_id'],
                                null,
                                $this->get_property('local_port'),
                                null,
                                null,
                                $this->get_property('network_type')
                            );
                            ...
                            elseif (($l_switch_port_group = $this->get_property('switch_port_group')))
                            {
                                $this->create_host_device(
                                    ...
                                    null,
                                    null,
                                    $l_switch_port_group['vs_port_group_title'],
                                    null,
                                    null,
                                    null
                                );
                        ...
                        if ($this->get_property('local_interface'))
                        {
                            $this->create_local_device(
                                ...
                                null,
                                null,
                                $this->get_property('local_interface'),
                                null,
                                null
                            );
    ...
    {
        if ($p_catdata["isys_virtual_device_host__cluster_storage"] || $p_catdata["isys_virtual_device_host__isys_catg_stor_list__id"] || $p_catdata["isys_virtual_device_host__isys_catg_ldevclient_list__id"] || $p_catdata["isys_virtual_device_host__isys_catg_drive_list__id"])
        {
     ...
     */
    public function create_host_device($p_catg_virtual_device_id, $p_stor_id = null, $p_ldevclient_id = null, $p_drive_id = null, $p_port_id = null, $p_switch_port = "", $p_ui_id = null, $p_cluster_storage = null, $p_cluster_ui = null)
    {
        $l_sql = "INSERT INTO isys_virtual_device_host SET " . "isys_virtual_device_host__isys_catg_virtual_device_list__id = " . $this->convert_sql_id(
                $p_catg_virtual_device_id
            ) . ", " . "isys_virtual_device_host__isys_catg_ldevclient_list__id = " . $this->convert_sql_id(
                $p_ldevclient_id
            ) . ", " . "isys_virtual_device_host__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_stor_id
            ) . ", " . "isys_virtual_device_host__isys_catg_drive_list__id = " . $this->convert_sql_id(
                $p_drive_id
            ) . ", " . "isys_virtual_device_host__isys_catg_port_list__id = " . $this->convert_sql_id(
                $p_port_id
            ) . ", " . "isys_virtual_device_host__switch_port_group = " . $this->convert_sql_text(
                $p_switch_port
            ) . ", " . "isys_virtual_device_host__cluster_storage = " . $this->convert_sql_text(
                $p_cluster_storage
            ) . ", " . "isys_virtual_device_host__cluster_ui = " . $this->convert_sql_text(
                $p_cluster_ui
     ...
     * @return  integer
     */
    public function save_host_device($p_host_device_id, $p_stor_id = null, $p_ldevclient_id = null, $p_drive_id = null, $p_port_id = null, $p_switch_port = "", $p_ui_id = null, $p_cluster_storage = null, $p_cluster_ui = null, $p_category_id = null)
    {
        $l_sql = "UPDATE isys_virtual_device_host SET " . "isys_virtual_device_host__isys_catg_ldevclient_list__id = " . $this->convert_sql_id(
                $p_ldevclient_id
            ) . ", " . "isys_virtual_device_host__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_stor_id
            ) . ", " . "isys_virtual_device_host__isys_catg_drive_list__id = " . $this->convert_sql_id(
    ...
    {
        $l_sql = "INSERT INTO isys_virtual_device_local SET " . "isys_virtual_device_local__isys_catg_virtual_device_list__id = " . $this->convert_sql_id(
                $p_catg_virtual_device_id
            ) . ", " . "isys_virtual_device_local__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_stor_id
            ) . ", " . "isys_virtual_device_local__isys_catg_port_list__id = " . $this->convert_sql_id(
                $p_port_id
            ) . ", " . "isys_virtual_device_local__isys_catg_ui_list__id = " . $this->convert_sql_id(
                $p_ui_id
            ) . ", " . "isys_virtual_device_local__isys_virtual_storage_type__id = " . $this->convert_sql_id(
                $p_storage_type
     ...
     * @return  integer
     */
    public function save_local_device($p_local_device_id = null, $p_stor_id = null, $p_port_id = null, $p_ui_id = null, $p_storage_type = null, $p_network_type = null, $p_category_id = null)
    {
        $l_sql = "UPDATE isys_virtual_device_local SET " . "isys_virtual_device_local__isys_catg_stor_list__id = " . $this->convert_sql_id(
                $p_stor_id
            ) . ", " . "isys_virtual_device_local__isys_catg_port_list__id = " . $this->convert_sql_id(
                $p_port_id
            ) . ", " . "isys_virtual_device_local__isys_catg_ui_list__id = " . $this->convert_sql_id(
    ...
    public function create($p_obj_id, $p_disk_image_location = "", $p_deviceType, $p_status = C__RECORD_STATUS__NORMAL, $p_description = "")
    {
        ...
        $l_sql = "INSERT INTO isys_catg_virtual_device_list SET " . "isys_catg_virtual_device_list__isys_obj__id = " . $this->convert_sql_id(
                $p_obj_id
            ) . ", " . "isys_catg_virtual_device_list__disk_image_location = " . $this->convert_sql_text(
                $p_disk_image_location
            ) . ", " . "isys_catg_virtual_device_list__device_type = " . $this->convert_sql_id(
                $p_deviceType
            ) . ", " . "isys_catg_virtual_device_list__status = " . $this->convert_sql_id(
                $p_status
            ) . ", " . "isys_catg_virtual_device_list__description = " . $this->convert_sql_text($p_description) . ";";
    ...
    public function save($p_virtual_device_list_id, $p_disk_image_location = "", $p_status = C__RECORD_STATUS__NORMAL, $p_description = "")
    {
        ...
        $l_sql = "UPDATE isys_catg_virtual_device_list SET " . "isys_catg_virtual_device_list__disk_image_location = " . $this->convert_sql_text(
                $p_disk_image_location
            ) . ", " . "isys_catg_virtual_device_list__status = " . $this->convert_sql_id(
                $p_status
            ) . ", " . "isys_catg_virtual_device_list__description = " . $this->convert_sql_text(
                $p_description
            ) . " " . "WHERE isys_catg_virtual_device_list__id = " . $this->convert_sql_id($p_virtual_device_list_id) . ";";
    ...
    public function save_element(&$p_cat_level, &$p_status, $p_create = false)
    {
        ...
        if ($l_dao_vm->get_objTypeID($l_host_system) === C__CATG__CLUSTER) $l_is_cluster = true;
        else $l_is_cluster = false;
        ...
        if ($p_create)
        {
            $l_id = $this->create(
                ...
                $_POST['C__CMDB__CATG__VD__DISK_IMAGE_LOCATION'],
                $_POST["device_type"],
                $l_status,
                $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . $this->get_category_id()]
            );

            $l_local_device_id = $this->create_local_device(
                $l_id,
                $_POST['C__CMDB__CATG__VD__LOCAL_STORAGE'],
                $_POST['C__CMDB__CATG__VD__LOCAL_NETWORK_PORT'],
                $_POST['C__CMDB__CATG__VD__LOCAL_INTERFACE'],
                $_POST['C__CMDB__CATG__VD__STORAGE_TYPE'],
                $_POST['C__CMDB__CATG__VD__NETWORK_TYPE']
            );

            $l_host_device_id = $this->create_host_device(
                $l_id,
                $l_host_stor_id,
                $l_host_ldev_client,
                $l_host_drive,
                $_POST['C__CMDB__CATG__VD__HOST_NETWORK_PORT'],
                html_entity_decode($_POST['C__CMDB__CATG__VD__SWITCH_PORT_GROUP'], null, $GLOBALS['g_config']['html-encoding']),
                $l_host_ui,
                $l_cluster_storage,
                $l_cluster_ui
            );
        ...
        {
            $this->save_local_device(
                $l_catdata["isys_virtual_device_local__id"],
                $_POST['C__CMDB__CATG__VD__LOCAL_STORAGE'],
                $_POST['C__CMDB__CATG__VD__LOCAL_NETWORK_PORT'],
                $_POST['C__CMDB__CATG__VD__LOCAL_INTERFACE'],
                $_POST['C__CMDB__CATG__VD__STORAGE_TYPE'],
            ...
            $this->save_host_device(
                $l_catdata["isys_virtual_device_host__id"],
                $l_host_stor_id,
                $l_host_ldev_client,
                $l_host_drive,
                $_POST['C__CMDB__CATG__VD__HOST_NETWORK_PORT'],
            ...
            return $this->save(
                $l_catdata["isys_catg_virtual_device_list__id"],
                $_POST['C__CMDB__CATG__VD__DISK_IMAGE_LOCATION'],
                $l_status,
                $_POST["C__CMDB__CAT__COMMENTARY_" . $this->get_category_type() . $this->get_category_id()]
            );

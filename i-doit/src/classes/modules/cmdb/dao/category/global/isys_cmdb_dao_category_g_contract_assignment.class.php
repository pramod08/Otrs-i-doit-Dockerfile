...
class isys_cmdb_dao_category_g_contract_assignment extends isys_cmdb_dao_category_global
{
                ...
                if ($l_contract_row['isys_connection__isys_obj__id'] !== null && $l_contract_row['isys_connection__isys_obj__id'] > 0)
                {
                    ...
                    else
                    {
                        $l_return[] = $l_quickinfo->get_quick_info(
                            ...
                            _L(
                                $l_contract_dao->get_objtype_name_by_id_as_string($l_contract['isys_obj__isys_obj_type__id'])
                            ) . ' &raquo; ' . $l_contract['isys_obj__title'] . ' (' . _L($l_contract['isys_contract_type__title']) . ')',
                            C__LINK__OBJECT
                        );
                    } // if
                } // if
    ...
    public function create($p_obj_id, $p_rec_status = C__RECORD_STATUS__NORMAL, $p_contract_start = null, $p_contract_end = null, $p_connected_contract = null, $p_description = '', $p_reaction_rate = null)
    {
        ...
        $l_sql = 'INSERT INTO isys_catg_contract_assignment_list (
				...
				isys_catg_contract_assignment_list__status) ' . 'VALUES( ' . $this->convert_sql_id($p_obj_id) . ', ' . $this->convert_sql_datetime(
                $p_contract_start
            ) . ', ' . $this->convert_sql_datetime($p_contract_end) . ', ' . $this->convert_sql_id(
                $l_dao_connection->add_connection($p_connected_contract)
    ...
    public function save($p_cat_id, $p_rec_status = C__RECORD_STATUS__NORMAL, $p_contract_start = null, $p_contract_end = null, $p_connected_contract = null, $p_description = '', $p_reaction_rate = null)
    {
        ...
        $l_sql            = 'SELECT isys_catg_contract_assignment_list__isys_obj__id,
					isys_catg_contract_assignment_list__isys_connection__id,
					isys_catg_contract_assignment_list__isys_catg_relation_list__id FROM isys_catg_contract_assignment_list ' . 'WHERE isys_catg_contract_assignment_list__id = ' . $this->convert_sql_id(
                $p_cat_id
            );
        ...
        $l_sql = 'UPDATE isys_catg_contract_assignment_list ' . 'SET ' . 'isys_catg_contract_assignment_list__contract_start = ' . $this->convert_sql_datetime(
                $p_contract_start
            ) . ', ' . 'isys_catg_contract_assignment_list__contract_end = ' . $this->convert_sql_datetime(
                $p_contract_end
            ) . ', ' . 'isys_catg_contract_assignment_list__isys_connection__id = ' . $this->convert_sql_id(
                $l_connection_id
            ) . ', ' . 'isys_catg_contract_assignment_list__reaction_rate__id = ' . $this->convert_sql_id(
                $p_reaction_rate
            ) . ', ' . 'isys_catg_contract_assignment_list__description = ' . $this->convert_sql_text(
                $p_description
            ...
            'connected_contract' => array_replace_recursive(
                ...
                [
                    ...
                    C__PROPERTY__DATA     => [
                        ...
                        C__PROPERTY__DATA__RELATION_TYPE    => C__RELATION_TYPE__CONTRACT,

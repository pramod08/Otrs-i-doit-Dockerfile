...
class isys_cmdb_dao_category_g_managed_objects extends isys_cmdb_dao_category_global
{
    ...
    public function create_element($p_cat_level, &$p_new_id)
    {
        ...
        if (!is_null($l_connected_objects))
        {
            ...
            {
                $l_assigned_objects = [];
                if (count($l_assigned_devices))
                {
                    $l_assigned_objects = array_flip($l_assigned_devices);
                } // if

                $l_res_objtypes        = $this->get_obj_type_by_catg([C__CATG__CLUSTER_ROOT]);
    ...
    private function update_administration_service($p_object_id, $p_connected_host, $p_is_cluster = false)
    {
        ...
        else
        {
            ...
            if ($l_res->num_rows())
            {
                ...
                return $l_dao->save(
                    $l_row['isys_catg_virtual_host_list__id'],
                    C__RECORD_STATUS__NORMAL,
                    $l_row['isys_catg_virtual_host_list__virtual_host'],
                    $l_row['isys_connection__isys_obj__id'],
                    $p_object_id,
                    $l_row['isys_catg_virtual_host_list__description']
                );
            }
            else
            {

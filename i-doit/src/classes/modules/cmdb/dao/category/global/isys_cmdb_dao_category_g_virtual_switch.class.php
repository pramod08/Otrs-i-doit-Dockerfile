...
class isys_cmdb_dao_category_g_virtual_switch extends isys_cmdb_dao_category_global
{
    ...
    {
        $l_sql = 'SELECT * FROM isys_catg_virtual_switch_list' . ' INNER JOIN isys_obj ON isys_obj__id = isys_catg_virtual_switch_list__isys_obj__id' . ' WHERE TRUE ' . $p_condition . ' ' . $this->prepare_filter(
                $p_filter
            );
    ...
    {
        $l_query = "SELECT * FROM isys_service_console_port " . "LEFT JOIN isys_catg_ip_list ON isys_catg_ip_list__id = isys_service_console_port__isys_catg_ip_list__id " . "LEFT JOIN isys_cats_net_ip_addresses_list ON isys_cats_net_ip_addresses_list__id = isys_catg_ip_list__isys_cats_net_ip_addresses_list__id " . "WHERE isys_service_console_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_vswitchID
            );
    ...
    {
        $l_query = "SELECT * FROM isys_vmkernel_port " . "LEFT JOIN isys_catg_ip_list ON isys_catg_ip_list__id = isys_vmkernel_port__isys_catg_ip_list__id " . "LEFT JOIN isys_cats_net_ip_addresses_list ON isys_cats_net_ip_addresses_list__id = isys_catg_ip_list__isys_cats_net_ip_addresses_list__id " . "WHERE isys_vmkernel_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_vswitchID
            );
    ...
    {
        $l_query = "SELECT * FROM isys_virtual_port_group " . "WHERE isys_virtual_port_group__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_vswitchID
            );
    ...
    {
        $l_update = "DELETE FROM isys_virtual_switch_2_port WHERE " . "isys_virtual_switch_2_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_catlevel
            );

        if (!$this->update($l_update)) return false;

        if (count($p_arPorts) == 0) return true;

        $l_update     = "INSERT INTO isys_virtual_switch_2_port " . "(isys_virtual_switch_2_port__isys_catg_virtual_switch_list__id, " . "isys_virtual_switch_2_port__isys_catg_port_list__id) " . "VALUES ";
        $l_is_correct = false;
    ...
    {
        $l_update = "DELETE FROM isys_service_console_port WHERE " . "isys_service_console_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_catlevel
            );

        if (!$this->update($l_update)) return false;

        if (count($p_scps) == 0) return true;

        $l_update = "INSERT INTO isys_service_console_port " . "(isys_service_console_port__isys_catg_virtual_switch_list__id, " . "isys_service_console_port__isys_catg_ip_list__id, " . "isys_service_console_port__title) " . "VALUES ";
        $l_exe    = false;
        foreach ($p_scps as $l_scp)
        {
            if (!empty($l_scp[1]) && $l_scp[1] > 0)
            {
                $l_update .= '(' . $this->convert_sql_id($p_catlevel) . ', ' . $this->convert_sql_id(
                        $l_scp[1]
    ...
    {
        $l_update = "DELETE FROM isys_vmkernel_port WHERE " . "isys_vmkernel_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_catlevel
            );

        if (!$this->update($l_update)) return false;

        if (count($p_vmks) == 0) return true;

        $l_update = "INSERT INTO isys_vmkernel_port " . "(isys_vmkernel_port__isys_catg_virtual_switch_list__id, " . "isys_vmkernel_port__isys_catg_ip_list__id, " . "isys_vmkernel_port__title) " . "VALUES ";
        $l_exe    = false;
    ...
    {
        $l_update = "DELETE FROM isys_virtual_port_group WHERE " . "isys_virtual_port_group__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_catlevel
            );

        if (!$this->update($l_update)) return false;

        if (count($p_pgs) == 0) return true;

        $l_update = "INSERT INTO isys_virtual_port_group " . "(isys_virtual_port_group__isys_catg_virtual_switch_list__id, " . "isys_virtual_port_group__title, " . "isys_virtual_port_group__vlanid) " . "VALUES ";
        $l_exe    = false;
        foreach ($p_pgs as $l_vmk)
        {
            if (!empty($l_vmk[0]))
            {
                $l_update .= "(" . $this->convert_sql_id($p_catlevel) . ", " . $this->convert_sql_text(
                        $l_vmk[0]
    ...
    {
        $l_update = "INSERT INTO isys_catg_virtual_switch_list SET " . "isys_catg_virtual_switch_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            ) . ", " . "isys_catg_virtual_switch_list__status = " . $this->convert_sql_id(
                $p_recStatus
            ) . ", " . "isys_catg_virtual_switch_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_virtual_switch_list__description = " . $this->convert_sql_text($p_description);

        if ($this->update($l_update) && $this->apply_update()) return $this->get_last_insert_id();
        else
            return false;
    } // function
    ...
    {
        $l_update = "UPDATE isys_catg_virtual_switch_list SET " . "isys_catg_virtual_switch_list__status = " . $this->convert_sql_id(
                $p_recStatus
            ) . ", " . "isys_catg_virtual_switch_list__title = " . $this->convert_sql_text(
                $p_title
            ) . ", " . "isys_catg_virtual_switch_list__description = " . $this->convert_sql_text(
                $p_description
    ...
    {
        $l_query = "SELECT * FROM isys_catg_port_list WHERE isys_catg_port_list__isys_obj__id = " . $this->convert_sql_id(
                $p_objID
            );
    ...
    {
        $l_query = "SELECT * FROM isys_virtual_switch_2_port WHERE isys_virtual_switch_2_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_vswitchID
            );
    ...
    {
        $l_query = "SELECT * FROM isys_virtual_switch_2_port " . "INNER JOIN isys_catg_port_list ON isys_catg_port_list__id = isys_virtual_switch_2_port__isys_catg_port_list__id " . "WHERE isys_virtual_switch_2_port__isys_catg_virtual_switch_list__id = " . $this->convert_sql_id(
                $p_vswitchID
            );
    ...
    {
        $l_query = "SELECT isys_obj__id, isys_obj__isys_obj_type__id, isys_obj__title FROM isys_virtual_device_host " . "INNER JOIN isys_catg_virtual_device_list ON isys_catg_virtual_device_list__id = isys_virtual_device_host__isys_catg_virtual_device_list__id " . "INNER JOIN isys_obj ON isys_obj__id = isys_catg_virtual_device_list__isys_obj__id " . "INNER JOIN isys_catg_virtual_machine_list ON isys_catg_virtual_machine_list__isys_obj__id = isys_obj__id " . "INNER JOIN isys_connection ON isys_connection__id = isys_catg_virtual_machine_list__isys_connection__id " . "WHERE isys_virtual_device_host__switch_port_group = " . $this->convert_sql_text(
                $p_pgName
            ) . " " . "AND isys_connection__isys_obj__id = " . $this->convert_sql_id(
                $p_objID

...
class isys_cmdb_dao_category_g_guest_systems extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver
{
    ...
    public function create($p_object_id, $p_status, $p_guest_systems, $p_description, $p_runs_on = null)
    {
        ...
        $l_sql = "INSERT INTO isys_catg_virtual_machine_list " . "SET " . "isys_catg_virtual_machine_list__status = '" . $p_status . "', " . "isys_catg_virtual_machine_list__description = " . $this->convert_sql_text(
                $p_description
            ) . ", " . "isys_catg_virtual_machine_list__isys_obj__id = '" . $p_guest_systems . "', " . "isys_catg_virtual_machine_list__vm = '2', " . "isys_catg_virtual_machine_list__isys_connection__id = " . $this->convert_sql_id(
                $l_dao_con->add_connection($p_object_id)
            ) . " ";
    ...
    public function save($p_id, $p_status, $p_connected_obj, $p_description, $p_runs_on = null)
    {
        if (is_numeric($p_id) && $p_id > 0)
        {
            ...
            $l_sql = "UPDATE isys_catg_virtual_machine_list " . "SET " . "isys_catg_virtual_machine_list__isys_connection__id = " . $this->convert_sql_id(
                    $this->handle_connection($p_id, $p_connected_obj)
                ) . ", " . "isys_catg_virtual_machine_list__status = '" . $p_status . "', " . "isys_catg_virtual_machine_list__vm = '2', " . "isys_catg_virtual_machine_list__description = " . $this->convert_sql_text(
                    $p_description
                ) . " ";
        ...
        foreach ($p_objects as $l_object_id)
        {
            if (is_numeric($l_object_id))
            {
                ...
                if (!isset($l_current_objects[$l_object_id]) || !$l_current_objects[$l_object_id])
                {
                    ...
                    $l_query = 'SELECT isys_catg_virtual_machine_list__id FROM isys_catg_virtual_machine_list WHERE isys_catg_virtual_machine_list__isys_obj__id = ' . $this->convert_sql_id(
                            $l_object_id
                        ) . ' ORDER BY isys_catg_virtual_machine_list__id DESC';
    ...
    public function get_count($p_obj_id = null)
    {
        ...
        $l_sql = "SELECT COUNT(guest.isys_obj__id) AS count FROM isys_catg_virtual_machine_list " . "INNER JOIN isys_obj guest ON isys_catg_virtual_machine_list__isys_obj__id = guest.isys_obj__id " . "INNER JOIN isys_connection ON isys_catg_virtual_machine_list__isys_connection__id = isys_connection__id " . "INNER JOIN isys_obj me ON me.isys_obj__id = isys_connection__isys_obj__id " . "WHERE TRUE AND isys_catg_virtual_machine_list__vm = '2' " . "AND isys_catg_virtual_machine_list__status = " . $this->convert_sql_int(
                C__RECORD_STATUS__NORMAL
            ) . " " . "AND guest.isys_obj__status = " . $this->convert_sql_int(C__RECORD_STATUS__NORMAL);
    ...
    {
        $l_sql = "SELECT guest.*, isys_catg_virtual_machine_list.*, isys_connection.*, isys_catg_relation_list__isys_obj__id " . "FROM isys_catg_virtual_machine_list " . "INNER JOIN isys_obj guest ON isys_catg_virtual_machine_list__isys_obj__id = guest.isys_obj__id " . "INNER JOIN isys_connection ON isys_catg_virtual_machine_list__isys_connection__id = isys_connection__id " . "INNER JOIN isys_obj me ON me.isys_obj__id = isys_connection__isys_obj__id " . "LEFT JOIN isys_catg_relation_list ON isys_catg_relation_list__id = isys_catg_virtual_machine_list__isys_catg_relation_list__id " . "WHERE TRUE AND isys_catg_virtual_machine_list__vm = '2' " . $p_condition . $this->prepare_filter(
                $p_filter
            ) . " ";
        ...
        switch ($_POST[C__GET__NAVMODE])
        {
            ...
            case C__NAVMODE__PURGE:
                if (!empty($p_cat_ids))
                {
                    ...
                    {
                        $l_sql = "UPDATE isys_catg_virtual_machine_list " . "INNER JOIN isys_connection ON isys_connection__id = isys_catg_virtual_machine_list__isys_connection__id " . "SET " . "isys_connection__isys_obj__id = NULL, " . "isys_catg_virtual_machine_list__status = '" . C__RECORD_STATUS__NORMAL . "', " . "isys_catg_virtual_machine_list__vm = '2', " . "isys_catg_virtual_machine_list__primary = NULL ";
                    ...
                    isys_event_manager::getInstance()
                        ->triggerCMDBEvent(
                            ...
                            $_GET[C__CMDB__GET__OBJECT],
                            $this->get_objTypeID($_GET[C__CMDB__GET__OBJECT]),
                            'LC__CMDB__CATG__GUEST_SYSTEMS',
                            serialize($l_changes)
                        );
                } // if
                break;

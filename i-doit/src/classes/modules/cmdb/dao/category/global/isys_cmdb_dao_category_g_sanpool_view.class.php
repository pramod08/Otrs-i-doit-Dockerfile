...
class isys_cmdb_dao_category_g_sanpool_view extends isys_cmdb_dao_category_g_virtual
{
    ...
    public function get_ldevserver()
    {
        ...
        while ($l_ldevserver_row = $l_data_res->get_row())
        {
            ...
            $l_ldevserver_arr[] = [
                ...
                "capacity"      => isys_convert::memory(
                    $l_ldevserver_row["isys_catg_sanpool_list__capacity"],
                    $l_ldevserver_row["isys_memory_unit__const"],
    ...
    public function set_chain_drive($p_id = null, $p_stage = null, $p_id2 = null)
    {
        ...
        else
        {
            ...
            switch ($p_stage)
            {
                // LDEV Clients
                case 1:
                    $l_sql      = "SELECT * FROM isys_catg_ldevclient_list WHERE isys_catg_ldevclient_list__id = " . $this->convert_sql_id(
                            $p_id2
                ...
                case 2:
                    ...
                    $l_sql = 'SELECT * FROM isys_ldevclient_fc_port_path ' . 'INNER JOIN isys_catg_fc_port_list ' . 'ON isys_catg_fc_port_list__id = isys_ldevclient_fc_port_path__isys_catg_fc_port_list__id ' . 'INNER JOIN isys_catg_hba_list ' . 'ON isys_catg_fc_port_list__isys_catg_hba_list__id = isys_catg_hba_list__id ' . 'WHERE isys_ldevclient_fc_port_path__isys_catg_ldevclient_list__id = ' . $this->convert_sql_id(
                            $p_id
                    ...
                    if ($l_con_res->num_rows() > 0)
                    {
                        ...
                        while ($l_row = $l_con_res->get_row())
                        {
                            ...
                            if (empty($l_index)) $l_index = count($this->m_chain[$p_id]);
    ...
    public function get_count($p_obj_id = null)
    {
        ...
        $l_sql = "SELECT count(isys_obj__id) AS count FROM isys_obj " . "LEFT JOIN isys_catg_fc_port_list ON isys_obj__id = isys_catg_fc_port_list__isys_obj__id " . "LEFT JOIN isys_catg_sanpool_list ON isys_obj__id = isys_catg_sanpool_list__isys_obj__id " . "LEFT JOIN isys_catg_ldevclient_list ON isys_obj__id = isys_catg_ldevclient_list__isys_obj__id " . "LEFT JOIN isys_catg_hba_list ON isys_obj__id = isys_catg_hba_list__isys_obj__id " . "WHERE TRUE ";
        if (!empty($l_obj_id))
        {
            $l_sql .= " AND (isys_obj__id = " . $this->convert_sql_id($l_obj_id) . ") ";
        }

        $l_sql .= " AND ((isys_catg_fc_port_list__id IS NOT NULL AND isys_catg_fc_port_list__status = '" . C__RECORD_STATUS__NORMAL . "') " . " OR (isys_catg_sanpool_list__id IS NOT NULL AND isys_catg_sanpool_list__status = '" . C__RECORD_STATUS__NORMAL . "') " . " OR (isys_catg_ldevclient_list__id IS NOT NULL AND isys_catg_ldevclient_list__status = '" . C__RECORD_STATUS__NORMAL . "') " . " OR (isys_catg_hba_list__id != NULL AND isys_catg_hba_list__status = '" . C__RECORD_STATUS__NORMAL . "'))";

...
class isys_cmdb_dao_status extends isys_cmdb_dao
{
    ...
    {
        if (!count($this->m_cmdb_status_cache))
        {
    ...
    {
        $this->update(
            'UPDATE isys_obj SET isys_obj__isys_cmdb_status__id = (SELECT isys_cmdb_status__id FROM isys_cmdb_status WHERE isys_cmdb_status__const = \'C__CMDB_STATUS__IDOIT_STATUS\') WHERE isys_obj__isys_cmdb_status__id = ' . $this->convert_sql_int(
                $p_id
            ) . ';'
        );

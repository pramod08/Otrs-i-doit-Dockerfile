...
class isys_cmdb_dao extends isys_cmdb_dao_nexgen
{
    ...
    public function set_object_cmdb_status($p_obj_id, $p_cmdb_status)
    {
        ...
        isys_cmdb_dao_status::instance($this->m_db)->add_change($p_obj_id, $p_cmdb_status);
    ...
    public function get_obj_id_by_ticket_id($p_id)
    {
        return $this->retrieve(
            'SELECT isys_catg_tickets_list__isys_obj__id FROM isys_catg_tickets_list WHERE isys_catg_tickets_list__rt_ticket__id = ' . $this->convert_sql_id($p_id) . ';'
        );
    } // Function
    ...
    {
        $l_query = 'UPDATE isysgui_catg SET isysgui_catg__status = ' . $this->convert_sql_int($p_status) . ' WHERE isysgui_catg__id = ' . $this->convert_sql_id(
                $p_catID
            ) . ';';
    ...
    {
        $l_res = $this->retrieve(
            'SELECT ' . ($p_complete ? '*' : 'isysgui_catg__id') . ' FROM isysgui_catg WHERE isysgui_catg__parent = ' . $this->convert_sql_id($p_id) . ';'
        );
        $l_res->free_result();
        ...
        if ($p_object_id > 0)
        {
            ...
            if (isset($l_connector_tables[$l_category_table]))
            {
                $l_delete_connectors = "DELETE isys_catg_connector_list FROM isys_catg_connector_list INNER JOIN {$l_category_table} ON " .
                    "{$l_connector_field} = isys_catg_connector_list__id WHERE {$l_category_table}__isys_obj__id = " . $this->convert_sql_id(
                        $p_object_id
                    ) . ";";
            ...
            if ($p_has_relation || $l_connector)
            {
                ...
                $l_res          = $this->retrieve(
                    "SELECT * FROM " . $l_category_table_for_relation . " WHERE (" . $l_category_table_for_relation . "__isys_obj__id = '" . $p_object_id . "')"
                );
            ...
            if ($l_category_table == 'isys_catg_ip_list')
            {
                $l_delete_ip_ids = '';
                $l_res           = $this->retrieve(
                    "SELECT " . $l_category_table . "__isys_cats_net_ip_addresses_list__id FROM " . $l_category_table . " WHERE " . $l_category_table . "__isys_obj__id = '" .
                    $p_object_id . "'"
                );
                $l_delete_ip     = false;
                while ($l_data = $l_res->get_row())
        ...
        if ($l_delete_relation || $l_delete_ip || $l_delete_connection)
        {
            ...
            if (is_array($p_cat_id))
            {
                ...
                while ($l_row = $l_res->get_row())
                {
                    ...
                    {
                        $l_delete = 'DELETE FROM isys_cats_net_ip_addresses_list WHERE isys_cats_net_ip_addresses_list__id = ' . $this->convert_sql_id(
                                $l_row['isys_cats_net_ip_addresses_list__id']
                            );
                        $this->update($l_delete);
                    } // if
                } // while
        ...
        if (is_array($p_category_id))
        {
            ...
            foreach ($p_category_id as $l_category_id => $l_ok)
            {
                if ($l_ok)
                {
                    ...
                    if (++$i < count($p_category_id)) $l_q .= " OR ";
                }
            }
    ...
    public function get_autotext($p_query, $p_source, $p_property)
    {
        ...
        $l_query = 'SELECT `' . $l_table . '__id`, `' . $l_field . '` ' . 'FROM `' . $l_table . '` ' . 'WHERE ' . $l_field . ' LIKE \'%' . $this->m_db->escape_string(
                $p_query
            ) . '%\';';
    ...
    {
        return $this->retrieve(
            'SELECT * FROM isys_obj INNER JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id WHERE isys_obj__id = ' . $this->convert_sql_id($p_objid) . ';'
        );
    } // function
    ...
    public function get_object_types_by_properties($p_properties = null, $p_order_by = null, $p_sort = null, $p_limit = null)
    {
        ...
        if (array_key_exists('status', $l_properties))
        {
            ...
            foreach ($l_properties['status'] as $l_value)
            {
                ...
                if (is_numeric($l_value)) $l_values[] = $l_table . '__status = ' . $this->convert_sql_id($l_value);
            } //foreach
            $l_query .= ' AND (' . implode(' OR ', $l_values) . ')';
        } // if titles
        ...
        if (array_key_exists('type_group', $l_properties))
        {
            ...
            foreach ($l_properties['type_group'] as $l_value)
            {
                ...
                if (is_numeric($l_value)) $l_values[] = $l_table . '__isys_obj_type_group__id = ' . $this->convert_sql_id($l_value);
            } //foreach
            $l_query .= ' AND (' . implode(' OR ', $l_values) . ')';
        } // if titles
    ...
    public function get_object_type_groups_by_properties($p_properties = null, $p_order_by = null, $p_sort = null, $p_limit = null)
    {
        ...
        if (array_key_exists('ids', $l_properties))
        {
            ...
            foreach ($l_properties['ids'] as $l_value)
            {
                ...
                if (is_numeric($l_value)) $l_values[] = $l_table . '__id = ' . $this->convert_sql_id($l_value);
            } //foreach
            $l_query .= ' AND (' . implode(' OR ', $l_values) . ')';
        } // if identifiers
     ...
     *
     * @param   string        $p_title
     * @param   integer|array $p_obj_type
     * @param   integer       $p_status
     *
     * @return  integer
     */
    public function get_obj_id_by_title($p_title, $p_obj_type = null, $p_status = null)
    {
        if (!empty($p_title))
        {
            ...
            {
                if (is_array($p_obj_type) && count($p_obj_type) > 0)
                {
            ...
            return $this->retrieve($l_sql . ';')
                ->get_row_value('isys_obj__id') ?: false;
        } // if
     ...
     * @return  boolean
     */
    public function obj_exists($p_obj_id, $p_ignore_cmdb_status = true)
    {
        if ($p_obj_id > 0)
        {
            return ($this->get_object($p_obj_id, $p_ignore_cmdb_status, 1)
                    ->count() > 0);
        }
        else
        {
     ...
     *
     * @param   integer $p_obj_id
     * @param   boolean $p_ignore_status Ignore status? Requires $p_obj_id to be set. Default is false.
     *
     * @param null      $p_limit
     *
     * @return isys_component_dao_result
     * @throws isys_exception_database
     */
    public function get_object($p_obj_id = null, $p_ignore_status = false, $p_limit = null)
    {
     ...
     * @throws isys_exception_database
     */
    public function search_objects($p_filter, $p_typeFilter = null, $p_groupFilter = null, $p_condition = "", $p_relation = false, $p_relation_only = false, $p_order_by = "isys_obj__title ASC", $p_limit = false, $p_status = C__RECORD_STATUS__NORMAL, $p_obj_type_black_list = null, $p_cat_filter = null)
    {
        ...
        if ($p_cat_filter !== null)
        {
            ...
            $l_specific_categories = array_map(
                function ($l_item)
                {
     ...
     * @author  Niclas Potthast <npotthast@i-doit.org>
     */
    public function get_object_by_id($p_objid, $p_ignore_status = false)
    {
        return $this->get_object($p_objid, $p_ignore_status, 1);
    } // function
     ...
     */
    public function get_catg_by_table_name($p_table)
    {
        if (strstr($p_table, "_list"))
        {
            $l_table = str_replace("_list", "", $p_table);
        }
        else
        {
            $l_table = $p_table;
        ...
        return $this->retrieve('SELECT * FROM isysgui_catg WHERE isysgui_catg__source_table = ' . $this->convert_sql_text($l_table) . ';');
    } // function
     ...
     * @author  Dennis Stuecken <dstuecken@i-doit.de>
     */
    public function get_catg_name_by_id_as_string($p_catg_id)
    {
        return $this->retrieve('SELECT isysgui_catg__title FROM isysgui_catg WHERE isysgui_catg__id = ' . $this->convert_sql_id($p_catg_id) . ';')
     ...
     * @author  Niclas Potthast <npotthast@i-doit.org>
     */
    public function get_cats_name_by_id_as_string($p_cats_id)
    {
        return $this->retrieve('SELECT isysgui_cats__title FROM isysgui_cats WHERE isysgui_cats__id = ' . $this->convert_sql_id($p_cats_id) . ';')
     ...
     */
    public function get_cat_custom_name_by_id_as_string($p_custom_id)
    {
        return $this->retrieve('SELECT isysgui_catg_custom__title FROM isysgui_catg_custom WHERE isysgui_catg_custom__id = ' . $this->convert_sql_id($p_custom_id) . ';')
    ...
    public function get_cat_id_by_const_name($p_const, $p_type = 'g')
    {
        $l_data = $this->retrieve(
            'SELECT isysgui_cat' . $p_type . '__id FROM isysgui_cat' . $p_type . ' WHERE isysgui_cat' . $p_type . '__const = ' . $this->convert_sql_text($p_const) . ';'
        )
            ->get_row_value('isysgui_cat' . $p_type . '__id');
    ...
    {
        return $this->retrieve(
            'SELECT isysgui_cat' . $p_type . '__const FROM isysgui_cat' . $p_type . ' WHERE isysgui_cat' . $p_type . '__id = ' . $this->convert_sql_id($p_id) . ';'
        )
            ->get_row_value('isysgui_cat' . $p_type . '__const');
    } // function
    ...
    {
        return ($this->get_type_by_id($p_typeid) !== null) ? true : false;
    } // function
    ...
    public function get_all_catg_by_obj_type_id($p_objtype_id, $p_catg_id = null)
    {
        ...
        $l_sql .= 'ORDER BY isysgui_catg__sort ASC;';
    ...
    {
        assert('is_int($p_record_status)');
        assert('is_bool($p_overview_only)');
    ...
    {
        $l_sql      = "SELECT * FROM isys_obj_type_2_isysgui_catg_custom AS oc
    ...
    public function get_all_categories($p_category_types = [])
    {
        ...
        {
            if (isset($l_result[$l_type])) continue;
    ...
    public function get_all_cats($p_isys_gui_cats__id = null, $p_condition = null)
    {
        ...
        $l_sql .= ' ORDER BY isysgui_cats__sort ASC;';
    ...
    public function get_all_catg_2_objtype_id($p_objtype_id, $p_condition = null, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        else $l_object_type_condition = '';

        $l_strSql = "SELECT *,  " . "(" . "SELECT COUNT(*) " . "FROM isys_obj_type_2_isysgui_catg " . "WHERE " .
            "isys_obj_type_2_isysgui_catg__isysgui_catg__id  = isysgui_catg__id " . $l_object_type_condition . ") selected " . "FROM isysgui_catg " . "WHERE TRUE";

        if ($p_status)
        {
            $l_strSql .= " AND isysgui_catg__status = '" . C__RECORD_STATUS__NORMAL . "' ";
        }
        ...
        $l_strSql .= " ORDER BY selected, isysgui_catg__sort;";
    ...
    public function update_view_all_catd_2_obj($p_obj_id, $p_arr_catd_id)
    {
        ...
        if ($l_daores != null)
        {
            ...
            if ($l_distributor != null)
            {
                ...
                if ($l_daores != null)
                {
                    while ($l_guicatd = $l_daores->get_row())
                    { // for each catd
                        $l_intSetVisible = 0;
                        $l_catdId        = $l_guicatd['isysgui_catd__id'];
                        if (count($p_arr_catd_id) > 0)
                        {
     ...
     * @param   string  $p_show_in_tree
     * @param   string  $p_sysid_prefix
     *
     * @return  mixed
     * @throws  isys_exception_dao
     */
    public function insert_new_objtype($p_groupid, $p_title = null, $p_const = null, $p_selfdefined = "1", $p_container = "0", $p_img_name = null, $p_icon = null, $p_sort = 65535, $p_status = C__RECORD_STATUS__NORMAL, $p_cats = null, $p_show_in_tree = "1", $p_sysid_prefix = null)
    {
        if (is_null($p_cats))
        {
            $l_cats = "null";
        ...
        if (is_null($p_title))
        {
            $l_strObjTypeName = "OBJECT_TYPE__" . time(); // Tmp_Name for ObjType
        }
        else
        {
        ...
        if (is_null($p_const))
        {
            $p_const = "C__" . strtoupper($l_strObjTypeName);
        } // if

        $p_const = $this->generate_unique_obj_type_constant($p_const);

        if (is_null($p_img_name))
        {
            $l_img_name = "null";
        }
        else
        {
            $l_img_name = $this->convert_sql_text($p_img_name);
        } // if

        if (!is_numeric($p_groupid) || !isset($p_groupid) || empty($p_groupid))
        {
            $p_groupid = C__OBJTYPE_GROUP__INFRASTRUCTURE;
        } // if

        $l_strSQL = "INSERT INTO isys_obj_type " . "(" . "isys_obj_type__isys_obj_type_group__id, " . "isys_obj_type__isysgui_cats__id, " . "isys_obj_type__title, " .
            "isys_obj_type__description, " . "isys_obj_type__selfdefined, " . "isys_obj_type__container, " . "isys_obj_type__idoit_obj_type_number, " .
            "isys_obj_type__obj_img_name, " . "isys_obj_type__const, " . "isys_obj_type__icon, " . "isys_obj_type__sort, " . "isys_obj_type__property, " .
            "isys_obj_type__status, " . "isys_obj_type__show_in_tree, " . "isys_obj_type__sysid_prefix " . ")" . " VALUES " . "(" . "'" . $p_groupid . "', " .
            // isys_obj_type__isys_obj_type_group__id
            "{$l_cats}, " . // isys_obj_type__isysgui_cats__id
            "'$l_strObjTypeName', " . // isys_obj_type__title
            "'', " . // isys_obj_type__description
            "'{$p_selfdefined}', " . // isys_obj_type__selfdefined
            "'{$p_container}', " . // isys_obj_type__container
            "'', " . // isys_obj_type__idoit_obj_type_number
            "{$l_img_name}, " . // isys_obj_type__obj_img_name
            "'" . $p_const . "', " . // isys_obj_type__const
            "'{$p_icon}', " . // isys_obj_type__icon
            "'{$p_sort}', " . // isys_obj_type__sort
            "'0', " . // isys_obj_type__property
            "'" . $p_status . "', " . // isys_obj_type__status
            "'{$p_show_in_tree}', " . // isys_obj_type__show_in_tree
            "'{$p_sysid_prefix}' " . // isys_obj_type__sysid_prefix
            ");";
    ...
    public function delete_object_type($p_nID)
    {
        ...
        //see if it's a deletable object type
        if (!$this->is_object_type_deletable($p_nID))
        {
            $l_obj_type = $g_comp_template_language_manager->{$this->get_objtype_name_by_id_as_string($p_nID)};
            throw new Exception("Object type: {$l_obj_type} is not self-defined.");
        }

        //see, if there are objects of this type
        $l_ret = $this->get_objects_by_type_id($p_nID);
        if ($l_ret->num_rows())
        {
            $l_obj_type = $g_comp_template_language_manager->{$this->get_objtype_name_by_id_as_string($p_nID)};
            throw new Exception("There are objects of {$l_obj_type} existing! Delete them in order to delete this type.");
        }

        $l_strSQL = "DELETE FROM isys_obj_type_2_isysgui_catg " . "WHERE " . "isys_obj_type_2_isysgui_catg__isys_obj_type__id = " . "'$p_nID';";
        $l_bRet   = $this->update($l_strSQL);

        if ($l_bRet)
        {
            $l_strSQL = "DELETE FROM isys_obj_type " . "WHERE isys_obj_type__id = " . $this->convert_sql_id($p_nID) . ";";
            $l_bRet   = $this->update($l_strSQL);
        }
    ...
    public function rank_record($p_object_id, $p_direction, $p_table, $p_checkMethod = null, $p_purge = false)
    {
        ...
        if ($l_ret && $l_ret->num_rows())
        {
            ...
            if ($p_purge === true) $l_intActRecStatus = C__RECORD_STATUS__DELETED;
            ...
            if ($p_checkMethod !== null)
            {
                ...
                {
                    $l_startOperation = $l_obj->$l_method(
                        $p_direction,
            ...
            if ($l_startOperation)
            {
                ...
                if (is_a($this, "isys_cmdb_dao_category") || $p_table == "isys_obj")
                {
                    ...
                    if (isset($l_row[$p_table . "__isys_catg_connector_list__id"]))
                    {
                        ...
                        $l_dao  = new isys_cmdb_dao_category_g_connector($this->m_db);
                    ...
                    case C__RECORD_STATUS__PURGE:
                        ...
                        else
                        {
                            ...
                            if ($l_connector_id > 0)
                            {
                                ...
                                $l_dao_cc->delete_cable_connection($l_dao_cc->get_cable_connection_id_by_connector_id($l_connector_id));
    ...
    public function delete_object($p_objID)
    {
        ...
        try
        {
            ...
            $l_sql = "DELETE isys_cats_net_ip_addresses_list FROM isys_cats_net_ip_addresses_list " . "INNER JOIN isys_catg_ip_list " .
                "ON isys_catg_ip_list__isys_cats_net_ip_addresses_list__id = isys_cats_net_ip_addresses_list__id " . "WHERE isys_catg_ip_list__isys_obj__id = " .
                (int) $p_objID . ";";
            ...
            {
                $l_sql = "DELETE isys_connection FROM isys_connection " . "INNER JOIN " . $l_ref . " ON " . "isys_connection__id = " . $l_ref . "__isys_connection__id " .
                    "WHERE (" . $l_ref . "__isys_obj__id = '" . $p_objID . "');";
    ...
    public function delete_object_and_relations($p_objID)
    {
        ...
        if (!is_numeric($p_objID)) throw new isys_exception_cmdb("Object-ID " . $p_objID . " not numeric!");
        ...
        try
        {
            ...
            // First we delete entries from related tables.
            $l_sql = "DELETE isys_cats_net_ip_addresses_list FROM isys_cats_net_ip_addresses_list " . "INNER JOIN isys_catg_ip_list " .
                "ON isys_catg_ip_list__isys_cats_net_ip_addresses_list__id = isys_cats_net_ip_addresses_list__id " . "WHERE isys_catg_ip_list__isys_obj__id IN(" . implode(
                    ',',
                    $l_objects_to_delete
                ) . ");";
    ...
    public function object_changed($p_object_id, $p_changed_by = null)
    {
        ...
        {
            $p_changed_by = $g_comp_session->get_current_username();
        } // if
        ...
        // refs #4218
        $this->update(
            sprintf('DELETE FROM isys_cache_qinfo WHERE isys_cache_qinfo__isys_obj__id %s;', $l_condition)
        );

        $l_sql = 'UPDATE isys_obj SET isys_obj__updated = NOW(), isys_obj__updated_by = ' . $this->convert_sql_text(
                $p_changed_by
            ) . ' WHERE isys_obj__id ' . $l_condition . ';';
     ...
     * @throws isys_exception_dao
     */
    public function insert_new_obj($p_obj_type_id, $p_unused = null, $p_strTitle = null, $p_strSYSID = null, $p_record_status = C__RECORD_STATUS__BIRTH, $p_hostname = null, $p_scantime = null, $p_import_date = false, $p_created = null, $p_created_by = null, $p_updated = null, $p_updated_by = null, $p_category = null, $p_purpose = null, $p_cmdb_status = null, $p_description = null)
    {
        global $g_comp_session;

        $l_username = $g_comp_session->get_current_username();
        ...
        // Insert object.
        if (!is_numeric($p_obj_type_id)) throw new isys_exception_cmdb("Object type invalid! Object not created.");

        // Get creation stats.
        $l_created    = is_null($p_created) ? "NOW()" : "'" . $p_created . "'";
        $l_created_by = is_null($p_created_by) ? $l_username : $p_created_by;

        // Get update stats.
        $l_updated_by = is_null($p_updated_by) ? $l_username : $p_updated_by;
        $l_updated    = is_null($p_updated) ? "NOW()" : "'" . $p_updated . "'";
        ...
        $l_cmdb_status = ($p_cmdb_status > 0) ? $p_cmdb_status : C__CMDB_STATUS__IN_OPERATION;
        ...
        $l_strSQL = "INSERT INTO isys_obj (" . "isys_obj__title, " . "isys_obj__isys_obj_type__id, " . "isys_obj__isys_cmdb_status__id, " . "isys_obj__status, " .
            ...
            "isys_obj__scantime," . "isys_obj__imported," . "isys_obj__description " . ") VALUES (" . $this->convert_sql_text(
                $p_strTitle
            ) . ", " . $this->convert_sql_id($p_obj_type_id) . ", " . $this->convert_sql_id($l_cmdb_status) . ", " . $this->convert_sql_id(
                $p_record_status
            ) . ", " . $this->convert_sql_text(
                $l_strSYSID
            ) . ", " . "{$l_created}," . "'" . $l_created_by . "'," . "{$l_updated}," . "'" . $l_updated_by . "'," . $this->convert_sql_text(
                $p_hostname
        ...
        if ($l_bRet = $this->update($l_strSQL))
        {
            ...
            if (empty($l_strSYSID))
            {
                ...
                $l_strSQL = 'UPDATE isys_obj SET isys_obj__sysid = ' . $this->convert_sql_text($l_strSYSID) . ' WHERE isys_obj__id = ' . $this->convert_sql_id(
                        $l_object_id
                    ) . ';';
            ...
            {
                $l_inventory_no = isys_cmdb_dao_category_g_accounting::instance($this->get_database_component())
                    ->replace_placeholders($l_auto_inventory, $l_object_id, $p_obj_type_id, $p_strTitle, $l_strSYSID);
                $l_insert       = 'INSERT INTO isys_catg_accounting_list
					(isys_catg_accounting_list__isys_obj__id, isys_catg_accounting_list__status, isys_catg_accounting_list__inventory_no)
					VALUES(' . $this->convert_sql_id($l_object_id) . ', \'' . C__RECORD_STATUS__NORMAL . '\', ' . $this->convert_sql_text($l_inventory_no) . ')';
        ...
        $l_strSQL = "INSERT INTO isys_catg_global_list (" . "isys_catg_global_list__isys_catg_global_category__id, " . "isys_catg_global_list__isys_purpose__id, " .
            "isys_catg_global_list__isys_obj__id, " . "isys_catg_global_list__description, " . "isys_catg_global_list__status) " . "VALUES (" . $this->convert_sql_id(
                $p_category
            ) . ", " . $this->convert_sql_id($p_purpose) . ", " . $this->convert_sql_id($l_object_id) . ", " . "''," . "'" . C__RECORD_STATUS__NORMAL . "'" . ");";
    ...
    public function generate_unique_obj_type_constant($p_obj_type_const, $p_obj_type_id = null)
    {
        ...
        while (($l_found = $this->retrieve(
            'SELECT isys_obj_type__id FROM isys_obj_type WHERE isys_obj_type__const = ' . $this->convert_sql_text($l_obj_type_const) . $l_condition . ';'
        )
     ...
     * @param int                     $p_record_status
     * @param string                  $p_hostname
     * @param int                     $p_scantime
     * @param int                     $p_created
     * @param string                  $p_created_by
     * @param int                     $p_updated
     * @param string                  $p_updated_by
     * @param int                     $p_cmdb_status
     * @param int                     $p_rt_cf_id
     * @param int                     $p_category
     * @param int                     $p_purpose
     *
     * @return bool Success?
     */
    public function update_object($p_object_id, $p_object_type_id = null, $p_title = null, $p_description = null, $p_sysid = null, $p_record_status = null, $p_hostname = null, $p_scantime = null, $p_created = null, $p_created_by = null, $p_updated = null, $p_updated_by = null, $p_cmdb_status = null, $p_rt_cf_id = null, $p_category = null, $p_purpose = null)
    {
        ...
        // Where to update:
        $l_obj_table    = 'isys_obj';
        $l_global_table = 'isys_catg_global_list';

        // How to update:
        $l_obj_query    = 'UPDATE ' . $l_obj_table . ' SET %s WHERE ' . $l_obj_table . '__id = ' . $p_object_id;
        $l_global_query = 'UPDATE ' . $l_global_table . ' SET %s WHERE ' . $l_global_table . '__' . $l_obj_table . '__id = ' . $p_object_id;
        ...
        if (!is_null($p_object_type_id))
        {
            $l_obj_sets['isys_obj_type__id'] = $this->convert_sql_id(
                $p_object_type_id
            );
        }
        // Title:
        ...
        else
        {
            ...
            global $g_comp_session;
            $l_obj_sets['updated_by'] = "'" . $g_comp_session->get_current_username() . "'";
        }
        // Category:
        if (!is_null($p_category))
        {
        ...
        else
        {
            if (!($p_cmdb_status > 0))
            {
                $l_obj_sets['isys_cmdb_status__id'] = $this->convert_sql_id(C__CMDB_STATUS__IN_OPERATION);
            }
        }

        // RT CF:
        if (!is_null($p_rt_cf_id))
        {
            $l_obj_sets['rt_cf__id'] = $this->convert_sql_id($p_rt_cf_id);
        }

        // Create SQL query for the object table:
        $l_merged_sets    = array_merge($l_sets, $l_obj_sets);
        $l_converted_sets = [];
        foreach ($l_merged_sets as $l_key => $l_value)
        {
            $l_converted_sets[] = $l_obj_table . '__' . $l_key . ' = ' . $l_value;
        }
        if (count($l_converted_sets) > 0)
        {
        ...
        $l_merged_sets    = array_merge($l_sets, $l_global_sets);
        $l_converted_sets = [];
        foreach ($l_merged_sets as $l_key => $l_value)
        {
            $l_converted_sets[] = $l_global_table . '__' . $l_key . ' = ' . $l_value;
        }
        if (count($l_converted_sets) > 0)
        {
            $l_query_part   = implode(', ', $l_converted_sets);
    ...
    public function assign_catg($p_obj_type, $p_category_id = null, $p_category_const = null)
    {
        ...
        if ($p_category_id > 0)
        {
            $l_sql = "DELETE FROM isys_obj_type_2_isysgui_catg " . "WHERE (isys_obj_type_2_isysgui_catg__isys_obj_type__id = '{$p_obj_type}' AND " .
                "isys_obj_type_2_isysgui_catg__isysgui_catg__id = " . $this->convert_sql_id(
                    $p_category_id
                ) . ");";
            ...
            if ($this->update($l_sql) && $this->apply_update())
            {
                $l_sql = "INSERT INTO isys_obj_type_2_isysgui_catg " . "(" . "isys_obj_type_2_isysgui_catg__isys_obj_type__id, " .
                    "isys_obj_type_2_isysgui_catg__isysgui_catg__id" . ")" . " VALUES " . "(" . $this->convert_sql_id(
                        $p_obj_type
    ...
    public function update_objtype_by_id($p_objtype_id, $p_arr_catg_id, $p_arr_overview_catg, $p_arr_update)
    {
        ...
        $p_arr_update['C__OBJTYPE__CONST'] = strtoupper(
            str_replace(
                [
                    "\"",
                    "'",
        ...
        // Create update SQL.
        if (count($p_arr_update) > 0)
        {
            // @todo for the time being isys_glob_utf8_decode the title there is still the issue with the encoding
            $l_strSql = "UPDATE isys_obj_type SET " . "isys_obj_type__title = '" . $this->m_db->escape_string($p_arr_update['C__OBJTYPE__TITLE']) . "' ," .
                "isys_obj_type__icon = '" . $this->m_db->escape_string($p_arr_update['C__OBJTYPE__ICON']) . "' ," . "isys_obj_type__isys_obj_type_group__id = " .
                $this->convert_sql_id($p_arr_update['C__OBJTYPE__GROUP_ID']) . " ," . "isys_obj_type__isysgui_cats__id  = " .
                $this->convert_sql_id($p_arr_update['C__OBJTYPE__CATS_ID']) . " ," . "isys_obj_type__description  = " .
                $this->convert_sql_text($p_arr_update['C__OBJTYPE__DESCRIPTION']) . " ,";

            if ($p_arr_update['C__OBJTYPE__SELF_DEFINED'])
            {
                $l_strSql .= "isys_obj_type__selfdefined  = '" . $this->m_db->escape_string($p_arr_update['C__OBJTYPE__SELF_DEFINED']) . "' ,";
            } // if

            if ($p_arr_update['C__OBJTYPE__IS_CONTAINER'] != "")
            {
                $l_strSql .= "isys_obj_type__container   = '" . $this->m_db->escape_string($p_arr_update['C__OBJTYPE__IS_CONTAINER']) . "' ,";
            } // if

            $p_arr_update['C__OBJTYPE__CONST'] = $this->generate_unique_obj_type_constant($p_arr_update['C__OBJTYPE__CONST'], $l_nObjTypeID);

            $l_strSql .= "isys_obj_type__status  = '" . C__RECORD_STATUS__NORMAL . "', " . "isys_obj_type__show_in_tree   = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__SHOW_IN_TREE']
                ) . "' ," . "isys_obj_type__idoit_obj_type_number   = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__TYPE_NUMBER']
                ) . "' ," . "isys_obj_type__obj_img_name   = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__IMG_NAME']
                ) . "' ," . "isys_obj_type__relation_master   = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__RELATION_MASTER']
                ) . "' ," . "isys_obj_type__overview = '" . $this->m_db->escape_string(
                    $p_arr_update['C__CMDB__OVERVIEW__ENTRY_POINT']
                ) . "' ," . "isys_obj_type__default_template = " . $this->convert_sql_id(
                    $p_arr_update['C__CMDB__OBJTYPE__DEFAULT_TEMPLATE']
                ) . " ," . "isys_obj_type__const   = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__CONST']
                ) . "', " . "isys_obj_type__color = '" . $this->m_db->escape_string(
                    $p_arr_update["C__OBJTYPE__COLOR"]
                ) . "', " . "isys_obj_type__show_in_rack = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__INSERTION_OBJECT']
                ) . "', " . "isys_obj_type__sort = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__POSITION_IN_TREE']
                ) . "', " . "isys_obj_type__sysid_prefix = '" . $this->m_db->escape_string(
                    $p_arr_update['C__OBJTYPE__SYSID_PREFIX']
                ) . "' " . "WHERE isys_obj_type__id = '" . $this->m_db->escape_string($l_nObjTypeID) . "'";
        ...
        if ($l_bRet)
        {
            ...
            $this->update(
                "DELETE FROM isys_obj_type_2_isysgui_catg_custom WHERE isys_obj_type_2_isysgui_catg_custom__isys_obj_type__id = " . $this->convert_sql_id($l_nObjTypeID) . ";"
            );

            // Delete assigned custom categories.
            $l_bRet = $this->update(
                "DELETE FROM isys_obj_type_2_isysgui_catg WHERE isys_obj_type_2_isysgui_catg__isys_obj_type__id = " . $this->convert_sql_id($l_nObjTypeID) . ";"
            );
        }

        if ($l_bRet && count($p_arr_catg_id) > 0)
        {
        ...
        if ($l_bRet)
        {
            ...
            $l_overview_catg_custom_delete = 'DELETE FROM isys_obj_type_2_isysgui_catg_custom_overview WHERE isys_obj_type__id = ' . $this->convert_sql_id(
                    $l_nObjTypeID
                ) . ';';
            $l_bRet                        = ($this->update($l_overview_catg_delete) && $this->update($l_overview_catg_custom_delete));
        } // if

        if ($l_bRet && count($p_arr_overview_catg) > 0)
        {
            $i = 0;
            ...
            for ($l_i = 0;$l_i < count($p_arr_overview_catg);$l_i++)
            {
                ...
                {
                    // Global category
                    $l_strSqlValues[] = "('" . $this->convert_sql_id($p_objtype_id) . "', '" . $this->convert_sql_id(
                            $l_all_catg_arr[$p_arr_overview_catg[$l_i]]
                        ) . "', '" . $i++ . "')";
                }
                elseif (isset($l_all_catg_custom_arr[$p_arr_overview_catg[$l_i]]))
                {
                    // Custom category
                    $l_strSqlValuesCustomCat[] = "('" . $this->convert_sql_id($p_objtype_id) . "', '" . $this->convert_sql_id(
                            $l_all_catg_custom_arr[$p_arr_overview_catg[$l_i]]
                        ) . "', '" . $i++ . "')";
                } // if
            }
    ...
    public function count_objects($p_objgroup = null, $p_objtype = null, $p_ignore_cmdb_status = false, $p_condition = "")
    {
        ...
        if (is_null($p_objgroup))
        {
            ...
            if (!$p_ignore_cmdb_status && $this->prepare_status_filter() != "") $l_q .= " AND " . $this->prepare_status_filter();
    ...
    public function convert_sql_decimal($p_value, $p_bNotApo = false)
    {
        ...
        if ($p_value > 0)
        {
            $l_nUserID = $g_comp_session->get_user_id();
     ...
     * @return  array
     */
    function isys_get_table_data_by_id($p_tbl, $p_id)
    {
    ...
    public function get_table_from_const($p_const, $p_category = null)
    {
        ...
        if (empty($p_category))
        {
            $l_sql = "SELECT isysgui_catg__source_table FROM isysgui_catg WHERE isysgui_catg__const = '" . $p_const . "'";

            return $l_dao->retrieve($l_sql)
                ->get_row_value('isysgui_catg__source_table');
        }
        else
        {
            ...
            {
                $l_sql .= " isysgui_cat" . $p_category . "__id= '" . $p_const . "'";
            }
            else
            {
                $l_sql .= " isysgui_cat" . $p_category . "__const = '" . $p_const . "'";
            } // if
            return $l_dao->retrieve($l_sql)
                ->get_row_value("isysgui_cat" . $p_category . "__source_table");
        } // if
    } // function
     ...
     * @return  boolean
     */
    public function check_category($p_obj_id, $p_class_name, $p_cat_id = null, $p_table_name = "")
    {
        if (!empty($p_class_name) && class_exists($p_class_name))
        {
            $l_dao_class = new $p_class_name($this->get_database_component());
            if ($p_cat_id > 0 && method_exists($l_dao_class, 'set_category_id'))
            {
                $l_dao_class->set_category_id($p_cat_id);
            } // if

            if (method_exists($l_dao_class, "get_count"))
            {
                if (strpos($p_table_name, "_list") <= 0)
                {
                    $l_table = $p_table_name . "_list";
                }
                else
                {
                ...
                if (method_exists($l_dao_class, 'set_source_table')) $l_dao_class->set_source_table($l_table);
                $l_amount = $l_dao_class->get_count($p_obj_id);

                return ($l_amount > 0);
            }
            else
            {
                $l_sql = "SELECT COUNT({$p_table_name}__id) as count
					FROM {$p_table_name}
					WHERE {$p_table_name}__status = " . $this->convert_sql_int(C__RECORD_STATUS__NORMAL) . "
					AND {$p_table_name}__isys_obj__id = " . $this->convert_sql_id($p_obj_id) . ";";

                $l_ret    = $this->retrieve($l_sql);
                $l_amount = $l_ret->get_row();
                $l_ret->free_result();

                return ($l_amount["count"] > 0);
            } // if
        } // if
    ...
    public function category_has_childs($p_cat_id, $p_table_name)
    {
        ...
        $l_sql = "SELECT isysgui_cat" . $l_cat . "__id AS id, isysgui_cat" . $l_cat . "__class_name AS class_name, isysgui_cat" . $l_cat . "__source_table AS table_name " .
            "FROM isysgui_cat" . $l_cat . " " . "WHERE isysgui_cat" . $l_cat . "__parent = " . $this->convert_sql_id(
                $p_cat_id
            );
    ...
    public function get_object_types_by_category($p_category_id, $p_category_type = 'g', $p_as_string = true, $p_complete_row = false)
    {
        ...
        if ($p_category_type == 'g')
        {
            $l_sql .= 'INNER JOIN isys_obj_type_2_isysgui_catg ON isys_obj_type__id = isys_obj_type_2_isysgui_catg__isys_obj_type__id ' .
                'WHERE isys_obj_type_2_isysgui_catg__isysgui_catg__id = ' . $this->convert_sql_int(
                    $p_category_id
                );
        }
        else
        {
            $l_sql .= 'LEFT JOIN isysgui_cats_2_subcategory ON isysgui_cats_2_subcategory__isysgui_cats__id__parent = isys_obj_type__isysgui_cats__id ' .
                ' WHERE isys_obj_type__isysgui_cats__id = ' . $this->convert_sql_id(
                    $p_category_id
                ) . ' OR isysgui_cats_2_subcategory__isysgui_cats__id__child = ' . $this->convert_sql_id($p_category_id) . ' GROUP BY isys_obj_type__id';
        } // if
     ...
     * @return int
     */
    public function get_last_obj_id_from_type($p_obj_type_id = null)
    {
        $l_sql = 'SELECT MAX(isys_obj__id) AS isys_obj__id FROM isys_obj WHERE TRUE ';

        if (isset($p_obj_type_id)) $l_sql .= 'AND isys_obj__isys_obj_type__id = ' . $this->convert_sql_id($p_obj_type_id);

        $l_result = $this->retrieve($l_sql)
            ->__to_array();

        return $l_result['isys_obj__id'];
    } // function
     ...
     * @return  string
     */
    public function replace_object_placeholder($p_placeholder, $p_obj_id)
    {
        if (!is_array($p_placeholder) && strstr($p_placeholder, '%'))
        {
            $l_object = $this->get_object($p_obj_id, false, 1)
                ->__to_array();
            ...
            return strtr($p_placeholder, $l_arr_placeholders);
        }
        else
        {
     ...
     * @author Van Quyen Hoang <qhoang@i-doit.org>
     */
    public function get_object_by_hostname_serial_mac($p_hostname = null, $p_serial = null, $p_mac = null, $p_title = null, $p_ip = null, $p_ip_long = null, $p_fqdn = null, $p_objtype_filter_arr = null)
    {
        $l_ignore_obj_types = C__OBJTYPE__RELATION . ',' . C__OBJTYPE__PARALLEL_RELATION . ',' . C__OBJTYPE__SOA_STACK . ',' . C__OBJTYPE__CABLE . ',' . C__OBJTYPE__CONTAINER;
        ...
        {
            $l_add_ip_query = true;
            if (strlen($l_ip_condition) > 1) $l_ip_condition .= 'OR ';
            ...
            if ($p_hostname || $p_fqdn) $l_ip_condition .= ') ';
        }
        ...
        if ($l_add_ip_query)
        {
            if ($p_title)
            {
                if (strlen($l_ip_condition) > 1) $l_ip_condition .= ' AND ';
        ...
        if ($p_serial)
        {
            ...
            if ($p_title)
            {
                if (strlen($l_model_condition) > 1) $l_model_condition .= ' AND ';
        ...
        if ($p_mac)
        {
            ...
            if ($p_title)
            {
                if (strlen($l_port_condition) > 1) $l_port_condition .= ' AND ';
        ...
        {
            $l_query_arr[] = $l_main_query . ' WHERE isys_obj__status = ' . $this->convert_sql_int(
                    C__RECORD_STATUS__NORMAL
                ) . ' AND isys_obj__title = ' . $this->convert_sql_text($p_title);
        } // if
    ...
    public function gui_get_catg_with_subcats_by_objtype_id($p_objtype_id, $p_hierarchical = false)
    {
        ...
        {
            $l_res = $this->retrieve(
                sprintf('SELECT * FROM isysgui_catg WHERE isysgui_catg__parent %s;', $this->prepare_in_condition(array_keys($l_categories)))
            );
    ...
    public function gui_get_cats_with_subcats_by_objtype_id($p_objtype_id, $p_hierarchical = false)
    {
        ...
        {
            $l_res = $this->retrieve(
                sprintf('SELECT * FROM isysgui_cats WHERE isysgui_cats__parent %s;', $this->prepare_in_condition(array_keys($l_categories)))
            );
            ...
            $l_res = $this->retrieve(
                sprintf(
                    'SELECT cats.*, isysgui_cats_2_subcategory__isysgui_cats__id__parent AS parent
				FROM isysgui_cats_2_subcategory
				LEFT JOIN isysgui_cats AS cats ON cats.isysgui_cats__id = isysgui_cats_2_subcategory__isysgui_cats__id__child
				WHERE isysgui_cats_2_subcategory__isysgui_cats__id__parent %s;',
                    $this->prepare_in_condition(array_keys($l_categories))
                )
            );
    ...
    protected function load_obj_types()
    {
        ...
        while ($l_row = $l_res->get_row())
        {
            ...
            self::$m_obj_types[$l_row['isys_obj_type__id']]['LC_isys_obj_type__title'] = _L($l_row['isys_obj_type__title']);
        } // while

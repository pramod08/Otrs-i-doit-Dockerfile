...
class isys_cmdb_dao_file extends isys_component_dao
{
    ...
    public function get_active_file_versions($p_group_cat = true, $p_limit = null, $p_objid = null, $p_order_by = null, $l_nRecStat = null)
    {
        ...
        $l_sql .= 'FROM isys_cats_file_list list
			LEFT OUTER JOIN isys_file_category cat
			ON (cat.isys_file_category__id = list.isys_cats_file_list__isys_file_category__id)
			INNER JOIN isys_file_version version
			ON (version.isys_file_version__id = list.isys_cats_file_list__isys_file_version__id)
			INNER JOIN isys_file_physical physical
			ON (physical.isys_file_physical__id = version.isys_file_version__isys_file_physical__id)
			LEFT OUTER JOIN isys_obj
			ON isys_cats_file_list__isys_obj__id = isys_obj__id

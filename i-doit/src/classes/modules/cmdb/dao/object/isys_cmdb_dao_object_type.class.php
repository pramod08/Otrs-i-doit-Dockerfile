...
class isys_cmdb_dao_object_type extends isys_cmdb_dao_object
{
     ...
     * @return  boolean
     */
    public function save_list_config($p_object_type_id, $p_list_config, $p_list_query, $p_list_row_click = true, $p_default_sorting = null, $p_sorting_direction = 'asc', $p_user_obj_id = null)
    {
     ...
     */
    public function &get_categories($p_object_type_id, $p_category_types = [
                ...
                switch ($l_cattype)
                {
                    case C__CMDB__CATEGORY__TYPE_GLOBAL:
                        ...
                        while ($l_row = $l_dao_result->get_row())
                        {
                            ...
                            array_walk(
                                $l_row,
                                function ($p_value, $p_key) use (&$l_replaced)
                                {
     ...
     */
    public function refresh_objtype_list_config($p_object_type_id = null, $p_execute_query = false)
    {
        $l_condition = '';
        ...
        if (count($l_res))
        {
            ...
            while ($l_row = $l_res->get_row())
            {
                $l_configs    = isys_format_json::decode($l_row['isys_obj_type_list__config']);
                $l_properties = $l_new_config = [];
                ...
                foreach ($l_configs as $l_index => $l_config)
                {
                    ...
                    if (!isset($l_cache[$l_property_method . '::' . $l_prop_key]))
                    {
                        ...
                        $l_sql = 'SELECT isys_property_2_cat__id
							...
							WHERE isys_property_2_cat__cat_const = ' . $this->convert_sql_text(
                                isys_factory::get_instance($l_class, $this->m_db)
                                    ->get_category_const()
                            ) . '
                    ...
                    if ($l_cache[$l_property_method . '::' . $l_prop_key] > 0)
                    {
                        ...
                        $l_new_config[$l_index] = $l_config;
                        $l_properties[]         = $l_cache[$l_property_method . '::' . $l_prop_key];
                    } // if
                } // foreach

                if (count($l_properties))
                {
                    $l_new_query = $l_dao->reset()
                        ->create_property_query_for_lists($l_properties, $l_row['isys_obj_type_list__isys_obj_type__id']);
                    ...
                    try
                    {
                        ...
                        $l_sql = 'UPDATE isys_obj_type_list
							...
							isys_obj_type_list__config = ' . $this->convert_sql_text(isys_format_json::encode($l_new_config)) . '
							WHERE isys_obj_type_list__id = ' . $this->convert_sql_id($l_row['isys_obj_type_list__id']) . ';';
                    ...
                    {
                        ; // If the execution fails, the query will not be saved.
                    } // try
                } // if
            } // while
        } // if

 ...
 * @license     http://www.i-doit.com/license
 */
class isys_cmdb_dao_list_catg_drive extends isys_cmdb_dao_list
{
    /**
    ...
    public function modify_row(&$p_arrRow)
    {
        ...
        {
            $p_arrRow["isys_catd_drive_type__title"] = _L($p_arrRow["isys_catd_drive_type__title"]) . " (" . $p_arrRow["isys_stor_raid_level__title"] . ")";
        }
        else
        {
            if (!empty($p_arrRow["isys_filesystem_type__title"]))
            {
                $p_arrRow["isys_catd_drive_type__title"] = _L($p_arrRow["isys_catd_drive_type__title"]) . " (" . _L($p_arrRow["isys_filesystem_type__title"]) . ")";
            } // if
        } // if

        $p_arrRow["isys_catg_drive_list__capacity"] = $g_loc->fmt_numeric(
                isys_convert::memory(
                    $p_arrRow["isys_catg_drive_list__capacity"],
        ...
        $p_arrRow["isys_catg_drive_list__free_space"] = $g_loc->fmt_numeric(
                isys_convert::memory(
                    $p_arrRow["isys_catg_drive_list__free_space"],
                    $l_free_space_unit['isys_memory_unit__const'],
                    C__CONVERT_DIRECTION__BACKWARD
                )
            ) . " " . $l_free_space_unit['isys_memory_unit__title'];
        ...
        $p_arrRow["isys_catg_drive_list__used_space"] = $g_loc->fmt_numeric(
                isys_convert::memory(
                    $p_arrRow["isys_catg_drive_list__used_space"],
                    $l_used_space_unit['isys_memory_unit__const'],
                    C__CONVERT_DIRECTION__BACKWARD
                )
            ) . " " . $l_used_space_unit['isys_memory_unit__title'];

 ...
 * @license     http://www.i-doit.com/license
 */
class isys_cmdb_dao_list_catg_file extends isys_cmdb_dao_list
{
    /**
    ...
    public function modify_row(&$p_row)
    {
        ...
        if ($p_row["isys_file_version__isys_file_physical__id"] !== null)
        {
            ...
            $l_file_name     = _L($p_row['isys_obj_type__title']) . ' > ' . $p_row['isys_obj__title'];
            ...
            $p_row["download"] = '<a target="_blank" class="btn btn-small" href="' . $l_download_link . '" onclick="event.stopPropagation();">' .
                '<img src="' . $g_dirs["images"] . '/icons/silk/disk.png" class="mr5" /><span>' . _L('LC__UNIVERSAL__DOWNLOAD_FILE') . '</span></a>';
        }
        else
        {
        ...
        if (!empty($p_row["isys_catg_file_list__link"]))
        {
            $p_row["isys_catg_file_list__link"] = isys_helper_link::create_anker(
                $p_row["isys_catg_file_list__link"],
                '_blank',

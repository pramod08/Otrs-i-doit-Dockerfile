        ...
        if (!$this->object_is_locked())
        {
            ...
            try
            {
                if (is_array($p_posts) && count($p_posts) > 0)
                {
                    ...
                    {
                        // Notify the user!
                        isys_notify::warning(_L('LC__CMDB__RANK_ERROR__ZERO_ID', [$p_table]), ['sticky' => true]);

                        // This will create a log message, without displaying the big red error-box.
                        new isys_exception_database(_L('LC__CMDB__RANK_ERROR__ZERO_ID', [$p_table]));
                    } // if
                    ...
                    if (count($p_posts) > 0)
                    {
                        ...
                        if (!($l_result = $this->m_cat_dao->rank_records(
                            $p_posts,
                            $p_direction,
                            $p_table,
                            [
                            ...
                            $l_purge)))
                        {
                            throw new isys_exception_cmdb(
                                "Could not delete category entries (" . var_export($p_posts, true) . ") (CMDB-DAO->rank_records)", C__CMDB__ERROR__ACTION_PROCESSOR
                            );
                        } // if

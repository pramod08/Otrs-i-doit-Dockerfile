...
class isys_cmdb_ui_category_g_connector extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        $l_new = (!isset($l_catdata["isys_catg_connector_list__id"]) || is_null($l_catdata["isys_catg_connector_list__id"]));
        ...
        $l_context[C__CONNECTOR__INPUT]  = _L("LC__UNIVERSAL__ALREADY_CONNECTED");
        $l_context[C__CONNECTOR__OUTPUT] = _L("LC__CMDB__CATG__UI_ASSIGNED_UI") . " %s";
                 ...
                 ] as $l_index)
        {
            ...
            while ($l_row = $l_data->get_row())
            {
                ...
                $l_connection = _L("LC__CMDB__CATG__UI_ASSIGNED_UI") . " " . $p_cat->get_connector_name_by_id(
                        $l_row["isys_catg_connector_list__isys_catg_connector_list__id"]
                    );

                if ($l_row["isys_catg_connector_list__isys_catg_connector_list__id"] != $_GET[C__CMDB__GET__CATLEVEL])
                {
                    if ($l_row["isys_catg_connector_list__isys_catg_connector_list__id"] > 0)
                    {
        ...
        if (isset($l_sibling))
        {
            $l_rules["C__CATG__CONNECTOR__SIBLING_IN"]["p_arData"]  = serialize($l_sibling[C__CONNECTOR__INPUT]);
            $l_rules["C__CATG__CONNECTOR__SIBLING_OUT"]["p_arData"] = serialize($l_sibling[C__CONNECTOR__OUTPUT]);
        }
        ...
        if ($l_new)
        {
            ...
            if ($l_schema_config == 'null') // migration did an error value is 'null' as string
            {
                $l_schema_config = [
                    "##INPUT## - OUT",
                    "- ##INPUT##",
            ...
            $l_schema_config["-1"] = _L('LC__UNIVERSAL__OWN') . ":";

            $l_rules["C__CATG__CONNECTOR__SUFFIX_SCHEMA"]["p_arData"]       = serialize($l_schema_config);
        ...
        if ($l_catdata['isys_catg_connector_list__assigned_category'] == 'C__CATG__UNIVERSAL_INTERFACE')
        {
            ...
            {
                $l_connection_type = '<img src="' . $g_dirs["images"] . 'empty.gif" width="15px" style="margin-right:5px;" alt="' . _L('LC__UNIVERSAL__NEW_VALUE') . '" />';
            } // if
    ...
    public function process_list(isys_cmdb_dao_category &$p_cat, $p_get_param_override = null, $p_strVarName = null, $p_strTemplateName = null, $p_bCheckbox = true, $p_bOrderLink = true, $p_db_field_name = null)
    {
        ...
        $l_dao_list = new isys_cmdb_dao_list_catg_connector($p_cat->get_database_component());
        ...
        $index_includes['contentbottomcontent'] = "content/bottom/content/catg__connector_list.tpl";
    } // function
} // class
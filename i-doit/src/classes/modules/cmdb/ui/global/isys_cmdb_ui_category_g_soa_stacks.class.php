...
class isys_cmdb_ui_category_g_soa_stacks extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        while ($l_row = $l_res->get_row())
        {
            ...
            if (count($l_catdata) > 0) $l_res_assigned = ($p_cat->get_assigned_object(
                $l_catdata["isys_catg_soa_stacks_list__id"],
                $l_relation_obj["isys_catg_relation_list__isys_obj__id"]
            ));
            else
            ...
            if ($l_selected) $l_arData[$l_relation_obj["isys_catg_relation_list__isys_obj__id"]] = $l_relation_obj["slave_title"] . ' >> ' . $l_relation_obj["master_title"];
        }

        $l_rel_res = $l_dao_relation->get_data(
            null,
            null,
            "AND (isys_catg_relation_list__isys_relation_type__id = '" . C__RELATION_TYPE__SOFTWARE . "' AND isys_catg_relation_list__isys_obj__id__slave = " . $p_cat->convert_sql_id(
                $_GET[C__CMDB__GET__OBJECT]
            ) . ") OR (isys_catg_relation_list__isys_relation_type__id = '" . C__RELATION_TYPE__CLUSTER_SERVICE . "' AND isys_catg_relation_list__isys_obj__id__slave = " . $p_cat->convert_sql_id(
                $_GET[C__CMDB__GET__OBJECT]
            ) . ")"
        );
        ...
        $g_comp_template->assign("soa_components", $l_cat_list);

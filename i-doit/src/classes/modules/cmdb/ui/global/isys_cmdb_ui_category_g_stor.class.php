...
class isys_cmdb_ui_category_g_stor extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        if ($l_res && $l_res->num_rows())
        {
            ...
            $l_rules["C__CATG__STORAGE_CONTROLLER"]["p_arData"] = serialize($l_controllers);
        } // if
        ...
        $l_rules["C__CATG__STORAGE_RAIDGROUP"]["p_arData"] = serialize($l_devices);
        ...
        $l_res = $l_dao_ctrl->get_devices(null, $l_gets[C__CMDB__GET__OBJECT], null, C__STOR_TYPE_DEVICE_HD);
        ...
        $l_res = $l_dao_ctrl->get_devices(null, $l_gets[C__CMDB__GET__OBJECT], $l_nStorID, C__STOR_TYPE_DEVICE_HD);
        ...
        if ($l_res && $l_num_disks)
        {
            while ($l_row = $l_res->get_row())
            {
                ...
                if (!isset($l_min_capacity) || isys_convert::memory(
        ...
        $l_rules["C__CATG__STORAGE_CONNECTION"]["p_arData"] = serialize($l_arHDAlloc);
        ...
        if ($l_res && $l_res->num_rows())
        {
            ...
            $l_rules["C__CATG__STORAGE_SANPOOL"]["p_arData"] = serialize($l_pools);
        } // if

        $l_rules["C__CATG__STORAGE_RAIDLEVEL"]["p_strTable"] = "isys_stor_raid_level";
        $l_rules["C__CATG__STORAGE_HOTSPARE"]["p_arData"]    = serialize(get_smarty_arr_YES_NO());

        if ($l_nStorID > 0)
        {
            // Fill in the values from the db.
            $l_catdata["isys_catg_stor_list__capacity"] = isys_convert::memory(
                $l_catdata["isys_catg_stor_list__capacity"],
                $l_catdata["isys_catg_stor_list__isys_memory_unit__id"],
                C__CONVERT_DIRECTION__BACKWARD
            );

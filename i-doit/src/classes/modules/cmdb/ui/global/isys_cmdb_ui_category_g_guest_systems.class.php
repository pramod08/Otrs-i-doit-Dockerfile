...
class isys_cmdb_ui_category_g_guest_systems extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        $l_rules["C__CATG__GUEST_SYSTEM_CONNECTED_OBJECT"]["p_strSelectedID"]                                         = $l_dao_con->get_object_id_by_connection(
            $l_catdata["isys_catg_guest_systems_list__isys_connection__id"]
        );
        $l_rules["C__CMDB__CAT__COMMENTARY_" . $p_cat->get_category_type() . $p_cat->get_category_id(
        )]["p_strValue"]                                                                                              = $l_catdata["isys_catg_guest_systems_list__description"];
    ...
    public function process_list(isys_cmdb_dao_category &$p_cat, $p_get_param_override = null, $p_strVarName = null, $p_strTemplateName = null, $p_bCheckbox = true, $p_bOrderLink = true, $p_db_field_name = null)
    {
        ...
        isys_component_template_navbar::getInstance()
            ...
            ->set_active(
                (isys_auth_cmdb::instance()
                    ->has_rights_in_obj_and_category(isys_auth::EDIT, $l_object_id, $p_cat->get_category_const())),
        ...
        if (!$p_cat->objtype_is_catg_assigned($l_obj_type, C__CATG__CLUSTER) && $p_cat->objtype_is_catg_assigned($l_obj_type, C__CATG__CLUSTER_MEMBERSHIPS))
        {
                ...
                while ($l_row_cluster_member = $l_res_cluster_member->get_row())
                {
                    ...
                    if (count($l_res_guest_system))
                    {
                        while ($l_row_guest_system = $l_res_guest_system->get_row())
                        {
                            ...
                            if ($l_row_guest_system['isys_catg_virtual_machine_list__primary'] == null || $l_row_guest_system['isys_catg_virtual_machine_list__primary'] == $l_object_id)
                            {
                                ...
                                $l_inherited_services[] = [
                                    ...
                                    'obj_type_title' => _L($p_cat->get_objtype_name_by_id_as_string($l_row_guest_system['isys_obj__isys_obj_type__id'])),
                                    ...
                                    'runs_on'        => _L(
                                            $p_cat->get_objtype_name_by_id_as_string($l_row_cluster_member['isys_obj__isys_obj_type__id'])

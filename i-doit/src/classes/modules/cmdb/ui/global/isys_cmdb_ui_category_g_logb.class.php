...
class isys_cmdb_ui_category_g_logb extends isys_cmdb_ui_category_global
{
    ...
    {
        global $index_includes, $g_comp_template, $g_comp_database, $g_comp_template_language_manager, $g_loc;
        ...
        if (is_null($l_catdata) || !$l_catdata) $l_catdata = $p_cat->get_result()
            ->__to_array();
        ...
        if ($l_userdata->num_rows() > 0)
        {
            ...
            $l_strUsertitle = "<a href=\"?" . C__CMDB__GET__OBJECT . "=" . $l_userdata["isys_cats_person_list__isys_obj__id"] . "\">" . $l_userdata["isys_cats_person_list__title"] . "</a>" . " (" . $l_userdata["isys_cats_person_list__first_name"] . $l_userdata["isys_cats_person_list__last_name"];
    ...
    public function process_list(isys_cmdb_dao_category &$p_cat, $p_get_param_override = null, $p_strVarName = null, $p_strTemplateName = null, $p_bCheckbox = true, $p_bOrderLink = true, $p_db_field_name = null)
    {
        ...
        $l_arTableHeader = [
            ...
            "isys_logbook__changes"          => _L("LC__CMDB__LOGBOOK__CHANGED_FIELDS"),
    ...
    public function get_changes_as_html_table($p_changes_array, $p_class_name = "listing")
    {
        ...
        if (is_array($l_changes_ar) && count($l_changes_ar) > 0)
        {
            ...
            $l_changes = "<table class=\"" . $p_class_name . "\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\">" . "<colgroup>" . "<col width=\"30%\" />" . "<col width=\"30%\" />" . "<col width=\"30%\" />" . "</colgroup>" . "<thead>" . "<tr>" . "<th>" . $g_comp_template_language_manager->{"LC__REGEDIT__VALUE"} . "</th>" . "<th>" . $g_comp_template_language_manager->{"LC_UNIVERSAL__FROM"} . "</th>" . "<th>" . $g_comp_template_language_manager->{"LC__TO"} . "</th>" . "</tr>" . "</thead>" . "<tbody>";
            ...
            foreach ($l_changes_ar as $l_field => $l_change)
            {
                if (strpos($l_field, '::') > 0)
                {
                    ...
                    if (class_exists($l_data[0]))
                    {
                        ...
                        if (method_exists($l_dao, 'get_properties_ng'))
                        {
                            ...
                            $l_lang_field       = _L($l_data_information[$l_data[1]][C__PROPERTY__INFO][C__PROPERTY__INFO__TITLE]);
                        } // if
                    } // if
                }
                else if (is_numeric($l_field) && is_array($l_change))
                {
                    ...
                    foreach ($l_change AS $l_property_key => $l_property_changes)
                    {
                        ...
                        // @see ID-2217 Removed the "isys_glob_unescape" functions
                        $l_changes .= '<tr class="' . $l_even_odd . '">' . '<td>' . $l_lang_field . '</td>' . '<td>' . $l_property_changes["from"] . '</td>' . '<td>' . $l_property_changes["to"] . '</td>' . '</tr>';
                    } // foreach
                ...
                {
                    $l_lang_field = _L("L" . str_replace('C__', 'C__CMDB__', $l_field));
                    ...
                    {
                        $l_lang_field = _L('LC__CMDB__CATG__DESCRIPTION');
                    }
                ...
                // @see ID-2217 Removed the "isys_glob_unescape" functions
                $l_changes .= '<tr class="' . $l_even_odd . '">' . '<td>' . $l_lang_field . '</td>' . '<td>' . $l_change["from"] . '</td>' . '<td>' . $l_change["to"] . '</td>' . '</tr>';
            } // foreach

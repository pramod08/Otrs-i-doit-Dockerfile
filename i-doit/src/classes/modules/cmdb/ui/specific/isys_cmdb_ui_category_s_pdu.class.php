    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        try
        {
            if (isys_tenantsettings::get('snmp.pdu.queries', false))
            {
                ...
                $l_rules["C__CMDB__CATS__PDU__ACC_ENERGY_PDU"]["p_strValue"] = $l_branch_dao->decimal_shift(
                    $l_snmp->cleanup($l_snmp->{$l_branch_dao->format($l_branch_dao->get_snmp_path("lgpPduPsEntryEnergyAccum"), $l_pdu, 1, 0)})
                );
                $l_rules["C__CMDB__CATS__PDU__ACC_POWER_PDU"]["p_strValue"]  = $l_snmp->cleanup(
                    $l_snmp->{$l_branch_dao->format($l_branch_dao->get_snmp_path("lgpPduPsEntryPwrTotal"), $l_pdu, 1, 0)}
                );
            }
        }
        catch (Exception $e)
        ...
        // Assign rules.
        $l_rules["C__CMDB__CATS__PDU__SNMP_QUERIES"]["p_arData"]        = serialize(get_smarty_arr_YES_NO());

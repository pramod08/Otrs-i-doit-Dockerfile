...
class isys_cmdb_ui_category_s_chassis extends isys_cmdb_ui_category_specific
{
     ...
     * Process method.
     *
     * @param   isys_cmdb_dao_category_s_chassis $p_cat
     *
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        if (isset($l_catdata['isys_cats_chassis_list__isys_catg_netp_list__id']))
        {
            ...
            $l_type           = 'C__CMDB__SUBCAT__NETWORK_INTERFACE_P';
        ...
        if ($l_value > 0)
        {
            $this->get_template_component()
                ->assign(
                    'view_field_content',
                    _L('LC__CMDB__CATS__CHASSIS__EXTERNAL_OBJECT') . ': ' . $l_quickinfo->get_quick_info(
                        $l_value,
                        $p_cat->get_obj_name_by_id_as_string($l_value),
                        C__LINK__OBJECT
                    )
                );
        }
        ...
        {
            $this->get_template_component()
                ->assign('view_field_content', _L('LC__CMDB__CATG__VD__LOCAL_DEVICE') . ': ' . $l_category_info['title']);
        }
        else
        {
            $this->get_template_component()
                ->assign('view_field_content', '- ' . _L('LC__CMDB__CATS__CHASSIS__NOTHING_ASSIGNED') . ' -');
        } // if

        $l_rules['C__CMDB__CATS__CHASSIS__LOCAL_ASSIGNMENT']['p_arData']                                              = serialize(
            $p_cat->get_local_devices_as_array($l_obj_id)
        );
        ...
        $l_rules['C__CMDB__CATS__CHASSIS__SLOT_ASSIGNMENT']['p_arData']                                               = serialize(
            $p_cat->callback_property_assigned_slots($l_request)
        );
     ...
     * @author  Van Quyen Hoang <qhoang@i-doit.org>
     */
    public function process_list(isys_cmdb_dao_category_s_chassis &$p_cat, $p_get_param_override = null, $p_strVarName = null, $p_strTemplateName = null, $p_bCheckbox = true, $p_bOrderLink = true, $p_db_field_name = null)
    {
        $this->list_view(
            null,
            $_GET[C__CMDB__GET__OBJECT],
            new isys_cmdb_dao_list_cats_chassis($p_cat),

...
class isys_ajax_handler_monitoring_livestatus extends isys_ajax_handler
{
        ...
        try
        {
            switch ($_GET['func'])
            {
                ...
                case 'load_livestatus_state':
                    $l_return['data'] = current($this->load_livestatus_states([$_POST[C__CMDB__GET__OBJECT]], ($_POST['force'] == 1 ? true : false)));
                    break;

                case 'load_livestatus_states':
                    $l_return['data'] = $this->load_livestatus_states(isys_format_json::decode($_POST['obj_ids']) ?: [], ($_POST['force'] == 1 ? true : false));
                    break;
            } // switch
        }
        catch (Exception $e)
    ...
    protected function load_livestatus($p_obj_id, $p_columns = null)
    {
            ...
            foreach ($p_obj_id as $l_obj_id)
            {
                try
                {
                    $l_return[] = [
                        ...
                        'message' => null,
                        'obj_id'  => $l_obj_id,
                        'data'    => $this->query_livestatus(
                            isys_cmdb_dao_category_g_monitoring::instance($this->m_database_component)
        ...
        {
            $l_return = $this->query_livestatus(
                isys_cmdb_dao_category_g_monitoring::instance($this->m_database_component)
     ...
     * @author  Leonard Fischer <lfischer@i-doit.org>
     */
    protected function load_livestatus_states(array $p_obj_ids, $p_force = false)
    {
        if (!$p_force)
        {
        ...
        if (count($p_obj_ids) > 0)
        {
            foreach ($p_obj_ids as $l_obj_id)
            {
                ...
                if (is_array($l_host) && !empty($l_hostname))
                {
                    ...
                    if (empty($l_host['isys_catg_monitoring_list__isys_monitoring_hosts__id']) || $l_host['isys_monitoring_hosts__type'] != C__MONITORING__TYPE_LIVESTATUS || $l_host['isys_monitoring_hosts__active'] != 1)
                    {
                        continue;
                    } // if

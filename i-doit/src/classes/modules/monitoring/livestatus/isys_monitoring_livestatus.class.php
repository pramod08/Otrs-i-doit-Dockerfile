...
class isys_monitoring_livestatus
{
    ...
    public static function factory($p_host)
    {
        if (empty($p_host))
        {
            throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__NO_CONFIG'), 0, false);
        } // if
    ...
    public function connect()
    {
        try
        {
            ...
            {
                throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__PHP_EXTENSION_MISSING'), 0);
            } // if
            ...
            if ($this->m_connection == false)
            {
                ...
                throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__COULD_NOT_CREATE_SOCKET'), 0);
            } // if
            ...
            if ($l_result == false)
            {
                throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__COULD_NOT_CONNECT_LIVESTATUS'), 0, false);
            } // if
    ...
    public function query(array $p_query, $p_force = false)
    {
        ...
        if ($l_read === false)
        {
            throw new isys_exception_general(
                _L('LC__MONITORING__LIVESTATUS_EXCEPTION__COULD_NOT_READ_FROM_SOCKET', socket_strerror(socket_last_error($this->m_connection))), false
            );
        } // if
        ...
        if ($l_status != "200")
        {
            throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__COULD_NOT_READ_FROM_SOCKET', $l_read));
        } // if
        ...
        if ($l_return === null)
        {
            throw new isys_exception_general(_L('LC__MONITORING__LIVESTATUS_EXCEPTION__INVALID_FORMAT'), 0, false);
        } // if

...
class isys_module_monitoring extends isys_module implements isys_module_interface
{
    ...
    public static function process_content_top($p_catdata)
    {
        ...
        if (defined('C__MONITORING__TYPE_LIVESTATUS'))
        {
            if (count(
    ...
    public function build_tree(isys_component_tree $p_tree, $p_system_module = true, $p_parent = null)
    {
        ...
        {
            $p_tree->add_node(
                C__MODULE__MONITORING . 0,
                $p_parent,
                _L('LC__MONITORING__LIVESTATUS_NDO__CONFIGURATION'),
                ...
                '',
                $g_dirs['images'] . 'icons/silk/cog_edit.png',
                (int) ($_GET[C__GET__TREE_NODE] == C__MODULE__MONITORING . 0)
            );

            $p_tree->add_node(
                C__MODULE__MONITORING . 1,
                $p_parent,
                _L('LC__MONITORING__EXPORT__CONFIGURATION'),
                isys_helper_link::create_url(
                    [
                        C__GET__MODULE_ID     => C__MODULE__SYSTEM,
                        C__GET__MODULE_SUB_ID => C__MODULE__MONITORING,
                ...
                '',
                $g_dirs['images'] . 'icons/silk/pencil.png',
                (int) ($_GET[C__GET__TREE_NODE] == C__MODULE__MONITORING . 1)
            );
        } // if
    } // function
    ...
    public function process_livestatus_ndo_config__row_modifier(&$p_row)
    {
        ...
        $p_row['isys_monitoring_hosts__active'] = ($p_row['isys_monitoring_hosts__active'] > 0) ? '<img src="' . $g_dirs['images'] . 'icons/silk/bullet_green.png" class="vam mr5" /><span class="vam">' . _L(
                'LC__UNIVERSAL__YES'
            ) . '</span>' : '<img src="' . $g_dirs['images'] . 'icons/silk/bullet_red.png" class="vam mr5" /><span class="vam">' . _L('LC__UNIVERSAL__NO') . '</span>';
    } // function
    ...
    protected function process_livestatus_ndo_config__edit($p_config_id = null)
    {
        ...
        $l_rules                                                         = [];
        $l_rules['C__MONITORING__CONFIG__ACTIVE']['p_strSelectedID']     = 1;
        $l_rules['C__MONITORING__CONFIG__ACTIVE']['p_arData']            = $l_yes_no;
        $l_rules['C__MONITORING__CONFIG__TYPE']['p_strSelectedID']       = C__MONITORING__TYPE_LIVESTATUS;
        $l_rules['C__MONITORING__CONFIG__TYPE']['p_arData']              = $l_monitoring_type;
        $l_rules['C__MONITORING__CONFIG__CONNECTION']['p_arData']        = $l_livestatus_connections;
        $l_rules['C__MONITORING__CONFIG__CONNECTION']['p_strSelectedID'] = C__MONITORING__LIVESTATUS_TYPE__TCP;
        ...
        if ($p_config_id !== null && $p_config_id > 0)
        {
            ...
            $l_rules['C__MONITORING__CONFIG__ACTIVE']['p_strSelectedID']     = $l_host['isys_monitoring_hosts__active'] ?: 1;
            ...
            $l_show = [
                $l_rules['C__MONITORING__CONFIG__TYPE']['p_strSelectedID']
            ];

            // Only hide the tcp/unix fields when the type is "livestatus".
            if ($l_rules['C__MONITORING__CONFIG__TYPE']['p_strSelectedID'] == C__MONITORING__TYPE_LIVESTATUS)
            {
                $l_show[] = $l_rules['C__MONITORING__CONFIG__CONNECTION']['p_strSelectedID'];
            } // if
        } // if

        $this->m_tpl->activate_editmode()
            ->assign('show_options', isys_format_json::encode($l_show))
        ...
        $index_includes['contentbottomcontent'] = self::get_tpl_dir() . 'livestatus_ndo_config_form.tpl';
    } // function
    ...
    protected function process_livestatus_ndo_config__list()
    {
        ...
        $this->m_tpl->assign('content_title', _L('LC__MONITORING'));
    ...
    protected function process_export_config__save($p_config_id = null, array $p_configuration = [])
    {
        ...
        {
            case 'check_mk':
                $l_options['site']         = isys_check_mk_helper_tag::prepare_valid_tag_name($p_configuration['C__MONITORING__CHECK_MK__SITE']);
                $l_options['multisite']    = !!$p_configuration['C__MONITORING__CHECK_MK__MULTISITE'];
                $l_options['lock_hosts']   = !!$p_configuration['C__MONITORING__CHECK_MK__LOCK_HOSTS'];
                $l_options['lock_folders'] = !!$p_configuration['C__MONITORING__CHECK_MK__LOCK_FOLDERS'];
    ...
    protected function process_export_config__edit($p_config_id = null)
    {
        ...
        $l_rules = [
            ...
            'C__MONITORING__CHECK_MK__MULTISITE'    => [
                ...
                'p_strSelectedID' => 0,
                'description'     => '(' . _L('LC__MONITORING__CHECK_MK__MULTISITE_INFO') . ')'
            ],

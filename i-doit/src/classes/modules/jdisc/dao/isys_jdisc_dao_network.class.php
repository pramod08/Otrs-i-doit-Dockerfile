...
class isys_jdisc_dao_network extends isys_jdisc_dao_data
{
    ...
    public function get_interface_types($p_type)
    {
        $l_result = [];
        $l_res    = $this->fetch(
            "SELECT id FROM interfacetypelookup WHERE name LIKE '%" . $p_type . "%' OR name LIKE '%" . ucfirst($p_type) . "%' OR name LIKE '%" . strtoupper($p_type) . "%'"
        );
    ...
    public function get_layer3_by_device($p_id, $p_raw = false, $p_all_layer3 = false, &$p_connections = [])
    {
        ...
        while ($l_row = $this->m_pdo->fetch_row_assoc($l_res))
        {
            ...
            if (!empty($l_row['ip4networkid']))
            {
                ...
                $l_layer3 = $this->does_layer3_exist_in_idoit(
                    C__CATS_NET_TYPE__IPV4,
                    $l_networkaddress,
                    $l_subnetmask,
                    $l_firstaddress_idoit,
                    $l_lastaddress_idoit
                );
                ...
                if (!$l_layer3)
                {
                    ...
                    $l_layer3_2 = $this->does_layer3_exist_in_idoit(
                        C__CATS_NET_TYPE__IPV4,
                        $l_networkaddress,
                        $l_subnetmask,
                        $l_firstaddress,
            ...
            else
            {
                ...
                if (!empty($l_row['ip4networkid']) && !empty($l_tmp) && !isset($l_already_imported[$l_tmp]) && $p_all_layer3 && !$l_layer3)
                {
                    ...
                    $l_new_obj = $l_dao->insert_new_obj(
                        ...
                        null,
                        null,
                        'By JDisc import: ip4network ID #' . $l_row['id']
                    );
                    ...
                    $l_net_dao->save(
                        ...
                        $l_lastaddress,
                        'By JDisc import: ip4network ID ' . $l_row['id'],
                        Ip::calc_cidr_suffix($l_subnetmask),
                        null,
                ...
                if (!$l_layer3)
                {
                    $l_layer3 = $l_net_dao->get_data(null, C__OBJ__NET_GLOBAL_IPV4)
                        ->get_row();
                } // if
        ...
        while ($l_row = $this->m_pdo->fetch_row_assoc($l_res))
        {
            ...
            {
                // The layer3-net does not exist - So we create it! Changed the IF-clause #4221
                if (!empty($l_row['ip6networkid']) && !empty($l_tmp) && isset($l_already_imported[$l_tmp]) && $p_all_layer3 && !$l_layer3)
                {
                    // We reset the "not-found" cache, because we create a new object.
                    unset($this->m_missing_net_cache[$l_tmp]);
                    $l_already_imported[$l_tmp] = true;
                    ...
                    $l_new_obj = $l_dao->insert_new_obj(
                        ...
                        null,
                        null,
                        'By JDisc import: ip6network ID #' . $l_row['id']
                    );
                    ...
                    $l_net_dao->save(
                        ...
                        $l_lastaddress,
                        'By JDisc import: ip6network ID ' . $l_row['id'],
                        Ip::calc_cidr_suffix_ipv6($l_row['subnetmask']),
                        null,
                        ((!is_null($l_dns_domain_id)) ? [$l_dns_domain_id] : null)
                    );
                ...
                if (!$l_layer3)
                {
                    $l_layer3 = $l_net_dao->get_data(null, C__OBJ__NET_GLOBAL_IPV6)
                        ->get_row();
                } // if
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CATG__IP_ADDRESS'),
                'const'             => 'C__CATG__IP',
        ...
        if (!empty($p_data) && !empty($p_idoit_data) && isset($p_data['address']))
        {
            if ($p_data['address'] > 0)
            {
                ...
                $p_connections[$p_idoit_data['isys_obj__id']]['properties'] = $l_net = [
                    ...
                    'title_lang' => _L('LC__CMDB__OBJTYPE__LAYER3_NET'),
                ...
                $l_return = [
                    ...
                    'properties' => [
                        ...
                        'ipv4_assignment' => [
                            'tag'        => 'ipv4_assignment',
                            'value'      => _L($this->m_ip_assignments[C__CATS_NET_TYPE__IPV4]['isys_ip_assignment__title']),
        ...
        if (!empty($p_data) && !empty($p_idoit_data))
        {
            ...
            $l_return = [
                ...
                'properties' => [
                    ...
                    'ipv6_assignment' => [
                        'tag'        => 'ipv6_assignment',
                        'value'      => _L($this->m_ip_assignments[C__CATS_NET_TYPE__IPV6]['isys_ipv6_assignment__title']),
                    ...
                    'ipv6_scope'      => [
                        ...
                        'id'         => C__CMDB__CATG__IP__GLOBAL_UNICAST,
    ...
    public function get_logical_ports_by_device($p_id, $p_raw = false)
    {
        ...
        {
            $l_local_ports = $this->create_port_map($this->get_current_object_id(), C__CMDB__SUBCAT__NETWORK_INTERFACE_L);
        } // if

        // At first we prepare the SQL to receive the ports.
        // Version 2.9
        $l_sql = 'SELECT mac.*, dl.id AS type_id, dl.singular AS type_custom, ' . 'os.id AS os_id, os.osversion AS os_custom, ift.name AS port_type ' . 'FROM mac ' .
            'JOIN device AS d ON d.id = mac.deviceid ' . 'JOIN devicetypelookup AS dl ON dl.id = d.type ' . 'LEFT JOIN operatingsystem AS os ON d.operatingsystemid = os.id ' .
            'LEFT JOIN interfacetypelookup AS ift ON ift.id = mac.iftype ' . 'WHERE (mac.ifdescr IS NOT NULL OR mac.ifphysaddress IS NOT NULL) AND deviceid = ' .
            $this->convert_sql_id(
                $p_id
            ) . ' AND mac.portid IS NULL AND mac.iftype IN (' . implode(
                ',',
                array_merge(
                    $this->m_interface_types['virtual']['content'],
                    $this->m_interface_types['vlan']['content'],
                    $this->m_interface_types['loopback']['content'],
...
//				}
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CMDB__CATG__NETWORK_TREE_CONFIG_PORT_L'),
                'const'             => 'C__CMDB__SUBCAT__NETWORK_INTERFACE_L',
                'category_type'     => C__CMDB__CATEGORY__TYPE_GLOBAL,
                'category_entities' => $l_return
            ];
        } // if
    ...
    public function get_ports_by_device($p_id, $p_raw = false)
    {
        ...
        if (isys_jdisc_dao_data::clear_data() === false)
        {
            $l_local_ports = $this->create_port_map($this->get_current_object_id(), C__CMDB__SUBCAT__NETWORK_PORT);
        } // if
        ...
        $l_sql = 'SELECT m.*, dl.id AS type_id, ' . 'dl.singular AS type_custom, os.id AS os_id, os.osversion AS os_custom, ' . 'mo.id AS if_id, ift.name AS port_type ' .
            ...
            $this->convert_sql_id(
                ...
                array_merge(
                    ...
                    $this->m_interface_types['loopback']['content'],
                    $this->m_interface_types['tunnel']['content'],
                    $this->m_interface_types['fibreChannel']['content']
                )
            ) . ')' . ' OR m.iftype IS NULL)';
        ...
        if ($l_num_rows > 0)
        {
            while ($l_row = $this->m_pdo->fetch_row_assoc($l_res))
            {
                ...
                {
                    $l_duplex_id = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__FULL')][0];
                }
                else
                {
                    $l_duplex_id = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__HALF')][0];
                }
                ...
                {
                    $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__KBIT_S'];
                    if ($l_row['ifspeed'] >= 1000000)
                    {
                        $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__MBIT_S'];
                        if ($l_row['ifspeed'] >= 1000000000)
                        {
                            $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__GBIT_S'];
                        } // if
                    } // if
                }
                else
                {
                    $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__BIT_S'];
                } // if
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CATD__PORT'),
                'const'             => 'C__CMDB__SUBCAT__NETWORK_PORT',
                'category_type'     => C__CMDB__CATEGORY__TYPE_GLOBAL,
                'category_entities' => $l_return
            ];
        } // if
    ...
    public function get_interfaces_by_device($p_id, $p_raw = false)
    {
        ...
        if ($this->m_import_type_interfaces === 1) return $l_return;
        ...
        $l_sql = 'SELECT mo.*, ms.socketdesignation ' . 'FROM mac AS m ' . 'INNER JOIN module AS mo ON mo.id = m.moduleid ' .
            'LEFT JOIN moduleslot AS ms ON ms.itemid = mo.id ' . 'LEFT JOIN device AS d ON d.id = m.deviceid ' . 'WHERE m.deviceid = ' . $this->convert_sql_id(
                $p_id
            ) . ' AND m.iftype NOT IN (' . implode(
                ',',
                array_merge(
        ...
        if (($l_if_amount = $this->m_pdo->num_rows($l_res)) > 0)
        {
            ...
            while ($l_row = $this->m_pdo->fetch_row_assoc($l_res))
            {
                if (!isset($this->m_network_interfaces[$l_row['id']]))
                {
                    ...
                    $l_title             = ($l_row['description'] !== '' && !is_numeric(
                            $l_row['description']
                        )) ? $l_row['description'] : ($l_row['model'] . (($l_row['serialnumber']) ? ' - ' . $l_row['serialnumber'] : ''));
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CMDB__CATG__NETWORK_TREE_CONFIG_INTERFACE_P'),
                'const'             => 'C__CMDB__SUBCAT__NETWORK_INTERFACE_P',
                'category_type'     => C__CMDB__CATEGORY__TYPE_GLOBAL,
                'category_entities' => $l_return
            ];
        } // if
    ...
    public function get_fc_ports_by_device($p_id, $p_raw = false)
    {
        ...
        $l_sql = 'SELECT m.*, dl.id AS type_id, ' . 'dl.singular AS type_custom, os.id AS os_id, os.osversion AS os_custom, ' . 'ift.name AS port_type ' . 'FROM mac AS m ' .
            ...
            'WHERE (m.ifdescr IS NOT NULL OR m.ifphysaddress IS NOT NULL) AND m.deviceid = ' . $this->convert_sql_id(
                $p_id
            ) . ' ' . 'AND m.portid IS NULL ' . 'AND m.iftype IN (' . implode(',', $this->m_interface_types['fibreChannel']['content']) . ')';
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__STORAGE_FCPORT'),
            ...
            if (isset($p_data['ifspeed']))
            {
                if ($p_data['ifspeed'] > 1000)
                {
                    if ($p_data['ifspeed'] >= 1000000)
                    {
                        ...
                        {
                            $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__GBIT_S'];
                            $l_speed_unit  = _L('LC__CMDB__PORT_SPEED__GBITS');
                            $l_speed_const = 'C__PORT_SPEED__GBIT_S';
                        }
                        else
                        {
                            $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__MBIT_S'];
                            $l_speed_unit  = _L('LC__CMDB__PORT_SPEED__MBITS');
                    ...
                    {
                        $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__KBIT_S'];
                        $l_speed_unit  = _L('LC__CMDB__PORT_SPEED__KBITS');
                ...
                {
                    $l_speed_type = $this->m_dialog_data['port_speed']['C__PORT_SPEED__BIT_S'];
                    $l_speed_unit  = _L('LC__CMDB__PORT_SPEED__BITS');
    ...
    public function prepare_network_interface(array $p_data, $p_new_id)
    {
        ...
        if (!empty($p_data))
        {
            ...
            $l_title                = ($p_data['description'] !== '' && !is_numeric(
                    $p_data['description']
                )) ? $p_data['description'] : ($p_data['model'] . (($p_data['serialnumber']) ? ' - ' . $p_data['serialnumber'] : ''));
            ...
            if (isset($l_dialog_data['manufacturer']))
            {
                $l_return['properties']['manufacturer'] = [
                    'tag'        => 'manufacturer',
                    'value'      => _L($l_dialog_data['manufacturer']['isys_iface_manufacturer__title']),
                    'id'         => $l_dialog_data['manufacturer']['isys_iface_manufacturer__id'],
                    'const'      => $l_dialog_data['manufacturer']['isys_iface_manufacturer__const'],
                    'title_lang' => _L($l_dialog_data['manufacturer']['isys_iface_manufacturer__title']),
            ...
            if (isset($l_dialog_data['model']))
            {
                $l_return['properties']['model'] = [
                    'tag'        => 'model',
                    'value'      => _L($l_dialog_data['model']['isys_iface_model__title']),
                    'id'         => $l_dialog_data['model']['isys_iface_model__id'],
                    'const'      => $l_dialog_data['model']['isys_iface_model__const'],
                    'title_lang' => _L($l_dialog_data['model']['isys_iface_model__title']),
            ...
            $l_return = [
                ...
                'properties' => [
                    ...
                    'port_type' => [
                        ...
                        'title_lang' => _L($this->m_dialog_data['port_type_logical'][$p_data['port_type']][2]),
                        'title'      => 'LC__CMDB__CATG__NETWORK__TYPE'
                    ],
                    //					'net' => array(
                    //						'tag' => 'net',
        ...
        if (!empty($p_data))
        {
            ...
            $l_return = [
                ...
                'properties' => [
                    ...
                    'port_type'   => [
                        ...
                        'title_lang' => _L($this->m_dialog_data['port_type'][$p_data['port_type']][2]),
                        'title'      => 'LC__CMDB__CATG__TYPE'
                    ],
                    'port_mode'   => [
                        'tag'        => 'port_mode',
                        'value'      => _L($this->m_dialog_data['port_mode']['Standard'][2]),
                        'id'         => $this->m_dialog_data['port_mode']['Standard'][0],
                        'const'      => $this->m_dialog_data['port_mode']['Standard'][1],
                        'title_lang' => _L($this->m_dialog_data['port_mode']['Standard'][2]),
                        'title'      => 'LC__CMDB__CATG__PORT__MODE'
                    ],
                    'negotiation' => [
                        'tag'        => 'negotiation',
                        'value'      => _L($this->m_dialog_data['port_negotiation'][_L('LC__PORT_NEGOTIATION__AUTO')][2]),
                        'id'         => $this->m_dialog_data['port_negotiation'][_L('LC__PORT_NEGOTIATION__AUTO')][0],
                        'const'      => $this->m_dialog_data['port_negotiation'][_L('LC__PORT_NEGOTIATION__AUTO')][1],
                        'title_lang' => _L($this->m_dialog_data['port_negotiation'][_L('LC__PORT_NEGOTIATION__AUTO')][2]),
            ...
            if (!empty($p_data['duplexmode']))
            {
                ...
                if ($p_data['duplexmode'] === 3 || $p_data['duplexmode'] === 2)
                {
                    ...
                    {
                        $l_duplex_id    = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__FULL')][0];
                        $l_duplex_const = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__FULL')][1];
                        $l_duplex_title = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__FULL')][2];
                    }
                    else
                    {
                        $l_duplex_id    = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__HALF')][0];
                        $l_duplex_const = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__HALF')][1];
                        $l_duplex_title = $this->m_dialog_data['port_duplex'][_L('LC__PORT_DUPLEX__HALF')][2];
                    }

                    $l_return['properties']['duplex'] = [
                        'tag'        => 'duplex',
                        'value'      => _L($l_duplex_title),
                        'id'         => $l_duplex_id,
            ...
            foreach ($this->m_port_array as $l_port_id => $l_port_data)
            {
                ...
                if (isset($l_port_data['update']) && isset($l_port_data['comparestring']) && $l_port_data['update'] > 0)
                {
                    ...
                    if ($l_port_data['comparestring'] !== $l_local_port_row['comparestring'])
                    {
                        ...
                        // Update this port
                        $this->update_port(
                            $l_local_port_row['isys_catg_port_list__id'],
                            $l_data[1],
                            $l_data[2],
                            $l_data[3],
                            $l_data[4],
...
//						' AND isys_catg_port_list__title = ' . $this->convert_sql_text($l_row['port_title2']);
                    ...
                    {
                        $l_sql_local_port     = $l_sql_port . $l_sql_condition1 . $this->convert_sql_text($l_row['mac1']) . $l_sql_condition2 . $this->convert_sql_text(
                                $l_row['port_title1']
                            );
                        $l_sql_external_port  = $l_sql_port . $l_sql_condition1 . $this->convert_sql_text($l_row['mac2']) . $l_sql_condition2 . $this->convert_sql_text(
                                $l_row['port_title2']
                            );
                    ...
                    {
                        $l_sql_external_port  = $l_sql_port . $l_sql_condition1 . $this->convert_sql_text($l_row['mac1']) . $l_sql_condition2 . $this->convert_sql_text(
                                $l_row['port_title1']
                            );
                        $l_sql_local_port     = $l_sql_port . $l_sql_condition1 . $this->convert_sql_text($l_row['mac2']) . $l_sql_condition2 . $this->convert_sql_text(
                                $l_row['port_title2']
                            );
...
//					$l_external_port_row = $l_port_dao->get_data(null, null, $l_external_port_condition)->get_row();
                ...
                elseif ($this->m_pdo->num_rows($l_res) > 1)
                {
                    ...
                    $this->m_log->debug(
                        '> Connection for "' . $l_port_data['mac'] . '" skipped. Because there are several connections to following mac-addresses: ' . $l_mac_addresses
                    );
                }
                else
                {
    ...
    public function connect_ports($p_local_port, $p_external_port)
    {
        ...
        $l_relation_type = $l_dao_rel->get_relation_type_by_category('C__CMDB__SUBCAT__NETWORK_PORT');
        if ($l_relation_type)
        {
        ...
        $l_rel_id = $l_dao_rel->create_relation(
            "isys_catg_connector_list",
            $p_local_port["isys_catg_connector_list__id"],
            $p_external_port["isys_catg_port_list__isys_obj__id"],
            $p_local_port["isys_catg_port_list__isys_obj__id"],
            $l_relation_type
        );

        $l_update = "UPDATE isys_catg_connector_list SET " . "isys_catg_connector_list__isys_cable_connection__id = " . $this->convert_sql_id(
                $l_connection_id
            ) . ", " . "isys_catg_connector_list__isys_catg_relation_list__id = " . $this->convert_sql_id(
                $l_rel_id
            ) . " " . "WHERE isys_catg_connector_list__id = " . $this->convert_sql_id(
                $p_local_port['isys_catg_connector_list__id']
        ...
        {
            $this->m_log->info(
                'Okay! Port "' . $p_local_port['isys_catg_port_list__mac'] . ' was successfully connected to "' . $p_external_port['isys_catg_port_list__mac'] . '"'
            );
        }
        else
        {
            $this->m_log->error(
                'Something did not work - Port "' . $p_local_port['isys_catg_port_list__mac'] . ' could not be connected with "' .
    ...
    public function update_ip_port_assignments()
    {
        ...
        if (is_array($this->m_port_array) && count($this->m_port_array) > 0)
        {
            foreach ($this->m_port_array AS $l_port)
            {
                ...
                {
                    $l_condition = 'AND isys_obj__title = ' . $this->convert_sql_text(
                            $l_port['obj_name']
                        ) . ' ' . 'AND isys_obj__isys_obj_type__id = ' . $this->convert_sql_id(
                            $l_port['obj_type']
                        ) . ' ' . 'AND isys_catg_port_list__title = ' . $this->convert_sql_text($l_port['value']);
                } // if
        ...
        if (is_array($this->m_logical_port_array) && count($this->m_logical_port_array) > 0)
        {
            foreach ($this->m_logical_port_array AS $l_port)
            {
                ...
                if ($l_loopback_tunnel === false)
                {
                    if (!empty($l_port['mac']))
                    {
                        $l_condition = 'AND isys_catg_log_port_list.isys_catg_log_port_list__title = ' . $this->convert_sql_text(
                                $l_port['value']
                    ...
                    else
                    {
                        $l_condition = 'AND isys_catg_log_port_list.isys_catg_log_port_list__title = ' . $this->convert_sql_text(
                                $l_port['value']
                            ) . ' AND mainObject.isys_obj__title = ' . $this->convert_sql_text(
                                $l_port['obj_name']
                            ) . ' ' . ' AND mainObject.isys_obj__isys_obj_type__id = ' . $this->convert_sql_id($l_port['obj_type']);
                    } // if
                ...
                if (isset($l_port['ip']) && is_array($l_port['ip']))
                {
                    foreach ($l_port['ip'] AS $l_ip)
                    {
                        ...
                        if ($l_loopback_tunnel && empty($l_external_port_row))
                        {
                            ...
                            $l_external_port_row = $l_dao_log_port->get_data(
                                null,
                                $l_ip_data['isys_obj__id'],
    ...
    public function get_access_point_by_device($p_id, $p_raw = false)
    {
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CMDB__CATS__ACCESS_POINT'),
            ...
            foreach ($this->m_network_interfaces_connection AS $l_device_id => $l_data)
            {
                if ($p_obj_id > 0)
                {
                    foreach ($l_data AS $l_port_id => $l_interface_id)
                    {
                        ...
                        if (isset(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT][$this->m_port_array[$l_port_id]['value'] . '|' . $p_obj_id . '|' .
                            $this->m_port_array[$l_port_id]['mac']]))
                        {
                            $l_idoit_port_id = self::$m_port_map
                            [C__CMDB__SUBCAT__NETWORK_PORT][$this->m_port_array[$l_port_id]['value'] . '|' . $p_obj_id . '|' . $this->m_port_array[$l_port_id]['mac']]
                            ['id'];
                        }
                        else
                        {
                            ...
                            $l_port_data_sql = 'SELECT isys_catg_port_list__id FROM isys_catg_port_list ' .
                                'INNER JOIN isys_obj ON isys_obj__id = isys_catg_port_list__isys_obj__id ' . 'WHERE isys_obj__id = ' . $this->convert_sql_id(
                                    $p_obj_id
                        ...
                        if ($l_interface_id > 0)
                        {
                            if (isset($this->m_network_interfaces[$l_interface_id]) && is_array($this->m_network_interfaces[$l_interface_id]))
                            {
                                $l_interface_data_sql = 'SELECT isys_catg_netp_list__id FROM isys_catg_netp_list ' . 'WHERE isys_catg_netp_list__isys_obj__id = ' .
                                    $this->convert_sql_id(
                                        $p_obj_id
                                    ) . ' ' . 'AND isys_catg_netp_list__title = ' . $this->convert_sql_text($this->m_network_interfaces[$l_interface_id]['value']);
                        ...
                        {
                            $l_update = 'UPDATE isys_catg_port_list SET isys_catg_port_list__isys_catg_netp_list__id = ' . $this->convert_sql_id(
                                    $l_idoit_interface_id
    ...
    public function update_vlan_assignments($p_obj_id, $p_device_id)
    {
        if ($this->m_import_vlans)
        {
            ...
            if ($this->m_pdo->num_rows($l_res_vlan) > 0)
            {
                ...
                while ($l_row_jd_vlan = $this->m_pdo->fetch_row_assoc($l_res_vlan))
                {
                    ...
                    if ($l_mac !== '' && $l_deviceid == $p_device_id)
                    {
                         ...
                         */
                        if (isset(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]))
                        {
                            $l_port = self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT][$l_port_title . '|' . $p_obj_id . '|' . $l_mac];
                            ...
                            if (isset($l_port['id']) && $l_port['id'] > 0)
                            {
                                ...
                                if (strpos(' ' . $l_port['vlans'], (string) $l_identifier))
                                {
                                    ...
                                    self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]['vlans'] = str_replace(
                                        ',' . (string) $l_identifier . ',',
                                        ',',
                                        ',' . self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]['vlans'] . ','
                                    );
                                ...
                                else
                                {
                                    ...
                                    $l_object_id = $l_vlans[$l_identifier] = $l_dao_layer2_net->insert_new_obj(
                                        C__OBJTYPE__LAYER2_NET,
                                        false,
                                        $l_row_jd_vlan['vlan_title'],
                                        null,
                                        C__RECORD_STATUS__NORMAL
                                    );

                                    $this->m_log->debug('New layer2-net "' . $l_row_jd_vlan['vlan_title'] . '" created.');
                                    $l_dao_layer2_net->create(
                                        $l_object_id,
                                        C__RECORD_STATUS__NORMAL,
                                        $l_vlanid
                                    );
                                ...
                                if ($l_object_id > 0)
                                {
                                     ...
                                     */
                                    $this->update(
                                        'REPLACE INTO isys_cats_layer2_net_assigned_ports_list (isys_catg_port_list__id, isys_cats_layer2_net_assigned_ports_list__isys_obj__id, isys_cats_layer2_net_assigned_ports_list__status) VALUES
                        ...
                        if (isset(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]))
                        {
                            $l_port = self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L][$l_port_title . '|' . $p_obj_id . '|' . $l_mac];
                            ...
                            if (isset($l_port['id']) && $l_port['id'] > 0)
                            {
                                ...
                                if (strpos(' ' . $l_port['vlans'], (string) $l_identifier))
                                {
                                    ...
                                    self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]['vlans'] = str_replace(
                                        ',' . (string) $l_identifier . ',',
                                        ',',
                                        ',' . self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L][$l_port_title . '|' . $p_obj_id . '|' . $l_mac]['vlans'] . ','
                                    );
                                ...
                                if ($l_object_id > 0)
                                {
                                     ...
                                     */
                                    $this->update(
                                        'REPLACE INTO isys_catg_log_port_list_2_isys_obj' .
                                        '(isys_catg_log_port_list__id,  isys_obj__id,  isys_catg_log_port_list_2_isys_obj__status) ' . 'VALUES ' . '(' . $this->convert_sql_id(
                                            $l_port['id']
                                        ) . ', ' . $this->convert_sql_id($l_object_id) . ', ' . C__RECORD_STATUS__NORMAL . ');'
                                    );
                                }
             ...
             */
            if (isset(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT]) && is_array(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT]))
            {
                foreach (self::$m_port_map[C__CMDB__SUBCAT__NETWORK_PORT] as $l_port)
                {
                    ...
                    {
                        $l_remove_list = array_map(
                            function ($p_val) use ($l_vlans)
                            {
                        ...
                        {
                            $this->update(
                                'DELETE FROM isys_cats_layer2_net_assigned_ports_list ' . 'WHERE isys_catg_port_list__id = \'' . $l_port['id'] . '\' AND ' .
                                'isys_cats_layer2_net_assigned_ports_list__isys_obj__id IN (' . implode(
                                    ',',
                                    $l_remove_list
                                ) . ')'
                            );
             ...
             * Delete VLANs which are not assigned anymore
             */
            if (isset(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L]) && is_array(self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L]))
            {
                foreach (self::$m_port_map[C__CMDB__SUBCAT__NETWORK_INTERFACE_L] as $l_port)
                {
                    ...
                    if ($l_port['vlans'] && $l_port['id'] > 0 && $l_port['objid'] == $p_obj_id)
                    {
                        ...
                        {
                            $this->update(
                                'DELETE FROM isys_catg_log_port_list_2_isys_obj ' . 'WHERE isys_catg_log_port_list__id = \'' . $l_port['id'] . '\' AND ' .
                                'isys_obj__id IN (' . implode(
                                    ',',
                                    $l_remove_list
                                ) . ')'
                            );
    ...
    public function update_port($p_port_id, $p_duplex_id, $p_port_type_id, $p_speed, $p_speed_unit, $p_active)
    {
        $l_sql = 'UPDATE isys_catg_port_list SET ' . 'isys_catg_port_list__isys_port_duplex__id = ' . $this->convert_sql_id($p_duplex_id) . ', ' .
            ...
            'isys_catg_port_list__isys_port_speed__id = ' . $this->convert_sql_id($p_speed_unit). ', ' . 'isys_catg_port_list__state_enabled = ' . $this->convert_sql_int($p_active) . ' ' .
    ...
    public function get_universal_interface_by_device($p_id, $p_idoit_objects, $p_raw = false)
    {
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CMDB__CATG__UNIVERSAL_INTERFACE'),
    ...
    public function cache_data($p_obj_id)
    {
        ...
        {
            parent::cache_data($p_obj_id, $this->m_port_array, self::C__CACHE__PORT);
            unset($this->m_port_array);
        } // if
        if (count($this->m_logical_port_array))
        {
            parent::cache_data($p_obj_id, $this->m_logical_port_array, self::C__CACHE__LOGICAL_PORT);
            unset($this->m_logical_port_array);
        } // if
        if (count($this->m_network_interfaces))
        {
            parent::cache_data($p_obj_id, $this->m_network_interfaces, self::C__CACHE__INTERFACE);
            unset($this->m_network_interfaces);
        } // if
        if (count($this->m_network_interfaces_connection))
        {
            parent::cache_data($p_obj_id, $this->m_network_interfaces_connection, self::C__CACHE__INTERFACE_CONNECTIONS);
            unset($this->m_network_interfaces_connection);
        } // if
        if (count($this->m_fc_port_array))
        {
            parent::cache_data($p_obj_id, $this->m_fc_port_array, self::C__CACHE__INTERFACE_CONNECTIONS);
            unset($this->m_fc_port_array);
        } // if
    } // function

    /**
     * Get hostname, fqdn and ip address by device if exist
     *
     * @param $p_device_id
     *
     * @return string|bool
    ...
    private function get_port_filter_query($p_type)
    {
                        ...
                        if (strpos($l_filter_key, '*') !== false)
                        {
                            ...
                            elseif ($l_left_side)
                            {
                                if (strpos(
                                        $this->m_type_title,
                                        $l_filter_key2
                                    ) === (strlen($this->m_type_title) - strlen($l_filter_key2)) - 1
                                )
                                {
                                    $l_type   = $l_filter_key;
                                    $l_os_arr = array_keys($this->m_port_filter[$l_type]);
                            ...
                            if (strpos($l_os_key, '*') !== false)
                            {
                                ...
                                elseif ($l_left_side)
                                {
                                    if (strpos(
                                            $this->m_osversion,
                                            $l_os_key2
                                        ) === (strlen($this->m_osversion) - strlen($l_os_key2)) - 1
                                    )
                                    {
                                        $l_filter      = isys_format_json::decode($this->m_port_filter[$l_type][$l_os_key]);
                                        $l_filter_type = isys_format_json::decode($this->m_port_filter_import_type[$l_type][$l_os_key]);
        ...
        if ($l_filter !== null || $l_filter_type !== null)
        {
            ...
            $l_sql_condition .= ' AND (';
            $l_filter_type_exclude = false;
            $l_filter_found        = false;
            if (count($l_filter_type))
            {
                foreach ($l_filter_type AS $l_filter_key => $l_filter_type_id)
                {
                    $l_filter_type_id = (int) $l_filter_type_id;
                    if ($l_filter_type_id === 0)
                    {
                        return '';
                    }
                    if ($l_filter_type_id === 3 && $l_filter[$l_filter_key] != '')
                    {
                        $l_filter_type_exclude = true;
                        $l_sql_condition .= ' ifdescr NOT ILIKE ' . $this->m_dao->convert_sql_text(
                                str_replace(
                                    '*',
                                    '%',
                                    $l_filter[$l_filter_key]
                                )
                            ) . ' AND (';
                    } // if
                    else
                    {
						...
						{
							// Logischer Port
							$l_filter_type_condition = 'IS NULL';
						}
						elseif(2 == $p_type)
						{
							// Physicalischer Port
							$l_filter_type_condition = 'IS NOT NULL';
						}
						else
						{
							$l_filter_type_condition = '';
						} // if
	*/
                        if ($l_filter_type_id === $p_type)
                        {
                            $l_filter_found = true;
                            if ($l_filter[$l_filter_key] !== '')
                            {
                                $l_title_check = 'ifdescr ILIKE ' . $this->m_dao->convert_sql_text(
                                        str_replace(
                                            '*',
                                            '%',
                                            $l_filter[$l_filter_key]
                            ...
                            //$l_sql_condition .= ' ('.$l_title_check.' ifphysaddress '.$l_filter_type_condition.') OR ';
                            $l_sql_condition .= ' (' . $l_title_check . ' ) OR ';
                        }
                    } // if
                } // foreach
            } // if
            $l_sql_condition = rtrim($l_sql_condition, ' AND ');
            $l_sql_condition = rtrim($l_sql_condition, ' OR ');
            $l_sql_condition .= ' ) ';
            if ($l_filter_type_exclude)
            {
                $l_sql_condition .= ' ) ';
            }
            if (strpos($l_sql_condition, 'AND ( )') > 0 || strpos($l_sql_condition, 'AND ( ( ) )') > 0)
            {
                $l_sql_condition = str_replace(
                    [
                        'AND ( )',
                        'AND ( ( ) )'
                    ],
    ...
    private function prepare_ports_for_cache_by_type($p_type, $p_object_id, $p_with_vlans = true)
    {
        ...
        switch ($p_type)
        {
            case C__CMDB__SUBCAT__NETWORK_PORT:
                ...
                break;
            case C__CMDB__SUBCAT__NETWORK_INTERFACE_L:
            ...
            default:
                ...
                $l_sql = 'SELECT isys_catg_connector_list__id, isys_catg_connector_list__isys_obj__id AS objid, isys_catg_connector_list__title AS title, \'\' AS vlans FROM isys_catg_connector_list ' .
                    'WHERE isys_catg_connector_list__assigned_category NOT IN (\'C__CMDB__SUBCAT__NETWORK_PORT\', \'C__CATG__POWER_CONSUMER\', \'C__CATG__UNIVERSAL_INTERFACE\')';
    ...
    public function __construct(isys_component_database $p_db, isys_component_database_pdo $p_pdo)
    {
        ...
        $this->m_ip_assignments = [
            C__CATS_NET_TYPE__IPV4 => isys_factory_cmdb_dialog_dao::get_instance($p_db, 'isys_ip_assignment')
                ->get_data(C__CATP__IP__ASSIGN__STATIC),
            C__CATS_NET_TYPE__IPV6 => isys_factory_cmdb_dialog_dao::get_instance($p_db, 'isys_ipv6_assignment')
        ...
        if (is_array($l_dialog_data))
        {
            ...
            {
                $this->m_dialog_data['port_type'][_L($l_data['isys_port_type__title'])] = [
        ...
        if (is_array($l_dialog_data))
        {
            foreach ($l_dialog_data AS $l_data)
            {
                $this->m_dialog_data['port_mode'][_L($l_data['isys_port_mode__title'])] = [
        ...
        if (is_array($l_dialog_data))
        {
            foreach ($l_dialog_data AS $l_data)
            {
                $this->m_dialog_data['port_negotiation'][_L($l_data['isys_port_negotiation__title'])] = [
        ...
        if (is_array($l_dialog_data))
        {
            foreach ($l_dialog_data AS $l_data)
            {
                $this->m_dialog_data['port_duplex'][_L($l_data['isys_port_duplex__title'])] = [
        ...
        $l_dialog_data = isys_factory_cmdb_dialog_dao::get_instance($this->m_db, 'isys_port_speed')->get_data();

...
class isys_jdisc_dao_cluster extends isys_jdisc_dao_data
{
    ...
    public function get_cluster_by_device($p_id, $p_raw = false, $p_all_clusters = false)
    {
        ...
        $l_sql = 'SELECT c.*, cs.name AS clusterservice, cs.id AS clusterserviceid FROM cluster AS c
        ...
        if ($this->m_pdo->num_rows($l_res) > 0)
        {
            while ($l_row = $this->m_pdo->fetch_row_assoc($l_res))
            {
                ...
                /*
                 * We skip the iteration if we already imported this cluster to this device,
                 * or if we don't want to import cluster which does not already exist in i-doit.
                 */
                if (in_array($l_row['name'], $l_already_imported) || (!$p_all_clusters && !$l_cluster))
                {
                    //$l_continue = false;
                    continue;
                } // if
                //if($l_continue)
                {
                    ...
                    else
                    {
                        ...
                        if (!$l_cluster)
                        {
                            ...
                            {
                                $l_objtype = C__OBJTYPE__CLUSTER_VRRP_HSRP;
                            }
                            else
                            {
                                $l_objtype = C__OBJTYPE__CLUSTER;
                            } // if
                        ...
                        {
                            if (in_array($l_row['type'], $this->m_virtual_cluster_types))
                            {
                            ...
                            if (!empty($l_row['clusterservice']))
                            {
                                ...
                                parent::set_jdisc_to_idoit_objects(
                                    'cluster_service_' . $l_row['clusterserviceid'],
                                    $l_clusterservice['isys_obj__id']
                                );
        ...
        {
            return [
                C__DATA__TITLE      => _L('LC__CMDB__CATG__CLUSTER_MEMBERSHIPS'),
            ...
            return [
                ...
                'properties' => [
                    'connected_object' => [
                        ...
                        'lc_title' => _L($p_data['isys_obj_type__title']),
                    ...
                    foreach ($l_data_cluster AS $l_cluster_service => $l_runs_on_objects)
                    {
                                        ...
                                        if (is_array($l_data_cluster_service) && in_array($l_cluster_service_key, $l_data_cluster_service))
                                        {
                                            ...
                                            if (!isset($l_cache_cluster_service_ips[$l_cluster_service_key]))
                                            {
                                                ...
                                                while ($l_row = $l_res->get_row())
                                                {
                                                    ...
                                                    {
                                                        $l_cache_cluster_service_ips[$l_cluster_service_key][$l_row['isys_catg_ip_list__id']] = Ip::validate_ipv6(
                                            ...
                                            if ((is_array($l_cache_cluster_service_ips[$l_cluster_service_key]) && !in_array(
                                                    $l_ip_long,
                                                    $l_cache_cluster_service_ips[$l_cluster_service_key]
                                                ))
                                            )
                                            {
                        ...
                        if (!isset($l_assigned_cluster_services[$l_cluster_service]))
                        {
                            $l_dao_clusterservice->create(
                                $l_cluster_object,
                                $l_cluster_service,
                                C__CLUSTER_TYPE__ACTIVE_ACTIVE,
                                $l_runs_on_objects,
                                null,
                                '',
                                C__RECORD_STATUS__NORMAL,
                        ...
                        if (in_array($l_cluster_object, $this->m_virtual_clusters))
                        {
                            if (is_array($l_runs_on_objects2) && count($l_runs_on_objects2) > 0)
                            {
                                foreach ($l_runs_on_objects2 AS $l_cluster_member_object_id)
                                {
                                    $l_guest_res = $l_dao_guest->get_data(null, $l_cluster_member_object_id);
                                    while ($l_guest_row = $l_guest_res->get_row())
                                    {
                                        $l_dao_guest->save(
                                            $l_guest_row['isys_catg_virtual_machine_list__id'],
                                            C__RECORD_STATUS__NORMAL,
                                            $l_cluster_object,
                                            '',
                                            $l_cluster_member_object_id
                                        );
                                    } // while
                                } // foreach
                            }
     ...
     */
    public function clear_cluster_memberships($p_obj_id)
    {
        ...
        foreach ($p_objects AS $l_key => $l_obj_id)
        {
            if (strpos($l_key, 'cluster_') !== false && strpos($l_key, 'cluster_service') === false)
            {
                ...
                if (is_numeric($l_cluster_id) && $l_cluster_id > 0)
                {
                    ...
                    while ($l_row = $l_cluster_members_res->get_row())
                    {
                        $l_members[strtolower($l_row['isys_obj__title'])] = $l_row['isys_obj__id'];
                    } // while

                    // Get all members of the cluster from jdisc
                    $l_sql = 'SELECT LOWER(d.name) AS name FROM cluster AS c
					LEFT JOIN clusterdevicerelation AS cdr ON cdr.clusterid = c.id
					LEFT JOIN device AS d ON d.id = cdr.deviceid
					LEFT JOIN clusterservice AS cs ON d.id = cs.deviceid
                    ...
                    if (count($l_members) > 0)
                    {
                        ...
                        {
                            $this->clear_cluster_memberships($l_member_id);
                        } // foreach
                    } // if
                } // if

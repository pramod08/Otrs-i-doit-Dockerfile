    ...
    public function execute()
    {
        if (!empty($this->query))
        {
            try
            {
                ...
                {
                    if (strpos($this->query, 'LIMIT'))
                    {
                        $this->query = preg_replace('/LIMIT [0-9,]+/', '', $this->query);
                    }
                    ...
                    while (1)
                    {
                        $portions = \isys_tenantsettings::get('report.file-export.portions', 325000);
                        ...
                        if ($l_num > 0)
                        {
                            ...
                            while ($l_row = $db->fetch_row_assoc($l_result))
                            {
                                ...
                                if ($i++ % 1000 == 0) $memory->outOfMemoryBreak();
                                ...
                                if ($l_first_row)
                                {
                                    ...
                                    foreach ($l_row AS $l_key => $l_row_value)
                                    {
                                        ...
                                        // This closes #5069
                                        if (isset($translatedKey[$l_origin_key]))
                                        {
                                            $i = 2;
                                ...
                                if ($this->worker) $this->worker->work($l_fixed_row);

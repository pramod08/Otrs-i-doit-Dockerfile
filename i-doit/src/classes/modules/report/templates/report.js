...
Lists.ReportList = Class.create(Lists.Objects, {
	...
	createFilter:function () {
		...
		$(this.table.id + '-options')
			...
			.insert({bottom:new Element('input', {'id':this.table.id + '-filter-data', className:'input input-mini'})});
...
window.build_table = function(name, data, ajax_pager, ajax_pager_url, ajax_pager_preload, max_pages) {
	...
	window.objecttype_list = new Lists.ReportList(name, {
		...
		ajax_pager_preload: ajax_pager_preload, // $this->m_dao_list->get_preload_pages()
		data: data,
		filter: "top",
		paginate: "top",

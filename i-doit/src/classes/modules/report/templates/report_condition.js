            ...
            new Ajax.Request('?ajax=1&call=report&func=add_contraint_to_property',
                {
                    ...
                    onSuccess: function(transport) {
                        ...
                        equation.each(function(i) {
                            ...
                            if (i == 'under_location') {
                                i = '[{isys type="lang" ident="LC__REPORT__FORM__CONDITIONS__BELOW"}]';
                                comparison_field.insert(new Element('option', {value:val}).update(i));
                            }
                            else if (i == 'subcnd') {
                                i = '[{isys type="lang" ident="LC__REPORT__FORM__CONDITIONS__ASSIGNED_ATTRIBUTE"}]';
                                comparison_field.insert(new Element('option', {value:val}).update(i));
                            } else {
                                comparison_field.insert(new Element('option', {value:val}).update(i));
                            }
                        ...
                        {
                            Element.remove(comparison_field.next());
                            ...
                            if (field != null) {
                                ...
                                if (Object.toJSON(field) == '[]') {
                                    ...
                                    value_field = new Element('select', {'id': valueId, 'name': valueName, 'className': 'reportDialog '+cndValueClass});
                                    ...
                                    span.down('select.'+cndComparisionClass).insert({
                                        after: value_field
                                    });
                                }
                            }
                            ...
                            {
                                span.down('input.'+cndValueClass).value = p_data.value;
                            }
                        ...
                        comparison_field.on('change', function(ele){
                            if(ele.findElement().value == 'subcnd') {
                                ...
                                if(comparison_field.next('a')) {
                                    comparison_field.next('input').disabled = true;
                                    comparison_field.next('input').hide();
                                    comparison_field.next('input').next('input').disabled = true;
                                ...
                                if(comparison_field.next('a')) {
                                    ...
                                    comparison_field.next('input').next('input').disabled = false;
                                    comparison_field.next('a').show();
                                }
                                if(comparison_field.next('a', 1)){

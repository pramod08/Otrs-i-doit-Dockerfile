...
class isys_module_manager extends isys_module implements isys_module_interface
{
    ...
    {
        if (isset(self::$m_trials[$p_module_identifier])) return self::$m_trials[$p_module_identifier];
     ...
     * @throws Exception
     */
    public function uninstall($p_identifier, $p_mandatorDBs = [])
    {
        ...
        try
        {
            ...
            $l_module = $this->get_modules(null, null, null, ' AND isys_module__identifier = \'' . addslashes($p_identifier) . '\'')
                ->__to_array();
                        ...
                        if (isset($l_package['type']))
                        {
                            if ($l_package['type'] == 'addon' && $l_module[$this->m_datasource['table'] . "__id"] >= 1010)
                            {
                                ...
                                foreach ($p_mandatorDBs as $l_mandatorDB)
                                {
                                    ...
                                    if (class_exists('isys_module_' . $p_identifier) && is_callable(
                                            [
                                    ...
                                    {
                                        $l_log->notice(
                                            sprintf('Calling uninstall method for mandator %s in %s', $l_mandatorDB->get_db_name(), 'isys_module_' . $p_identifier)
                                        );
                                        call_user_func(
                                            [
                                                'isys_module_' . $p_identifier,
        ...
        {
            /* Cancel transaction */
            if (isset($l_dao) && is_object($l_dao)) $l_dao->cancel_update();
    ...
    public function install($p_package)
    {
        ...
        {
            $l_dao = isys_component_dao::instance($g_comp_database);
            ...
            if (!($l_id = $this->is_installed($p_package['identifier'])))
            {
                ...
                $l_sql = 'INSERT INTO ' . $this->m_datasource['table'] . ' SET ' . $this->m_datasource['table'] . '__title = \'' .
                    ...
                    '__const = \'C__MODULE__' . strtoupper(
                        $p_package['identifier']
                    ...
                    C__RECORD_STATUS__NORMAL . '\', ' . $this->m_datasource['table'] . '__date_install = NOW()';
            ...
            else
            {
                if ($l_id > 0)
                {
                    $l_sql = 'UPDATE ' . $this->m_datasource['table'] . ' SET ' . $this->m_datasource['table'] . '__title = \'' .
                        (isset($p_package['name']) ? $p_package['name'] : $p_package['title']) . '\', ' . $this->m_datasource['table'] . '__identifier = \'' .
                        $p_package['identifier'] . '\', ' . $this->m_datasource['table'] . '__const = \'C__MODULE__' . strtoupper(
                            $p_package['identifier']
    ...
    public function set_parent_module($p_child_module_id, $p_parent_module_id)
    {
        ...
        $l_sql = 'UPDATE ' . $this->m_datasource['table'] . ' SET ' . $this->m_datasource['table'] . '__parent = ' .
            (($p_parent_module_id > 0) ? '\'' . $p_parent_module_id . '\'' : 'NULL') . ' ' . 'WHERE ' . $this->m_datasource['table'] . '__id = \'' . $p_child_module_id . '\'';
    ...
    public function activate($p_identifier)
    {
        ...
        if ($g_comp_database->query($l_sql))
        {
            if (class_exists('isys_module_' . $p_identifier)) @call_user_func('isys_module_' . $p_identifier . '::activate');
    ...
    public function deactivate($p_identifier)
    {
        ...
        if ($g_comp_database->query($l_sql))
        {
            if (class_exists('isys_module_' . $p_identifier)) @call_user_func('isys_module_' . $p_identifier . '::deactivate');
    ...
    public function module_loader()
    {
        try
        {
            //assert('!is_null(isys_application::instance()->database)');
            ...
            // Initialize modules.
            $l_directory = isys_application::instance()->app_path . '/src/classes/modules/';
    ...
    private function get_installed_modules()
    {
        if (isys_application::instance()->database)
        {
            if (!count($this->m_installed))
            {
                $l_dao     = isys_component_dao::instance(isys_application::instance()->database);
                $l_sql     = 'SELECT ' . $this->m_datasource['table'] . '__id AS id , ' . $this->m_datasource['table'] . '__status as status, ' .
                    $this->m_datasource['table'] . '__title as title, ' . $this->m_datasource['table'] . '__const as const, ' . $this->m_datasource['table'] .
                    '__class as class, ' . $this->m_datasource['table'] . '__icon as icon, ' . $this->m_datasource['table'] . '__identifier as identifier FROM ' .

...
class isys_import_handler_cabling extends isys_import_handler_csv
{
    ...
    private $m_cabling_objects = C__OBJTYPE__PATCH_PANEL;
    private $m_cabling_type = C__CATG__CONNECTOR;

    // Object of smarty plugin f_text
    private $m_connector_type = null;

    // Determines the cabling type
    private $m_create_patch_panels = false;

    // Determines the cable type
    /**
     * @var isys_cmdb_dao_category_g_connector
     */
    private $m_dao;

    // Determines whether to check existing connectors or not
    private $m_dao_cable;
    ...
    public function render_list()
    {
        ...
        if (is_array($this->m_data))
        {
            ...
            {
                if (!is_numeric($l_key)) continue;

                if ($l_key == 0)
                {
                    // head
                    $this->m_max_columns = count($l_line);
    ...
    public function render_row($p_line, $p_iterator, $p_head = false)
    {
        ...
        else
        {
            ...
            $l_html_output = '<td><a href="javascript:" onclick="Cabling.remove_row(' . $p_iterator . ')"><img src="' . $g_dirs['images'] . 'icons/silk/cross.png" title="Reihe entfernen"></td>';
            ...
            $l_html_output .= '</tr>';
            $l_row_start = '<tr id="row_' . $p_iterator . '" class="import_row ' . ($p_iterator % 2 == 0 ? 'CMDBListElementsOdd' : 'CMDBListElementsEven') . '" style="cursor:default;">';
                ...
                if ((floatval($l_key + 1 / 2) % 2.0) > 0)
                {
                    ...
                    if ($l_counter_connector % 2 == 0)
                    {
                        ...
                        if ((isset($l_data[$l_key]) && $l_data[$l_key] != '-' && $l_data[$l_key] != ''))
                        {
                            ...
                            $this->m_dao_connection->delete_cable_connection($l_current_data);
                            // Delete connection from input
                            if (isset($l_rear_connector_data)) $this->m_dao_connection->delete_cable_connection($l_rear_connector_data);

                            $l_cable_id = $this->m_dao_connection->add_cable_connection($l_cable);

                            $l_list_id = $this->m_dao_cable->create_connector('isys_catg_cable_list', $l_cable);

                            $this->m_dao_cable->save(
                                $l_list_id,
                            ...
                            $this->m_dao_connection->save_connection(
                                $l_current_data,
                                $l_rear_connector_data,
                                $l_cable_id
                            );
                            $this->m_log->notice(
                                'Connection between ' . $l_input_object . ' (' . $l_rear_connector_title . ') and ' . $l_ouput_object . ' (' . $l_current_title . ') established.'
                            );
                        }
                    }
                    else
    ...
    public function prepare()
    {
        ...
        $this->m_max_columns = count($this->m_data[0]);
        ...
        foreach ($this->m_data AS $l_data_key => $l_data)
        {
            ...
            if (!is_numeric($l_data_key)) continue;
                    ...
                    else
                    {
                        ...
                        if ($l_counter_objects % 2 != 0)
                        {
                            ...
                            else
                            {
                                if ($l_key < ($this->m_max_columns - 1) && !empty($l_data[$l_key + 4]))
                                {
                                    ...
                                    {
                                        $this->m_log->notice(_L('LC_UNIVERSAL__OBJECT') . " with titel '" . $l_title . "' not found.");
                                        $this->m_log->notice("Mode for creating non existing objects is activated.");
                                        $l_pp_obj_id = $this->m_dao->insert_new_obj(
                                            $this->m_cabling_objects,
                                            false,
                                            $l_title,
                                            null,
                                            C__RECORD_STATUS__NORMAL
                                        ...
                                        $this->m_log->notice(_L('LC_UNIVERSAL__OBJECT') . " '" . $l_title . "'  created with ID '" . $l_pp_obj_id . "'");
                                    }
                                    elseif (!$l_pp_obj_id)
                                    {
                                        $this->m_log->notice(_L('LC_UNIVERSAL__OBJECT') . " with titel '" . $l_title . "' not found.");
                                    ...
                                    $this->create_input_output(
                                        $l_data,
                                        $l_data_key,
                                        $l_key,
                                        false,
                                        $l_connector_key,
                                        self::C__CABLING__INPUT_OUTPUT,
                                        C__CATG__CONNECTOR,
                                        $l_last_key
                                    );
                        ...
                        else
                        {
                            ...
                            if (isset($l_data[$l_key + 2]) && !empty($l_data[$l_key + 2]))
                            {
                                ...
                                if ($l_title == '-' || $l_title == '')
                                {
                                    $l_title = _L('LC__CMDB__OBJTYPE__CABLE') . ' ' . ($this->m_dao->get_last_id_from_table('isys_obj') + 1);
                                }
                                $l_cable_id = $this->m_dao->insert_new_obj(
                                    C__OBJTYPE__CABLE,
                                    false,
                                    $l_title,
                                    null,
     ...
     * @param      $p_connector_type
     * @param bool $p_check_exists_connector
     * @param bool $p_check_exists_objects
     *
     * @return isys_import_handler_cabling
     * @author Van Quyen Hoang <qhoang@i-doit.org>
     */
    public function set_options($p_cabling_type, $p_connector_type, $p_create_patch_panels = false, $p_cabling_object = C__OBJTYPE__PATCH_PANEL, $p_wiring_system = null, $p_cable_type = null, $p_typefilter = null)
    {
        $this->m_cabling_type           = $p_cabling_type;
        $this->m_connector_type         = $p_connector_type;
        $this->m_create_patch_panels    = $p_create_patch_panels;
        $this->m_cabling_objects        = $p_cabling_object;
        $this->m_cable_type             = $p_cable_type;
        $this->m_wiring_system          = $p_wiring_system;

        $this->m_typefilter = $p_typefilter;

        if (is_array($this->m_typefilter))
        {
            $l_key = array_search(C__OBJTYPE__CABLE, $this->m_typefilter);
            unset($this->m_typefilter[$l_key]);
        }
        $l_typefilter_as_string = $this->m_dao->get_object_types_by_category(C__CATG__CABLING, 'g', true, false);
        ...
        foreach ($p_line AS $l_key => $l_title)
        {
            ...
            if ((floatval($l_key + 1 / 2) % 2.0) > 0)
            {
                ...
                if ($l_counter != $l_key)
                {
                    if ($l_counter == ($l_key - 2))
                    {
                        $l_html_output .= $this->render_connector_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_CONNECTORS'), 'skip', $l_key, self::C__CABLING__OUTPUT, true, -2);
                    }
                    else
                    {
                        $l_html_output .= $this->render_connector_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_CONNECTORS'), 'skip', $l_key, self::C__CABLING__INPUT, true, 2);
                    }
                }
                else
                {
                    if ($l_counter_connector % 2 == 0)
                    {
                        $l_html_output .= $this->render_connector_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_CONNECTORS'), 'skip', $l_key, self::C__CABLING__INPUT, true);
                    }
                    else
                    {
                        $l_html_output .= $this->render_connector_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_CONNECTORS'), 'skip', $l_key, self::C__CABLING__OUTPUT, true);
                    }
                }
            ...
            else
            {
                ...
                if ($l_counter_objects % 2 == 0)
                {
                    $l_html_output .= $this->render_object_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_OBJECTS'), 'skip', $l_key, self::C__CABLING__CABLE, true, false, true);
                }
                else
                {
                    $l_html_output .= $this->render_object_cell(_L('LC__MODULE__IMPORT__CABLING__ALL_OBJECTS'), 'skip', $l_key, self::C__CABLING__OBJECT, false, false, true);
                }
                $l_counter_objects++;
            }
    ...
    private function render_hidden_row()
    {
        ...
        $l_counter_objects   = 1;
        $l_counter_connector = 1;
        $l_html_output       = '<td><a href="javascript:"><img src="' . $g_dirs['images'] . 'icons/silk/cross.png" title="Reihe entfernen"></td>';
     ...
     * @author Van Quyen Hoang <qhoang@i-doit.org>
     */
    private function render_connector_cell($p_title, $p_row, $p_cell, $p_input_output = C__CABLING__INPUT, $p_multiedit = false, $p_key_addition = 0)
    {
        ...
        $l_title = $this->m_smarty_plugin->navigation_edit($g_comp_template, $l_params);

        if ($p_multiedit)
        {
            $l_title .= '<img class="vam" src="' . $g_dirs['images'] . '/icons/silk/cog.png" title="Optionen" style="cursor: pointer;" onclick="$(\'title_identifier\').value=this.previous().id;Cabling.set_suffix_format_preselection(this.previous());popup_open(\'multiedit_options\', 700, 280);$$(\'.suf\').each(function(e){e.appear();})">';
        }
        ...
        if ($p_multiedit)
        {
            $l_html_output = '<td style="border-bottom:#000000 solid 1px;" data-column="' . ($p_cell + $p_key_addition) . '" data-row="' . $p_row . '" class="cabling_table_cell_multiedit" ' . $l_data_type . '>';
        }
        else
        {
     ...
     * @author Van Quyen Hoang <qhoang@i-doit.org>
     */
    private function render_object_cell($p_title, $p_row, $p_cell, $p_cable_object = self::C__CABLING__OBJECT, $p_is_cable = false, $p_last_object = false, $p_multiedit = false)
    {
        ...
        $l_params    = [
            ...
            'p_strClass'        => 'normal',
            'p_bInfoIconSpacer' => '0',
            //'p_onChange' => 'Cabling.check_object(this, '.(($p_is_cable)? 'true': 'false').', false)'
        ];
        ...
        else
        {
            ...
            if ($p_row != 'skip' && $p_row != 'skip2')
            {
                ...
                $l_params['p_strSuggestView']       = 'row_' . $p_row . '_' . $p_cell;
                $l_params["p_strSuggestParameters"] = "parameters: { " . "typeFilter: '" . $this->m_typefilter_as_string . "' " . "}, " . "selectCallback: \"Cabling.check_object($('row_" . $p_row . "_" . $p_cell . "'), false, true)\"";
            }
        }
        ...
        {
            $l_title .= '<img class="vam" src="' . $g_dirs['images'] . '/icons/silk/zoom.png" title="Objekt prüfen" style="cursor: pointer;">';
        }
        elseif (!$p_is_cable)
        {
            $l_title .= '<img class="vam" src="' . $g_dirs['images'] . '/icons/silk/zoom.png" title="Objekt prüfen" style="cursor: pointer;" onclick="Cabling.check_object($(\'row_' . $p_row . '_' . $p_cell . '\'), ' . (($p_is_cable) ? 'true' : 'false') . ', false)">';
        }
        ...
        {
            $l_style       = 'style="cursor:default;background:#e77777 url(\'' . $g_dirs['images'] . 'gradient.png\') repeat-x;border-left:1px solid #000000;border-right:1px solid #000000;"';
            $l_html_output = '<td data-column="' . $p_cell . '" data-row="' . $p_row . '" ' . $l_style . ' ' . $l_data_type . ' class="' . $l_class . '" ' . $l_default_bgcolor . '>';
            $l_html_output .= $l_title;
            $l_html_output .= '</td>';
        }
        else
        {
            $l_html_output = '<td ' . $l_style . ' data-column="' . $p_cell . '" data-row="' . $p_row . '" ' . $l_data_type . ' class="' . $l_class . '" ' . $l_default_bgcolor . '>';
            $l_html_output .= $l_title;
            $l_html_output .= '</td>';
        }
    ...
    private function create($p_cabling_type, $p_obj_id, $p_title, $p_direction, $p_connected_connector = null, $p_cable_name = null, $p_connector_sibling = null)
    {
        ...
        $l_list_id      = null;
        $l_connector_id = $this->m_dao->create(
            $p_obj_id,
            $p_direction,
            $this->m_wiring_system,
            $this->m_connector_type,
            $p_title,
            null,
            $p_connector_sibling,
            $p_connected_connector,
            $this->m_array_classes[$p_cabling_type][1]
        );
        switch ($p_cabling_type)
        {
            case C__CMDB__SUBCAT__NETWORK_PORT:
                $l_dao_class = $this->get_class_for_import(C__CMDB__SUBCAT__NETWORK_PORT);
            ...
            case C__CATG__CONTROLLER_FC_PORT:
                $l_dao_class = $this->get_class_for_import(C__CATG__CONTROLLER_FC_PORT);
                // Cannot use the create method of the dao class. Because the connector will be created at the same time
            ...
            case C__CATG__UNIVERSAL_INTERFACE:
                $l_dao_class = $this->get_class_for_import(C__CATG__UNIVERSAL_INTERFACE);
                // Cannot use the create method of the dao class. Because the connector will be created at the same time
    ...
    private function update_category_connector($p_table, $p_connector_id, $p_id)
    {
        ...
        {
            $l_update = 'UPDATE ' . $p_table . ' SET ' . $p_table . '__isys_catg_connector_list__id = ' . $this->m_dao->convert_sql_id(
                    $p_connector_id
                ) . ' ' . 'WHERE ' . $p_table . '__id = ' . $this->m_dao->convert_sql_id($p_id);
     ...
     * @author Van Quyen Hoang <qhoang@i-doit.org>
     */
    private function create_input_output(&$p_data, $p_row, $p_cell, $p_end_object = false, $p_connector_key = null, $p_create_input_output = self::C__CABLING__INPUT_OUTPUT, $p_category_id = C__CATG__CONNECTOR, $p_last_key = 0)
    {
        ...
        if ($p_last_key > $p_cell)
        {
            ...
            if (($p_create_input_output == self::C__CABLING__INPUT_OUTPUT || $p_create_input_output == self::C__CABLING__OUTPUT))
            {
                ...
                if ($l_connector_output_title != '-' && !empty($l_connector_output_title))
                {
                    ...
                    while ($l_row = $l_connector_output_id_res->get_row())
                    {
                        ...
                        {
                            $l_assigned_object = $this->m_dao_connection->get_assigned_object(
                                $l_row['isys_catg_connector_list__isys_cable_connection__id '],
                ...
                else
                {
                    ...
                    $this->m_dao->save(
                        $l_connector_output_id,
                        C__CONNECTOR__OUTPUT,
                        $this->m_wiring_system,
                        $this->m_connector_type,
                        $l_connector_output_title,
                        $l_row['isys_catg_connector_list__isys_catg_connector_list__id'],
                        $l_row['isys_catg_connector_list__description'],
                        C__RECORD_STATUS__NORMAL,
                        null,
                        null
                    );

                    $l_connector_input_id = ($l_row['isys_catg_connector_list__isys_catg_connector_list__id'] > 0) ? $l_row['isys_catg_connector_list__isys_catg_connector_list__id'] : null;
            ...
            if (($p_create_input_output == self::C__CABLING__INPUT_OUTPUT || $p_create_input_output == self::C__CABLING__INPUT) && $p_category_id == C__CATG__CONNECTOR)
            {
                $l_connector_input_title = trim($p_data[$p_cell - 1]);
                if ($l_sibling_title != $l_connector_input_title && $l_connector_input_id == null)
                {
                    if ($l_connector_input_title != '-' && !empty($l_connector_input_title) && $p_category_id == C__CATG__CONNECTOR)
                    {
                ...
                else
                {
                    ...
                    if (!$p_end_object)
                    {
                        $this->m_dao->save(
                            ...
                            $this->m_wiring_system,
                            $this->m_connector_type,
                            ((empty($l_connector_input_title)) ? $l_row['isys_catg_connector_list__title'] : $l_connector_input_title),
                            $l_row['isys_catg_connector_list__isys_catg_connector_list__id'],
                            $l_row['isys_catg_connector_list__description'],
                            C__RECORD_STATUS__NORMAL,
                            null,
        ...
        else
        {
            ...
            if (($p_create_input_output == self::C__CABLING__INPUT_OUTPUT || $p_create_input_output == self::C__CABLING__INPUT) && $p_category_id == C__CATG__CONNECTOR && $p_cell > 0)
            {
                ...
                else
                {
                    ...
                    $l_connector_output_id = ($l_row['isys_catg_connector_list__isys_catg_connector_list__id'] > 0) ? $l_row['isys_catg_connector_list__isys_catg_connector_list__id'] : null;
            ...
            if (($p_create_input_output == self::C__CABLING__INPUT_OUTPUT || $p_create_input_output == self::C__CABLING__OUTPUT))
            {
                ...
                if ($l_connector_output_id == null)
                {
                    ...
                    $l_connector_output_id = $this->create(
                        $p_category_id,
                        $p_data[$p_cell],
                        $l_connector_output_title,
                        C__CONNECTOR__OUTPUT,
                ...
                else
                {
                    ...
                    if (!$p_end_object)
                    {
                        $this->m_dao->save(
                            ...
                            $this->m_wiring_system,
                            $this->m_connector_type,
                            ((empty($l_connector_output_title)) ? $l_row['isys_catg_connector_list__title'] : $l_connector_output_title),
                            $l_row['isys_catg_connector_list__isys_catg_connector_list__id'],
                            $l_row['isys_catg_connector_list__description'],
                            C__RECORD_STATUS__NORMAL,
                            null,
    ...
    public function __construct($p_log = null, $p_file_name = null, $p_data = null)
    {
        ...
        if (is_array($p_data))
        {
            ...
            {
                if (is_numeric($l_key)) $this->m_data[] = $l_data;
            }
        }
        ...
        $this->m_dao_connection = isys_cmdb_dao_cable_connection::instance($g_comp_database);

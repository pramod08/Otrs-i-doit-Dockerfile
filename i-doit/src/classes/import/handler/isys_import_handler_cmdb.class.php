...
class isys_import_handler_cmdb extends isys_import_handler
{
    ...
    /**
     * Nagios component DAO.
     *
     * @var  isys_component_dao_nagios
     */
    protected $m_component_dao_nagios = null;
    /**
     ...
     *
     */
    protected $m_logbook_source = C__LOGBOOK_SOURCE__IMPORT;
    /**
    ...
    public function parse($p_xml_data = null)
    {
        try
        {
            ...
            if (!isset($l_xml_data->objects->object) || count($l_xml_data->objects->object) <= 0)
            {
            ...
            foreach ($l_xml_data->objects->object as $l_object)
            {
                ...
                $this->m_log->debug(
                    sprintf(
                        'Found object %s (%s) of type %s.',
                        $l_title,
                        $l_object_id,
                        $l_type[C__DATA__VALUE]
                    )
                );
                ...
                foreach ($l_object->data->category as $l_category)
                {
                    ...
                    else
                    {
                        if (!defined($l_cat_const))
                        {
                            $this->m_log->notice(
                                printf(
                                    'Category %s\'s constant has not been defined yet.',
                                    $l_cat_const
                                )
                            );
                        }
                    ...
                    foreach ($l_category->cat_data as $l_category_data)
                    {
                        ...
                        foreach ($l_category_data_array as $l_category_data_key => $l_category_data_value)
                        {
                            ...
                            // Recursion:
                            $l_property                                                = $this->parse_properties(
                                $l_category_data,
                                $l_category_data_key,
                                $l_category_data_value
                            );
        ...
        try
        {
            ...
            foreach ($this->m_data as $l_object_id => $l_object_values)
            {
                            ...
                            foreach ($l_category_data_values[self::C__PROPERTIES] as $l_property_id => $l_property)
                            {
                                ...
                                if (count($l_found_references) > 0)
                                {
                                    foreach ($l_found_references as $l_ref)
                                    {
                                        if (isset($l_ref['id']) && isset($l_ref['type']) && isset($l_ref['sysid']))
                                        {
                                            ...
                                            if (isset($l_ref['ref_id']) && isset($l_ref['ref_type']) && isset($l_ref['ref_title']))
                                            {
                                                ...
                                                if ($l_found_category_datas !== false)
                                                {
                                                    ...
                                                    {
                                                        if ($l_category_values['const'] === 'C__CMDB__SUBCAT__STORAGE__DEVICE' && $l_ref['ref_type'] === 'C__CATG__CONTROLLER')
                                                        {
                                                            /*
															 * @todo
															 ...
															 */
                                                            $this->m_prepared[$l_object_id][self::C__CATEGORIES][$l_found_category_data['category_type_id'] . '_' . $l_found_category_data['category_id']][self::C__PRIORITY] += $this->m_prepared[$l_object_id][self::C__CATEGORIES][$l_category_type_id . '_' . $l_category_id][self::C__PRIORITY] + 10000;
                                                        }
                                                        else
                                                        {
                                                            // Use also this category's priority:
                                                            $this->m_prepared[$l_object_id][self::C__CATEGORIES][$l_found_category_data['category_type_id'] . '_' . $l_found_category_data['category_id']][self::C__PRIORITY] += $this->m_prepared[$l_object_id][self::C__CATEGORIES][$l_category_type_id . '_' . $l_category_id][self::C__PRIORITY] + 1;
                                                        } // if
                                                    } // foreach
                                                } // if
                                            } // if
                                        ...
                                        else if (isset($l_ref['id']) && isset($l_ref['type']))
                                        {
                                            // We've found a property that's related to a category data:
                                            // Look for this referenced category data:
                                            $l_found_category_datas = $this->fetch_category_reference(
                                            ...
                                            if ($l_found_category_datas !== false)
                                            {
                                                ...
                                                foreach ($l_found_category_datas as $l_found_category_data)
                                                {
                                                    if ($l_category_values['const'] === 'C__CMDB__SUBCAT__STORAGE__DEVICE' && $l_ref['type'] === 'C__CATG__CONTROLLER')
                                                    {
                                                        /*
														 * @todo
														 * If category C__CMDB__SUBCAT__STORAGE__DEVICE has a reference to a RAID group there may be a
                ...
                if (isset($this->m_prepared[$l_object_id][self::C__CATEGORIES]) && is_array($this->m_prepared[$l_object_id][self::C__CATEGORIES]))
                {
                    ...
                    uksort(
                        $this->m_prepared[$l_object_id][self::C__CATEGORIES],
                        [
                            $this,
                            'sort_categories'
                        ]
            ...
            if (is_array($this->m_prepared)) uksort(
                $this->m_prepared,
                [
                    $this,
                    'sort_objects'
    ...
    {
        if (isset($this->m_replaced_titles[$p_obj_id])) return $this->m_replaced_titles[$p_obj_id];
        else return false;
    } //function
    ...
    public function import()
    {
        ...
        if (defined('C__MODULE__NAGIOS'))
        {
            ...
            $this->m_component_dao_nagios        = isys_component_dao_nagios::instance($this->m_db);
        } // if
        ...
        $this->m_log->info(
            sprintf(
                ...
                $this->m_object_counter['created'],
                $this->m_object_counter['updated'],
                $this->m_object_counter['category_updated'],
                $this->m_object_counter['category_skipped']
            )
        );
        //$this->m_log->flush_verbosity(true, false);
        ...
        $l_changed_objects = array_keys($this->m_prepared);
        $this->m_log->info('Updating changed_by attribute and deleting quickinfo cache for ' . count($l_changed_objects) . ' object(s)');
        $this->m_log->debug(' -> ' . implode(', ', $l_changed_objects));

        if ($this->m_options['update-object-changed'])
        {
    ...
    {
        if (count($this->m_logbook_entries))
        {
    ...
    {
        // Location fix should be called in the destuctor.
        try
        {
    ...
    private function parse_properties($p_data, $p_key, $p_value)
    {
        ...
        switch (gettype($p_value))
        {
            ...
            default:
                $this->m_log->warning(
                    sprintf('Unknown property type %s.', gettype($p_value))
                );
                break;
        } // switch
    ...
    private function find_references($p_object_id, $p_category_type_id, $p_category_id, $p_category_data_id, $p_property_id, $p_data_arr = null)
    {
        ...
        switch (gettype($l_value))
        {
            ...
            case 'array':
                ...
                {
                    // Next step:
                    $l_new_result = $this->find_references(
                        $p_object_id,
                        $p_category_type_id,
                        $p_category_id,
                        $p_category_data_id,
        ...
        else if (isset($l_property['id']) && isset($l_property['type']) && isset($l_property['sysid']))
        {
            // Reference to object:
            $l_new_result['id']    = $l_property['id'];
            $l_new_result['type']  = $l_property['type'];
            $l_new_result['sysid'] = $l_property['sysid'];
            if (isset($l_property['ref_id'])) $l_new_result['ref_id'] = $l_property['ref_id'];
            if (isset($l_property['ref_type'])) $l_new_result['ref_type'] = $l_property['ref_type'];
            if (isset($l_property['ref_title'])) $l_new_result['ref_title'] = $l_property['ref_title'];
        ...
        else if (isset($l_property['id']) && isset($l_property['const']))
        {
    ...
    private function fetch_object_reference($p_current_object_id, $p_object_id, $p_type, $p_sysid, $p_value)
    {
        $this->m_log->debug(
            sprintf(
                'Look for object %s of type %s with sysid %s and title %s...',
                $p_object_id,
                $p_type,
                $p_sysid,
                $p_value,
        ...
        foreach ($this->m_data as $l_object_id => $l_object_values)
        {
            ...
            if ($l_object_id == $p_object_id && $l_object_values['type']['const'] == $p_type && $l_object_values['sysid'] == $p_sysid && $l_object_values[C__DATA__TITLE] == $p_value)
            {
    ...
    private function import_dummy_objects()
    {
        ...
        if (is_array($this->m_info[self::C__OBJECTS]))
        {
            foreach ($this->m_info[self::C__OBJECTS] as $l_object_id => $l_object_info)
            {
                ...
                {
                    $this->m_log->debug(
                        'Looks like current object is unknown, but we also take a look at the SYSID.'
                    );
                ...
                {
                    $this->m_log->debug(
                        'Looks like current object is unknown, but we also take a look at the Title and Type.'
                    );
                    ...
                    if (defined($l_object_info[self::C__PROPERTIES]['type']))
                    {
                        ...
                        {
                            $this->m_cached_title_constants[$l_title_constant] = $this->m_cmdb_dao->get_obj_id_by_title(
                                $l_title,
                                $l_constant
                            );
                        } //if
                        if ($this->m_cached_title_constants[$l_title_constant])
                        {
                            ...
                            {
                                $this->m_cached_objects[$this->m_cached_title_constants[$l_title_constant]] = $this->m_cmdb_dao->get_object_by_id(
                                    $this->m_cached_title_constants[$l_title_constant]
                                );
                            } //if
                        }
                        $this->m_found_objects[$l_object_id] = $this->compare_objects(
                ...
                else if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__PARTLY)
                {
                    $this->m_log->debug(
                        '  Match found in database, but it\'s not exactly the same. Updating.'
                    );
                ...
                else if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__DIFFERENT)
                {
                    $this->m_log->debug(
                    ...
                    $this->m_log->debug(
                        sprintf(
                            '  Check whether object type %s exists.',
                            $l_object_info[self::C__PROPERTIES]['type']
                        )
                    );
                    ...
                    else
                    {
                        ...
                        $l_new_obj_type_id                        = $this->m_cmdb_dao->insert_new_objtype(
                            null,
                            null,
                            $l_object_info[self::C__PROPERTIES]['type']
                        );
                        ...
                        if (!isset($this->m_object_type_ids[$l_object_id]))
                        {
                            throw new isys_exception_general(
                                sprintf(
                                    '  Failed to create new object type %s.',
                                    $l_object_info[self::C__PROPERTIES]['type']
                                )
                            );
                        }
                        $this->m_log->debug(
                            sprintf(
                                '  New object type %s created as a dummy.',
                                $l_object_info[self::C__PROPERTIES]['type']
                            )
                        );
                        // Log:
    ...
    private function import_objects()
    {
        ...
        foreach ($this->m_prepared as $l_object_id => $l_object_values)
        {
            ...
            // Preparation:
            $this->m_object_ids[$l_object_id] = $l_object_id;
            $l_object_title                   = ($this->get_replaced_title($l_object_id)) ? $this->get_replaced_title($l_object_id) : $l_object_values[C__DATA__TITLE];
            ...
            if ($this->m_mode !== self::C__APPEND)
            {
                ...
                {
                    $this->m_log->debug(
                        'Looks like current object is unknown, but we also take a look at the Object Title and Object Type.'
                    );

                    // Look for object by its sysid in database and compare result
                    // with this object:
                    $l_title = $l_object_values['title'];
                ...
                else if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__PARTLY)
                {
                    ...
                    try
                    {
                        if (is_array($l_object_values['categories']) && array_key_exists(
                                C__CMDB__CATEGORY__TYPE_GLOBAL . "_" . C__CATG__GLOBAL,
                                $l_object_values['categories']
                            )
                    ...
                    // Log:
                    if (is_array($l_object_values['categories']) && array_key_exists(C__CMDB__CATEGORY__TYPE_GLOBAL . "_" . C__CATG__GLOBAL, $l_object_values['categories']))
                    {
                        ...
                        if (_L($l_current_obj_data['isys_obj_type__title']) != $l_object_values['type']['value'])
                        {
                            $l_serialized_changes['isys_cmdb_dao_category_g_global::type'] = [
                                'from' => _L($l_current_obj_data['isys_obj_type__title']),
                        ...
                        if ($this->m_cmdb_dao->get_record_status_as_string($l_current_obj_data['isys_obj__status']) != $this->m_cmdb_dao->get_record_status_as_string(
                        ...
                        if ($l_changes > 0)
                        {
                            $this->m_event_manager->triggerImportEvent(
                                ...
                                $this->m_object_ids[$l_object_id],
                                constant($this->m_object_type_ids[$l_object_id]),
                                $l_object_values['categories'][C__CMDB__CATEGORY__TYPE_GLOBAL . "_" . C__CATG__GLOBAL]['title'],
                ...
                {
                    $this->m_log->debug(
                        'No match found in database. Create new object in a moment.'
                    );
                    $this->m_log->debug(
                        sprintf(
                            'Check whether object type %s exists.',
                    ...
                    {
                        $this->m_log->debug(
                            'Object type is unknown. It will be now created.'
                        );
                    ...
                    $this->m_object_ids[$l_object_id] = $this->m_cmdb_dao->insert_new_obj(
                        ...
                        true,
                        date($this->m_date_format, $l_object_values['created']),
                        $l_object_values['created_by'],
                        date($this->m_date_format, $l_object_values['updated']),
                        $l_object_values['updated_by'],
                        null,
                        null,
                        $l_object_values['cmdb_status'],
            ...
            else if ($this->m_mode === self::C__APPEND)
            {

                $this->m_log->debug(
                    'Create new object in a moment.'
                );
                $this->m_log->debug(
                    sprintf(
                        'Check whether object type %s exists.',
    ...
    private function import_categories()
    {
        ...
        {
            $this->m_log->debug(
                "Importing object $l_object_id: " . $l_object_values[C__DATA__TITLE] . " (" . $l_object_values['sysid'] . ")"
            );
            ...
            if (is_array($l_object_values[self::C__CATEGORIES]))
            {
                foreach ($l_object_values[self::C__CATEGORIES] as $l_category_key => $l_category_values)
                {
                    ...
                    {
                        $l_category_id      = C__CATG__CUSTOM_FIELDS;
                        $l_category_type_id = C__CMDB__CATEGORY__TYPE_GLOBAL;
                    } //if

                    // Translate category name:
                    $l_category_name = _L($this->m_all_categories[$l_category_type_id][$l_category_id]['title']);
                    ...
                    {
                        $this->m_log->debug(
                            'Category type ' . $l_category_type_id . ' does not exist. Skipping.'
                        );
                        continue;
                    } //if

                    if (!empty($this->m_ignored_categories) && isset($this->m_ignored_categories[$l_category_type_id]) && in_array(
                            $l_category_id,
                            $this->m_ignored_categories[$l_category_type_id]
                        )
                    )
                    {
                    ...
                    if (isset($l_daos_processed[$l_cat_class]) && $l_category_id != C__CATG__CUSTOM_FIELDS)
                    {
                        $this->m_log->debug(
                            "Category $l_cat_class ($l_category_id) was already processed by the import. Skipping."
                        );
                    ...
                    $l_cat_dao = isys_factory_cmdb_category_dao::get_instance_by_id(
                        (int) $l_category_type_id,
                        (int) $l_category_id,
                        $this->m_db
                    );
                    ...
                    {
                        $this->m_log->debug(
                            'Handle custom category ' . $l_category_values['title']
                        );
                        ...
                        {
                            if (isset($l_category_data_values['properties']) && is_array($l_category_data_values['properties']) && count(
                                    $l_category_data_values['properties']
                                )
                            )
                            {
                                foreach ($l_category_data_values['properties'] as $l_tag => $l_values)
                                {
                                    ...
                                    if ($l_type == 'f_popup')
                                    {
                                        ...
                                        {
                                            $l_config[$l_key]['popup']      = 'dialog_plus';
                                            $l_config[$l_key]['title']      = $l_values['title'];
                                            $l_config[$l_key]['identifier'] = $l_values['identifier'];
                                        }
                                    }
                        ...
                        if (count($l_module_data) > 0)
                        {
                            foreach ($l_module_data as $l_candidate)
                            {
                                ...
                                else if ($l_candidate['isysgui_catg_custom__config'] == $l_serialized_config)
                                {
                                    $l_found = true;
                                }
                                if ($l_found === true)
                        ...
                        {
                            $this->m_log->notice(
                                'Custom category ' . $l_category_values['title'] . ' not found. Creating...'
                            );
                            ...
                            {
                                $this->m_log->warning(
                                    'Failed to create custom category ' . $l_category_values['title'] . ' [' . $l_category_values['const'] . ']. Skipping.'
                                );
                                continue;
                            } //if

                            $l_status = $l_custom_module->assign(
                                $l_catg_custom_id,
                                constant($this->m_object_type_ids[$l_object_id])
                            );
                            if ($l_status === false)
                            {
                                $this->m_log->warning(
                                    'Failed to assign custom category ' . $l_category_values['title'] . ' [' . $l_category_values['const'] . '] to object type ' . $this->m_object_type_ids[$l_object_id] . '. Skipping.'
                                );
                                continue;
                            } //if
                    ...
                    {
                        $this->m_cat_info[$l_category_key]['property_info'] = $l_cat_dao->get_properties();
                    } //if
                    ...
                    if (is_array($l_category_values[self::C__CATEGORY_ENTITIES]))
                    {
                        ...
                        foreach ($l_category_values[self::C__CATEGORY_ENTITIES] as $l_category_data_id => $l_category_data_values)
                        {
                            ...
                            foreach ($l_category_data_values['properties'] as $l_key => $l_value)
                            {
                                ...
                                if ($l_class && $l_method)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (($l_import_method == 'object_import' && is_array(
                                                    $l_value
                                                )) || $l_import_method == 'model_title_import' || $l_import_method == 'convert_import' || $l_import_method == 'dialog_import' || $l_import_method == 'dialog_plus_import' || $l_import_method == 'object_image_import' || (array_key_exists(
                                                    'ref_id',
                                                    $l_category_data_values['properties'][$l_key]
                                                ) && ($l_category_data_values['properties'][$l_key]['ref_id'] === null || !is_numeric($l_category_data_values['properties'][$l_key][C__DATA__VALUE])))
                                        )
                                        {
                                            ...
                                            {
                                                $this->m_log->warning(
                                                    sprintf('Transformation failed (method %s from class %s in category %s).', $l_import_method, $l_class, $l_category_name)
                                                );
                                            } // if
                                        }
                                        else
                            ...
                            else if ($l_mode === self::C__OVERWRITE)
                            {
                                $l_category_data_status = self::C__CREATE;
                                ...
                                if ($l_already_overwritten === false && $l_cat_class != 'isys_cmdb_dao_category_g_global')
                                {
                                    ...
                                    $l_cleared = $this->m_cmdb_dao->clear_data(
                                        $this->m_object_ids[$l_object_id],
                            ...
                            else if ($l_mode === self::C__MERGE || $l_mode === self::C__USE_IDS)
                            {
                                // There are one or more data sets fetched from
                                // database:
                                if (count($l_object_category_dataset) == 0)
                                {
                                    $l_category_data_status = self::C__CREATE;
                                }
                                else
                                {
                                    ...
                                    $this->m_log->debug(
                                        'Checking whether one of the datasets matches the properties of category "' . $l_category_name . '"...'
                                    );
                                    ...
                                    {
                                        if (!$this->m_cat_info[$l_category_key]['is_multi-valued'] && isset($l_object_category_dataset[0][$this->m_cat_info[$l_category_key]['table'] . '__id']))
                                        {
                                    ...
                                    if (isset($l_comparison[self::C__COMPARISON__SAME]) && count($l_comparison[self::C__COMPARISON__SAME]) > 0)
                                    {
                                        if ($this->m_cat_info[$l_category_key]['is_multi-valued'])
                                        {
                                    ...
                                    if ($l_candidate_found === null && isset($l_comparison[self::C__COMPARISON__PARTLY]) && count(
                                            $l_comparison[self::C__COMPARISON__PARTLY]
                                        ) > 0
                                    )
                                    {
                            ...
                            if ($l_category_data_status !== self::C__KEEP)
                            {
                                if ($l_category_data_status === self::C__CREATE)
                                {
                                    if (!$this->m_cat_info[$l_category_key]['is_multi-valued'])
                                    {
                                        ...
                                        if ($l_res->num_rows() == 1 && $l_cat_class != 'isys_cmdb_dao_category_g_global')
                                        {
                                            ...
                                            if ($l_cat_class === 'isys_cmdb_dao_category_g_accounting' && defined($this->m_object_type_ids[$this->m_object_ids[$l_object_id]]))
                                            {
                                ...
                                if ($l_mode == self::C__MERGE && count($l_object_category_dataset) > 0 && $l_category_id != C__CATG__CUSTOM_FIELDS)
                                {
                                ...
                                else
                                {
                                    ...
                                    foreach ($l_category_data_values['properties'] as $l_key => $l_value)
                                    {
                                        if (!$l_used_properties[$l_key][C__PROPERTY__PROVIDES][C__PROPERTY__PROVIDES__IMPORT]) continue;

                                        // Need to unconvonvert:
                                        if (!isset($l_category_data_values['properties'][$l_key][C__DATA__VALUE . '_converted']) && isset($l_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0]) && isset($l_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1]))
                                        {
                                ...
                                if (count($l_category_data_values[self::C__PROPERTIES]) > 0)
                                {
                                    ...
                                    if ($l_category_data_status === self::C__UPDATE)
                                    {
                                        ...
                                        elseif (count($l_object_category_dataset) == 1)
                                        {
                                    ...
                                    else if ($l_category_data_status === self::C__CREATE)
                                    {
                                        ...
                                        if ($this->m_cat_info[$l_category_key]['is_multi-valued'])
                                        {
                                            ...
                                            if ($l_event_counter[self::C__CREATE] <= isys_settings::get('logbook.changes.multivalue-threshold', 25))
                                            {
                                                // Create summarized logbook entry for multi value categories
                                                $l_collected_category_changes[$l_event_counter[self::C__CREATE]] = $this->m_mod_logbook->prepare_changes(
                                                    $l_cat_dao,
                                                    null,
                                                    $l_category_data_values
                                                );
                                    ...
                                    // Sync category:
                                    $l_new_category_data_id = $l_cat_dao->sync(
                                        $l_category_data_values,
                                        $this->m_object_ids[$l_object_id],
                                        $l_category_data_status
                                    );
                                ...
                                else
                                {
                                    ...
                                    $this->m_log->debug(
                                        sprintf(
                                            '== There is nothing to sync for the category %s skipping...',
                                            $l_category_name
                                        )
                                    );
                                }
                    ...
                    else if ($l_changed_categories === false && $this->m_found_objects[$l_object_id] === self::C__COMPARISON__SAME)
                    {
                        $this->m_object_counter['category_skipped']++;
                    } // if
                } // foreach category
     ...
     * @throws isys_exception_cmdb
     */
    private function compare_category_data(&$p_category_data_values, &$p_object_category_dataset, &$p_used_properties, &$p_comparison, &$p_badness, &$p_mode, &$p_category_id, &$p_unit_key, &$p_category_data_ids, &$p_local_export, &$p_dataset_id_changed, &$p_dataset_id, &$p_logger, &$p_category_name = null, &$p_table = null, &$p_cat_multi = null, &$p_category_type_id = null, &$p_category_ids = null, &$p_already_used_data_ids = null)
    {
        $l_count_properties = count($p_category_data_values[self::C__PROPERTIES]);
        ...
        foreach ($p_object_category_dataset as $l_dataset_key => $l_dataset)
        {
            ...
            foreach ($p_category_data_values[self::C__PROPERTIES] as $l_key => $l_value)
            {
                ...
                // Need to unconvert:
                if (isset($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0]) && isset($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1]))
                {
                    ...
                    else
                    {
                        ...
                        if ($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] == 'connection') $l_local = $l_dataset['isys_connection__isys_obj__id'];
                        ...
                        else
                        {
                            if (isset($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1]))
                            {
                                ...
                                if ($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] != 'connection' && $p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] != 'get_yes_or_no')
                                {
                                    ...
                                    if (isset($p_local_export))
                                    {
                                        ...
                                        if (count($l_local_export_data) > 0)
                                        {
                                            ...
                                            if (is_array($l_local_export_data))
                                            {
                                                ...
                                                {
                                                    if ($p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] == 'get_ip_reference') $l_local = $l_local_export_data['ref_title'];
                        ...
                        if (gettype($l_import) != 'array' && $p_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] != 'contact')
                        {
                            ...
                            if ((!empty($l_local_check) || !empty($l_import_check)) && $l_local != $l_import)
                            {
                                ...
                                {
                                    $l_import = _L($l_value['title_lang']);
                                    ...
                                    $l_local     = _L($l_local_arr['title_lang']);
                                } //if
                ...
                else
                {
                    ...
                    {
                        if (array_key_exists(
                            $p_used_properties[$l_key][C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD_ALIAS],
                            $l_dataset
            ...
            else if ($p_badness[$p_dataset_id] > $l_threshhold && $p_cat_multi)
            {
                $p_logger->debug(
    ...
    private function compare_objects($p_database_object, &$p_import_object, &$p_object_id, &$p_object_status = null)
    {
        ...
        {
            if (defined($p_import_object['type']['const'])) $l_type = constant($p_import_object['type']['const']);
            else $l_type = $this->m_cmdb_dao->get_objtype_id_by_const_string($p_import_object['type']['const']);
        }
        else if (isset($p_import_object['type_id']))
        {
            $l_type = $p_import_object['type_id'];
        }
        else if (is_string($p_import_object['type']))
        {
        ...
        {
            $l_title = $p_import_object[C__DATA__VALUE];
        }
        elseif (isset($p_import_object[C__DATA__TITLE]) && isset($p_import_object['first_name']))
        {
            $l_title = $p_import_object[C__DATA__TITLE];
        } // if
        ...
        if (isset($p_import_object['category_types'][C__CMDB__CATEGORY__TYPE_GLOBAL][C__CATG__GLOBAL][self::C__CATEGORY_ENTITIES][$p_object_id][self::C__PROPERTIES]['description'][C__DATA__VALUE]) && $l_existing_object['isys_obj__description'] !== $p_import_object['category_types'][C__CMDB__CATEGORY__TYPE_GLOBAL][C__CATG__GLOBAL][self::C__CATEGORY_ENTITIES][$p_object_id][self::C__PROPERTIES]['description'][C__DATA__VALUE])
        {
            $this->m_log->debug('Object description is different.');
            $l_badness++;
        } // if

        if (isset($p_import_object['status']) && ((bool) isys_settings::get('import.object.keep-status')) === false)
        {
        ...
        */
        ...
        else if ($l_badness >= self::C__COMPARISON__THRESHOLD)
        {
            $p_object_id = -1;
     ...
     * @param $p_helper_arr
     */
    private function merge_missing_data(&$p_category_data_values, $p_object_category_dataset, $p_category_key, $p_dataset_key, $p_category_ids, $p_category_data_ids, $p_mode, &$p_helper_arr, $p_class_name = '')
    {
        ...
        foreach ($this->m_cat_info[$p_category_key]['property_info'] as $l_property_key => $l_property_value)
        {
            if ($l_property_value[C__PROPERTY__PROVIDES][C__PROPERTY__PROVIDES__IMPORT])
            {
                $l_table_value     = null;
                $l_local_value_arr = [];
                if (empty($p_category_data_values['properties'][$l_property_key][C__DATA__VALUE]) || in_array(
                        $p_category_data_values['properties'][$l_property_key][C__DATA__VALUE],
                        $this->m_possible_empty_values
                    )
                )
                {
                    if (isset($l_property_value[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD_ALIAS])) $l_property_field = $l_property_value[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD_ALIAS];
                    else $l_property_field = $l_property_value[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD];

                    if (isset($p_object_category_dataset[$p_dataset_key]) && is_array(
                        ...
                        if ($l_table_value !== null)
                        {
                            ...
                            if (isset($l_property_value[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1]))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!isset($l_property_value[C__PROPERTY__DATA][C__PROPERTY__DATA__REFERENCES][1]) || !strpos(
                                            $l_property_field,
                                            $l_property_value[C__PROPERTY__DATA][C__PROPERTY__DATA__REFERENCES][1]
                                        )
                                    )
                                    {
                                        ...
                                        if ($l_local_export)
                                        {
                                            ...
                                            if (is_array($l_local_export_data) && count($l_local_export_data) > 0)
                                            {
                                                ...
                                                if ($l_property_value[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] == 'assigned_connector' || $l_property_value[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1] == 'get_net_dns_server')
                                                {
                                                ...
                                                if ($l_table_value === null) $l_table_value = $l_helper->$l_import_method([C__DATA__VALUE => $l_local_export_data]);
                    ...
                    // Append missing data:
                    if ((!isset($p_category_data_values['properties'][$l_property_key]) || !isset($p_category_data_values['properties'][C__DATA__VALUE])) && $l_table_value !== null)
                    {
                ...
                elseif (isset($p_category_data_values['properties'][$l_property_key]['sysid']) && !is_numeric(
                        $p_category_data_values['properties'][$l_property_key][C__DATA__VALUE]
                    )
                )
                {
    ...
    public function __construct($p_log, $p_db, $p_cmdb_dao = null)
    {
        ...
        $p_log->set_destruct_flush((bool) isys_settings::get('logging.cmdb.import', false));

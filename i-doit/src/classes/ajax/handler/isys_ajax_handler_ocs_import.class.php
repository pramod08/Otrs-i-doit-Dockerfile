        ...
        try
        {
            if ($l_daoOCS->does_snmp_exist())
            {
                ...
                while ($l_row = $l_snmp_inventory->get_row())
                {
                    ...
                    $l_data_res                    = $l_dao_comp->retrieve(
                        "SELECT isys_obj__imported, isys_obj__isys_obj_type__id FROM isys_obj WHERE isys_obj__hostname = " . $l_dao_comp->convert_sql_text(
                            $l_row["NAME"]
                        ) . ";"
                    );
                    $l_row["imported"]             = null;

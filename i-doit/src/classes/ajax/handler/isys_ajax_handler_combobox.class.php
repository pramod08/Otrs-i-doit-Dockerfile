...
class isys_ajax_handler_combobox extends isys_ajax_handler
{
    ...
    public function init()
    {
        ...
        switch ($_GET['func'])
        {
            ...
            case 'save':
                $l_data = $this->save(
                    isys_format_json::decode($_POST['data']),
                    $_POST['table'],
                    isys_format_json::decode($_POST['parent']),
                    $_POST['condition']
                );
                ...
                {
                    $this->save_field('isys_contact_tag', $l_data, C__RELATION_TYPE__USER, 'isys_relation_type__id');
                } // if
            ...
            case 'save_relation_type':
                $l_data = $this->save_relation_type(
                    $_POST['relation_type__title'],
                    $_POST['relation_type__master'],
                    $_POST['relation_type__slave']
                );
                break;
        } // switch
    ...
    public function load_extended($p_table, $p_parenttable = '', $p_parenttable_id = 0, $p_condition = null)
    {
        ...
        {
            $l_sql = 'SELECT * ' . 'FROM ' . $p_table . ' ' . 'WHERE ' . $p_table . '__' . $p_parenttable . '__id = ' . $l_dao->convert_sql_int(
                    $p_parenttable_id
        ...
        {
            $l_title                                            = _L(trim($l_row[$p_table . '__title']));
    ...
    public function load_sub($p_id = null, $p_table, $p_child_table)
    {
        ...
        $l_sql = 'SELECT ' . $p_child_table . '__id AS id, ' . $p_child_table . '__title AS title ' . 'FROM ' . $p_child_table . ' WHERE ' . $p_child_table . '__' . $p_table . '__id = ' . $p_id;
    ...
    public function save(array $l_data, $p_table, $p_parent = [], $p_condition = null)
    {
        ...
        foreach ($l_data as $i => $l_dataset)
        {
            ...
            if ($l_dataset['id'] == '-')
            {
                ...
                $l_sql = 'INSERT INTO ' . $p_table . ' SET ' . $p_table . '__title = ' . $l_dao->convert_sql_text(
                        $l_dataset['name']
                    ) . ', ' . $p_table . '__status = ' . $l_dao->convert_sql_int(C__RECORD_STATUS__NORMAL) . ' ' . ((!empty($p_condition)) ? ',' . $p_condition : ';');

                // When we have a dependency, we have to write another SQL query.
                if ($p_parent['selected_id'] > 0 && !empty($p_parent['table']))
                {
                    $l_sql = 'INSERT INTO ' . $p_table . ' (' . $p_table . '__title, ' . $p_table . '__status, ' . $p_table . '__' . $p_parent['table'] . '__id' . ') VALUES (' . $l_dao->convert_sql_text(
                            $l_dataset['name']
                        ) . ', ' . $l_dao->convert_sql_int(C__RECORD_STATUS__NORMAL) . ', ' . $l_dao->convert_sql_int($p_parent['selected_id']) . ');';
                } // if
    ...
    protected function save_cat_data($p_obj_id, $p_table, array $p_data)
    {
        ...
        foreach ($p_data as $l_dataset)
        {
            ...
            if ($l_dataset['id'] == '-')
            {
                ...
                $l_sql = 'INSERT INTO ' . $p_table . ' SET ' . $p_table . '__title = ' . $l_dao->convert_sql_text(
                        $l_dataset['name']
                    ) . ', ' . $p_table . '__isys_obj__id = ' . $l_dao->convert_sql_id($p_obj_id) . ', ' . $p_table . '__status = ' . $l_dao->convert_sql_int(

...
class isys_ajax_handler_dhcp extends isys_ajax_handler
{
    ...
    protected function create_dhcp_range($p_obj_id, $p_from, $p_to)
    {
        ...
        return $l_dhcp_dao->check_and_merge_new_dhcp_range_inside_existing(
            $p_obj_id,
            $this->m_types[$this->m_post['type']],
            $p_from,
            $p_to
        );
    } // function
    ...
    protected function create_static_range($p_obj_id, $p_from, $p_to)
    {
        ...
        $l_g_ip->update_ip_assignment_by_ip_range($p_obj_id, C__CATP__IP__ASSIGN__STATIC, $p_from, $p_to);

    ...
    public function init()
    {
        ...
        if ($l_cache === false)
        {
            ...
            while ($l_module = $l_modules->get_row())
            {
                ...
                if (class_exists($l_module['isys_module__class']))
                {
                    if (constant($l_module['isys_module__class'] . '::DISPLAY_IN_MAIN_MENU'))
                    {
                        ...
                        {
                            $l_module_url = isys_application::instance()->www_path . (defined(
                                    $l_module['isys_module__class'] . '::MAIN_MENU_REWRITE_LINK'
                                ) ? $l_module['isys_module__identifier'] : '?' . C__GET__MODULE_ID . '=' . $l_module['isys_module__id']);
                            ...
                            if (!is_null($l_module['isys_module__parent']))
                            {
                                ...
                                if (!isset($l_parent_modules[$l_module['isys_module__parent']]))
                                {
                                    ...
                                    $l_module_url    = isys_application::instance()->www_path . (defined(
                                            $l_module['isys_module__class'] . '::MAIN_MENU_REWRITE_LINK'
                                        ) ? $l_parent_module['isys_module__identifier'] : '?' . C__GET__MODULE_ID . '=' . $l_parent_module['isys_module__id']);
                                    ...
                                    $l_parent_modules[$l_module['isys_module__parent']] = [
                                        ...
                                        'sort'   => (isset($l_sort_array[$l_parent_module['isys_module__title']])) ? $l_sort_array[$l_parent_module['isys_module__title']] : 99 + $l_counter,
                                        'title'  => $l_parent_module['isys_module__title'],
                                        'url'    => $l_module_url,
                                        'icon'   => strstr(
                                            $l_module['isys_module__icon'],
                                            '/'
                                        ) ? $g_config['www_dir'] . $l_parent_module['isys_module__icon'] : $l_parent_module['isys_module__icon'],
                                        'childs' => []
                                    ];
                                } // if
                                ...
                                $l_parent_modules[$l_module['isys_module__parent']]['childs'][$l_module['isys_module__title']] = [
                                    ...
                                    'icon' => strstr(
                                        $l_module['isys_module__icon'],
                                        '/'
                                    ) ? $g_config['www_dir'] . $l_module['isys_module__icon'] : $l_module['isys_module__icon']
                                ];
                        ...
                        if (is_array($l_additional_links))
                        {
                            foreach ($l_additional_links AS $l_key => $l_content)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (is_a($l_auth_instance, 'isys_auth') && !$l_auth_instance->is_allowed_to(isys_auth::VIEW, $l_key)) continue;
                                } // if
            ...
            if (isys_auth_system::instance()
                ->is_allowed_to(isys_auth::VIEW, 'WORKFLOW')
            )
            {
                $l_links['LC__CMDB__CATG__WORKFLOW'] = [
            ...
            if (isys_auth_cmdb::instance()->is_allowed_to(isys_auth::VIEW, 'OBJ_IN_TYPE/C__OBJTYPE__LAYER3_NET'))
            {
        ...
        if (count($l_parent_modules) > 0)
        {
            foreach ($l_parent_modules AS $l_parent_module_id => $l_content)
            {
                ...
                if (count($l_childs) > 0)
                {
                    $l_out_string = '<li id="parent_' . $l_parent_module_id . '" class="' . $l_disable_class . '" onmouseover="$(\'module-dropdown\').show_childs(\'childs_' . $l_parent_module_id . '\');" >' . '<a href="' . $l_parent_url . '" ><img class="vam module-icon" src="' . $l_parent_icon . '" alt=" ">' . '<span>' . _L(
                            $l_parent_module_title
                        ) . '</span><img class="fr vam m5" src=\'' . isys_application::instance()->www_path . 'images/icons/tree_icon_right.png\' />' . '</a></li>';
                    ...
                    {
                        $l_out_string .= '<li><a href="' . $l_child_info['url'] . '">' . '<img class="vam module-icon" src="' . $l_child_info['icon'] . '" alt=" "><span>' . _L(
                                $l_child_title
                            ) . '</span>' . '</a></li>';
                    } // foreach
        ...
        if (count($l_links) > 0)
        {
            foreach ($l_links AS $l_title => $l_content)
            {
                ...
                $l_out[] = '<li class="' . $l_disable_class . '"><a href="' . $l_url . '" ' . $l_onclick . ' onmouseover="$(\'module-dropdown\').close_all_childs();">' . '<img class="vam module-icon" src="' . $l_icon . '" alt=" "><span>' . _L(
                        $l_title
                    ) . '</span>' . '</a></li>';
            } // foreach
        } // if

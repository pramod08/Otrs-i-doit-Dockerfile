...
class isys_ajax_handler_validate_field extends isys_ajax_handler
{
            ...
            if (!in_array($l_dao_name, $l_already_called_dao) && class_exists($l_dao_name))
            {
                ...
                foreach ($l_properties as $l_property)
                {
                    if ($l_property[C__PROPERTY__CHECK][C__PROPERTY__CHECK__MANDATORY])
                    {
                        ...
                        if ($l_property[C__PROPERTY__UI][C__PROPERTY__UI__TYPE] == C__PROPERTY__UI__TYPE__POPUP && ($l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__TYPE] == C__PROPERTY__INFO__TYPE__OBJECT_BROWSER || $l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__TYPE] == C__PROPERTY__INFO__TYPE__N2M))
                        {
                            $l_ui_id .= '__HIDDEN';
                        }
                        else if ($l_property[C__PROPERTY__UI][C__PROPERTY__UI__TYPE] == C__PROPERTY__UI__TYPE__DIALOG_LIST)
                        {
    ...
    private function get_validation_by_category($p_cat_type, $p_cat_id)
    {
        ...
        foreach ($l_properties as $l_property => $l_property_data)
        {
            ...
            {
                $l_property_names[$l_property] = _L($l_property_data[C__PROPERTY__INFO][C__PROPERTY__INFO__TITLE]);
            } // if
        ...
        else if ($p_cat_type == C__CMDB__CATEGORY__TYPE_SPECIFIC)
        {
            $l_cattype = 's';
        }
        else
        ...
        if (count($l_virtual_properties) > 0)
        {
            foreach ($l_virtual_properties AS $l_virtual_property)
            {
                if (isset($l_configuration[$l_virtual_property])) unset($l_configuration[$l_virtual_property]);

                if (isset($l_property_names[$l_virtual_property])) unset($l_property_names[$l_virtual_property]);

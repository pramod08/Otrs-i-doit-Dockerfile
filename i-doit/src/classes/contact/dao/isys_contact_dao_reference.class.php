...
class isys_contact_dao_reference extends isys_component_dao
{
    ...
    public function insert_data_item($p_id)
    {
        if ($p_id > 0)
        {
            if (!array_key_exists($p_id, $this->m_arDataItems)) $this->m_arDataItems[$p_id] = true;
        }
    ...
    {
        $l_query = "SELECT * FROM isys_obj " . "LEFT JOIN isys_contact_2_isys_obj ON isys_obj__id = isys_contact_2_isys_obj__isys_obj__id " . "WHERE isys_obj__id = " . $this->convert_sql_id(
                $p_data_item_id
            );
    ...
    {
        return count($this->m_arDataItems);
    }
    ...
    public function get_data_item_dao($p_type)
    {
        switch ($p_type)
        {
            ...
            case C__CONTACT__DATA_ITEM__PERSON_EXTERN:
    ...
    public function get_data_item_data($p_id)
    {
        ...
        switch ($l_data_item["isys_obj__isys_obj_type__id"])
        {
            case C__OBJTYPE__PERSON:
     ...
     * - Create isys_contact_data_item entries
     *
     * @param integer $p_id
     *
     * @return boolean
    ...
    public function save($p_id = null)
    {
        ...
        if (count($this->m_arDataItems) > 0)
        {
            foreach ($this->m_arDataItems as $l_item => $l_bool)
            {
                $l_q = "INSERT INTO isys_contact_2_isys_obj (isys_contact_2_isys_obj__isys_contact__id, isys_contact_2_isys_obj__isys_obj__id) " . "VALUES " . "(" . $this->convert_sql_id(
                        $p_id
                    ) . ", " . $this->convert_sql_id($l_item) . ")";

...
class isys_log
{
    ...
    {
        $l_levels = [
            self::C__DEBUG   => _L('LC_UNIVERSAL__LOG_LEVEL__DEBUG'),
            self::C__INFO    => _L('LC_UNIVERSAL__LOG_LEVEL__INFO'),
            self::C__NOTICE  => _L('LC_UNIVERSAL__LOG_LEVEL__NOTICE'),
            self::C__WARNING => _L('LC_UNIVERSAL__LOG_LEVEL__WARNING'),
            self::C__ERROR   => _L('LC_UNIVERSAL__LOG_LEVEL__ERROR'),
            self::C__FATAL   => _L('LC_UNIVERSAL__LOG_LEVEL__FATAL_ERROR')
        ];
    ...
    {
        assert(
            '$p_level == self::C__FATAL || $p_level == self::C__ERROR ||
    ...
    public function flush_verbosity($p_print = true, $p_standalone = true)
    {
        ...
        foreach ($l_verbosity as $l_timestamp => $l_msg)
        {
            ...
            $l_content .= sprintf(
                '[%s] %s: %s',
                date('Y-m-d H:i:s', $l_time),
                strtoupper($this->level_as_string($l_msg['level'])),
                $l_msg['message']
            );

...
class isys_auth_dao extends isys_component_dao
{
    ...
    public function build_path(array &$p_paths, array $p_row)
    {
        ...
        // This happenes, because we ALWAYS have the 'view' right.
        $p_paths[$l_method][$l_id][] = 1;
    ...
    public function create_paths($p_obj_id, $p_module_id, array $p_path_data = [])
    {
        ...
        foreach ($p_path_data as $l_method => $l_params)
        {
            foreach ($l_params as $l_param => $l_rights)
            {
                ...
                if ($l_param != 0 || $l_param != -1 || $l_param == isys_auth::WILDCHAR)
                {
                    ...
                    $l_inserts[] = '(' . $this->convert_sql_id($p_obj_id) . ',  ' . $this->convert_sql_int($l_rights_bitwise) . ',  ' . $this->convert_sql_id(
                            $p_module_id
                    ...
                    if ($l_param == isys_auth::WILDCHAR)
                    {
                        ...
                        $l_inserts[] = '(' . $this->convert_sql_id($p_obj_id) . ',  ' . $this->convert_sql_int($l_rights_bitwise) . ',  ' . $this->convert_sql_id(
                                $p_module_id
                            ) . ',  ' . $this->convert_sql_text(strtoupper($l_method)) . ', ' . $this->convert_sql_int(C__RECORD_STATUS__NORMAL) . ')';
                    } // if
                } // if
            } // foreach
        } // foreach
        ...
        {
            $l_sql = 'INSERT INTO isys_auth (isys_auth__isys_obj__id, isys_auth__type, isys_auth__isys_module__id, isys_auth__path, isys_auth__status) VALUES ' . implode(
                    ', ',
                    $l_inserts
                ) . ';';
            if ($this->update($l_sql) && $this->apply_update())
    ...
    public function get_all_persons()
    {
        $l_sql = 'SELECT DISTINCT(isys_obj__id) FROM isys_auth
			...
			WHERE isys_obj__isys_obj_type__id = ' . $this->convert_sql_id(C__OBJTYPE__PERSON) . ';';
    ...
    {
        //assert('is_numeric($p_from_object_id) && $p_from_object_id >= 0');
        //assert('is_numeric($p_to_object_id) && $p_to_object_id >= 0');
        ...
        try
        {
            ...
            if ($l_res->num_rows() > 0)
            {
                ...
                {
                    $l_paths[]  = $l_row;
                    $l_values[] = '(' . $this->convert_sql_id($p_to_object_id) . ', ' . $this->convert_sql_int($l_row['isys_auth__type']) . ', ' . $this->convert_sql_int(
                            $l_row['isys_auth__isys_module__id']
                        ) . ', ' . $this->convert_sql_text($l_row['isys_auth__path']) . ', ' . $this->convert_sql_int($l_row['isys_auth__status']) . ')';
                } // while

...
class isys_export_cmdb_object extends isys_export_cmdb
{
    ...
    private static $m_cat_skip = [
        C__CMDB__CATEGORY__TYPE_GLOBAL   => [
            C__CATG__OVERVIEW                      => true,
            C__CATG__VIRTUAL                       => true,
            C__CATG__CABLING                       => true,
            //C__CATG__OBJECT => true,
            C__CATG__CLUSTER_ROOT                  => true,
            C__CATG__VIRTUAL_HOST_ROOT             => true,
            C__CATG__VIRTUAL_MACHINE__ROOT         => true,
            C__CATG__NETWORK                       => true,
            C__CMDB__SUBCAT__NETWORK_PORT_OVERVIEW => true,
            C__CATG__LOGBOOK                       => true,
            C__CATG__NAGIOS_HOST_FOLDER            => true,
            C__CATG__NAGIOS_HOST_TPL_FOLDER        => true,
            C__CATG__NAGIOS_SERVICE_FOLDER         => true,
            C__CATG__NAGIOS_SERVICE_TPL_FOLDER     => true,
            C__CATG__CMK_FOLDER                    => true
        ],
        C__CMDB__CATEGORY__TYPE_SPECIFIC => [//C__CMDB__SUBCAT__FILE_ACTUAL => true,
    ...
    {
        if (is_int($p_category_type)) return self::$m_cat_skip[$p_category_type];
        else return self::$m_cat_skip;
    } // function
     ...
     * @return  boolean
     */
    public static function is_catg_exportable($p_cat_data_information)
    {
    ...
    private static function fetch_exportable_category_data_information($p_category_type)
    {
        ...
        while ($l_row = $l_res->get_row())
        {
            ...
            if (!is_array($l_properties))
            {
                ...
                //file_put_contents($g_dirs['temp'] . 'misformed_properties.txt', $l_row[$l_class_name].";\n", FILE_APPEND);
     ...
     */
    public function add_skip($p_category_id, $p_type = C__CMDB__CATEGORY__TYPE_GLOBAL)
    {
    ...
    public function export($p_object_ids, $p_categories = [], $p_record_status = C__RECORD_STATUS__NORMAL, $p_duplicate = false, $p_merge = false)
    {
        try
        {
            ...
            else if (is_numeric($p_object_ids))
            {
                $l_object_ids[] = $p_object_ids;
            }
            else if (is_string($p_object_ids))
            {
                $l_object_ids = explode(',', $p_object_ids);
            } // if

            // Prepare categories:
            if (count($p_categories) <= 0 || !is_array($p_categories))
            {
                $p_categories = [
                    C__CMDB__CATEGORY__TYPE_GLOBAL   => [],
                    C__CMDB__CATEGORY__TYPE_SPECIFIC => [],
            ...
            foreach ($l_object_ids as $l_object_id)
            {
                ...
                if (!is_numeric($l_object_id) || $l_object_id <= 0)
                {
                    throw new isys_exception_general(
                        sprintf(
                            'Object identifier is invalid. (value: %s)',
                            $l_object_id
                        )
                    );
                } // id
                ...
                $this->m_export[$l_object_id] = $this->export_categories(
                    $l_object_id,
                    $p_categories,
                    $p_record_status,
                    $p_duplicate,
    ...
    public function export_properties($p_object_properties, $p_row, $p_cattype, $p_disable_provides = false)
    {
        ...
        foreach ($p_object_properties as $l_tag => $l_property)
        {
            ...
            else if (!isset($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK]))
            {
                // No helper class.
            ...
            else if (isset($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK]))
            {
                ...
                if (class_exists($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0]))
                {
                    ...
                    $l_helper = new $l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0](
                        $p_row, $this->m_database, $l_property[C__PROPERTY__DATA], $l_property[C__PROPERTY__FORMAT], $l_property[C__PROPERTY__UI]
                    );
                    ...
                    if (method_exists($l_helper, $l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][1]))
                    {
                        ...
                        )
                        {
                            $l_new['value'] = call_user_func(
                                ...
                                $p_row[$l_property[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD_ALIAS]]
                            );
                        }
                        else
                        {
                            $l_new['value'] = call_user_func(
                                ...
                                $p_row[$l_property[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD]]
                            );
                        } // if
                    }
                    else
                        ...
                        if (is_numeric($l_cattype))
                        {
                            foreach ($l_categories as $l_catid => $l_catdata)
                            {
                                foreach ($l_catdata as $l_catentryID => $l_catentry)
                                {
                                    ...
                                    {
                                        if ($l_catid === C__CATG__GLOBAL)
                                        {
                                            foreach ($l_catentry AS $l_key => $l_property)
                                            {
                                                if ($l_property[C__DATA__TAG] == 'cmdb_status') continue;
    ...
    private function export_categories($p_object_id, $p_categories, $p_record_status = C__RECORD_STATUS__NORMAL, $p_duplicate = false, $p_merge = false)
    {
        ...
        $l_output['head'] = [
            ...
            'type'        => [
                ...
                'title'        => _L($l_otrow['isys_obj_type__title']),
                 ...
                 ] as $l_cattype)
        {
            ...
            try
            {
                ...
                {
                    $l_all_categories = $l_cat->get_categories_as_array(
                        $l_object_type,
                        $p_object_id,
                        $l_cattype,
                    ...
                    foreach ($l_durables as $l_durable)
                    {
                        if (class_exists($l_durable['isysgui_catg__class_name']))
                        {
                            $l_all_categories[$l_durable['isysgui_catg__id']] = [
                                ...
                                'title'        => _L($l_durable['isysgui_catg__title']),
                ...
                elseif ($l_cattype == C__CMDB__CATEGORY__TYPE_CUSTOM)
                {
                    ...
                    $l_all_categories = $l_cat->get_custom_categories_as_array(
                        $l_object_type,
                        $p_object_id,
                        $l_cattype,
                        C__RECORD_STATUS__NORMAL,
            ...
            if (is_array($l_all_categories) && count($l_all_categories))
            {
                foreach ($l_all_categories as $l_category_id => $l_data)
                {
                    ...
                    if (count($p_categories[$l_cattype]) !== 0)
                    {
                        ...
                        else if ($l_cattype == C__CMDB__CATEGORY__TYPE_GLOBAL && $p_categories && !isset($p_categories[$l_cattype][$l_category_id]))
                        {
                            continue;
                        } // if
                    }
                    ...
                    $l_dataresult = $l_category_dao->get_data(
                        null,
                        $p_object_id,
                        $l_condition,
                        null,
                    ...
                    if ($l_cattype == C__CMDB__CATEGORY__TYPE_CUSTOM)
                    {
                        ...
                        {
                            $l_prop_export                                        = $this->export_properties(
                                $l_category_data_information,
                                $l_merge_test,
                                $l_cattype
                            );
                    ...
                    else
                    {
                        if (empty($l_table))
                        {
                            $l_table = (!strpos($l_data['source_table'], '_list') && !strpos(
                                    $l_data['source_table'],
                                    '_2_'
                                )) ? $l_data['source_table'] . "_list" : $l_data['source_table'];
                        } // if
        ...
        if($l_has_operating_system)
        {
            // Remove operating system from software assignment otherwise the entry will also be created in mass-template
            $l_os_data = $l_output[C__CMDB__CATEGORY__TYPE_GLOBAL][C__CATG__OPERATING_SYSTEM];
            unset($l_os_data['head']);
            $l_data_id = key($l_os_data);
            if(isset($l_output[C__CMDB__CATEGORY__TYPE_GLOBAL][C__CATG__APPLICATION][$l_data_id]))
            {
                unset($l_output[C__CMDB__CATEGORY__TYPE_GLOBAL][C__CATG__APPLICATION][$l_data_id]);
            } // if
        } // if

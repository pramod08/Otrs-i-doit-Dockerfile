...
class isys_export_type_xml extends isys_export_type
{
                ...
                if (is_object($l_val) && method_exists($l_val, "get_data"))
                {
                    ...
                    else
                    {
                        if (is_array($l_data[0]) && count($l_data[0]) > 0)
                        {
                            ...
                            {
                                if ($l_dkey == "title") $l_dtitle = $l_dval;

                                if (!empty($l_dval) && $l_dkey != C__CATEGORY_DATA__METHOD && $l_dkey != C__CATEGORY_DATA__HELPER) $l_xml .= " " . $l_dkey . "=\"" . htmlspecialchars(
                                        $l_dval
                                    ) . "\"";
                            }
                            $l_xml .= " >";
    ...
    public function parse($p_array, $p_export_format = null, $p_translate = false)
    {
        ...
        if (!is_array($p_array))
        {
            throw new isys_exception_general(
                'Input not an array. (isys_export_type_xml->parse())'
            );
        }
        ...
        $l_xml .= "\t<head>\n" . "\t\t<datetime>" . date("Y-m-d H:i:s") . "</datetime>\n" . "\t\t<mandator language=\"" . htmlentities(
                $g_comp_session->get_language()
            ) . "\" id=\"" . htmlentities($g_comp_session->get_current_mandator_as_id()) . "\">" . $this->cdata(
                $g_comp_session->get_mandator_name()
        ...
        {
            if (!isset($l_array["head"])) continue;
            ...
            $l_xml .= "\t\t\t<id>" . $l_value["id"] . "</id>\n" . "\t\t\t<title>" . $l_title . "</title>\n" . "\t\t\t<sysid>" . $l_sysid . "</sysid>\n" . "\t\t\t<created by=\"" . $l_value["created_by"] . "\">" . $l_value["created"] . "</created>\n" . "\t\t\t<updated by=\"" . $l_value["updated_by"] . "\">" . $l_value["updated"] . "</updated>\n" . "\t\t\t<type " . "id=\"" . $l_value["type"]["id"] . "\" " . "const=\"" . $l_value["type"]["const"] . "\" " . "title_lang=\"" . $l_value["type"]["title_lang"] . "\" " . "group=\"" . $l_value["type"]["group"] . "\" " . "sysid_prefix=\"" . $l_value["type"]["sysid_prefix"] . "\">" . $l_type_title . "</type>\n" . "\t\t\t<status lc_title='" . $this->get_status_lc(
                    $l_value["status"]
                ) . "'>" . $l_value["status"] . "</status>\n" . "\t\t\t<cmdb_status>" . $l_value["cmdb_status"] . "</cmdb_status>\n" . "\t\t\t<description>" . $l_description . "</description>\n" . "\n";
            ...
            if (is_array($l_cathead) && count($l_cathead) > 0)
            {
                foreach ($l_cathead AS $l_cathead_key => $l_cathead_value)
                {
                    if ($l_cathead_key == 'title')
                    {
                        if ($p_translate === true)
                        {
                            $l_cathead_value = _L($l_cathead_value);
                        }
                    }
            ...
            foreach ($l_catdata as $l_category_id => $l_properties)
            {
                ...
                foreach ($l_properties as $l_property)
                {
                    ...
                    if (isset($l_property[C__DATA__TITLE]))
                    {
                        ...
                        if ($p_translate === true)
                        {
                            $l_title = $g_comp_template_language_manager->get($l_title);
                        }
                    }
                        ...
                        case 'object':
                            ...
                            switch (get_class($l_property[C__DATA__VALUE]))
                            {
                                case 'isys_export_data':
                                    ...
                                    if (is_array($l_data) && count($l_data) > 0)
                                    {
                                        foreach ($l_data as $l_key => $l_item)
                                        {
                                            if (is_numeric($l_key))
                                            {
                                                ...
                                                if (!$l_parent) $l_xml .= "\t\t\t\t\t\t" . "<" . $l_property[C__DATA__TAG] . " title=\"" . htmlspecialchars(
                                                        $l_title
                                                    ) . "\">\n";
                                    ...
                                    if ($l_parent) $l_xml .= "\t\t\t\t\t\t" . "</" . $l_property[C__DATA__TAG] . ">\n";
            ...
            foreach ($p_data as $l_ptag => $l_pval)
            {
                ...
                if (!is_array(($l_pval)))
                {
                    ...
                    if (!is_null($l_pval) && $l_pval != "")
                    {
                        ...
                        {
                            if ($l_ptag == "id" && $p_object_status == C__RECORD_STATUS__TEMPLATE) continue;
                            $l_xml .= $p_tabs;
                ...
                else
                {
                    ...
                    if (isset($l_pval["attributes"]))
                    {
                        ...
                        {
                            if ($l_akey == "id" && $p_object_status == C__RECORD_STATUS__TEMPLATE) continue;
    ...
    private function get_status_lc($p_const)
    {
        ...
        {
            case 1:
                $l_lc = 'LC__CMDB__RECORD_STATUS__BIRTH';
                break;
            case 2:
                $l_lc = 'LC__CMDB__RECORD_STATUS__NORMAL';
                break;
            case 3:
                $l_lc = 'LC__CMDB__RECORD_STATUS__ARCHIVED';
                break;
            case 4:
                $l_lc = 'LC__RECORD_STATUS__Deleted';
                break;
            default:
                $l_lc = 'LC__CMDB__RECORD_STATUS__NORMAL';
        }

        return $g_comp_template_language_manager->get($l_lc);
    }
    ...
    {
        if (!is_null($p_encoding)) $this->m_encoding = $p_encoding;
    }
}

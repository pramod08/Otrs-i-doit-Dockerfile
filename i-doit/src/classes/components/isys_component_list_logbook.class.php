...
class isys_component_list_logbook
{
     ...
     * @global $g_comp_template_language_manager
     * @author  Niclas Potthast <npotthast@i-doit.de> - 2005-11-29
     * @version Niclas Potthast <npotthast@i-doit.de> - 2007-04-13
     * @desc    return the html table
     */
    public function getTempTableHtml($p_filter = null)
    {
        ...
        $l_strRet      = "";
        $l_nRowCounter = 0;
        $l_objTemplate = new isys_component_template($g_comp_template_language_manager);
        $l_objDAORes   = $this->getTableResult($p_filter);
        $l_bOrderLink  = $this->m_bOrderLink;

        $l_dao_logbook = new isys_component_dao_logbook($g_comp_database);

        if (!$l_objDAORes)
        {
            throw new isys_exception_general(
                get_class($this) . ": getTempTableHtml doesn't have any results."
            );
        }

        $l_navbar->set_page_results($l_objDAORes->num_rows());
        $l_navbar->set_active(true, C__NAVBAR_BUTTON__FORWARD)
        ...
        if ($this->m_arTableHeader && ($l_objDAORes->num_rows() != 0))
        {
            ...
            foreach ($this->m_arTableHeader as $l_key => $value)
            {
                ...
                if ($this->isTableColumn($l_key))
                {
                    ...
                    if ($l_bOrderLink)
                    {
                        $l_strRet .= "\t\t<th " . "title=\"" . $l_strTooltip . "\" " . "onClick=" . "\"";

                        $l_strRet .= "document.isys_form.dir.value=" . "'" . isys_glob_get_order(
                            ) . "'; " . "document.isys_form.sort.value=" . "'" . $l_key . "'; " . "document.isys_form.submit();" . "\"" . ">";
                        ...
                        if ($l_sort == $l_key)
                        {
                            ...
                            $l_strRet .= " <img class=\"fr\" style=\"margin-top:8px;margin-right:-2px;\" src=\"" . $g_dirs["images"] . "/list/" . strtolower(
                                    $l_dir
                                ) . ".gif\" />";
                        }
        ...
        if ($l_objDAORes->num_rows() == 0)
        {
            ...
            $l_strTemp = $g_comp_template_language_manager->get("LC__CMDB__FILTER__NOTHING_FOUND_STD");
        }
        else
        {
            while ($l_row = $l_objDAORes->get_row(IDOIT_C__DAO_RESULT_TYPE_ALL))
            {
                ...
                $l_strRet .= "\t<tr " . "class=\"" . $l_objTemplate->row_background_color(
                        $l_nRowCounter
                    ) . "\" " . "onmouseover=\"this.style.cursor='pointer'; this.className='" . $l_objTemplate->row_background_color(
                        $l_nRowCounter
                    ) . "Highlight';\" " . "onmouseout=\"this.className='" . $l_objTemplate->row_background_color(
                        $l_nRowCounter
                    ) . "';\" " . "onclick=\"this.className='" . $l_objTemplate->row_background_color($l_nRowCounter) . "Highlight';";

                $l_strRet .= "\" style=\"border-top: 1px solid #31ACC2;\">\n";
                ...
                $l_row['isys_logbook_level__title']  = _L($l_row['isys_logbook_level__title']);
                $l_row['isys_logbook_source__title'] = _L($l_row['isys_logbook_source__title']);

                if (isset($l_row['isys_logbook__obj_type_static']))
                {
                    $l_row['isys_logbook__obj_type_static'] = _L($l_row['isys_logbook__obj_type_static']);
                } // if
                ...
                foreach ($this->m_arTableHeader as $l_key => $value)
                {
                    ...
                    if ($l_key == "+")
                    {
                        if ($_POST["filter_archive"] == "1") $l_archive = "true";
                        else $l_archive = "false";

                        $l_strRet .= "\t\t<td class=\"logexpand\" onclick=\"expandEntry(" . $l_row["isys_logbook__id"] . ", {$l_archive}, '" . C__MODULE__LOGBOOK . "');";
                    }
                    else
                    ...
                    if ($l_key == "+")
                    {
                        $l_strRet .= " class=\"center\" id=\"ec" . $l_row["isys_logbook__id"] . "\">+</td>\n";
                    }
                    else
                        $l_strRet .= ">" . $l_strTablecellContent . "</td>\n";
                }
                $l_strRet .= "\t</tr>\n";
                $l_strRet .= "<tr id=\"tr" . $l_row["isys_logbook__id"] . "\" style=\"display:none;\" class=\"" . $l_objTemplate->row_background_color(
                        $l_nRowCounter
                    ) . "\">" . "<td></td>" . "<td colspan=\"5\">" . "<div id=\"logb" . $l_row["isys_logbook__id"] . "\" " . "onclick=\"collapseEntry(" . $l_row["isys_logbook__id"] . ");\">" . "</div>" . "</td>" . "</tr>";
     ...
     * @author  Dennis Blümer <dbluemer@i-doit.org>
     */
    public function getGroupedTableHtml($p_groupRow)
    {
        global $g_comp_template_language_manager, $g_comp_database, $g_dirs;

        $l_strRet      = "";
        $l_nRowCounter = 0;
        $l_objTemplate = new isys_component_template($g_comp_template_language_manager);
        $l_objDAORes   = $this->getTableResult(null);
        $l_bOrderLink  = $this->m_bOrderLink;
        ...
        {
            if (empty($l_row[$p_groupRow])) $l_groupedResult[$g_comp_template_language_manager->get("LC__CMDB__OBJTYPE_GROUP__OTHER")][] = $l_row;
            else
                $l_groupedResult[$l_row[$p_groupRow]][] = $l_row;
        }
            ...
            foreach ($this->m_arTableHeader as $l_key => $value)
            {
                ...
                if ($this->isTableColumn($l_key))
                {
                    ...
                    if ($l_bOrderLink)
                    {
                        ...
                        $l_javascript = "document.isys_form.dir.value='" . isys_glob_get_order(
                            ) . "'; document.isys_form.sort.value='" . $l_key . "'; document.isys_form.submit();";
        ...
        if ($l_objDAORes->num_rows() == 0)
        {
            $l_strRet .= '<tr><td>' . _L("LC__CMDB__FILTER__NOTHING_FOUND_STD") . '</td></tr>';
        }
        else
        {
            foreach ($l_groupedResult as $l_key => $l_group)
            {
                $l_strRet .= '<tr class="bold"><td>' . $l_key . '</td></tr>';
                ...
                foreach ($l_group as $l_row)
                {
                    ...
                    foreach ($this->m_arTableHeader as $l_key => $value)
                    {
                        ...
                        if ($l_key == "+")
                        {
                            $l_strRet .= " style=\"text-align:center;\" id=\"ec" . $l_row["isys_logbook__id"] . "\">+</td>\n";
                        }
                        else
                        {
                            $l_strRet .= ">" . $l_strTablecellContent . "</td>\n";
                    ...
                    $l_strRet .= "<tr id=\"tr" . $l_row["isys_logbook__id"] . "\" style=\"display:none;\" class=\"" . $l_objTemplate->row_background_color(
                            $l_nRowCounter
                        ) . "\">" . "<td></td>" . "<td>" . "<div id=\"logb" . $l_row["isys_logbook__id"] . "\" " . "onmouseover=\"this.style.cursor='pointer';\" " . "onclick=\"collapseEntry(" . $l_row["isys_logbook__id"] . ");\"></div></td>" . "<td></td><td></td></tr>";
     ...
     *         the maximal count of values to be translated is 20.
     */
    protected function replaceLinkValues(&$p_strString, $p_arRow)
    {
        ...
        while ($l_nBegin !== false)
        {
            ...
            //replace "[{...}]" with value from p_arRow
            //get value to be translated
            $l_strTranslate = substr(
                $p_strString,
                $l_nBegin + 2,
                ($l_nEnd - 2) - $l_nBegin
            );
    ...
    protected function get_dialog_table_as_case($p_table, $p_ref_field, $p_as_ref_field)
    {
        ...
        if ($l_res->num_rows() > 0)
        {
            ...
            {
                $l_case .= ' WHEN ' . $l_row[$p_table . '__id'] . ' THEN ' . $this->m_listdao->convert_sql_text($l_row[$p_table . '__title']);
            } // while
    ...
    protected function getTableResult($p_filter)
    {
        ...
        $l_logbook_level_case  = $this->get_dialog_table_as_case('isys_logbook_level', 'isys_logbook__isys_logbook_level__id', 'isys_logbook_level__title');
        $l_logbook_event_case  = $this->get_dialog_table_as_case('isys_logbook_event', 'isys_logbook__isys_logbook_event__id', 'isys_logbook_event__title');
        $l_logbook_source_case = $this->get_dialog_table_as_case('isys_logbook_source', 'isys_logbook__isys_logbook_source__id', 'isys_logbook_source__title');

        $l_strSQL = "SELECT SQL_CALC_FOUND_ROWS isys_logbook.* " .
            (strlen($l_logbook_level_case) > 0 ? ',' . $l_logbook_level_case : '') .
            (strlen($l_logbook_event_case) > 0 ? ',' . $l_logbook_event_case : '') .
            (strlen($l_logbook_source_case) > 0 ? ',' . $l_logbook_source_case : '') .
            " FROM " . $this->m_strTempTableName . " ";
        ...
        $l_strSQL .= 'INNER JOIN isys_catg_logb_list ON isys_catg_logb_list__isys_logbook__id = isys_logbook__id ';

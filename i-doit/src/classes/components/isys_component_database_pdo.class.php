...
class isys_component_database_pdo extends isys_component_database
{
    ...
    public function query($p_query, $p_unbuffered = false)
    {
        ...
        {
            isys_application::instance()->container['notify']->emergency($this->escape_string($e->getMessage()));
    ...
    public function reconnect()
    {
        ...
        try
        {
            $l_dsn = $this->m_dbType . ":host=" . $this->m_host . ";port=" . $this->m_port . ";dbname=" . $this->m_db_name . ";user=" . $this->m_user . ";password=" . $this->m_pass;

            if ($l_dsn) $this->m_db_link = new PDO($l_dsn);
            else return false;
    ...
    public function __construct($p_dbType, $p_host, $p_port, $p_user, $p_password, $p_databasename, $p_dsn = '')
    {
        try
        {
            ...
            $this->m_db_link = new PDO(
                $l_dsn, $p_user, $p_password, [
                    PDO::ATTR_TIMEOUT => isys_settings::get('system.pdo.pgsql.timeout', 5),

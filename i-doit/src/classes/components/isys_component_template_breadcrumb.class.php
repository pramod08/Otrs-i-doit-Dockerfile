...
class isys_component_template_breadcrumb extends isys_component_template
{
    ...
    public function process($p_plain = false, $p_append = null, $l_module_id = null, $p_prepend = null, $p_linkClass = null)
    {
        ...
        {
            $l_out = '<li class="home"><a href="' . isys_application::instance(
                )->www_path . '" title="Home"><img src="' . $g_dirs['images'] . 'home.png"  height="12" alt="i-doit" /></a></li>';
        } // if
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        $l_out .= $this->build_link(
                                            isys_glob_build_url($l_current_url),
                                            $l_bc_title
                                        );
                                    } // if
                                } // if
    ...
    {
        global $g_comp_template_language_manager, $g_config, $g_strMandatorName, $g_db_system;

        $l_strOut = "";
        $l_arNavi = [];

        $l_strNaviLink = $this->build_link(
            $g_config["www_dir"],
            $g_comp_template_language_manager->get($g_strMandatorName),
            $g_comp_template_language_manager->get('LC__NAVIGATION__BREADCRUMB__BACK_TO_MAIN_VIEW')
        );
    ...
    private function build_link($p_strURL, $p_strTitle, $p_strTooltip = '', $p_linkClass = '')
    {
        ...
        if (!empty($p_strTitle))
        {
            $p_strTitle = $g_comp_template_language_manager->get(html_entity_decode(stripslashes($p_strTitle), null, $GLOBALS['g_config']['html-encoding']));
        } // if

        if (empty($p_strTitle))
        {
            $p_strTitle = $g_comp_template_language_manager->{'LC__NAVIGATION__BREADCRUMB__NO_TITLE'};
        } // if

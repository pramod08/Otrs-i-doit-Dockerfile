...
class isys_component_dao_object_table_list extends isys_component_dao
{
    ...
    public function get_result($p_strTableName = null, $p_object_id, $p_cRecStatus = null)
    {
        ...
        else throw new isys_exception_dao_cmdb("isys_component_dao_object_table_list::\$m_cat_dao undefined.");
    }
    ...
    public function set_rec_status($p_cRecStatus)
    {
        ...
        $g_comp_template->smarty_tom_add_rule("tom.content.navbar.cRecStatus.p_bDisabled=0");
        $g_comp_template->smarty_tom_add_rule("tom.content.navbar.cRecStatus.p_arData=" . serialize($this->get_rec_array()));
        $g_comp_template->smarty_tom_add_rule("tom.content.navbar.cRecStatus.p_strSelectedID=" . $this->m_cRecStatus);
    ...
    public function get_rec_array()
    {
        ...
        $l_arData[C__RECORD_STATUS__NORMAL] = $g_comp_template_language_manager->{"LC__CMDB__RECORD_STATUS__NORMAL"} . ($l_cRecCounts[C__RECORD_STATUS__NORMAL] !== null ? " (" . $l_cRecCounts[C__RECORD_STATUS__NORMAL] . ")" : '');

        $l_arData[C__RECORD_STATUS__ARCHIVED] = $g_comp_template_language_manager->{"LC__CMDB__RECORD_STATUS__ARCHIVED"} . ($l_cRecCounts[C__RECORD_STATUS__ARCHIVED] !== null ? " (" . $l_cRecCounts[C__RECORD_STATUS__ARCHIVED] . ")" : '');

        $l_arData[C__RECORD_STATUS__DELETED] = $g_comp_template_language_manager->{"LC__CMDB__RECORD_STATUS__DELETED"} . ($l_cRecCounts[C__RECORD_STATUS__DELETED] !== null ? " (" . $l_cRecCounts[C__RECORD_STATUS__DELETED] . ")" : '');

        if (defined(
                "C__TEMPLATE__STATUS"
            ) && C__TEMPLATE__STATUS == 1
        ) $l_arData[C__RECORD_STATUS__TEMPLATE] = 'Template' . ($l_cRecCounts[C__RECORD_STATUS__TEMPLATE] !== null ? " (" . $l_cRecCounts[C__RECORD_STATUS__TEMPLATE] . ")" : '');
    ...
    public function __construct($p_object)
    {
        ...
        else if ($p_object instanceof isys_component_database)
        {
        ...
        else
        {
            throw new isys_exception_general(
                "\$p_object invalid. (" . get_class($p_object) . ") - Use isys_component_database or isys_cmdb_dao_category.  " . __FILE__ . ":" . __LINE__
            );
        }

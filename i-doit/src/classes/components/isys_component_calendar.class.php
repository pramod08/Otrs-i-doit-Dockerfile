...
class isys_component_calendar extends isys_component
{
    ...
    public function add_holidays($p_year = null)
    {
        ...
        return $this->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__NEW_YEAR'), 1, 1))
            ->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__VALENTINES_DAY'), 14, 2))
            ->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__SAINT_PATRICKS_DAY'), 17, 3))
            ->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__EASTER'), $l_day, $l_month, $l_year))
            ->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__HALLOWEEN'), 31, 10))
            ->add_event(isys_component_calendar_event::factory(_L('LC__CALENDAR_EVENT__CHRISTMAS'), 24, 12));
    } // function
    ...
    public function render($p_render_template = true)
    {
        ...
        if ($p_render_template)
        {
            ...
            return $g_comp_template->assign('year', $this->m_options['year'])
    ...
    protected function retrieve_day_data($p_timestamp)
    {
        ...
        return [
            ...
            'events'      => (count($l_date_event) > 0) ? $l_date_event : false,
            'events_json' => (count($l_date_event) > 0) ? isys_format_json::encode($l_date_event) : false
        ];
    } // function

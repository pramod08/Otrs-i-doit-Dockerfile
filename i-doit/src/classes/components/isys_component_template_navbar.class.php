...
class isys_component_template_navbar
{
    ...
    public function show_navbar()
    {
        ...
        if (self::$m_locked === true) return;

        $l_pages = [];

        $this->assign_current_url();

        $l_pagestart = isys_glob_get_param("navPageStart");
        if ($l_pagestart < $this->m_nav_page_count)
        {
            $l_current_page = ceil($l_pagestart / isys_glob_get_pagelimit()) + 1;
        }
        else
        {
            $l_current_page = 1;
        ...
        $l_arConstantString = [
            ...
            "C__NAVBAR_BUTTON__NEW"           => C__RS__RIGHT_NEW,
            "C__NAVBAR_BUTTON__EDIT"          => C__RS__RIGHT_EDIT,
            "C__NAVBAR_BUTTON__DUPLICATE"     => C__RS__RIGHT_NEW,
            "C__NAVBAR_BUTTON__ARCHIVE"       => C__RS__RIGHT_ARCHIVE,
            "C__NAVBAR_BUTTON__DELETE"        => C__RS__RIGHT_DELETE,
            "C__NAVBAR_BUTTON__PURGE"         => C__RS__RIGHT_PURGE,
            "C__NAVBAR_BUTTON__RECYCLE"       => C__RS__RIGHT_RECYCLE,
            "C__NAVBAR_BUTTON__QUICK_PURGE"   => C__RS__RIGHT_PURGE,
        ...
        $g_comp_template->assign('navbar_buttons', $l_navbar_buttons);
        ...
        {
            $g_comp_template->assign("pages", $l_pages)
    ...
    {
        global $g_dirs, $g_ajax_calls, $g_comp_database;

        $l_nModuleID = C__MODULE__CMDB;
        $l_bRight    = true;
        ...
        if (is_numeric($p_right))
        {
            ...
            if (isset($_GET[C__GET__SETTINGS_PAGE]) && $_GET[C__GET__SETTINGS_PAGE] == 'login' && $_GET[C__GET__MODULE_ID] == C__MODULE__SYSTEM && $_GET[C__GET__MODULE_SUB_ID] == C__MODULE__USER_SETTINGS)
            {
                $l_bRight = true;
            }
            else
        ...
        if ($this->m_visible[$p_cNavButton])
        {
            ...
            if ($this->m_active[$p_cNavButton] == false)
            {
            ...
            else
            {
                ...
                switch ($l_cNavMode)
                {
                    ...
                    case C__NAVMODE__BACK:
                        if ($l_cNavMode == C__NAVMODE__BACK) $l_element_id .= 'C__NAVMODE__BACK';

                        $l_strJS = "onclick=\"" . $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.navMode.value='$l_cNavMode';change_page('{$l_strValNavPageStart}'";
                    ...
                    case C__NAVMODE__PRINT:
                        ...
                        else if ($_GET[C__CMDB__GET__CATS])
                        {
                            $l_res      = $l_dao->get_cats_by_const($_GET[C__CMDB__GET__CATS]);
                            $l_cat_spec = "s";
                        }
                        else if ($_GET[C__CMDB__GET__OBJECTTYPE])
                        {
                        }
                        else return;

                        if ($l_cat_spec != "" && $_GET[C__CMDB__GET__CATG] != C__CATG__OVERVIEW)
                        {
                            $l_row = $l_res->get_row();

                            if (!class_exists($l_row['isysgui_cat' . $l_cat_spec . '__class_name'])) return;

                            $l_cat = new $l_row['isysgui_cat' . $l_cat_spec . '__class_name']($g_comp_database);

                            if (!empty($_GET[C__CMDB__GET__CATG_CUSTOM]))
                            {
                                if (method_exists($l_cat, 'set_catg_custom_id')) $l_cat->set_catg_custom_id($_GET[C__CMDB__GET__CATG_CUSTOM]);
                            }
                        ...
                        $l_strLoc = C__GET__MODULE_ID . "=" . C__MODULE__EXPORT;
                        if ($_GET[C__CMDB__GET__OBJECTTYPE])
                        {
                            ...
                            $l_strLoc .= "&filter='+get_current_filter()+'";

                            if (!$_GET[C__CMDB__GET__CATG] && !$_GET[C__CMDB__GET__CATS]) $l_strLoc .= "&data='+id_collector()+'";
                        }
                        ...
                        $l_strJS = "onclick=\"isys_popup_open('" . isys_glob_build_url(
                                $l_strLoc
                    ...
                    case C__NAVMODE__NEW:
                        ...
                        $l_strJS = "style=\"\" " . "onclick=\"" . (empty($this->m_js_onclick[$p_cNavButton]) ? $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.navMode.value='$l_cNavMode'; document.isys_form.submit();" . $this->m_add_onclick[$p_cNavButton] : $this->m_js_onclick[$p_cNavButton]) . "\"";

                        if (defined("C__MODULE__TEMPLATES") && isset($_GET[C__CMDB__GET__OBJECTTYPE]) && !isset($_GET[C__CMDB__GET__OBJECT]))
                        {
                            $l_add .= "<a style=\"border-left:1px dotted #888;padding-left:3px;\" class=" . $l_element_class . " href=\"javascript:\" onclick=\"new Effect.toggle(this.next('div'), 'slide', {duration:0.2});\">" . "<img src=\"" . $g_dirs['images'] . "icons/silk/bullet_arrow_down.png\" />" . "</a>" . "<div id=\"new_overlay\" style=\"display:none;\">" . "<ul>" . "<li>" . "<a href=\"javascript:\" onclick=\"" . "document.isys_form.navMode.value='$l_cNavMode'; document.isys_form.submit();" . "\">" . "<img src=\"" . $l_strIcon . "\" width=\"15px\" height=\"15px\" alt=\"\" /> " . _L(
                                    "LC__TEMPLATES__NEW_OBJECT"
                                ) . "</a>" . "</li>" . "<li>" . "<a href=\"?" . C__GET__MODULE_ID . "=" . C__MODULE__TEMPLATES . "&" . C__GET__SETTINGS_PAGE . "=3&" . C__CMDB__GET__OBJECTTYPE . "=" . $_GET[C__CMDB__GET__OBJECTTYPE] . "\">" . "<img src=\"" . $l_strImagesDir . $this->m_icon[C__NAVBAR_BUTTON__EDIT] . "\" width=\"15px\" height=\"15px\" alt=\"\" /> " . _L(
                                    "LC__TEMPLATES__NEW_OBJECT_FROM_TEMPLATE"
                                ) . "</a>" . "</li>" . "<li>" . "<a href=\"javascript:\" onclick=\"" . "document.isys_form.template.value='1'; document.isys_form.navMode.value='$l_cNavMode'; document.isys_form.submit();" . "\">" . "<img src=\"" . $l_strImagesDir . $this->m_icon[C__NAVBAR_BUTTON__DUPLICATE] . "\" width=\"15px\" height=\"15px\" alt=\"\" /> " . _L(
                                    "LC__TEMPLATES__NEW_TEMPLATE"
                                ) . "</a>" . "</li>" . "</ul>" . "</div>";
                        }
                    ...
                    case C__NAVMODE__EDIT:
                        ...
                        {
                            $l_strJS = "onclick=\"" . $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.sort.value=''; document.isys_form.navMode.value='$l_cNavMode'; form_submit(); " . $this->m_add_onclick[$p_cNavButton] . "\"";
                        }
                        else
                        {
                    ...
                    case C__NAVMODE__DELETE:
                        if ($l_cNavMode == C__NAVMODE__DELETE) $l_element_id .= 'C__NAVMODE__DELETE';
                    case C__NAVMODE__RECYCLE:
                        if ($l_cNavMode == C__NAVMODE__RECYCLE) $l_element_id .= 'C__NAVMODE__RECYCLE';
                    case C__NAVMODE__COMPLETE:
                        if ($l_cNavMode == C__NAVMODE__COMPLETE) $l_element_id .= 'C__NAVMODE__COMPLETE';
                        $l_strJS = "onclick=\"" . (empty($this->m_js_onclick[$p_cNavButton]) ? $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.navMode.value='$l_cNavMode'; form_submit(null, null, null, null, null, get_listSelection4Submit());" . $this->m_add_onclick[$p_cNavButton] : $this->m_js_onclick[$p_cNavButton]) . "\"";
                        break;
                    ...
                    case C__NAVMODE__PURGE:
                        ...
                        {
                            $l_submit = "replace_listSelection();$('isys_form').submit();";
                        } // if

                        // We only want the new "confirmation" for objects, so we look if catgID and catsID are empty.
                        if ($_GET[C__CMDB__GET__VIEWMODE] == C__CMDB__VIEW__LIST_OBJECT || empty($_GET[C__CMDB__GET__CATG]) && empty($_GET[C__CMDB__GET__CATS]) && empty($_GET[C__GET__MODULE_ID]))
                        {
                            $l_strJS = 'onclick="' . (empty($this->m_js_onclick[$p_cNavButton]) ? $this->m_add_onclick_prepend[$p_cNavButton] . "purge_object('" . $l_cNavMode . "', '" . $g_ajax_calls . "', '" . _L(
                                        'LC_UNIVERSAL__PURGE_NOTIFICATION'
                                    ) . "');" . $this->m_add_onclick[$p_cNavButton] : $this->m_js_onclick[$p_cNavButton]) . '"';
                        }
                        else
                        {
                            $l_strJS = 'onclick="' . (empty($this->m_js_onclick[$p_cNavButton]) ? $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.navMode.value='" . $l_cNavMode . "'; " . $l_submit . $this->m_add_onclick[$p_cNavButton] : $this->m_js_onclick[$p_cNavButton]) . '"';
                        }
                        break;
                    ...
                    case C__NAVMODE__DUPLICATE:
                        ...
                        $l_strJS = (!empty($this->m_js_onclick[$p_cNavButton])) ? $this->m_js_onclick[$p_cNavButton] : "onclick=\"if ($$('input[name=id[]]:checked').length === 0) {idoit.Notify.warning('" . _L(
                                'LC__DUPLICATE__NO_OBJECTS_SELECTED'
                            ) . "', {life:5});} else {get_popup('duplicate', 'editMode=1&" . C__CMDB__GET__OBJECTTYPE . "=" . $_GET[C__CMDB__GET__OBJECTTYPE] . "', '720', '480');}\"";
                        break;

                    default:
                        $l_element_id .= $l_cNavMode;
                        $l_strJS = "onclick=\"" . (empty($this->m_js_onclick[$p_cNavButton]) ? $this->m_add_onclick_prepend[$p_cNavButton] . "document.isys_form.navMode.value='$l_cNavMode'; document.isys_form.submit();" . $this->m_add_onclick[$p_cNavButton] : $this->m_js_onclick[$p_cNavButton]) . "\"";
                        break;
                } // switch
                ...
                if (isset($this->m_overlay[$p_cNavButton]))
                {
                    ...
                    foreach ($this->m_overlay[$p_cNavButton] as $l_li)
                    {
                        $l_add .= '<li><a href="' . $l_li['href'] . '" id="' . $l_li['id'] . '" onclick="' . $l_li['onclick'] . '">
							<img class="vam" src="' . $l_strImagesDir . $l_li['icon'] . '" width="15px" height="15px" alt="" /> <span>' . _L($l_li['title']) . '
							</span></a></li>';
                    } // foreach

                    $l_add .= "</ul></div>" . "<script type=\"text/javascript\">" . "var new_left_" . $p_cNavButton . " = ($('new_overlay_" . $p_cNavButton . "').previous().previous().offsetLeft);" . "$('new_overlay_" . $p_cNavButton . "').setStyle({left: new_left_" . $p_cNavButton . "+'px'});" . "</script>";
                } // if
            } // if

            // Tooltip:
            $l_strTooltip = str_replace("\"", "'", _L($l_strTooltip));

            // Title:
            $l_strTitle = str_replace("\"", "'", _L($l_strTitle));
            ...
            $l_strOut .= "<div id=\"" . $l_element_id . "\" title=\"" . $l_strTooltip . "\" data-navmode=\"" . $l_cNavMode . "\" class=\"" . $l_element_class . "\" " . $l_strJS . $l_access_key . ">" . "<img src=\"" . $l_strIcon . "\" width=\"15px\" height=\"15px\" alt=\"\" />" . "<span class=\"" . $l_strClassSPAN . "\" style=\"vertical-align:middle\">&nbsp;" . $l_strTitle . "&nbsp;</span>" . "</div>" . $l_add;
        } // if
    ...
    private function assign_current_url()
    {
        ...
        $l_current_url = $_SERVER["QUERY_STRING"];
        $l_current_url = str_replace("&ajax=1", "", $l_current_url);
        $l_current_url = str_replace("&call=category", "", $l_current_url);

        if (defined('C__MODULE__NAGIOS') && C__MODULE__NAGIOS)
        {
            // Specific nagios handling, because we display (cmdb) object-types in nagios-module context.
            if (in_array(
            ...
            {
                $l_current_url = C__GET__MODULE_ID . '=' . C__MODULE__NAGIOS;
            }
        }
        $g_comp_template->assign('current_link', $l_current_url);
    } // function

...
class isys_component_dao_table extends isys_component_dao
{
    ...
    public function clean_temp_tables()
    {
        ...
        if (count($l_arDiff))
        {
            ...
            foreach ($l_arDiff as $val)
            {
                ...
                if (!$l_bRet)
                {
                    throw new isys_exception_dao(
                        "Could not delete temp table '$val'"
                    );
                }
            }
            ...
            if ($l_bRet)
            {
                ...
                {
                    throw new isys_exception_dao(
                        "Could not commit transaction for deletion of temp tables!"
                    );
                }
            }
            else
    ...
    {
        $l_bRet         = false;
        $l_strTempTable = isys_glob_get_obj_list_table_name(ISYS_NULL, $p_SesID);

        $this->begin_update();

        // Delete own session entry in db
        $l_strSQL = "DELETE FROM " . "isys_user_session " . "WHERE " . "isys_user_session__php_sid = " . "'$p_SesID';";

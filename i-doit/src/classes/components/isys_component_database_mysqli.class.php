...
class isys_component_database_mysqli extends isys_component_database
{
    ...
    public function query($p_query, $p_unbuffered = false)
    {
        try
        {
            ...
            {
                throw new isys_exception_database_mysql('MySQLi error: Lost link to database.');
            }

        }
        catch (isys_exception_database_mysql $e)
        {
            if (isset(isys_application::instance()->container['notify']))
            {
    ...
    public function reconnect()
    {
        try
        {
            if (!$this->m_db_link = mysqli_connect(
                isys_glob_create_tcp_address($this->m_host, $this->m_port),
                $this->m_user,
                $this->m_pass,
                true
            )
            ...
            {
                throw new isys_exception_database_mysql("Could not select database: " . $this->m_db_name, mysqli_connect_error());
            }
        }
        catch (isys_exception_database_mysql $e)
        {
        ...
        try
        {
            ...
            if ($this->is_connected())
            {
                ...
                if (!$this->select_database($p_databasename))
                {
                    ...
                    throw new isys_exception_database_mysql(
                        "Could not select database: " . $p_databasename . $l_message, $this->get_last_error_as_string(), $this->get_last_error_as_id()
                    );
                } // if

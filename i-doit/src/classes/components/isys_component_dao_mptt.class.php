...
class isys_component_dao_mptt extends isys_component_dao
{
    ...
    {
        $l_q = "SELECT * FROM " . "{$this->m_src_tbl} " . "INNER JOIN isys_obj " . "ON isys_obj__id = " . "{$this->m_src_field_node} " . "WHERE " . "{$this->m_src_field_node}=" . (int) $p_nodeid . " " . "ORDER BY isys_obj__title";
    ...
    public function has_children($p_root_object_id, $p_child_object_id)
    {
        ...
        if ($p_root_object_id > 0)
        {
            ...
            if ($l_node && isset($l_node[$this->m_src_field_left]) && isset($l_node[$this->m_src_field_right]))
            {
                ...
                $l_q = "SELECT " . $this->m_src_field_id . ", " . $this->m_src_field_node . ", " . $this->m_src_field_parent_id . ", " . $this->m_src_field_const . ", " . $this->m_src_field_left . ", " . $this->m_src_field_right . ", " . "isys_obj__title, " . "isys_obj_type__id, " . "isys_obj_type__title " .
                ...
                $l_q .= "WHERE {$this->m_src_field_left} BETWEEN " . $l_node[$this->m_src_field_left] . " AND " . $l_node[$this->m_src_field_right] . ' AND ' . $this->m_src_field_node . ' = ' . $this->convert_sql_int(
                        $p_child_object_id
                    );
    ...
    public function get_outer_by_left_right($p_left, $p_right)
    {
        if (is_numeric($p_left) && is_numeric($p_right))
        {
            $l_q = "SELECT " . "*" . "FROM " . "{$this->m_src_tbl} " . "WHERE " . "{$this->m_src_field_left} < $p_left " . "AND " . "{$this->m_src_field_right} > $p_right " . "GROUP BY " . "{$this->m_src_tbl}__isys_obj__id " . "ORDER BY " . "{$this->m_src_field_left} ASC" . ";";
    ...
    public function read($p_id, $p_callback = null, $p_userdata = null)
    {
        ...
        if ($l_treeres)
        {
            if ($l_treeres->num_rows())
            {
                ...
                $l_subres = $this->get_left_by_left_right(
                    $l_startrow[$this->m_src_field_left],
                    $l_startrow[$this->m_src_field_right],
                    ...
                    while ($l_subrow = $l_subres->get_row())
                    {
                        ...
                        if ($l_use_callback)
                        {
                            ...
                            $p_callback->mptt_read(
                                ...
                                $l_subrow[$this->m_src_field_id],
                                $l_subrow[$this->m_src_field_node],
                                $l_subrow[$this->m_src_field_parent_id],
                                $l_subrow[$this->m_src_field_const],
                                $l_subrow[$this->m_src_field_left],
                                $l_subrow[$this->m_src_field_right],
                                $p_userdata,
    ...
    private function action_handler_add($p_data)
    {
        ...
        if (!array_key_exists("node_id", $p_data)) return "Please set node_id!";
        if (!array_key_exists("parent_id", $p_data)) return "Please set parent_id!";

        /* Check if node is existent */
        $l_existsdao = $this->retrieve(
            "SELECT " . "* " . "FROM " . "{$this->m_src_tbl} " . "WHERE " . "{$this->m_src_field_node}={$p_data["node_id"]};"
        );

        /* If yes, break proc */
        if ($l_existsdao && $l_existsdao->num_rows() > 0) return "Node already existent!";

        /* Add node */
        $this->update(
            "INSERT INTO " . "{$this->m_src_tbl} " . "(" . "{$this->m_src_field_node}, " . "{$this->m_src_field_parent_id}, " . "{$this->m_src_field_const}, " . "{$this->m_src_field_left}, " . "{$this->m_src_field_right} " . ") VALUES (" . "'{$p_data["node_id"]}', " . "'{$p_data["parent_id"]}', " . "'{$p_data["const"]}', " . "'0', " . "'0' " . ");"
        );
    ...
    private function action_handler_update($p_data)
    {
        ...
        $l_q = "UPDATE " . "{$this->m_src_tbl} " . "SET " . "{$this->m_src_field_node}='" . $l_nodeid . "', " . $l_strextra . "WHERE " . "{$this->m_src_field_node}='" . $l_nodeid . "';";
    ...
    private function action_handler_delete($p_data)
    {
        ...
        /* Delete node */
        $this->update(
            "DELETE FROM " . "{$this->m_src_tbl} " . "WHERE " . "{$this->m_src_field_node} = {$p_data["node_id"]};"
        );
    ...
    {
        if (!array_key_exists("node_id", $p_data)) return "Please set node_id";
        if (!array_key_exists("parent_id", $p_data)) return "Please set node_id";

        /* Delete node */
        $this->update(
            "UPDATE " . "{$this->m_src_tbl} " . "SET " . "{$this->m_src_field_parent_id} = {$p_data["parent_id"]} " . "WHERE " . "{$this->m_src_field_node} = {$p_data["node_id"]};"
        );
    ...
    private function _write($p_callback = null, $p_node_id = C__OBJ__ROOT_LOCATION, &$p_left = null)
    {
        /* Get node data */
        ...
        if ($l_noderes && $l_noderes->num_rows() > 0)
        {
            ...
            /* Before writing anything, we process the action stack,
               ...
               the whole tree! */
            if ($l_noderow[$this->m_src_field_node] == C__OBJ__ROOT_LOCATION)
            {
            ...
            $l_parentres = $this->get_by_parent_id($l_noderow[$this->m_src_field_node]);

            while ($l_parentrow = $l_parentres->get_row())
            {
                $l_right = $this->_write($p_callback, $l_parentrow[$this->m_src_field_node], $l_right);
            }
            ...
            if ($p_callback)
            {
                ...
                $l_dowrite = $p_callback->mptt_write(
                    $p_node_id,
                    $l_noderow[$this->m_src_field_parent_id],
                    $l_noderow[$this->m_src_field_const],
                    $p_left,
                    $p_right
                );
            }
            ...
            if ($l_dowrite)
            {
                /* Update transaction */
                {
                    $l_q = "UPDATE " . "{$this->m_src_tbl} " . "SET " . "{$this->m_src_field_left} = '$p_left', " . "{$this->m_src_field_right} = '$l_right' " . "WHERE " . "{$this->m_src_field_node} = '$p_node_id';";

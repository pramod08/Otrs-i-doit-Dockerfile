        ...
        switch (true)
        {
            ...
            case is_object($val):
                ...
                else
                {
                    ...
                    {
                        $pXml = isset($pXml) ? $pXml . $this->serialize_val($v, $k, false, false, false, false, $use) : $this->serialize_val(
                            $v,
                            $k,
                            false,
                            false,
...
class nusoap_fault extends nusoap_base
{
    ...
    function serialize()
    {
        ...
        $return_msg = '<?xml version="1.0" encoding="' . $this->soap_defencoding . '"?>' . '<SOAP-ENV:Envelope SOAP-ENV:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"' . $ns_string . ">\n" . '<SOAP-ENV:Body>' . '<SOAP-ENV:Fault>' . $this->serialize_val(
                $this->faultcode,
                'faultcode'
            ) . $this->serialize_val($this->faultactor, 'faultactor') . $this->serialize_val($this->faultstring, 'faultstring') . $this->serialize_val(
                $this->faultdetail,
                'detail'
...
class nusoap_xmlschema extends nusoap_base
{
    ...
    function serializeSchema()
    {
        ...
        foreach ($this->complexTypes as $typeName => $attrs)
        {
            ...
            {
                $contentStr = "   <$schemaPrefix:restriction base=\"" . $this->contractQName(
...
class soap_transport_http extends nusoap_base
{
    ...
    var $password = '';
    var $path = '';
    var $persistentConnection = false;    // HTTP response status line
    var $port = '';
    var $protocol_version = '1.0';
    var $proxy = null;    // cURL handle
    var $request_method = 'POST';    // cURL custom options
    var $response_status_line;        // force cURL use
    var $scheme = '';            // proxy information (associative array)
    var $uri = '';
    var $url = '';
    var $useSOAPAction = true;
    var $use_curl = false;
    var $username = '';    // keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, certpassword (optional), verifypeer (optional), verifyhost (optional)
    ...
    function io_method()
    {
        if ($this->use_curl || ($this->scheme == 'https') || ($this->scheme == 'http' && $this->authtype == 'ntlm') || ($this->scheme == 'http' && is_array(
                    $this->proxy
                ) && $this->proxy['authtype'] == 'ntlm')
        ) return 'curl';
        if (($this->scheme == 'http' || $this->scheme == 'ssl') && $this->authtype != 'ntlm' && (!is_array(
                    $this->proxy
                ) || $this->proxy['authtype'] != 'ntlm')
        ) return 'socket';
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
            ...
            if (defined('CURLOPT_CONNECTIONTIMEOUT')) $CURLOPT_CONNECTIONTIMEOUT = CURLOPT_CONNECTIONTIMEOUT;
            else
                $CURLOPT_CONNECTIONTIMEOUT = 78;
            if (defined('CURLOPT_HTTPAUTH')) $CURLOPT_HTTPAUTH = CURLOPT_HTTPAUTH;
            else
                $CURLOPT_HTTPAUTH = 107;
            if (defined('CURLOPT_PROXYAUTH')) $CURLOPT_PROXYAUTH = CURLOPT_PROXYAUTH;
            else
                $CURLOPT_PROXYAUTH = 111;
            if (defined('CURLAUTH_BASIC')) $CURLAUTH_BASIC = CURLAUTH_BASIC;
            else
                $CURLAUTH_BASIC = 1;
            if (defined('CURLAUTH_DIGEST')) $CURLAUTH_DIGEST = CURLAUTH_DIGEST;
            else
                $CURLAUTH_DIGEST = 2;
            if (defined('CURLAUTH_NTLM')) $CURLAUTH_NTLM = CURLAUTH_NTLM;
            else
                $CURLAUTH_NTLM = 8;
...
//		}
    ...
    function setCredentials($username, $password, $authtype = 'basic', $digestRequest = [], $certRequest = [])
    {
        ...
        elseif ($authtype == 'digest')
        {
            if (isset($digestRequest['nonce']))
            {
                ...
                $this->setHeader(
                    'Authorization',
                    'Digest username="' . $username . '", realm="' . $digestRequest['realm'] . '", nonce="' . $nonce . '", uri="' . $this->digest_uri . $opaque . '", cnonce="' . $cnonce . '", nc=' . sprintf(
                        "%08x",
                        $digestRequest['nc']
    ...
    function getResponse()
    {
        ...
        if ($this->io_method() == 'socket')
        {
            ...
            // close filepointer
            if ((isset($this->incoming_headers['connection']) && strtolower($this->incoming_headers['connection']) == 'close') || (!$this->persistentConnection) || feof(
                    $this->fp
                )
            )
            {
...
class wsdl extends nusoap_base
{
    ...
    var $schemas = [];    // User-specified cURL options
    var $status = '';            // whether to always try to use cURL
    // for HTTP authentication
    var $timeout = 0;                // Username for HTTP authentication
    var $use_curl = false;                // Password for HTTP authentication
    var $username = '';                // Type of HTTP authentication
    var $wsdl;        // Certificate for HTTP SSL authentication
                        ...
                        for ($ii = 0;$ii < count($list2);$ii++)
                        {
                            if (!$list2[$ii]['loaded'])
                            {
                                ...
                                if ($url != '')
                                {
                                    ...
                                    {
                                        $url = $wsdlparts['scheme'] . '://' . $wsdlparts['host'] . (isset($wsdlparts['port']) ? ':' . $wsdlparts['port'] : '') . substr(
                                                $wsdlparts['path'],
                                                0,
                                                strrpos($wsdlparts['path'], '/') + 1
                                            ) . $urlparts['path'];
                                    }
            ...
            switch ($this->status)
            {
                ...
                case 'portType':
                    switch ($name)
                    {
                        ...
                        default:
                            $m                                                                                      = isset($attrs['message']) ? $this->getLocalPart(
                                $attrs['message']
                            ) : '';
                ...
                case 'binding':
                    switch ($name)
                    {
                        ...
                        case 'body':
                            ...
                            {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus] = array_merge(
    ...
    function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = false, $unqualified = false)
    {
        ...
        elseif ($phpType == 'array')
        {
            ...
            if (is_null($value))
            {
                ...
                {
                    $xml = "<$name$elementNS xsi:nil=\"true\" xsi:type=\"" . $this->getPrefixFromNamespace(
                            'http://schemas.xmlsoap.org/soap/encoding/'
                        ) . ":Array\" " . $this->getPrefixFromNamespace('http://schemas.xmlsoap.org/soap/encoding/') . ':arrayType="' . $this->getPrefixFromNamespace(
                            $this->getPrefix($typeDef['arrayType'])
            ...
            else
            {
                $xml = "<$name$elementNS xsi:type=\"" . $this->getPrefixFromNamespace(
                        ...
                        $this->getPrefix($typeDef['arrayType'])
                    ) . ":" . $this->getLocalPart($typeDef['arrayType']) . "[$rows$cols]\">" . $contents . "</$name>";
            }
        }
        ...
        if (isset($typeDef['elements']) && is_array($typeDef['elements']))
        {
            ...
            foreach ($typeDef['elements'] as $eName => $attrs)
            {
                ...
                else
                {
                    ...
                    if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) && isset($v) && is_array(
                            $v
    ...
    function wsdl($wsdl = '', $proxyhost = false, $proxyport = false, $proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $curl_options = null, $use_curl = false)
    {
        ...
        if (is_array($curl_options)) $this->curl_options = $curl_options;
        $this->use_curl = $use_curl;
        $this->fetchWSDL($wsdl);
    }
}
...
class nusoap_parser extends nusoap_base
{
    ...
    var $depth_array = [];
    var $document = '';
    var $fault = false;            // incoming SOAP body (text)
    ...
    var $root_header = '';
    var $root_struct = '';
    var $root_struct_name = '';    // parsed SOAP Body
    var $root_struct_namespace = '';        // parsed SOAP Header
    var $soapheader = null;    // incoming SOAP headers (text)
...
class nusoap_client extends nusoap_base
{
    ...
    var $authtype = '';                // Username for HTTP authentication
    var $bindingType = '';                // Password for HTTP authentication
    var $certRequest = [];                // Type of HTTP authentication
    var $cookies = [];        // Certificate for HTTP SSL authentication
    var $curl_options = [];    // SOAP headers in request (text)
    var $decode_utf8 = true;        // SOAP headers from response (incomplete namespace resolution) (text)
    var $defaultRpcParams = false;        // SOAP Header from response (parsed)
    var $document = '';                // SOAP body response portion (incomplete namespace resolution) (text)
    var $endpoint;
    var $endpointType = '';        // overrides WSDL endpoint
    /**
     * @var      fault
     * @access   public
     */
     ...
     */
    var $faultstring;
    var $forceEndpoint = '';            // character set encoding of incoming (response) messages
    var $http_encoding = false;
    var $operations = [];                // HTTP connection timeout
    var $password = '';        // HTTP response timeout
    var $persistentConnection = false;            // soap|wsdl, empty for WSDL initialization error
    var $proxyhost = '';
    var $proxypassword = '';    // This is no longer used
    var $proxyport = '';                // HTTP request
    var $proxyusername = '';                // HTTP response
    var $request = '';            // SOAP payload of response
    var $requestHeaders = false;            // Cookies from response or for request
    var $response = '';        // toggles whether the parser decodes element content w/ utf8_decode()
    var $responseData = '';        // WSDL operations, empty for WSDL initialization error
    var $responseHeader = null;    // User-specified cURL options
    var $responseHeaders = '';            // WSDL operation binding type
    var $response_timeout = 30;            // whether to always try to use cURL

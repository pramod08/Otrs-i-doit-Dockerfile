...
class isys_library_ldap extends ldapi_acc
{
    ...
    {
        return new self(
            $p_hostname, $p_dn, $p_password, $p_port, $p_protocol_version, $p_tls, $p_debug_level
        );
    } // function
     ...
     * @param int    $p_scope
     */
    public function search($p_dn, $p_filter, $p_attributes = [], $p_attributes_only = 0, $p_sizelimit = null, $p_timelimit = null, $p_deref = null, $p_scope = C__LDAP_SCOPE__RECURSIVE)
    {
     ...
     * @throws  Exception
     */
    public function __construct($p_hostname, $p_dn, $p_password, $p_port = 389, $p_protocol_version = 3, $p_tls = false, $p_debug_level = 0)
    {
        ...
        if (($this->m_connected = $this->connect()))
        {
            ...
            if ($p_tls)
            {
                ...
                {
                    throw new Exception(
                        "Failed to start TLS (" . $this->get_ldap_error(
                        ) . "). Check http://php.net/ldap-start-tls for help configuring secure ldap connections. " . "Host: " . $p_hostname . ":" . $p_port . ". User: " . $p_dn . ", TLS: On"
                    );
                } // if
            } // if

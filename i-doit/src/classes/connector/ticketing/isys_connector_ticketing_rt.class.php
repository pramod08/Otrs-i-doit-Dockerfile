...
class isys_connector_ticketing_rt extends isys_connector_ticketing
{
    ...
    protected $m_mandator = 'i-doit mandator';
    /**
    ...
    public function create_new_ticket_url($p_object_id)
    {
        ...
        $l_ticket_new_url = [
            'url'          => str_replace('/REST/1.0/', '', $this->m_protocol->get_url()) . "/Ticket/Create.html?IDoitMandator=" . $g_comp_session->get_mandator_name(
    ...
    public function get_tickets_by_cmdb_object($p_object_id)
    {
        ...
        $l_custom_fields = [
            ...
            $this->m_mandator     => $g_comp_session->get_mandator_name()
        ];
     ...
     * @return string
     */
    public function create_ticket($p_queue, $p_requestor, $p_subject, $p_cc, $p_admincc, $p_owner, $p_status, $p_priority, $p_initial_priority, $p_final_priority, $p_estimated_time, $p_starts, $p_due, $p_text, $p_customfields = [])
    {
        $l_contentArr = [];
     ...
     * @return string
     */
    public function edit_ticket($p_id, $p_queue, $p_requestor, $p_subject, $p_cc, $p_admincc, $p_owner, $p_status, $p_priority, $p_initial_priority, $p_final_priority, $p_estimated_time, $p_starts, $p_due, $p_text, $p_customfields = [])
    {
        $l_contentArr = [];
    ...
    {
        $l_content = '';
        if (is_array($p_params)) foreach ($p_params as $l_key => $l_value)
        {
            if (!empty($l_value))
            {
                ...
                if (preg_match('/^RT\/([\.0-9]+) ([0-9][0-9][0-9]) (.*?)$/i', $l_block, $l_register))
                {
                    ...
                    if ($l_status_code != 200)
                    {
                        throw new isys_exception_general(
                            'Could not send request to RT version ' . $l_version . ': ' . $l_status . ' (Status code: ' . $l_status_code . ')'
                        );
                    } // if
                }
                else

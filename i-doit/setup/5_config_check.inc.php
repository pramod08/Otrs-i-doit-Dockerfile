    ...
    foreach ($g_config as $l_key => $l_data)
    {
        ...
        switch ($l_key)
        {
            ...
            case "config.dir.fileman.image":
                ...
                if ($l_data["content"] != "" && strrpos($l_data["content"], DIRECTORY_SEPARATOR) < (strlen(
                            $l_data["content"]
                        ) - 1)
                ) $g_config["$l_key"]["content"] .= DIRECTORY_SEPARATOR;
                // check for folder existence
                if ($l_key == ("config.dir.fileman.temp" || "config.dir.fileman.image") && $l_key != "config.dir.fileman.file" && !file_exists(
                        $l_data["content"]
            ...
            case "config.db.root.password":
                ...
                if (!empty($l_dbHost) && !empty($l_dbPort))
                {
                    ...
                    if (!$l_dbLink->connect_error)
                    {
                        ...
                        if ($l_is_mariadb)
                        {
                            $l_db = 'MariaDB Version: ' . $l_db_version;

                            if (version_compare($l_db_version, MARIADB_VERSION_MINIMUM, "<") == -1)
                            {
                                $l_success   = false;
                                $l_css_class = 'stepLineStatusBad';
                                $l_message   = MARIADB_VERSION_MINIMUM . ' REQUIRED.<br /><a href=\"https://i-doit.atlassian.net/wiki/x/IQANAQ\" target=\"_blank\">See our Knowledge Base article for help!</a>';
                            } // if
                        }
                        else
                        {
                            $l_db = 'MySQL Version: ' . $l_db_version;

                            if (version_compare($l_db_version, MYSQL_VERSION_MINIMUM, "<") == -1)
                            {
                                $l_success   = false;
                                $l_css_class = 'stepLineStatusBad';
                                $l_message   = MYSQL_VERSION_MINIMUM . ' REQUIRED, ' . MYSQL_VERSION_MINIMUM_RECOMMENDED . ' RECOMMENDED.<br /><a href=\"https://i-doit.atlassian.net/wiki/x/IQANAQ\" target=\"_blank\">See our Knowledge Base article for help!</a>';
                            } // if
                        } // if

                        // New checks for MySQL >= 5.6 or MariaDB >= 10.0
                        $s_steps .= '<tr><td></td><td class="stepLineData">' . $l_db . '</td><td class="' . $l_css_class . '">' . $l_message . '</td></tr><tr><td colspan="3" class="stepLineSeperator"></td></tr>';
                    }
                    else
                    {
                        $l_success = false;
                ...
                if (!$l_dbLink->connect_error)
                {
                    ...
                    if (isset($l_mysql) && is_array($l_mysql))
                    {
                        foreach ($l_mysql as $l_mkey => $l_mconfig)
                        {
                            ...
                            {
                                $l_err = true;
                                $l_data["content"] .= "<br><span style='color: #CC0000;'><strong>" . $l_mkey . "</strong> should be at least " . $l_mconfig[1] . "M (currently " . (number_format(
                                        floor($l_mconfig[0]) / 1024 / 1024,
                                        2
                                    )) . "M)!</span>";
                            }
        ...
        if ($l_bShowError)
        {
            if ($l_data["name"] != "")
            {
                $s_steps .= "<tr>" . "<td>&nbsp;</td>" . "<td class=\"stepLineData\">" . $l_data["name"] . ": " . $l_data["content"] . "</td>" . "<td class=\"" . (($l_success) ? "stepLineStatusGood" : "stepLineStatusBad") . "\">$l_message</td>" . "</tr>";

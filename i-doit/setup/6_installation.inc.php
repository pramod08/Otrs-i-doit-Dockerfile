...
function process_after_posttransfer()
{
    ...
    function ___add_status(&$p_out, $p_text, $p_status, $p_error = null)
    {
        ...
        $p_out .= "<tr>" . "<td>&nbsp;</td>" . "<td class=\"stepLineData\">" . $p_text . "</td>" . "<td class=\"" . (($p_status) ? "stepLineStatusGood" : "stepLineStatusBad") . "\">" . (($p_status) ? "OK" : $l_error) . "</td>" . "</tr>";
        $p_out .= "<tr>" . "<td colspan=\"3\" class=\"stepLineSeperator\">" . "</td>" . "</tr>";
    }

    function ___rollback($p_rollback, &$p_out)
    {
        if (count($p_rollback) > 0)
        {
    ...
    if ($_POST["install_now"] == "1")
    {
        ...
        if ($l_nErrors == 0)
        {
            $l_ret = $g_dbLink->query(
                "CREATE DATABASE IF NOT EXISTS `" . $g_config["config.mandant.name"]["content"] . "` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci"
            );
            if ($l_ret === false) $l_nErrors++;
        ...
        if ($l_nErrors == 0)
        {
            ...
            $l_ret = mysql_import(
                ...
                $g_settings["mysqlDumpSystem"],
                $l_out,
                $g_dbLink
            );

            if ($l_ret != 1) $l_nErrors++;
        ...
        {
            $l_ret = $g_dbLink->query(
                "ALTER TABLE " . $g_config["config.mandant.name"]["content"] . ".isys_obj " . "AUTO_INCREMENT = " . $g_dbLink->escape_string(
                    $g_config["config.mandant.autoinc"]['content']
                ) . ";"
            );
        ...
        if ($l_nErrors == 0)
        {
            ...
            */
            ...
            $l_grant = "GRANT " . $g_settings["mysqlPrivileges"] . " " . "ON " . $g_config["config.db.name"]["content"] . ".* " . "TO '" . $g_config["config.db.username"]["content"] . "'@'localhost'";
            if ($g_config["config.db.password"]["content"] != "")
            {
                $l_grant .= " IDENTIFIED BY '" . $g_config["config.db.password"]["content"] . "'";
            }
        ...
        if ($l_nErrors == 0)
        {
            ...
            */
            ...
            $l_grant = "GRANT " . $g_settings["mysqlPrivileges"] . " " . "ON " . $g_config["config.mandant.name"]["content"] . ".* " . //"ON *.* ".
                "TO '" . $g_config["config.db.username"]["content"] . "'@'localhost'";
            if ($g_config["config.db.username"]["content"] != "")
            {
                $l_grant .= " IDENTIFIED BY '" . $g_config["config.db.password"]["content"] . "'";
            }
        ...
        if ($l_nErrors == 0)
        {
            ...
            if (isset($l_configData) && !empty($l_configData))
            {
                ...
                {
                    if ($l_key == "config.modules") continue;

                    $l_data["content"] = str_replace(
                        "\\",
                        "\\\\",
                        $l_data["content"]
                    );

                    $l_configData = str_replace(
                        "%" . $l_key . "%",
                        $l_data["content"],
                        $l_configData
                    );
                ...
                $l_ret = $g_dbLink->query(
                    "REPLACE INTO isys_settings SET isys_settings__key = 'system.dir.file-upload', isys_settings__value = '" . $g_dbLink->escape_string(
                        @$g_config['config.dir.fileman.file']["content"]
                    ) . "';"
                );
                if ($l_ret === false) $l_nErrors++;

                $l_ret = $g_dbLink->query(
                    "REPLACE INTO isys_settings SET isys_settings__key = 'system.dir.image-upload', isys_settings__value = '" . $g_dbLink->escape_string(
                        @$g_config['config.dir.fileman.image']["content"]
                    ) . "';"
                );
                if ($l_ret === false) $l_nErrors++;
        ...
        else
        {
            ...
            $l_res     = $g_dbLink->query(
                'SELECT isys_module__id FROM ' . $g_config["config.mandant.name"]["content"] . '.isys_module WHERE isys_module__const = "C__MODULE__QCW";'
            );

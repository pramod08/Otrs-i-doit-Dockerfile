...
))
{
    ...
    {
        if (strlen($match[1])) $phpinfo[$match[1]] = [];
        elseif (isset($match[3])) $phpinfo[end(array_keys($phpinfo))][$match[2]] = isset($match[4]) ? [
            $match[3],
            $match[4]
        ] : $match[3];
        else
...
foreach ($phpinfo as $name => $section)
{
    if (count($section) > 0)
    {
        ...
        {
            if (is_array($val)) $l_out .= "      <$key>" . strip_tags($val[0]) . ", " . strip_tags($val[1]) . "</$key>\n";
            elseif (is_string($key)) $l_out .= "      <$key>" . strip_tags($val) . "</$key>\n";
        }
        $l_out .= "    </" . str_replace(" ", "_", strtolower($name)) . ">\n";
    }
}

...
$g_comp_template->assign('steps', $l_update_gui->get_steps())
...
class isys_update_gui
{
            ...
            if (is_array($_SESSION['update_data']['step-3']))
            {
                ...
                foreach ($_SESSION['update_data']['step-3'] as $l_key => $l_tenant)
                {
                    if (strpos($l_key, 'mandator_') !== false)
                    {
                        ...
                        if (count($l_migration_log[$l_tenant]) > 0)
                        {
                            $l_logged_data = true;
                        } // if
                    } // if
    ...
    public function completion()
    {
        ...
        if ($_SESSION['error'] >= 1)
        {
            $l_message = '<h3 color="red">Error!</h3>' . '<p>There are <strong>' . $_SESSION['error'] . '</strong> errors occurred. Your i-doit could run unstable now.<br />Visit our support forum at <a href="http://www.i-doit.org/" target="_blank">http://www.i-doit.org/</a> for any help.</p>' . '<p>Detailed debug information can be found in the update log located at "' . $this->m_dirs['temp'] . '" on your i-doit web-server.</p>';
        }
        else
        {
            $l_message = '<h3>Congratulations!</h3>' . '<p>Your i-doit installation has been successfully updated to a newer version.</p>' . '<p>Detailed debug information can be found at "' . $this->m_dirs['temp'] . '" on your i-doit web-server.</p>';
        } // if
    ...
    protected function check_for_update()
    {
        ...
        if (defined('C__IDOIT_UPDATES_PRO')) $g_updatexml = C__IDOIT_UPDATES_PRO;
        else
        ...
        if (count($l_local_updates) > 0)
        {
            foreach ($l_local_updates as $l_local_update)
            {
                if ($l_latest_update["version"] != $l_local_update["version"] && $l_latest_update != null)
                {
                    ...
                    $l_return['message'] = 'There is a new i-doit update available! Version ' . $l_latest_update['version'] . ' (Revision: ' . $l_latest_update['revision'] . '), released on ' . $l_latest_update['release'] . '. Simply press "Download and extract" to proceed!';
                }
                else
                {
    ...
    {
        global $g_comp_template, $g_absdir;

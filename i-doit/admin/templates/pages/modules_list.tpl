<form action="?req=modules" method="post" id="modules_form">
	...
	<hr style="border:0;" />
	[{foreach from=$modules item="mandator"}]
		...
		<fieldset class="overview" style="margin-top:10px;">
			...
			<table cellpadding="2" cellspacing="0" width="100%" class="sortable mt10">
				...
				<tbody>
				[{foreach from=$mandator.modules item="row"}]
				<tr class="[{cycle values="even,odd"}]">
					<td>
						[{if $row.type != 'core' || !$row.active}]
						<input type="checkbox" name="module[[{$mandator.id}]][]" value="[{$row.identifier}]" />
						[{else}]
						<input type="checkbox" name="disabled_checkbox" disabled="disabled" title="You cannot disable or uninstall core modules." />
						[{/if}]
					</td>
					...
					<td><span class="[{if $row.licenced === true}]green"><img src="../images/icons/silk/bullet_green.png" class="vam" border="0" alt="+" /> Yes[{else if $row.licenced === false}]red"><img src="../images/icons/silk/bullet_red.png" class="vam" border="0" alt="-" /> No[{else}]grey">[{if $mandator.licenced}]Yes[{else}]No[{/if}][{/if}]</span></td>
				</tr>
				[{/foreach}]
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	$('modules_form').on(
	    'click',
	    'table.sortable tr td button.install-module',
	    function(event, element)
	    {
		    var module = element.getAttribute('data-identifier'), tenant = element.getAttribute('data-tenant'), tenantName = element.getAttribute('data-tenant-name');

		    if (confirm('Are you sure you want to {0} module "{1}" in tenant "{2}"?'.replace('{0}', element.getAttribute('data-type')).replace('{1}', module).replace('{2}', tenantName))) {

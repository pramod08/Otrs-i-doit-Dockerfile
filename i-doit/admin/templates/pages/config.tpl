		...
		<div style="margin-top:20px;">
			<table cellpadding="2" cellspacing="0" width="100%" class="sortable m10">
				...
				<tr>
					<td>Password</td>
					<td>
						<input type="text" name="db_pass1" id="db_pass1" onfocus="this.clear();" value="***" />
					</td>
				</tr>
				<tr>
					<td>Verify Password</td>
					<td>
						<input type="text" name="db_pass2" id="db_pass2" onfocus="this.clear();" value="***" />
					</td>
				</tr>
				<tr>
					<td>Database Name</td>
					...
					new Ajax.Request('?req=config&action=save',
							{
								...
								onComplete: function (response) {
									var message = 'Unknown Error. Config was not saved!';
									if (response.responseJSON) {
										message = response.responseJSON.message;

										if (!response.responseJSON.success) {
											$('configSaveResult').removeClassName('note').addClassName('error');
										}
										else $('configSaveResult').removeClassName('error').addClassName('note');
									}

									$('configSaveResult').update(message).show();
								}
							}
					);
				}
